...
if ($nboftargetok) {
	...
	if ($nbofpublishneedchangelog)
	{
		...
		{
			print 'cd ~/git/powererp_'.$MAJOR.'.'.$MINOR.'; git log `git rev-list --boundary '.$MAJOR.'.'.$MINOR.'..origin/develop | grep ^- | cut -c2- | head -n 1`.. --no-merges --pretty=short --oneline | sed -e "s/^[0-9a-z]* //" | grep -e \'^FIX\|NEW\|CLOSE\' | sort -u | sed \'s/FIXED:/FIX:/g\' | sed \'s/FIXED :/FIX:/g\' | sed \'s/FIX :/FIX:/g\' | sed \'s/FIX /FIX: /g\' | sed \'s/CLOSE/NEW/g\' | sed \'s/NEW :/NEW:/g\' | sed \'s/NEW /NEW: /g\' > /tmp/aaa';
		}
		else			# For a maintenance release
		{
			#print 'cd ~/git/powererp_'.$MAJOR.'.'.$MINOR.'; git log '.$MAJOR.'.'.$MINOR.'.'.($BUILD-1).'.. --no-merges --pretty=short --oneline | sed -e "s/^[0-9a-z]* //" | grep -e \'^FIX\|NEW\' | sort -u | sed \'s/FIXED:/FIX:/g\' | sed \'s/FIXED :/FIX:/g\' | sed \'s/FIX :/FIX:/g\' | sed \'s/FIX /FIX: /g\' | sed \'s/CLOSE/NEW/g\'| sed \'s/NEW :/NEW:/g\' | sed \'s/NEW /NEW: /g\' > /tmp/aaa';
			print 'cd ~/git/powererp_'.$MAJOR.'.'.$MINOR.'; git log '.$MAJOR.'.'.$MINOR.'.'.($BUILD-1).'.. | grep -v "Merge branch" | grep -v "Merge pull" | grep "^ " | sed -e "s/^[0-9a-z]* *//" | grep -e \'^FIX\|NEW\|CLOSE\' | sort -u | sed \'s/FIXED:/FIX:/g\' | sed \'s/FIXED :/FIX:/g\' | sed \'s/FIX :/FIX:/g\' | sed \'s/FIX /FIX: /g\' | sed \'s/CLOSE/NEW/g\' | sed \'s/NEW :/NEW:/g\' | sed \'s/NEW /NEW: /g\' > /tmp/aaa';
		}
		print "\n";
		if (! $ret)
		{
	...
	if ($nboftargetneedbuildroot)
	{
		...
		$ret=`rm -f  $BUILDROOT/$PROJECT/build/powererp_*.deb`;
		...
		$ret=`rm -f  $BUILDROOT/$PROJECT/dev/powererp_changes.txt`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/dev/README`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/powererp_screenshot2.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/powererp_screenshot3.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/powererp_screenshot4.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/powererp_screenshot5.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/powererp_screenshot6.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/powererp_screenshot7.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/powererp_screenshot8.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/powererp_screenshot9.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/powererp_screenshot10.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/powererp_screenshot11.png`;
		$ret=`rm -f  $BUILDROOT/$PROJECT/doc/images/powererp_screenshot12.png`;
