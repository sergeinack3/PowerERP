...
foreach ($includeconstants as $countrycode => $tmp) {
	fputs($fp, '<powererp_constants country="'.$countrycode.'">'."\n");
	foreach ($tmp as $constname => $constvalue) {
		$valueforchecksum=(empty($constvalue)?'0':$constvalue);
		$checksumconcat[]=$valueforchecksum;
		fputs($fp, '    <constant name="'.$constname.'">'.$valueforchecksum.'</constant>'."\n");
	}
	fputs($fp, '</powererp_constants>'."\n");
}

fputs($fp, '<powererp_htdocs_dir includecustom="'.$includecustom.'">'."\n");
...
fputs($fp, '  </dir>'."\n");
fputs($fp, '</powererp_htdocs_dir>'."\n");

asort($checksumconcat); // Sort list of checksum
//var_dump($checksumconcat);
fputs($fp, '<powererp_htdocs_dir_checksum>'."\n");
...
fputs($fp, '<powererp_script_dir version="'.$release.'">'."\n");
...
fputs($fp, '  </dir>'."\n");
fputs($fp, '</powererp_script_dir>'."\n");

asort($checksumconcat); // Sort list of checksum
fputs($fp, '<powererp_script_dir_checksum>'."\n");
fputs($fp, md5(join(',', $checksumconcat))."\n");
fputs($fp, '</powererp_script_dir_checksum>'."\n");
