...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
	...
	if ($user->rights->societe->contact->creer) {
		if ($action == 'create') {
			...
			$doleditor = new DolEditor('note_public', $object->note_public, '', 80, 'powererp_notes', 'In', 0, false, empty($conf->global->FCKEDITOR_ENABLE_NOTE_PUBLIC) ? 0 : 1, ROWS_3, '90%');
			print $doleditor->Create(1);
			print '</td></tr>';

			// Note Private
			print '<tr><td class="tdtop"><label for="note_private">'.$langs->trans("NotePrivate").'</label></td><td colspan="3">';
			$doleditor = new DolEditor('note_private', $object->note_private, '', 80, 'powererp_notes', 'In', 0, false, empty($conf->global->FCKEDITOR_ENABLE_NOTE_PRIVATE) ? 0 : 1, ROWS_3, '90%');
			print $doleditor->Create(1);
			print '</td></tr>';
			...
			if ($object->user_id) {
				...
				$result = $powererp_user->fetch($object->user_id);
				print $powererp_user->getLoginUrl(1);
			} else {
	...
	if (!empty($id) && $action != 'edit' && $action != 'create') {
		...
		if ($action == 'create_user') {
			...
			print $form->formconfirm($_SERVER["PHP_SELF"]."?id=".$object->id, $langs->trans("CreatePowerERPLogin"), $text, "confirm_create_user", $formquestion, 'yes');
		}
		...
		print '<tr><td>'.$langs->trans("PowerERPLogin").'</td><td>';
		if ($object->user_id) {
			$powererp_user = new User($db);
			$result = $powererp_user->fetch($object->user_id);
			print $powererp_user->getLoginUrl(-1);
			...
			if (!$object->user_id && $user->rights->user->user->creer) {
				print '<a class="aaa" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&action=create_user&token='.newToken().'">'.img_picto($langs->trans("CreatePowerERPLogin"), 'add', 'class="pictofixedwidth"').$langs->trans("CreatePowerERPLogin").'</a>';
			}
		}
		print '</td></tr>';
