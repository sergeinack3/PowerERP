...
class CdavLib
{
	...
	public function toVCalendar($calid, $obj)
	{
		...
		if (!empty($obj->soc_nom)) {
			$caldata .= "\\n*POWERERP-SOC: ".$obj->soc_nom;
		}
		if (!empty($obj->soc_phone)) {
			$caldata .= "\\n*POWERERP-SOC-TEL: ".$obj->soc_phone;
		}
		if (!empty($obj->firstname) || !empty($obj->lastname)) {
			$caldata .= "\\n*POWERERP-CTC: ".trim($obj->firstname.' '.$obj->lastname);
		}
		if (!empty($obj->phone) || !empty($obj->phone_perso) || !empty($obj->phone_mobile)) {
			$caldata .= "\\n*POWERERP-CTC-TEL: ".trim($obj->phone.' '.$obj->phone_perso.' '.$obj->phone_mobile);
		}
		if (strpos($obj->other_users, ',')) { // several
			$caldata .= "\\n*POWERERP-USR: ".$obj->other_users;
		}
		$caldata .= "\n";
