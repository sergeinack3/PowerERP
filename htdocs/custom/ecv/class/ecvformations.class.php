...
class ecvformations extends Commonobject{ 

	public $errors = array();
	public $rowid;
	public $ref;

	public $element='ecvformations';
	public $table_element='ecvformations';
	...
	public function fetchAll($sortorder = '', $sortfield = '', $limit = 0, $offset = 0, $filter = '', $filtermode = 'AND')
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$line->etablissement 	=  $obj->etablissement;
				$line->niveau 	    =  $obj->niveau;
				$line->intitule 	=  $obj->intitule;
				$line->filiere 	    =  $obj->filiere;
	...
	public function fetch($id)
	{
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->etablissement 	    =  $obj->etablissement;
	...
	public function select_niveau($niveau='' ,$nameselect='niveau'){
	    
	    // return 'fdgsdfgdfg';
   		global $langs;
	    $moreforfilter.='<select class="niveau flat" id="niveau" name="'.$nameselect.'" >';
	    $moreforfilter.='<option value="0" ></option>';
		$moreforfilter.='<option value="Qualifiant"  >'.$langs->trans("Qualifiant").'</option>';
		$moreforfilter.='<option value="Bac"  >Bac </option>';
		$moreforfilter.='<option value="Bac+1" >Bac+1 </option>';
		$moreforfilter.='<option value="Bac+2" >Bac+2 </option>';
		$moreforfilter.='<option value="Bac+3" >Bac+3 </option>';
		$moreforfilter.='<option value="Bac+5" >Bac+5 </option>';
		$moreforfilter.='<option value="Master_specialisé" >'.$langs->trans("Master_specialisé").' </option>';
		$moreforfilter.='<option value="Master_recherche" >'.$langs->trans("Master_recherche").' </option>';
		$moreforfilter.='<option value="Doctorat" >'.$langs->trans("Doctorat").' </option>';
	    $moreforfilter.='</select>';
	    $moreforfilter = str_replace('value="'.$niveau.'"', 'value="'.$niveau.'" selected', $moreforfilter);
	    $moreforfilter.='<script>$(document).ready(function(){$("#niveau").select2()})</script>';
	    return $moreforfilter;
	}
} 
