...
$modname = $langs->trans("list_interventions");
...
$filter .= (!empty($srch_date)) ? " AND date = '".$date."'" : "";
...
$filter .= (!empty($srch_resultat)) ? " AND resultat like '%".$srch_resultat."%'" : "";
...
print '<thead>';
	...
	print '<tr class="liste_titre">';
		...
		print_liste_field_titre($langs->trans("Ref_l"), $_SERVER["PHP_SELF"], "rowid", "", $param, 'align="left"', $sortfield, $sortorder);
	...
	print '<tr class="liste_titre nc_filtrage_tr">';
		...
		print '<td align="center"><input type="text" class="datepicker" id="srch_date" name="srch_date" value="'.$srch_date.'" autocomplete="off"/></td>';
		...
		print '<td align="center">'.$gestpanne->resultat_intervention($srch_resultat,"srch_resultat").'</td>';
		...
		for ($i=0; $i < count($interventions->rows) ; $i++) {
			...
			print '<tr '.$bc[$var].' >';
	    		print '<td align="left" style="">'; 
		    		print '<a href="'.dol_buildpath('/gestionpannes/interventions/card.php?id='.$item->rowid.'&id_panne='.$item->fk_panne,2).'" >'.$item->rowid.'</a>';
	    		print '</td>';
