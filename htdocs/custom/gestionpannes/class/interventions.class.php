...
class interventions extends Commonobject{ 

	public $errors = array();
	public $rowid;
	public $ref;

	public $element='interventions';
	public $table_element='interventions';
	...
	//	die($sql);
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$line->fk_panne    = $obj->fk_panne;
				$line->description    = $obj->description;
				$line->resultat    = $obj->resultat;
				$line->guide    = $obj->guide;
...
public function select_tache($selected=0,$name='select_',$showempty=1,$filter=''){
	...
	public function select_matriel($id=0){
		global $conf;
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				$moreforfilter.='<option value="'.$obj->rowid.'"';
	            if ($obj->rowid == $id) $moreforfilter.=' selected';
