...
$modname = $langs->trans("list_respo");
...
$srch_responsablemintenece        = GETPOST('responsablemintenece');
	$srch_responsablemintenece= GETPOST('srch_responsablemintenece');
$filter .= (!empty($srch_responsablemintenece)) ? " AND responsablemintenece like '%".$srch_responsablemintenece."%'" : "";



$filter .= (!empty($srch_rowid)) ? " AND rowid = ".$srch_rowid."" : "";
$filter .= (!empty($srch_type)) ? " AND responsablemintenece like '%".$srch_responsablemintenece."%'" : "";
...
$nbrtotal = $responsablemintenece->fetchAll($sortorder, $sortfield, $limit, $offset, $filter);
...
field($langs->trans("nom_respo"),'nom_respo');
...
print '<td align="center"><input style="max-width: 129px;" type="text" class="" id="srch_responsablemintenece" name="srch_responsablemintenece" value="'.$srch_responsablemintenece.'"/></td>';
...
$colspn = 2;
if (count($responsablemintenece->rows) > 0) {
for ($i=0; $i < count($responsablemintenece->rows) ; $i++) {
$var = !$var;
$item = $responsablemintenece->rows[$i];

print '<tr '.$bc[$var].' >';
print '<td align="center" style="">'; 
print '<a href="'.dol_buildpath('/gestionpannes/responsablemintenece/card.php?id='.$item->rowid,2).'" >';
print $item->rowid;
print '</a>';
print '</td>';

print '<td align="center">';
print $item->responsablemintenece;
print '</td>';
print '<td align="center">';
