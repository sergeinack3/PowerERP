...
$modname = $langs->trans("types_de_pannes");
...
$filter .= (!empty($srch_type)) ? " AND typepanne like '%".$srch_type."%'" : "";
...
$nbrtotal = $typepanne->fetchAll($sortorder, $sortfield, $limit, $offset, $filter);
	...
	print '<table id="table-1" class="noborder" style="width: 100%;" >';
		print '<thead>';
			...
			print '<tr class="liste_titre nc_filtrage_tr">';
				print '<td align="center"><input size="1" type="text" class="" id="srch_rowid" name="srch_rowid" value="'.$srch_rowid.'"/></td>';

				print '<td align="left"><input type="text" style="width:100%;" class="" id="srch_type" name="srch_type" value="'.$srch_type.'"/></td>';
		...
		print '<tbody>';
			...
			$colspn = 2;
			if (count($typepanne->rows) > 0) {
			for ($i=0; $i < count($typepanne->rows) ; $i++) {
				$var = !$var;
				$item = $typepanne->rows[$i];

				print '<tr '.$bc[$var].' >';
		    		print '<td align="center" style="">'; 
		    		print '<a href="'.dol_buildpath('/gestionpannes/typepanne/card.php?id='.$item->rowid,2).'" >';
					print $item->rowid;
		    		print '</a>';
		    		print '</td>';
			
		    		print '<td align="left">';
		    		print $item->typepanne;
		    		print '</td>';
		    		print '<td align="center">';
		    		print '</td>';
				...
				print '<tr><td align="center" colspan="3">'.$langs->trans("NoResults").'</td></tr>';
