...
if ($action == 'update' && $request_method === 'POST') {
    ...
    $typeurgent2  = GETPOST('typeurgent');
    ...
    $isvalid = $typeurgent->update($id, $data);
...
if($action == "edit"){
    ...
    $typeurgent->fetchAll('','',0,0,' and rowid = '.$id);
    $item = $typeurgent->rows[0];

    print '<table class="border" width="100%">';
    print '<tbody>'; 
    $var  = true;
    print '<tr '.$bc[$var].'>';
        print '<td width="20%">'.$langs->trans('typeurgent').'</td>';
        print '<td width="80%"><input type="text" class="" id="typeurgent" name="typeurgent" value="'.$item->typeurgent.'" required="required" autocomplete="off" style="width:100%"/>';
    print '</td>';
    print '</tr>';
