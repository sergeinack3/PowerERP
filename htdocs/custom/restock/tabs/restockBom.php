...
/**
 *  \file	   htdocs/restock/restockBom.php
 *  \ingroup	stock
 *  \brief	  Page to manage restock on Bom
 */
...
if ($action != "createrestock" && $action != "createsubof") {
	$title=$langs->trans("RestockBomOrderProduct");

	llxHeader('', $title, 'EN:Customers_Orders|FR:Commandes_Clients|ES:Pedidos de clientes', '');
	
	$head = bomPrepareHead($object);
	
	dol_fiche_head($head, 'restock', $langs->trans("BillOfMaterials"), -1, 'bom');
...
if ($action=="") {
	...
	print '<form action="restockBom.php" method="post" name="formulaire">';
	...
	$tblRestockTemp =$object->lines;
	
	$nbcomponent=0;
	foreach ($tblRestockTemp as $lgncomponent) {
		//var_dump($lgncomponent);
		// on ajoute le composant trouvé au tableau des composants
		$tblRestock[$nbcomponent] = new Restock($db);

		$tblRestock[$nbcomponent]->id = $lgncomponent->fk_product;
		$product_static = new Product($db);
		$product_static->fetch($lgncomponent->fk_product);

		$tblRestock[$nbcomponent]->ref_product = $lgncomponent->ref;
		$tblRestock[$nbcomponent]->libproduct = $lgncomponent->label;
		$tblRestock[$nbcomponent]->nbFactory = $lgncomponent->qty;
	...
	foreach ($tblRestock as $lgnRestock) {
		...
		|| $conf->global->RESTOCK_PRODUCT_TYPE_SELECT ==0)) {
			...
			if (count($product_fourn_list) > 0) {
				// si le besoin est n�gatif cela signifie que l'on a assez , 
	...
	*/
	...
	print '<td align=left>';
	// on mémorise la réfdu bom sur la commande fournisseur
	...
	*/
	...
	// derniére étape : la création des commandes fournisseurs
	// on récupère la liste des produits à commander
	$tblproduct=explode("-", GETPOST("prodlist"));

	// on récupère la listes des commandes fournisseurs si on en a s�lectionn�
	//$tblcommande=explode("-", GETPOST("cmdfournlist"));

	// on va utiliser un tableau pour stocker les commandes fournisseurs
	$tblCmdeFourn=array();
	// on parcours les produits pour récupérer les fournisseurs, les produits et les quantités
	foreach ($tblproduct as $idproduct) {
	...
	// on va maintenant cr�er les sous OF
	foreach ($tblSousOF as $fk_product => $qtyProduct) {
		$idCmdFourn = 0;
		// si il y d�j� un of en brouillon du produit, on augmente sa quantit�
		if ($conf->global->RESTOCK_FILL_FACTORY_DRAFT > 0) {
			// on v�rifie qu'il n'y a pas OF d�j� ouvert
		...
		// sinon on part en cr�ation de l'OF
		if ($idSubFactory == 0) {
			$objectsf = new Factory($db);
			...
			$objectsf->duration_planned=(GETPOST('workload-'.$fk_product.'hour')?GETPOST('workload-'.$fk_product.'hour')*3600:0);
			$objectsf->duration_planned+=+(GETPOST('workload-'.$fk_product.'min')?GETPOST('workload-'.$fk_product.'min')*60:0);
	...
	// une fois que c'est terminé, on affiche les commandes fournisseurs cr�e
	// on crée les commandes et on les listes sur l'écran
	// une fois que c'est terminé, on affiche la ou les commandes fournisseurs cr�e
	if (count($tblCmdeFourn) == 1) {
		// on cr�e les commandes et on les listes sur l'écran
		header("Location: ".DOL_URL_ROOT."/fourn/commande/card.php?id=".$idCmdFourn);
	} else {
		// on cr�e les commandes et on les listes sur l'écran
		header("Location: ".DOL_URL_ROOT."/fourn/commande/list.php?search_refsupp=".GETPOST("reforderfourn"));
	...
	if ($lgncomponent->nbcomposed > 0) {
		...
		foreach($lgncomponent->composedby as $currentcomponent) {
			...
			foreach ($tblRestockNew as $lgncomponentpresent) {
				// si on a d�j� r�f�renc� le produit on l'ajoute
...
//					print 'updt'.$currentcomponent[0]."-".$currentcomponent[3].'('.$currentcomponent[1].')<br>';
			...
			// si le produit n'est pas pr�sent dans le tableau on le rajoute
			if (! $btrouve) {
