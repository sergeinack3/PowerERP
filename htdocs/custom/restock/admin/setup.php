...
if ($action == 'setvalue') {
	...
	// gestion de la transformation apr�s remise de la commande client en commande fournisseur
	powererp_set_const(
	...
	// select le type de produit s�lectionn�
	powererp_set_const(
					$db, "RESTOCK_PRODUCT_TYPE_SELECT", GETPOST('producttypeselect', 'int'), 
					'chaine', 0, '', $conf->entity
	);

	// Gestion de la cr�ation automatique des commandes clients
