...
if ($action == 'create') {
	...
	print load_fiche_titre($langs->trans("NewEntrepot"), '', 'object_' . $object->picto);
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("Entrepot"), '', 'object_' . $object->picto);
...
if ($action == 'create' || $action == 'edit') {
	print '
		<script> 
			const user = document.querySelector(".field_fk_user");
			const capacite = document.querySelector(".field_capacite");
			const type = document.getElementById("type");

			document.addEventListener("click", (e) => {
				if (type.value == 1) {
					capacite.style.display = "table-row";
					user.style.display = "none";
				} else if (type.value == 2) {
					user.style.display = "table-row";
					capacite.style.display = "none";
				}
			});
		</script>
	';
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	$head = entrepotPrepareHead($object);
	print dol_get_fiche_head($head, 'card', $langs->trans("Entrepot"), -1, $object->picto);

	$formconfirm = '';

	// Confirmation to delete
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteEntrepot'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' . dol_buildpath('/localisation/entrepot_list.php', 1) . '?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	// 		$filedir = $conf->localisation->dir_output . '/' . $object->element . '/' . $objref;
	// 		$urlsource = $_SERVER["PHP_SELF"] . "?id=" . $object->id;
	// 		$genallowed = $permissiontoread; // If you can read, you can build the PDF to read content
	// 		$delallowed = $permissiontoadd; // If you can create/edit, you can remove a file on card
	// 		print $formfile->showdocuments('localisation:Entrepot', $object->element . '/' . $objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
	// 	}

	// 	// Show links to link elements
	// 	$linktoelem = $form->showLinkToObjectBlock($object, null, array('entrepot'));
	...
	// 	$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-list-alt imgforviewmode', dol_buildpath('/localisation/entrepot_agenda.php', 1) . '?id=' . $object->id);
	...
	$diroutput = $conf->localisation->dir_output;
	$trackid = 'entrepot' . $object->id;
