...
/**
 * \file    test/phpunit/LocalisationFunctionalTest.php
...
/**
 * Class LocalisationFunctionalTest
 *
 ...
 *
 * @package Testlocalisation
 */
class LocalisationFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modLocalisation')->click();
		} else {
			// Disable the module
			$this->byHref('modLocalisation')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/localisation/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('localisation/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/localisation/admin/about.php');
		$this->authenticate();
		return $this->assertContains('localisation/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('localisationwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modLocalisation_LocalisationTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modLocalisation_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
