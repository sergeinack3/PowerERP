	...
	if ($object->status == $object::STATUS_DRAFT) {
			...
			while ($i < count($All_book)) {
				...
				if (isset($_GET['idEdit']) && $_GET['idEdit'] == $All_book[$i]->rowid) {
					...
					print '	<tr class="oddeven">
						<td class="left"><a href="conteneur_card.php?id=' . $All_book[$i]->rowid . '&save_lastsearch_values=1"> ' . $object->getRefConteneur($All_book[$i]->conteneur) . '</a></td>
		...
		if (empty($All_book)) {
			...
			while ($l < count($All_book)) {
				...
				print '	<tr class="oddeven">
						<td class="left"><a href="conteneur_card.php?id=' . $All_book[$l]->rowid . '&save_lastsearch_values=1"> ' . $object->getRefConteneur($All_book[$l]->conteneur) . '</a></td>
						<td class="center">' . date_format($date, 'd M Y') . '</td>
