...
/**
 *  \file       transport_contact.php
 *  \ingroup    parcautomobile
 *  \brief      Tab for contacts linked to Transport
 */
...
$hookmanager->initHooks(array('transportcontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->parcautomobile->transport->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Transport')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Transport"), -1, $object->picto);
