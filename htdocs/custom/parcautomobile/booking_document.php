...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->parcautomobile->multidir_output[$object->entity ? $object->entity : $conf->entity]."/booking/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->rights->parcautomobile->booking->write; // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
}
...
if ($object->id) {
	...
	$permissiontoadd = 1;
	//$permtoedit = $user->rights->parcautomobile->booking->write;
	$permtoedit = 1;
	$param = '&id='.$object->id;
