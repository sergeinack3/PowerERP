	...
	protected function loadDefaultActions()
	{
		if (! $this->disable_default_actions)
		{
			...
			else if ($action == 'updateMask')
			{
				...
				if ($maskconst) $res = powererp_set_const($db, $maskconst, $mask,'chaine', 0, '', $conf->entity);
			...
			else if ($action == 'setmod')
			{
				...
				powererp_set_const($db, $this->num_model_const_name, $value, 'chaine', 0, '', $conf->entity);
			}
			...
			else if ($action == 'deldoc')
			{
				...
				{
					powererp_del_const($db, $this->doc_model_const_name, $conf->entity);
				}
			}
			...
			else if ($action == 'setdefaultdoc')
			{
				...
				if (powererp_set_const($db, $this->doc_model_const_name, $value, 'chaine', 0, '', $conf->entity))
				{
