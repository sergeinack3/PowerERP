...
class Purchase extends CommonObject
{
    ...
    */
    ...
    /**
     *    \brief      Remove a supplier price for a product in this purchase (optionally it removes too the price on PowerERP)
                ...
                foreach($p['prices'] as $ii => $pr){
                    if ($ii == $vars['pr_ii']){
                        if ($vars['action'] == 'del_line_supp_and_price'){
                            // delete also the record ond PowerERP of this price of this supplier for this product
    ...
    function saveQuotationForm()
    {
        ...
        // == save data regarding to products on supplier record price for each product (product_fournisseur_price, PowerERP table)
