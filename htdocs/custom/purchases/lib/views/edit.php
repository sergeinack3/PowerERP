...
****************************************************/
    ...
    // == load orders linked to this purchase & count how many products there are without being ordered
        ...
        if (count($ordersIDs) > 0){
            ...
            if ($resql) {
                while($row = $resql->fetch_assoc()){ $orders[$row['rowid']] = $row;
                }
            }
        }
...
<script>
    ...
    function js_request_preview(){
        ...
        /* = find products marked to be ordered now = */
            ...
            if (Object.keys(product_order_list).length == 0){
                alert("<?= str_replace('"','',$langs->trans("purchasesInfoMsg04")) ?>");
                return;
            }
    ...
    * so it's like a preview before to create the order. In fact, clicking "create order" this module will create an standard PowerERP order, in draft mode
     */
    ...
    function js_delete_purchase(){
        if (confirm("<?= str_replace('"','',html_entity_decode($langs->trans('purchasesDelSure','',0))) ?>")){
    ...
    function js_del_line_supp(pid,pr_ii){
        ...
        /* == modal dialog box asking about delete the price from PowerERP == */
	$("#debug")
