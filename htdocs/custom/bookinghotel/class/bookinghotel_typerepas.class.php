...
class bookinghotel_typerepas extends Commonobject{
	...
	public function select_all_bookinghotel_typerepas($selected=0,$name='select_',$showempty=1,$val="rowid",$opt="label",$id='',$attr='',$action="",$list=false){
	    global $conf;
	    $moreforfilter = '';
	    $nodatarole = '';
	    $id = (!empty($id)) ? $id : $name;
	    ...
	    $moreforfilter.='<select onchange="ShowNotesReservRepas()" width="100%" '.$attr.' class="flat" id="select_'.$id.'" name="'.$name.'" '.$nodatarole.' required>';
	    if ($showempty) $moreforfilter.='<option value="0">&nbsp;</option>';

		$this->db->begin();


    	$sql = "SELECT ".$val.",".$opt.",notes FROM ".MAIN_DB_PREFIX.get_class($this);
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				$moreforfilter.='<option value="'.$obj->$val.'" data-notes="'.addslashes($obj->notes).'"';
