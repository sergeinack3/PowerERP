...
$arrayofparameters = array(
    'libelle'=>array('type'=>'string' ,'enabled'=>1),
    'description'=>array('type'=>'html' ,'enabled'=>1),
    'Date_start'=>array('type'=>'datetime' ,'enabled'=>1),
    'Date_end'=>array('type'=>'datetime' ,'enabled'=>1),
    // 'payrollmad_state'=>array('type'=>'yesno' ,'enabled'=>1),
    //'PAYROLLMOD_MYPARAM3'=>array('type'=>'category:'.Categorie::TYPE_CUSTOMER, 'enabled'=>1),
    //'PAYROLLMOD_MYPARAM4'=>array('type'=>'emailtemplate:thirdparty', 'enabled'=>1),
    //'PAYROLLMOD_MYPARAM5'=>array('type'=>'yesno', 'enabled'=>1),
    //'PAYROLLMOD_MYPARAM5'=>array('type'=>'thirdparty_type', 'enabled'=>1),
    //'PAYROLLMOD_MYPARAM6'=>array('type'=>'securekey', 'enabled'=>1),
    //'PAYROLLMOD_MYPARAM7'=>array('type'=>'product', 'enabled'=>1),
);
...
// $sql = 'SELECT * FROM '.MAIN_DB_PREFIX.' payrollmod_session ';
// $req = $db->query($sql);
// num_rows($req);
// // $object = $db->fetch_object($sql);
// var_dump($object);die();
...
if ($action == 'updateMask') {
    ...
    if ($maskconstorder) {
        $res = powererp_set_const($db, $maskconstorder, $maskorder, 'chaine', 0, '', $conf->entity);
        if (!($res > 0)) {
            $error++;
        }
    }
    ...
    foreach ($dirmodels as $reldir) {
        $file = dol_buildpath($reldir."core/modules/payrollmod/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
        if (file_exists($file)) {
            $filefound = 1;
    ...
    if (!empty($tmpobjectkey)) {
        $constforval = 'PAYROLLMOD_'.strtoupper($tmpobjectkey)."_ADDON";
...
if($action == 'update'){
    ...
    (!empty(GETPOST('libelle'))) ? $_SESSION['libelle'] = GETPOST('libelle', 'alpha') : '';
    (!empty(GETPOST('description'))) ? $_SESSION['description'] = GETPOST('description', 'alpha') : '';
    (!empty(GETPOST('Date_start'))) ? $_SESSION['date_start'] = GETPOST('Date_start', 'alpha') : '';
    (!empty(GETPOST('Date_end'))) ? $_SESSION['date_end'] = GETPOST('Date_end', 'alpha') : '';

    header('Location: ./payrollmod_model.php');
    exit;
}
...
$modpayroll = new modPayrollmod($db);
...
print dol_get_fiche_head($head, 'Session de paye', $langs->trans($page_name), -1, "payrollmod@payrollmod");

// (!empty(GETPOST('payrollmad_state'))) ? $_SESSION['payrollmad_state'] = GETPOST('payrollmad_state', 'alpha') : '';

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("PayrollmodSetupPage").'</span><br><br>';
// $modpayroll->init();
    ...
    foreach ($arrayofparameters as $constname => $val) {
        if ($val['enabled']==1) {
            ...
            if($val['type'] == 'datetime'){
                print '<input type="date" required="required" id="'.$constname.'" name="'.$constname.'" value="'.(GETPOST($constname, 'alpha') ?GETPOST($constname, 'alpha') : $conf->global->{$constname}).'">';
            }

            if($val['type'] == 'string'){
                 print '<input name="'.$constname.'"  class="flat '.(empty($val['css']) ? 'minwidth200' : $val['css']).'" value="'.$conf->global->{$constname}.'" placeholder="Session-'.date("m-Y").'" >';
             }
            ...
            elseif ($val['type'] == 'yesno') {
                ...
                print '<input required="required" type="text" class="flat" id="'.$constname.'" name="'.$constname.'" value="'.(GETPOST($constname, 'alpha') ?GETPOST($constname, 'alpha') : $conf->global->{$constname}).'">';
                    ...
                    while (($file = readdir($handle)) !== false) {
                        if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
                            ...
                            if ($module->isEnabled()) {
                                ...
                                $constforvar = 'PAYROLLMOD_'.strtoupper($myTmpObjectKey).'_ADDON';
