...
/**
 * \file    test/phpunit/PayrollmodFunctionalTest.php
 * \ingroup payrollmod
...
/**
 * Class PayrollmodFunctionalTest
 *
 ...
 *
 * @package Testpayrollmod
 */
class PayrollmodFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modPayrollmod')->click();
		} else {
			// Disable the module
			$this->byHref('modPayrollmod')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/payrollmod/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('payrollmod/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/payrollmod/admin/about.php');
		$this->authenticate();
		return $this->assertContains('payrollmod/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('payrollmodwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modPayrollmod_PayrollmodTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modPayrollmod_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
