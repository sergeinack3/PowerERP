...
switch($action) {
    ...
    //retrive the data
        ...
        if( ($object->third_party == '-1') || ($object->task == '-1') || ($object->task == '-1') || ($object->project == '-1') ){

            setEventMessage('FieldsNotComplete', 'errors');
...
if(empty($conf->global->MAIN_DISABLE_FULL_SCANLIST)) {
    ...
    if($resql) {
        ...
        print '</td><td>';
        print '<input type = "submit" value = "'.$langs->trans('add').'" name="add">';
...
//                print "<td>".print_generic('third_party', 'rowid', $obj->fk_third_party, 'rowid', 'description')."</td>";
                ...
                print "<td>".($duration == '')?'':formatTime($duration, 0)."</td>";
    ...
    print '</form>'."\n";
    // new button
   print '<a href = "'.$_SERVER["PHP_SELF"].'?action=create" class="butAction"role="button">'.$langs->trans('New');
