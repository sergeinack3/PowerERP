...
/**
 *  \file       releve_contact.php
 *  \ingroup    station
 *  \brief      Tab for contacts linked to Releve
 */
...
$hookmanager->initHooks(array('relevecontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->station->releve->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Releve')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Releve"), -1, $object->picto);
