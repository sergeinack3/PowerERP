 ...
 * Class for Config
 */
class Config extends CommonObject
{
	/**
	 ...
	 */
	public $table_element = 'station_config';
	...
	//  */
	// public $table_element_line = 'station_configline';
	...
	//  */
	// protected $childtablesoncascade = array('station_configdet');
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->station->config->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->station->config->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->station->config->config_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('CONFIG_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'config/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'config/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->station->dir_output.'/config/'.$oldref;
				$dirdest = $conf->station->dir_output.'/config/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->station->dir_output.'/config/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'CONFIG_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'CONFIG_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'CONFIG_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Config").'</u>';
		...
		$url = dol_buildpath('/station/config_card.php', 1).'?id='.$this->id;
		...
		$hookmanager->initHooks(array('configdao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new ConfigLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_config = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->STATION_CONFIG_ADDON)) {
			$conf->global->STATION_CONFIG_ADDON = 'mod_config_standard';
		}

		if (!empty($conf->global->STATION_CONFIG_ADDON)) {
			$mybool = false;

			$file = $conf->global->STATION_CONFIG_ADDON.".php";
			$classname = $conf->global->STATION_CONFIG_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_config';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->CONFIG_ADDON_PDF)) {
				$modele = $conf->global->CONFIG_ADDON_PDF;
			}
		}
 ...
 */
class ConfigLine extends CommonObjectLine
{
	// To complete with content of an object ConfigLine
	// We should have a field rowid, fk_config and position
