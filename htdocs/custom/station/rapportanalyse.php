...
for ($i = 0; $i < count($chauffeurs); $i++) {
    ...
    $resql = $db->query($sql);
    $ChauffeurTransport[] = array($chauffeurs[$i]->name, $db->num_rows($resql));
...
print '<h2>&nbsp;STATISTIQUES DE TRANSIT</h2>';
    ...
    <div class="ponct-cont">
        ...
        <form action="./rapportanalyse.php" method="POST">
            <th style="display:flex; flex-direction: row; gap: 10px; padding-bottom: 20px;">
                <div>
                    Début &nbsp; <input name="date-debut-ponc" type="date" required value="<?= $date_debut_ponc ?>">
                </div>
                <div>
                    Fin &nbsp; <input name="date-fin-ponc" type="date" required value="<?= $date_fin_ponc ?>">
                </div>
                <div>
