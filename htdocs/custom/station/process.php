...
if (empty($reshook)) {
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '_ID_'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				$backtopage = $backurlforlist;
			} else {
				$backtopage = dol_buildpath('/parcautomobile/transport_card.php', 1) . '?id=' . ((!empty($id) && $id > 0) ? $id : '_ID_');
			}
		}
	}
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($object->type == 0) {
		...
		if (!$it || isset($_GET['changeCont'])) {
			...
			print '	<form action="' . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '" method="POST">
								...
								<tr class="oddeven">
									<td>
										<select name="conteneur" required>
											<option value="">Selectionner le conteneur</option>';
			$i = 0;
			while ($i < count($conteneur)) {
				if ($conteneur[$i]->conteneur == $it->conteneur) {
					print '					<option value="' . $conteneur[$i]->conteneur . '" selected>' . $conteneur[$i]->refCont . '</option>';
				} else {
			...
			print '			<input type="hidden" name="st_action" value="create_conteneur" />
								<tr class="oddeven">
									<td><a href="conteneur_card.php?id=' . $it->conteneur . '">' . $it->refCont . '</a></td>';
			if ($object->status == $object::STATUS_DRAFT) {
	...
	if (empty($produit)) {
		...
		while ($n < count($produit)) {
			print '	<tr class="oddeven"> 
						<td class="left"><a href="../../product/card.php?id=' . $produit[$n]->produit . '">' . $produit[$n]->RefProd . '</a></td>
	...
	if (!empty($object->table_element_line)) {
		...
		print '	<form name="addproduct" id="addproduct" action="' . $SERVER["PHP_SELF"] . '?id=' . $object->id . (($action != 'editline') ? '' : '#line' . GETPOST('lineid', 'int')) . '" method="POST">
