...
/**
 *  \file       partis_contact.php
 *  \ingroup    station
 *  \brief      Tab for contacts linked to Partis
 */
...
$hookmanager->initHooks(array('partiscontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->station->partis->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Partis')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Partis"), -1, $object->picto);
