...
$sql8 = "SELECT * FROM " . MAIN_DB_PREFIX . "user WHERE job = 'Chauffeur'";
$resql8 = $db->query($sql8);
while ($ite = $db->fetch_object($resql8)) {
    array_push($chauffeurs, array("id" => $ite->rowid, "name" => $ite->login));
}
$total3 = 0;

foreach ($chauffeurs as $item) {
    $sql7 = "SELECT * FROM " . MAIN_DB_PREFIX . "parcautomobile_transport WHERE chauffeur_id = " . $item['id'];
    $resql7 = $db->query($sql7);
    array_push($ChauffeurData, array($item['name'], round($resql7->num_rows)));
    $total3 += $resql7->num_rows;
}
...
<script>
    window.onload = function() {
        var chartChauffeur = new CanvasJS.Chart("ChauffeurContainer", {
            animationEnabled: true,
            theme: "light2",
            title: {
                text: "Voyages effetues par les chaffeurs"
            },
            axisY: {
            ...
            data: [{
                type: "column",
                yValueFormatString: "# voyages",
                dataPoints: <?php echo json_encode($ChauffeurData, JSON_NUMERIC_CHECK); ?>
            }]
        });
        chartChauffeur.render();
    }
</script>
...
<div class="ctn-bord">
    ...
    print '<div style="display:flex; flex-wrap: wrap; gap: 40px;"><div class="flex:1"><div class="div-table-responsive-no-min">
            <table class="noborder centpercent">
            <tbody> 
                <tr class="liste_titre">
                    <th>Moyenne de l`etat des vehicules</th>
                </tr>
                <tr>
                    <td>';
    ...
    $total1 = 0;
    $VehiculesEtat[] = array("Vehicules neuf", round($nombre_veh_neuf));
    $VehiculesEtat[] = array(("Vehicule en bon etat"), round($nombre_veh_bonetat));
    $VehiculesEtat[] = array(("Vehicule endommage"), round($nombre_veh_endommage));
    $VehiculesEtat[] = array(("Vehicule en reparation"), round($nombre_veh_reparation));
    $VehiculesEtat[] = array(("Vehicule detruit"), round($nombre_veh_detruite));
    ...
    print '<div class="flex:1"><div class="div-table-responsive-no-min">
            ...
            <tbody> 
                <tr class="liste_titre">
                    <th>Moyenne de la disponibilite des vehicules</th>
                </tr>
                <tr>
                    <td>';
    $total2 = 0;
    $VehiculesDispo[] = array("Vehicules disponible", round($nombre_veh_dispo));
    $VehiculesDispo[] = array(("Vehicule non disponible"), round($nombre_veh_nondispo));
    ...
    $dolgraph2->setHeight('200');
    $dolgraph2->draw('jojob');
    print $dolgraph2->show($total2 ? 0 : 1);
    print ' </td></tr></tbody></table></div></div><br>';

    ///////////////////////// Troisieme stat

    print '<div class=""><div class="div-table-responsive-no-min">
            <table class="noborder centpercent">
            <tbody> 
                <tr class="liste_titre">
                    <th>Moyennes des voyages des chauffeurs</th>
                </tr>
                <tr>
                    <td>';

    $dolgraph3 = new DolGraph();
    $dolgraph3->SetData($ChauffeurData);
    $dolgraph3->setShowLegend(0);
    $dolgraph3->setShowPercent(0);
    $dolgraph3->SetType(array('bars'));
    $dolgraph3->setHeight('200');
    $dolgraph3->draw('demandeNumber');
