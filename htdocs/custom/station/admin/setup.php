...
$arrayofparameters = array(
	'STATION_VOLUMECRITIQUE' => array('type' => 'string', 'css' => 'minwidth500', 'enabled' => 1),
	'STATION_POURCENTAGEMAX' => array('type' => 'string', 'css' => 'minwidth500', 'enabled' => 1),
...
if ($useFormSetup && (float) DOL_VERSION >= 15) {
	...
	$item = $formSetup->newItem('STATION_VOLUMECRITIQUE');
...
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "station@station");
...
print_barre_liste($langs->trans("conf_tiers"), $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'bank-building.png', 0, $newcardbutton, '', $limit, 0, 0, 1);
...
print '<td>Entité</td>';
print '<td class="center">' . $langs->trans("pourcentage") . '</td>';
print '<td class="center">' . $langs->trans("Statut") . '</td>';
print "</tr>\n";

//Display partis
$sqlConfig = 'SELECT * FROM ' . MAIN_DB_PREFIX . 'station_partis';
...
if ($num > 0) {
	while ($config = $db->fetch_object($resqlConfig)) {
		...
		$sqlEnt = 'SELECT * FROM ' . MAIN_DB_PREFIX . 'societe WHERE rowid = ' . $entite;
		...
		// var_dump($status);
		if ($status == 1) {
			$statusTitle = $langs->trans("Actif");
		} elseif ($status == 5) {
			$statusTitle = $langs->trans("Configured");
		} elseif ($status == 9) {
			$statusTitle = $langs->trans("Disabled");
		} else {
			$statusTitle = $langs->trans("Draft");
		}
		// $statusTitle = ($status == 1) ? "Actif" : "Brouillon";
		// $statusTitle1 = ($status == 1) ? "Ouvert" : " Clos";

		print '<tr class="oddeven">';
		print 		'<td class="nowrap">';
		print  			'<a href="' . DOL_URL_ROOT . '/custom/station/partis_card.php?id=' . $rowid . '&save_lastsearch_values=1" alt="" class="inline-block">';
		print			'<img src="' . DOL_URL_ROOT . '/custom/station/img/object_partis.png" class="paddingright classfortooltip">' . $ref;
		print			'</a></td>';
		print 		'<td class="tdoverflowmax200">';
		print  			'<a href="' . DOL_URL_ROOT . '/societe/card.php?socid=' . $rowid . '&save_lastsearch_values=1" class="classfortooltip refurl">';
	...
	$Tot = $db->fetch_array($resqlTot);
	print '
		<tr class="liste_total">
			<td>' . $langs->trans("Total") . '</td>
			<td></td>
			<td class="center">' . $Tot["Total_pourcentage"] . '</td>
			<td></td>
		</tr>';
} else {
	print "<tr><td colspan='4'> Aucun enregistrement trouvé. </td></tr>";
}
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_' . strtolower($myTmpObjectKey) . '_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'STATION_' . strtoupper($myTmpObjectKey) . '_ADDON';
