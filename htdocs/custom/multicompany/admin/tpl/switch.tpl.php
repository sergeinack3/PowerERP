		...
		$.get( "<?php echo dol_buildpath('/multicompany/core/ajax/functions.php',1); ?>", {
			...
			function (result) {
				if (result == 1) {
					...
					//var switch_off = '<?php echo img_picto('','off','','',true); ?>';
					//button.attr('src', switch_off).trigger('load');
					button.attr('title', title).attr('alt', title);
					button.removeClass('fa-check-square-o multicompany-button-' + type + '-on');
					button.addClass('fa-square-o multicompany-button-' + type + '-off');
					if (type == 'active') {
						//$('#visible_' + id).attr('src', switch_off).trigger('load');
						$('#visible_' + id).attr('title', title).attr('alt', title);
						$('#visible_' + id).removeClass('fa-check-square-o multicompany-button-visible-on');
						$('#visible_' + id).removeClass('multicompany-button-visible-off');
						$('#visible_' + id).addClass('fa-square-o multicompany-button-disabled');
					}
				} else {
		...
		$.get( "<?php echo dol_buildpath('/multicompany/core/ajax/functions.php',1); ?>", {
			...
			function (result) {
				if (result == 1) {
					...
					//var switch_on = '<?php echo img_picto('','on','','',true); ?>';
					//button.attr('src', switch_on).trigger('load');
					button.attr('title', title).attr('alt', title);
					button.removeClass('fa-square-o multicompany-button-' + type + '-off');
					button.addClass('fa-check-square-o multicompany-button-' + type + '-on');
					if (type == 'active') {
						$('#visible_' + id).removeClass('fa-check-square-o multicompany-button-disabled');
						$('#visible_' + id).addClass('fa-square-o multicompany-button-visible-off');
					}
				} else {
