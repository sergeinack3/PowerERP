...
if(is_array($list_tables))
{
	...
	foreach($list_tables as $table)
	{
		...
		{
			$list_champ = $this->db->DDLInfoTable($table);
				...
				if(is_array($champ) && in_array('entity',$champ))
				{
					...
					if ($resql)
					{
						...
						else
						{
							print "... [KO !!]\n;";
							$error++;
							$errorcode = dol_print_error($this->db);
						}
					}
					else
					{
	...
	$sql.= " WHERE entity = " . $entity_new_master;
	dol_syslog($script_file." sql=".$sql, LOG_DEBUG);
	$resql=$this->db->query($sql); //echo $sql."\n";
	if ($resql)
	{
		$i = 0;
		...
		while ($i < $num)
		{
			...
			$sql2.= " AND entity = 1";
			$resql2=$this->db->query($sql2); //echo $sql2."\n";
			if (!$resql2)
			{
			...
			$resql3=$this->db->query($sql3); //echo $sql3."\n";
			if (!$resql3)
			{
