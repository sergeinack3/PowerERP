...
class ActionsMulticompany
{
	 ...
	 *	Constructor
	 *
	 *	@param	PowerDB	$db		Database handler
	 */
...
*/
		...
		if (is_array($this->dao->entities))
		{
			...
			foreach ($this->dao->entities as $entity)
			{
				if ($entity->active == 1 && ($entity->visible == 1 || ($user->admin && ! $user->entity)))
				{
					...
					if (! empty($user->login) && ! empty($this->conf->global->MULTICOMPANY_TRANSVERSE_MODE) && ! empty($user->entity) && $this->checkRight($user->id, $entity->id) < 0) continue;
	...
	public function checkRight($id, $entity)
	{
		...
		if ($this->dao->fetch($entity) > 0)
		{
			...
			if ($this->dao->verifyRight($entity, $id) || $this->user->admin)
			{
				return 1;
			}
			else
	...
	private function getTopRightMenu()
	{
		...
		if (($level = checkMultiCompanyVersion()) !== 0)
		{
			$text = $langs->trans("MultiCompanyUpgradeIsNeeded");
			if ($level === 1) $text = $langs->trans("MultiCompanyIsOlderThanPowerERP");
			if ($level === -1) $text = $langs->trans("PowerERPIsOlderThanMulticompany");

			$msg = get_htmloutput_mesg(img_warning('default') . ' ' . $text, '', 'mc-upgrade-alert', 1);
			$out.= '
			<script type="text/javascript">
			$(document).ready(function() {
