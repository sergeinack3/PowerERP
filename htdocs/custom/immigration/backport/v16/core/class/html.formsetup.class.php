...
class FormSetupItem
{
	...
	public function saveConfValue()
	{
		...
		if ($this->type != 'title') {
			$result = powererp_set_const($this->db, $this->confKey, $this->fieldValue, 'chaine', 0, '', $this->entity);
			if ($result < 0) {
				return -1;
			} else {
				return 1;
	...
	public function generateInputFieldHtml()
	{
		...
		$doleditor = new DolEditor($this->confKey, $this->fieldValue, '', 160, 'powererp_notes', '', false, false, $conf->fckeditor->enabled, ROWS_5, '90%');
