...
/**
 *  \file       cat_procedures_note.php
 *  \ingroup    immigration
 *  \brief      Tab for notes on Cat_procedures
 */
...
$langs->loadLangs(array("immigration@immigration", "companies"));
...
$hookmanager->initHooks(array('cat_proceduresnote', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->immigration->multidir_output[!empty($object->entity) ? $object->entity : $conf->entity]."/".$object->id;
}
...
$help_url = '';
$title = $langs->trans('Cat_procedures').' - '.$langs->trans("Notes");
...
if ($id > 0 || !empty($ref)) {
	...
	print dol_get_fiche_head($head, 'note', $langs->trans("Cat_procedures"), -1, $object->picto);
