...
/**
 *  \file       procedures_contact.php
 *  \ingroup    immigration
 *  \brief      Tab for contacts linked to Procedures
 */
...
$hookmanager->initHooks(array('procedurescontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->immigration->procedures->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Procedures')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Procedures"), -1, $object->picto);
