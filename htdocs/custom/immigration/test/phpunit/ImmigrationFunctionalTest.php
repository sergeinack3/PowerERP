...
/**
 * \file    test/phpunit/ImmigrationFunctionalTest.php
 * \ingroup immigration
...
/**
 * Class ImmigrationFunctionalTest
 *
 ...
 *
 * @package Testimmigration
 */
class ImmigrationFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modImmigration')->click();
		} else {
			// Disable the module
			$this->byHref('modImmigration')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/immigration/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('immigration/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/immigration/admin/about.php');
		$this->authenticate();
		return $this->assertContains('immigration/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('immigrationwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modImmigration_ImmigrationTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modImmigration_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
