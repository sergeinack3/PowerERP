 ...
 */
function proceduresPrepareHead($object)
{
	...
	$head[$h][0] = dol_buildpath("/immigration/procedures_card.php", 1).'?id='.$object->id;
	$head[$h][1] = $langs->trans("Card");
	$head[$h][2] = 'card';
	$h++;

	if ($object->status != $object::STATUS_CANCELED){
		$head[$h][0] = dol_buildpath("/immigration/procedures_documents_card.php", 1).'?id='.$object->id;
		$head[$h][1] = $langs->trans("Documentations");
		$head[$h][2] = 'documents';
		$h++;
	}
	...
	if ($showtabofpagecontact) {
		$head[$h][0] = dol_buildpath("/immigration/procedures_contact.php", 1).'?id='.$object->id;
