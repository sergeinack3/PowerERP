...
/**
 *  \file       cat_procedures_contact.php
 *  \ingroup    immigration
 *  \brief      Tab for contacts linked to Cat_procedures
 */
...
$hookmanager->initHooks(array('cat_procedurescontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->immigration->cat_procedures->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Cat_procedures')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Cat_procedures"), -1, $object->picto);
