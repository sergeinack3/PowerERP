...
/**
 *  \file       procedures_document.php
 *  \ingroup    immigration
 *  \brief      Tab for documents linked to Procedures
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->immigration->multidir_output[$object->entity ? $object->entity : $conf->entity]."/procedures/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'document', $langs->trans("Procedures"), -1, $object->picto);
	...
	$permissiontoadd = 1;
	//$permtoedit = $user->rights->immigration->procedures->write;
	$permtoedit = 1;
	$param = '&id='.$object->id;

	//$relativepathwithnofile='procedures/' . dol_sanitizeFileName($object->id).'/';
	$relativepathwithnofile = 'procedures/'.dol_sanitizeFileName($object->ref).'/';
