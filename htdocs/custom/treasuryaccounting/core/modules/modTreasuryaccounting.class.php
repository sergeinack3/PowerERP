...
/**
 * 	\defgroup   Treasury accounting     Module treasury_accounting
 *  \file       treasuryaccounting/core/modules/modTreasuryaccounting.class.php
...
class modTreasuryaccounting extends PowererpModules
{
	...
	public function __construct($db)
	{
        ...
        $this->description = "Description of module Treasury accounting";
		...
		$this->config_page_url = array("setup.php@treasuryaccounting");

		// Dependencies
		$this->hidden = false;			// A condition to hide module
		$this->depends = array("modAccounting");		// List of modules id that must be enabled if this module is enabled
		...
		// );
		$this->const = array(
            0 => array('ACCOUNTANCY_ENABLE_FKDOCDET','chaine','1','',0,'current',0),
            1 => array('MODULE_TREASURYACCOUNTING','chaine',$this->version,'',0,'allentities',0)
        );
		...
		if (! isset($conf->treasuryaccounting) || ! isset($conf->treasuryaccounting->enabled))
        {
        	$conf->treasuryaccounting=new stdClass();
        	$conf->treasuryaccounting->enabled=0;
        }
        ...
        */
        ...
        if ((int) DOL_VERSION > 9.0) {
            $leftmenu = "transfer";
        } elseif ($conf->expensereport->enabled) {
            $leftmenu = "dispatch_expensereport";
        } elseif ($conf->fournisseur->enabled) {
            $leftmenu = "dispatch_supplier";
        } else {
            $leftmenu = "dispatch_customer";
        }

        $this->menu[$r]=array('fk_menu'=>'fk_mainmenu=accountancy,fk_leftmenu='.$leftmenu,
            'type'=>'left',
            'titre'=>'MenuAccountancyTreasuryJournal',
            'mainmenu'=>'',
            'leftmenu'=>'',
            'url'=>'/treasuryaccounting/index.php',
            'langs'=>'treasuryaccounting@treasuryaccounting',
            'position'=>100,
	...
	public function init($options='')
	{
        ...
        $this->_load_tables('/treasuryaccounting/sql/');
