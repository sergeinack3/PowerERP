...
if ($action == 'create') {
	...
	// foreach($fk_product as $product){
	// 	var_dump($product);
	// }


	// var_dump($object->try_Isset());
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($object->status == $object::STATUS_USED && $permissiontoread) {
		...
		if (isset($new_periode) && !empty($new_periode)) {
			print '<td class="center">';
			print '<span class="opacitymedium">La périodicité de votre amortissement est estimé à : ' . $new_periode . ' ' . $type_periode . '</span>';
			print '</td>';
		} else {
			print '<td>';
			print '<span class="opacitymedium">Aucune</span>';
			print '</td>';
		}
			...
			if ($permissiontoadd || ($permissiontoadd && $permissiontoputinapproval)) {
				...
				if ($permissiontoadd) {
					if ($object->status == $object::STATUS_VALIDATED) {
						...
						if ($permissiontoconsumption && $object->fk_user_trans == (int) $user->id) {
							print '<a class="butAction relative_div_" href="' . dol_buildpath('/immobilisations/views/immobilisations_card.php', 1) . '?id=' . $object->id . '&amp;action=consumption&amp;token=aa9c7b4cde4c5fb45500d69c2c86b843" title="' . $langs->trans('initConsumption') . '">' . $langs->trans('initConsumption') . '</a>';
						} else {
				...
				if ($permissiontoadd) {
					...
					if ($object->status == $object::STATUS_USED) {
						...
						if ($permissiontoaccount) {
							print '<a class=" butAction relative_div_" href="' . dol_buildpath('/immobilisations/views/immobilisations_echeancier.php', 1) . '?id=' . $object->id . '" title="' . $langs->trans('accountWritingn') . '">' . $langs->trans('accountacy') . '</a>';
						}
					}
				}
