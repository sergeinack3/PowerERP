...
/**
 *  \file       immobilisations_contact.php
 *  \ingroup    immobilisations
 *  \brief      Tab for contacts linked to Immobilisations
 */
...
$hookmanager->initHooks(array('immobilisationscontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->immobilisations->immobilisations->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Immobilisations')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Immobilisations"), -1, $object->picto);
