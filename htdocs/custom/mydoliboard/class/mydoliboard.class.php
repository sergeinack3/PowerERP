...
class Mydoliboard extends CommonObject
{
	...
	function fetch($rowid)
	{
		...
		$sql = "SELECT label, description, paramfields, mainmenu, leftmenu, elementtab";
		$sql.= " , xlstemplate, perms, titlemenu, langs, author, active";
		...
		if ($resql) {
			if ($this->db->num_rows($resql) > 0) {
				...
				$this->xlstemplate	= $res['xlstemplate'];
	...
	function create($user='')
	{
		...
		$this->xlstemplate = trim($this->xlstemplate);
		...
		$sql.= ", ".(! isset($this->xlstemplate)?'NULL':"'".$this->db->escape($this->xlstemplate)."'");
	...
	{
		global $langs, $powererp_main_db_prefix;
		...
		if ($resql) {
			...
			while ($rec = $this->db->fetch_array($resql)) {
				...
				if ($powererp_main_db_prefix != 'llx_')
					$sqlquerydisp= " ".preg_replace('/llx_/i', $powererp_main_db_prefix, $sqlquerydisp);
				...
				if (!empty($this->elementtab) && $this->idreftab != "") {
					switch($this->elementtab) {
						case 'Societe' :
							$sqlquerydisp=str_replace("#FILTERTAB#", " AND s.rowid=".$this->idreftab, $sqlquerydisp);
							break;
					...
					while ($rec = $this->db->fetch_object($resboardsql)) {
						...
						foreach ($rec as $valfield) {
							...
							if ($tabletype[$nbcol]==1) { 
								...
								// date
								$sz.="<td align=center width='70px' >";
								$sz.=dol_print_date($this->db->jdate($valfield), 'day')."</td>";
								...
								// all the other type
								$sz.="<td align=center width=".$width."% title='type=".$tabletype[$nbcol]."'>".$valfield."</td>";
							}
	...
	function genboardXLS($bloc, &$excelobj)
	{
		global $langs, $powererp_main_db_prefix;
		// récupération des requetes de ce bloc
		$sql = "SELECT * FROM ".MAIN_DB_PREFIX."mydoliboardsheet";
		...
		if ($resql) {
			...
			while ($rec = $this->db->fetch_array($resql)) {
				if ($sheetCount > 0) {
					$arraysheets = $excelobj->getAllSheets();
					$btrouve=false;
					foreach ($arraysheets as $tmpsheet) {
						if ($tmpsheet->getTitle() == $rec['exportsheet']) {
							$btrouve=true;
							$objWorkSheet = $tmpsheet;
						}
					}
					if (!$btrouve) {
						// si on doit créer l'onglet car il n'existe pas
						$objWorkSheet = $excelobj->createSheet($sheetCount);
						$objWorkSheet->setTitle($rec['exportsheet']?$rec['exportsheet']:$rec['titlesheet']);
					}
				} else {
					// si on doit créer l'onglet car il n'existe pas
					$objWorkSheet = $excelobj->createSheet($sheetCount);
				...
				if ($num>0) {
					// boucle sur les colonnes
					$nbcol = ($rec['startexportcol'] >0 ? $rec['startexportcol'] : 0 );
					$nblgn = ($rec['startexportrow'] >0 ? $rec['startexportrow'] : 1 );

					while ($finfo = $resboardsql->fetch_field()) {
						// sur la premiere ligne les entetes
						$objWorkSheet->setCellValueByColumnAndRow($nbcol, $nblgn, $langs->trans($finfo->name));

						$tabletype[$nbcol]=$finfo->type;
						$nbcol++;
					}
					$nblgn++;
					while ($rec2 = $this->db->fetch_object($resboardsql)) {
						// boucle sur les colonnes
						$nbcol=($rec['startexportcol'] >0 ? $rec['startexportcol'] : 0 );
						foreach ($rec2 as $valfield ) {
							// boolean
							if ($tabletype[$nbcol]==1) { 
								$objWorkSheet->setCellValueByColumnAndRow($nbcol, $nblgn, yn($valfield));
							} elseif ($tabletype[$nbcol]==10) {
								$objWorkSheet->setCellValueByColumnAndRow($nbcol, $nblgn, dol_print_date($this->db->jdate($valfield), 'day'));
							} elseif ($tabletype[$nbcol]==253) {
								// text
								$objWorkSheet->setCellValueByColumnAndRow($nbcol, $nblgn, $valfield);
							} elseif ($tabletype[$nbcol]==5 || $tabletype[$nbcol]==3 || $tabletype[$nbcol]==246) {
								// numérique
								$objWorkSheet->setCellValueByColumnAndRow($nbcol, $nblgn, round($valfield, 2));
							} else { 
								$objWorkSheet->setCellValueByColumnAndRow($nbcol, $nblgn, $valfield, 2);
							}
							$nbcol++;
						}
						$nblgn++;
	...
	{
		global $langs, $conf, $powererp_main_db_prefix;
		...
		if ($resql) {
			...
			for ($i=0; $i < $nbcol; $i++) {
				...
				for ($j=0; $j < $ligne; $j++) {
					...
					elseif ($headertype[$i]==5 || $headertype[$i]==8 || $headertype[$i]==3 || $tabletype[$nbcol]==246) {
						// numérique
	...
	function update($user='')
	{
		...
		$sql.= " WHERE name ='".$this->db->encrypt('MAIN_MODULE_MYDOLIBOARD_TABS_'.$this->rowid, 1)."'";
		$this->db->query($sql);
		...
		$sql .= ", xlstemplate ='".$this->db->escape($this->xlstemplate)."'";
		...
		if ($this->db->query($sql)) {
			...
			if ($this->active) {
				...
				if ($this->elementtab) {
					...
					$tabinfo.=':+mydoliboard_'.$this->rowid.':'.$this->titlemenu;
					$tabinfo.=':@mydoliboard:/mydoliboard/mydoliboard.php?idboard='.$this->rowid.'&id=__ID__';
...
//print $sql;
	...
	{
		$sql = "SELECT rowid, label, description, perms, langs, titlemenu,";
		$sql.=" mainmenu, leftmenu, elementtab, author, active, paramfields";
		$sql.=" FROM ".MAIN_DB_PREFIX."mydoliboard";
		...
		if ($res) {
			...
			while ($rec = $this->db->fetch_array($res)) {
				...
				$cat['xlstemplate']	= $rec['xlstemplate'];
				$cat['perms']		= $rec['perms'];
				$cat['langs']		= $rec['langs'];
	...
	function getSheetsArray( $pageid=0 )
	{
		...
		if ($res) {
			...
			while ($rec = $this->db->fetch_array($res)) {
				...
				$cat['exportsheet']		= $rec['exportsheet'];
				$cat['startexportcol']	= $rec['startexportcol'];
				$cat['startexportrow']	= $rec['startexportrow'];
	...
	function getexporttable($code)
	{
		...
		$tmp.="<xlstemplate>".$this->xlstemplate."</xlstemplate>\n";
		...
		foreach ($this->listsUsed as $key=> $value ) {
			...
			$tmp.="\t \t<exportsheet>".$value['exportsheet']."</exportsheet>\n";
			$tmp.="\t \t<startexportcol>".$value['startexportcol']."</startexportcol>\n";
			$tmp.="\t \t<startexportrow>".$value['startexportrow']."</startexportrow>\n";		
	...
	function importlist($xml)
	{
		...
		if ($sxe === false) {
			print  "Erreur lors du chargement du XML\n";
		...
		$this->xlstemplate=	$arraydata['xlstemplate'];
		...
		if ($result) {
			...
			if (count($tblsheets) >0)
				foreach ($tblsheets as $sheet) {
					...
					$newsheet->exportsheet		= $sheet['exportsheet'];
					$newsheet->startexportcol	= $sheet['startexportcol'];
					$newsheet->startexportrow	= $sheet['startexportrow'];
...
//				break;
	...
	function fetch($rowid)
	{
		...
		$sql = "SELECT fk_mdbpage, titlesheet, description, displaycell, cellorder,";
		$sql.= " perms, langs, author, active, querymaj, querydisp, graphtype,";
		$sql.= " exportsheet, startexportcol, startexportrow";
		$sql.= " FROM ".MAIN_DB_PREFIX."mydoliboardsheet";
		$sql.= " WHERE rowid = '".$rowid."'";
		...
		if ($resql) {
			if ($this->db->num_rows($resql) > 0) {
				...
				$this->exportsheet		= $res['exportsheet'];
				$this->startexportcol	= $res['startexportcol'];
				$this->startexportrow	= $res['startexportrow'];
	...
	function create($user='')
	{
		...
		$this->exportsheet		=(!is_array($this->exportsheet)?trim($this->exportsheet):'');
		//$this->startexportcol	=(!is_array($this->startexportcol)?trim($this->startexportcol):0);
		//$this->startexportrow	=(!is_array($this->startexportrow)?trim($this->startexportrow):0);
		...
		$sql.= ", '".$this->db->escape($this->exportsheet)."'";
		$sql.= ", ".($this->startexportcol?$this->startexportcol:"null");
		$sql.= ", ".($this->startexportrow?$this->startexportrow:"null");
		$sql.= ")";
print $sql.'<br>';
		dol_syslog(get_class($this).'::create sql='.$sql);
		if ($this->db->query($sql)) {
			// la récup de l'id avant le commit sinon ca déconne
	...
	function update($user='')
	{
		...
		$sql .= ", author			='".$this->db->escape($this->author)."'";
		$sql .= ", active			=".$this->db->escape($this->active);
		$sql .= ", exportsheet		='".$this->db->escape($this->exportsheet)."'";
		$sql .= ", startexportcol	=".($this->startexportcol?$this->startexportcol:'NULL');
		$sql .= ", startexportrow	=".($this->startexportrow?$this->startexportrow:'NULL');
