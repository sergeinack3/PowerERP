...
function mydoliboard_admin_prepare_head ()
{
	...
	$head[$h][0] = dol_buildpath('/mydoliboard/admin/setup.php', 1);
	$head[$h][1] = $langs->trans("Setup");
	$head[$h][2] = 'admin';
	
	$h++;
	$head[$h][0] = dol_buildpath('/mydoliboard/admin/about.php', 1);
...
function select_xlstemplate($selected='', $htmlname='xlstemplate')
{
	global $langs; //, $conf;
	require_once DOL_DOCUMENT_ROOT.'/core/lib/files.lib.php';	
	$tmpfiles=dol_dir_list(DOL_DATA_ROOT.'/docxlstemplates', 'files', 0, '', '', 'name', SORT_ASC, 0);

	$tmp="<select name=".$htmlname.">";
	$tmp.="<option value='' >".$langs->trans("NoExportAllowed")."</option>";
	foreach ($tmpfiles as $key => $value)
		$tmp.="<option value='".$value["name"]."'".($selected==$value["name"]?" selected ":"").">".$value["name"]."</option>";
	$tmp.="</select>";
	return $tmp;
}
...
function blocsheet($pageid, $listsUsed, $cellsheet)
{
	...
	foreach ($listsUsed as $key=> $value ) {
		if ($value['displaycell']==$cellsheet) {
			$var=!$var;
			$sz.= '<tr '.$bc[$var].' pageid="'.$pageid.'"';
			$sz.= ' displaycell="'.$value['displaycell'].'" cellorder="'.$value['cellorder'].'">'."\n";
			$sz.= '<td align=center class="upArrow"><img src="./img/1uparrow.png"></td>';
			$sz.= '<td align=center class="downArrow"><img src="./img/1downarrow.png"></td>';
