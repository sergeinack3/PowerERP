...
function createContainer(w, h, el, altText, opts) {
  ...
  var playButtonStyles = opts && opts.playButtonStyles ? parseStyles(opts.playButtonStyles) : [
    ...
    'border:' + borderSize + 'px solid ' + playColor,
    'position:absolute',
    'top:50%',
    'left:50%',
    'margin:-' + ((playSize/2) + borderSize) + 'px',
    'padding: 0px'
  ].join(';');
  var playButtonIconStyles = opts && opts.playButtonIconStyles ? parseStyles(opts.playButtonIconStyles) : [
    'width: 0',
    'height: 0',
    'border-top:' + playButtonSize + 'px solid transparent',
    'border-bottom:' + playButtonSize + 'px solid transparent',
    //'border-left: 14px solid rgba(0, 0, 0, 0.5)',
    'border-left:' + playButtonSize + 'px solid ' + playColor,
    'position: absolute',
    'left: 45%',
    'top: 0px',
    'bottom: 0px',
...
function process(el, gifs, options) {
  ...
  w = gifWidth;//el.getAttribute('data-gifffer-width');
  h = gifHeight;//el.getAttribute('data-gifffer-height');
