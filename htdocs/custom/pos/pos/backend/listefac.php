...
// Security check
$fieldid = (! empty($ref)?$refDoli9or10:'rowid');
...
$arrayfields=array(
    'f.'.$refDoli9or10=>array('label'=>$langs->trans("Ref"), 'checked'=>1),
...
$sql.= ' f.rowid as facid, f.'.$refDoli9or10.',f.ref_client, f.type, f.note_private, f.note_public, f.increment, f.fk_mode_reglement, ';
if (version_compare(DOL_VERSION, 14) >= 0) { $sql .= 'f.total_ht as total_ht, f.total_tva as total_vat, f.total_ttc,'; }
else { $sql .= 'f.total as total_ht, f.tva as total_vat, f.total_ttc,'; }
...
if ($search_ref) $sql .= natural_search('f.'.$refDoli9or10, $search_ref);
...
$sql.= ' GROUP BY f.rowid, f.'.$refDoli9or10.', ref_client, f.type, f.note_private, f.note_public, f.increment, f.fk_mode_reglement, ';
if (version_compare(DOL_VERSION, 14) >= 0) { $sql .= 'f.total_ht, f.total_tva, f.total_ttc,'; }
else { $sql .= 'f.total, f.tva, f.total_ttc,'; }
...
//mysql strict
$sql.= ' ca.name, ca.rowid, f.fk_user_valid';
//
...
if ($resql)
{
    ...
    if (! empty($arrayfields['f.'.$refDoli9or10]['checked']))          print_liste_field_titre($arrayfields['f.'.$refDoli9or10]['label'],$_SERVER['PHP_SELF'],'f.'.$refDoli9or10,'',$param,'',$sortfield,$sortorder);
    ...
    print '<tr class="liste_titre">';
	// Ref
	if (! empty($arrayfields['f.'.$refDoli9or10]['checked']))
	{
    ...
    if ($num > 0)
    {
        ...
        while ($i < min($num,$limit))
        {
    		...
    		if (! empty($arrayfields['f.'.$refDoli9or10]['checked']))
    		{
                ...
                $filename=dol_sanitizeFileName($refDoli9or10);
                $filedir=$conf->facture->dir_output . '/' . dol_sanitizeFileName($objDoli9or10);
