 ...
 * Copyright (C) 2005-2010 Regis Houssin         <regis@powererp.fr>
...
if ($action == 'add_paiement' || ($action == 'confirm_paiement' && $confirm=='yes'))
{
    ...
    foreach ($_POST as $key => $value)
    {
        ...
        {
            $cursorticketsid = substr($key,7);
            $amounts[$cursorticketsid] = price2num($_POST[$key]);
            $totalpaiement = $totalpaiement + $amounts[$cursorticketsid];
            $tmpfacture=new tickets($db);
            $tmpfacture->fetch($cursorticketsid);
            $amountsresttopay[$cursorticketsid]=price2num($tmpfacture->total_ttc-$tmpfacture->getSommePaiement());
            if ($amounts[$cursorticketsid] && $amounts[$cursorticketsid] > $amountsresttopay[$cursorticketsid])
            {
                $addwarning=1;
...
if ($action == 'confirm_paiement' && $confirm == 'yes')
{
    ...
    if (! $error)
    {
        $result=$paiement->addPaymentToBank($user,'payment','(CustomerticketsPayment)',$_POST['accountid'],$_POST['socid'],$_POST['chqemetteur'],$_POST['chqbank']);
        if ($result < 0)
        {
            $errmsg=$paiement->error;
            $error++;
    ...
    if (! $error)
    {
        ...
        // If payment dispatching on more than one tickets, we keep on summary page, otherwise go on invoice card
        $invoiceid=0;
        ...
        if ($invoiceid > 0)
        {
			...
			$sql.= ' WHERE pf.fk_tickets = '.$invoiceid.' AND p.fk_paiement = c.id AND pf.fk_paiement = p.rowid';
			$sql.= ' ORDER BY dp, tms';
			...
			$sql ="UPDATE ".MAIN_DB_PREFIX."pos_tickets set customer_pay=".$totalpaye." WHERE rowid=".$invoiceid;
			$result = $db->query($sql);

        	$loc =dol_buildpath('/pos/backend/tickets.php',1).'?id='.$invoiceid;
        }
...
if ($action == 'create' || $action == 'confirm_paiement' || $action == 'add_paiement')
{
    ...
    $result=$tickets->fetch($ticketsid);

    if ($result >= 0)
    {
        $tickets->fetch_thirdparty();
        ...
        if ($action == 'add_paiement')
        {
            ...
            $formquestion[$i++]=array('type' => 'hidden','name' => 'ticketsid', 'value' => $tickets->id);
            $formquestion[$i++]=array('type' => 'hidden','name' => 'socid', 'value' => $tickets->socid);
            $formquestion[$i++]=array('type' => 'hidden','name' => 'type',  'value' => $tickets->type);
        }

        // Invoice with Paypal transaction
        if ($conf->paypalplus->enabled && $conf->global->PAYPAL_ENABLE_TRANSACTION_MANAGEMENT && ! empty($tickets->ref_int))
        {
        	if (! empty($conf->global->PAYPAL_BANK_ACCOUNT)) $accountid=$conf->global->PAYPAL_BANK_ACCOUNT;
        	$paymentnum=$tickets->ref_int;
        }

        print '<form id="payment_form" name="add_paiement" action="'.$_SERVER["PHP_SELF"].'" method="POST">';
        print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
        print '<input type="hidden" name="action" value="add_paiement">';
        print '<input type="hidden" name="ticketsid" value="'.$tickets->id.'">';
        print '<input type="hidden" name="socid" value="'.$tickets->socid.'">';
        print '<input type="hidden" name="type" value="'.$tickets->type.'">';
        print '<input type="hidden" name="thirdpartylabel" id="thirdpartylabel" value="'.dol_escape_htmltag($tickets->thirdparty->name).'">';

        print '<table class="border" width="100%">';

        // Third party
        print '<tr><td><span class="fieldrequired">'.$langs->trans('Company').'</span></td><td colspan="2">'.$tickets->thirdparty->getNomUrl(4)."</td></tr>\n";
        ...
        $html->select_types_paiements((GETPOST('paiementcode')?GETPOST('paiementcode'):$tickets->mode_reglement_id),'paiementcode','',2);
        print "</td>\n";
        ...
        if ($conf->banque->enabled)
        {
            ...
            //if ($tickets->type == 2) print '<td><span class="fieldrequired">'.$langs->trans('AccountToDebit').'</span></td>';
            print '<td>';
         ...
         */
        $sql = 'SELECT f.rowid as ticketsid, f.ticketsnumber, f.total_ttc, f.type, ';
        $sql.= ' f.date_closed as df';
        $sql.= ' FROM '.MAIN_DB_PREFIX.'pos_tickets as f';
        $sql.= ' WHERE f.fk_soc = '.$tickets->socid;
        ...
        if ($resql)
        {
            ...
            if ($num > 0)
            {
                ...
                print '<td>'.$langs->trans('tickets').'</td>';
                ...
                while ($i < $num)
                {
                    ...
                    $invoice=new tickets($db);
                    $invoice->fetch($objp->ticketsid);
					...
					//{
	                    ...
	                    // Add remind amount
	                    $namef = 'amount_'.$objp->ticketsid;
	                    $nameRemain = 'remain_'.$objp->ticketsid;

	                    if ($action != 'add_paiement')
	                    {
	                        if ($conf->use_javascript_ajax && !empty($conf->global->MAIN_JS_ON_PAYMENT))
	                        {
	                            print img_picto($langs->trans('AddRemind'),'rightarrow.png','id="'.$objp->ticketsid.'" "');
	                        }
        ...
        if ($action == 'add_paiement')
        {
            ...
            $html->form_confirm($_SERVER['PHP_SELF'].'?ticketsid='.$tickets->id.'&socid='.$tickets->socid.'&type='.$tickets->type,$langs->trans('ReceivedCustomersPayments'),$text,'confirm_paiement',$formquestion,$preselectedchoice);
        }
