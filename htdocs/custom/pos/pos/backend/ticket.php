...
/**
 *	\file       htdocs/pos/backed/tickets.php
 *	\ingroup    pos
 *	\brief      Page to see an tickets
 */
...
if (GETPOST('action','alpha') == 'confirm_deletetickets' )
{
	$result=$object->delete_tickets();
	if($result>0) {
		setEventMessage($langs->trans("ticketsWasRemoved"));
		header('Location: ' . dol_buildpath('/pos/backend/liste.php', 1) . '?id=' . $id);
	}
}

if (GETPOST('action','string') == 'confirm_abandontickets' )
{
	$result=$object->set_canceled();

	setEventMessage($langs->trans("ticketsWasAbandoned"));

	Header('Location: '.$_SERVER["PHP_SELF"].'?id='.$id);
}

if (GETPOST('action','string') == 'confirm_facturetickets' )
{
...
$head = tickets_prepare_head($object);
dol_fiche_head($head, 'tickets', $langs->trans("tickets"), 0, 'tickets');
...
if ($result > 0)
{
	...
	print '<td colspan="5">';
	$morehtmlref = '';
	//print $html->showrefnav($object,'id','',1,'ticketsnumber','ticketsnumber',$morehtmlref);
	print $html->showrefnav($object,'ref','',1,'ticketsnumber','ref',$morehtmlref);
	print '</td>';
	print '</tr>';
	...
	*/
	...
	$sql.= ' WHERE pf.fk_tickets = '.$object->id.' AND p.fk_paiement = c.id AND pf.fk_paiement = p.rowid';
	$sql.= ' ORDER BY dp, tms';
	...
	*/
	...
	if ($object->statut>0 && $object->statut<3)
	{
		...
		{
			print '<a class="butAction" href="'.dol_buildpath('/pos/backend/tickets.php?action=facture&id='.$object->id,1).'">'.$langs->trans('CreateBill').'</a>';
		}
		else{
			print '<span class="butActionRefused" title="'.$langs->trans("DisabledBecauseticketsHasFacture").'">'.$langs->trans('CreateBill').'</span>';
		}
		...
		{
			print '<a class="butAction" href="paiement.php?ticketsid='.$object->id.'&amp;action=create">'.$langs->trans('DoPayment').'</a>';
			print '<a class="butAction" href="'.dol_buildpath('/pos/backend/tickets.php?action=abandon&id='.$object->id,1).'">'.$langs->trans('ClassifyCanceled').'</a>';
		}
		$url = '../frontend/tpl/tickets.tpl.php?id='.$object->id;
		print '<a class="butAction" href='.$url.' target="_blank">'.$langs->trans('PrintCopy').'</a>';

		print '<a class="butAction" href="'.dol_buildpath('/pos/backend/tickets.php',1).'?id='.$object->id.'&action=mail">'.$langs->trans('MailCopy').'</a>';

	}
	if ($object->statut == 3)
	{
		print '<a class="butAction" href="'.dol_buildpath('/pos/backend/tickets.php?action=reopen&id='.$object->id,1).'">'.$langs->trans('ReOpen').'</a>';
	}
	if ($object->is_erasable())
	{
		print '<a class="butAction" href="'.dol_buildpath('/pos/backend/tickets.php?action=delete&id='.$object->id,1).'">'.$langs->trans('Delete').'</a>';
	}
	else
	{
	...
	{
		$ret=$form->form_confirm($_SERVER["PHP_SELF"].'?id='.$id, $langs->trans('Deletetickets'), $langs->trans('ConfirmDeletetickets',$object->ticketsnumber), 'confirm_deletetickets','','',1);
		if ($ret == 'html') print '<br>';

	}
	if (GETPOST('action','string') == 'abandon')
	{
		$ret=$form->form_confirm($_SERVER["PHP_SELF"].'?id='.$id, $langs->trans('ClassifyCanceled'), $langs->trans('ConfirmAbandontickets',$object->ticketsnumber), 'confirm_abandontickets','','',1);
		if ($ret == 'html') print '<br>';
	}
	if (GETPOST('action','string') == 'facture')
	{
		$ret=$form->form_confirm($_SERVER["PHP_SELF"].'?id='.$id, $langs->trans('CreateBill'), $langs->trans('ConfirmCreateBill',$object->ticketsnumber), 'confirm_facturetickets','','',1);
		if ($ret == 'html') print '<br>';
	...
	if( GETPOST('action','string') == 'mail')
	{
		...
		$formmail->withtopic=$conf->global->MAIN_INFO_SOCIETE_NOM.': '.$langs->trans("CopyOftickets").' '.$object->ticketsnumber;
		$formmail->withfile=0;
		$formmail->withbody= POS::fillMailticketsBody($id);
