...
if (! $sortfield) $sortfield='f.date_tickets';
...
if(count($toselect)>0 && !GETPOST('invoince')){
	foreach ($toselect as $tickets){
		$ticketsdel = new tickets($db);
		$ticketsdel->id = $tickets;
		$ticketsdel->delete_tickets();
	}
}
if(count($toselect)>0 && GETPOST('invoince')){
	$ticketss = array();
	foreach ($toselect as $ticketsid){
		$tickets = new tickets($db);
		$tickets->fetch($ticketsid);

		if(!array_key_exists($tickets->socid, $ticketss)) {
			$ticketss[$tickets->socid][0] = $ticketsid;
		}
		else{
			array_push($ticketss[$tickets->socid], $ticketsid);
		}
	}

	if($ticketss) {
		foreach ($ticketss[1] as $client) {
			$pos = new tickets($db);
			$pos->fetch($client,'');
			...
			if(!$pos->linkedObjects['facture']) {
				...
				if (!$result) {
					setEventMessages('Error to create invoice for tickets -> ' . $pos->ref,'','errors');
					$viewstatut = 1;
				}
				else{
					setEventMessage('Invoice for tickets -> ' . $pos->ref . ' created');
				}
			}
			else{
				setEventMessages('Error: invoice already exist for tickets -> ' . $pos->ref,'','errors');
				$viewstatut = 1;
			}
		}
	}
...
if (!$user->rights->pos->backend)
{
	...
	$sql = 'SELECT ';
	$sql.= ' f.rowid as ticketsid, f.type, f.ticketsnumber, f.total_ht, f.total_ttc,';
	...
	if ($_POST['search_ref'])
	{
		 $sql.= ' AND f.ticketsnumber LIKE \'%'.$db->escape(trim($_POST['search_ref'])).'%\'';
	}
	...
	{
		if ($year > 0)
			$sql.= " AND f.date_tickets BETWEEN '".$db->idate(dol_get_first_day($year,$month,false))."' AND '".$db->idate(dol_get_last_day($year,$month,false))."'";
		else
		$sql.= " AND date_format(f.date_tickets, '%m') = '".$month."'";
	}
	else if ($year > 0)
	{
		$sql.= " AND f.date_tickets BETWEEN '".$db->idate(dol_get_first_day($year,1,false))."' AND '".$db->idate(dol_get_last_day($year,12,false))."'";
	}
	if ($_POST['sf_ref'])
	{
		$sql.= ' AND f.ticketsnumber LIKE \'%'.$db->escape(trim($_POST['sf_ref'])) . '%\'';
		}
	if ($sall)
	{
		$sql.= ' AND (s.nom LIKE \'%'.$db->escape($sall).'%\' OR f.ticketsnumber LIKE \'%'.$db->escape($sall).'%\' OR f.note LIKE \'%'.$db->escape($sall).'%\' OR fd.description LIKE \'%'.$db->escape($sall).'%\')';
	}
	if (! $sall)
	{
		$sql.= ' GROUP BY f.rowid, f.ticketsnumber, f.total_ht, f.total_ttc,';
		$sql.= ' f.date_tickets,';
	...
	if ($resql)
	{
		...
		$txtListe = $langs->trans('ticketssCustomers');

		if ($viewstatut <> '')
		{
			$txtListe = $txtListe." - ".$ticketsstatic->LibStatut($viewstatut);
		}

		if ($viewtype <> '')
		{
			$txtListe = $txtListe." - ".$langs->trans("StatusticketsReturned");
		}

		print_barre_liste($txtListe.' '.($socid?' '.$soc->nom:''),$page,'liste.php',$param,$sortfield,$sortorder,'',$num);

		$i = 0;
		print '<form method="POST" name="liste_tickets" action="'.$_SERVER["PHP_SELF"].'">'."\n";
		print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
		print '<input type="hidden" name="formfilteraction" id="formfilteraction" value="list">';
		print '<input type="hidden" name="action" value="list">';
		print '<table class="liste" width="100%">';
		print '<tr class="liste_titre">';
		print_liste_field_titre($langs->trans('Ref'),$_SERVER['PHP_SELF'],'f.ticketsnumber','',$param,'',$sortfield,$sortorder);
		print_liste_field_titre($langs->trans('Date'),$_SERVER['PHP_SELF'],'f.date_tickets','',$param,'align="center"',$sortfield,$sortorder);
		...
		if ($num > 0)
		{
			...
			while ($i < min($num,$limit))
			{
				...
				$ticketsstatic->ref=$objp->ticketsnumber;
				$paiement = $ticketsstatic->getSommePaiement();
				...
				print $ticketsstatic->LibStatut($objp->fk_statut,1);
				print "</td>";
				// Action column
				if(version_compare(DOL_VERSION,4.0)>=0) {
					...
					{
						$selected = 0;
						if (in_array($objp->ticketsid, $arrayofselected)) $selected = 1;
						print '<input id="cb' . $objp->ticketsid . '" class="flat checkforselect" type="checkbox" name="toselect[]" value="' . $objp->ticketsid . '"' . ($selected ? ' checked="checked"' : '') . '>';
					}
					print '</td>';
				}
				else {
