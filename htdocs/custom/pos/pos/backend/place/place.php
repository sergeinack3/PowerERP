...
$sql = "SELECT rowid, name, status, description, fk_tickets";
...
if ($resql)
{
	...
	print_liste_field_titre($langs->trans("Associatedtickets"),$_SERVER["PHP_SELF"],"fk_tickets","",$params,"",$sortfield,$sortorder);
...
//	print_liste_field_titre($langs->trans("Status"),$_SERVER["PHP_SELF"],"s.status","",$params,'align="right"',$sortfield,$sortorder);
	...
	while ($i < min($num,$conf->liste_limit))
	{
        ...
        if(!$placewil->is_inUse())
        {
	        if($placewil->is_notInUse())
	        {
	        	if($user->rights->pos->backend)
	        	{
	        		print "<td></td>\n";
	        		print '<td align="left">';
					print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?action=closeplace&id='.$placewil->id.'">'.$langs->trans("CloseIt").'</a>';
					print '</td>';
	        	}
	        	else
	        	{
	        ...
	        elseif ($placewil->is_closed())
	        {
	        	if($user->rights->pos->backend)
	        	{
	        		print "<td></td>\n";
	        		print '<td align="left">';
					print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?action=openplace&id='.$placewil->id.'">'.$langs->trans("OpenIt").'</a>';
					print '</td>';
	        	}
	        	else
	        	{
		...
		{
			print '<td>'.$placewil->getticketsUrl(1).'</td>';
			print "<td></td>\n";
		}
        print '<td></td>';
        print '<td></td>';
