...
$sql.= " sum(if(f.type=1,l.total_ht * -1,l.total_ht)) as amount, sum(if(f.type=1,l.total_ttc * -1,l.total_ttc)) as amount_ttc, sum(l.qty) as qty";
$sql.= " FROM ".MAIN_DB_PREFIX."pos_tickets as f, ".MAIN_DB_PREFIX."pos_ticketsdet as l, ".MAIN_DB_PREFIX."product as p";
...
$sql.= " AND l.fk_tickets = f.rowid";
$sql.= " AND f.fk_statut in (1,2)";

if ($date_start && $date_end) {
	$sql.= " AND f.date_tickets >= '".$db->idate($date_start)."' AND f.date_tickets <= '".$db->idate($date_end)."'";
}
if ($selected_cat === -2)	// Without any category
{
