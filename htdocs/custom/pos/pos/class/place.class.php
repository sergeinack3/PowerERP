...
class Place extends CommonObject
{
    ...
    function create($user)
    {
        ...
        $sql .= ", 1";
        $sql .= "," . ($this->fk_tickets ? $this->fk_tickets : 0);
    ...
    function fetch($rowid, $ref = '')
    {
        ...
        if ($result) {
            if ($this->db->num_rows($result)) {
                ...
                $this->fk_tickets = $obj->fk_tickets;
    ...
    function delete($rowid = 0)
    {
        ...
        if ($resql) {
            ...
            if ($obj->nb > 0) {
                $this->error = "ErrorRecordHasticketss";
                return -2;
            }
        } else {
    ...
    function can_be_deleted()
    {
        ...
        $sql .= " WHERE t.fk_place = " . $this->id . " OR f.fk_place = " . $this->id;
        $resql = $this->db->query($sql);
        if ($resql) {
            $obj = $this->db->fetch_object($resql);
    ...
    /**
     *        Renturns clicable ticketss name
     * @param        withpicto        Include picto in link
     * @return        string            String avec URL
     */
    function getticketsUrl($withpicto = 0)
    {
        global $langs, $conf;
        $result = '';
        ...
        $sql .= ' WHERE entity = ' . $conf->entity;
        $sql .= " AND rowid=" . $this->fk_tickets;

        $resql = $this->db->query($sql);
        if ($resql) {
            $obj = $this->db->fetch_object($resql);
            $lien = '<a href=' . dol_buildpath('/pos/backend/tickets.php', 1) . '?id=' . $this->fk_tickets . '>';
            $lienfin = '</a>';
            if ($withpicto) $result .= ($lien . img_object($langs->trans("Showtickets") . ' ' . $obj->ticketsnumber, 'barcode') . $lienfin . ' ');
            $result .= $lien . $obj->ticketsnumber . $lienfin;
            return $result;
        } else {
            dol_print_error($this->db);
        }
    ...
    function is_inUse()
    {
        if ($this->status && $this->fk_tickets > 0) {
            return true;
        } else {
            return false;
        }
    ...
    /**
     * Set the place as free of ticketss
     *
     ...
     *
     * @param    $id_tickets    Id of tickets to associate to the place
     * @return    number    > 0 OK, < 0 ko
     */
    function set_place($id_tickets)
    {
        $sql = "UPDATE " . MAIN_DB_PREFIX . "pos_places SET";
        $sql .= " fk_tickets = " . $id_tickets;
        $sql .= " WHERE rowid=" . $this->id;
