...
if($action == 'update'){
    ...
    $resignation->fetch($id);
    $isvalid = $resignation->update($id, $data);
...
if( $id || $action == "delete" ){
    ...
    $resignation->fetchAll('','',0,0,' and rowid = '.$id);
    $item = $resignation->rows[0];
        ...
        print '<div aria-hidden="false" '.$cl.' aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="moadl_adttend" class="fade in" align="center">';
            print '<div class="div_request">';
                ...
                print '<table class="tbl_hrm">';
                    ...
                    print '<tr>';
                        print '<td> '.$langs->trans("resignation_for").' </td>';
                        print '<td colspan="3"> <input type="text" class="label" name="label" value="'.$item->label.'" style="width:92%;" > </td>';
                    print '</tr>';

                    print '<tr>';
                        print '<td>'.$langs->trans("Employe").'</td>';
                        print '<td colspan="3">'. $resignation->select_user($item->employe,'employe',1,1).'<span class="branche_name"></span></td>';
                    print '</tr>';
                   
                    print '<tr>';
                        print '<td>'.$langs->trans("notice_date").'</td>';
                        print '<td width="25%"><input type="text" name="date_notice" autocomplete="off" value="'.date('d/m/Y', strtotime($item->date_notice)).'" class="datepicker55" > </b>';
                        print '<td>'.$langs->trans("date_resignation").' </b>';
                    ...
                    print '<tr>';
                        print '<td>'.$langs->trans("Description").' </b>';
                        print '<td colspan="3"><textarea name="description" onchange="textarea_autosize(this)" class="description">'.$item->reason.'</textarea></td>';
                    print '</tr>';
    ...
    $relativepathwithnofile = '/resignation/'.$id.'/';
