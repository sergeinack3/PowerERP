...
if(empty($id_candidature)){
	$id_candidature=$srch_candidature;
}
// die($id_poste);
...
$filter .= (!empty($id_candidature)) ? " AND candidature = ".$id_candidature."" : "";
...
if(!empty($candidature)){
	...
	$head = menu_candidature($candidature);
	if($action != 'add'){
	    dol_fiche_head(
	        $head,
	        'cv',
	...
	print '<form method="get" action="'.$_SERVER["PHP_SELF"].'?candidature='.$id_candidature.'">'."\n";
		...
		print '<table id="table-1" class="noborder tablerecrutement" style="width: 100%;" >';
			...
			print '<tr class="liste_titre">';
				...
				print_liste_field_titre($langs->trans("nom"),$_SERVER["PHP_SELF"], "nom", '', '', 'align="center"', $sortfield, $sortorder);
				...
				if (count($cv->rows) > 0) {
					for ($i=0; $i < count($cv->rows) ; $i++) {
						...
						print '<tr '.$bc[$var].' >';
				    		...
				    		print '<a href="'.dol_buildpath('/recrutement/cv/card.php?id='.$item->rowid.'&candidature='.$item->candidature,2).'" >';
					    		print $item->nom;
				    		print '</a>';
				    		print '</td>';
				    		// print '<td align="center" style=""><a href="'.dol_buildpath('/recrutement/candidatures/card.php?id='.$item->candidature,2).'"> '.$candidatures->sujet.' </a></td>';
							...
							if($item->candidature){
								 if($item->type == 'fichier' && $item->fichier){
                                	...
                                	$dt_files = getAdvancedPreviewUrl('recrutement', '/candidatures/'.$item->candidature.'/cv/'.$item->rowid.'/'.$minifile, 1, '&entity='.(!empty($object->entity)?$object->entity:$conf->entity));
								 	if($ext == 'pdf'){
                                    	print ' <a href="'.$dt_files['url'].'" class="'.$dt_files['css'].'" mime="'.$dt_files['mime'].'" target="'.$dt_files['target'].'" title="'.$minifile.'" >'.img_mime('test.pdf').'</a>' ;
								 	}else{
                                    	print ' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=recrutement&file=candidatures/'.$item->candidature.'/cv/'.$item->rowid.'/'.$minifile.'"  title="'.$minifile.'">'.img_mime('test.'.$ext).'</a>' ;
								 	}
			       	...
			       	print ' <div class="board-item" >';
			       		print ' <div class="board-item-content" style="margin:0 20px 20px">';
			       			print '<div class="item-content">';
			       				...
			       				$cd = '';
			       				if($candidature){
				       				$cd='&candidature='.$candidature;
			       				}
			       				print '<div class="detailcontainer">';
			       					print '<div ><a href="'.dol_buildpath('recrutement/cv/card.php?id='.$item->rowid.$cd,2).'"><strong>'.$item->nom.'</strong></a></div>';
			       					// if($item->poste){
