...
class hrm_termination extends Commonobject{ 
	...
	public $element='hrm_termination';
	public $table_element='hrm_termination';
	...
	public function fetchAll($sortorder = '', $sortfield = '', $limit = 0, $offset = 0, $filter = '', $filtermode = 'AND')
	{
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$line->date          =  $obj->date;
				$line->date_notice  =  $obj->date_notice;
	...
	public function fetch($id)
	{
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->date         =  $obj->date;
				$this->date_notice 	=  $obj->date_notice;
    ...
    function getNomUrl($withpicto=0, $option='', $get_params='', $notooltip=0, $save_lastsearch_value=-1)
    {
        ...
        $result='';
        $label='';
        $url = dol_buildpath('/gestionhrm/hrm_termination/card.php?id='.$this->id,2);
        ...
        if ($ref) {
            ...
            $result .= $linkstart;
            if ($withpicto) 
                $result.= '<img height="16" src="'.dol_buildpath('/gestionhrm/img/icon_hrm_termination.png',2).'" >&nbsp;';
