...
if (! empty($datedeb) && ! empty($datefin)) {
	...
	$sql.= ' fa.fk_soc,';
	$sql.= ' p.color, p.step,';
	$sql.= ' fa.total_ht ,';  // NOTE : préciser le type de total?
	$sql.= ' fa.fk_statut ';
	$sql.= ' FROM '.MAIN_DB_PREFIX.'facture_fourn as fa';
	$sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'process as p';
	$sql.= " ON fa.rowid = p.fk_element and p.element='facture_fourn'";
...
//	$return['query']=$sql;
	...
	if ($resql) {
		...
		while ($j < $num) {
			...
			$event['color'] = "#".$bgcolor;
			$event['url'] = dol_buildpath("fourn/facture", 1)."/card.php?id=".$objp->rowid;
