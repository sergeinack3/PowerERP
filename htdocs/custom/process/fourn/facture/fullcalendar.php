...
if (!$selectlist && !$selectagenda && !$choiceperiod) {
	...
	switch (substr($conf->global->ProcessDisplayAction, 0, 1)) {
		case 'W':
			$choiceperiod= "week";
			break;
		case 'D':
	...
	if (substr($conf->global->ProcessDisplayAction, -1, 1) == "L")
		$selectlist ="list";
}
...
llxHeader('', "AgendaProcess", "", '', 0, 0, $arrayofjs, $arrayofcss, '');
 ...
 */
$title=$langs->trans("AgendaProcess")."-".$langs->trans("SuppliersBills");
...
print '<option '.($statut=="0"?' selected ':'').' value="0">'.$generic_status->LibStatut(0,0,0).'</option>';  	// brouillon 
print '<option '.($statut=="1"?' selected ':'').' value="1">'.$generic_status->LibStatut(1,0,0).'</option>';  	// brouillon 
print '<option '.($statut=="2"?' selected ':'').' value="2">'.$generic_status->LibStatut(2,0,0).'</option>';  	// brouillon 
print '<option '.($statut=="3"?' selected ':'').' value="3">'.$generic_status->LibStatut(3,0,0).'</option>';  	// brouillon 
print '<option '.($statut=="4"?' selected ':'').' value="4">'.$generic_status->LibStatut(3,1,0).'</option>';  	// brouillon 
...
print "window.open('".dol_buildpath("/process/fourn/facture/", 1);
...
print "	$.ajax({\n";
print "url: '".dol_buildpath("/process/fourn/facture/ajax/", 1);
