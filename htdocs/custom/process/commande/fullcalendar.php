...
if (!$selectlist && !$selectagenda && !$choiceperiod) {
	...
	switch (substr($conf->global->ProcessDisplayCommande, 0, 1)) {
		case 'W':
			$choiceperiod = "week";
			break;
		case 'D':
	...
	if (substr($conf->global->ProcessDisplayCommande, -1, 1) == "L")
		$selectlist = "list";
}
...
if (! empty($user->societe_id))
	$periodsoc=$user->societe_id;
$result = restrictedArea($user, 'commande', 0, '');
...
$title=$langs->trans("CalendarProcess")." - ".$langs->trans("Orders");
...
print "window.open('".dol_buildpath("/process/commande/", 1);
...
print "	$.ajax({\n";
print "url: '".dol_buildpath("/process/commande/ajax/", 1);
