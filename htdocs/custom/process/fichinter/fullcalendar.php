...
/**
 *	\file	   	htdocs/process/fichinter/fullcalendar.php
 *	\ingroup		process
 *	\brief	  	Page of fichinter process
 */
...
if (!$selectlist && !$selectagenda && !$choiceperiod) {
	...
	switch (substr($conf->global->ProcessDisplayFichinter, 0, 1)) {
		case 'W':
			$choiceperiod = "week";
			break;
		case 'D':
	...
	if (substr($conf->global->ProcessDisplayFichinter, -1, 1) == "L")
		$selectlist = "list";
}
...
$title=$langs->trans("CalendarProcess")." - ".$langs->trans("InterventionPlanned");
...
print '<th  align=left >'.$langs->trans("Interventional").'</th>';
print "</tr>\n";
...
if ($conf->contrat->enabled) {
	...
	print '<tr><td>'.$langs->trans("Contract");
	print "</td><td colspan=2>";
	$showempty=1;
	print $formcontract->select_contract(
					$periodsoc, $periodcontract, 
					'periodcontract', $maxlength=16, $showempty
	);
	print '</td></tr>';
}

if ($conf->myschedule->enabled) {
	print '<tr><td valign=top>'.$langs->trans("DisplayFichInter");
	...
	print '<table>';
	print '<tr><td bgcolor=yellow>'.$langs->trans("NoUserAffected")."</td><td bgcolor=#CEFFCE>".$langs->trans("NotYetMade")."</td></tr>\n";
	print '<tr><td bgcolor=#FFCCCC>'.$langs->trans("NotYetPlanned")."</td><td bgcolor=#C0C0C0>".$langs->trans("Made")."</td></tr>\n";
	print "</table>";
	print '</td></tr>';
}
print '</table>';
...
print "window.open('".dol_buildpath("/process/fichinter/", 1);
...
if ($periodcontract > 0)
	print "&periodcontract=".$periodcontract;
...
print "	$.ajax({\n";
print "url: '".dol_buildpath("/process/fichinter/ajax/", 1);
