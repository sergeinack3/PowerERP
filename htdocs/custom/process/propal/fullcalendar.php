...
if (!$selectlist && !$selectagenda && !$choiceperiod) {
	...
	switch (substr($conf->global->ProcessDisplayPropal, 0, 1)) {
		case 'W':
			$choiceperiod = "week";
			break;
		case 'D':
	...
	if (substr($conf->global->ProcessDisplayPropal, -1, 1) == "L")
		$selectlist = "list";
}
...
$title=$langs->trans("CalendarProcess")." - ".$langs->trans("Proposal");
...
print '<select class="flat" name="filterbystatut">';
$generic_status = new Propal($db);
print '<option '.($statut==-1? " selected ":""). 'value="-1">&nbsp;</option>';
print '<option '.($status=="0"?' selected ':'').' value="0">'.$generic_status->LibStatut(0,0).'</option>';  	// brouillon 
print '<option '.($status=="1"?' selected ':'').' value="1">'.$generic_status->LibStatut(1,0).'</option>';  	// brouillon 
print '<option '.($status=="2"?' selected ':'').' value="2">'.$generic_status->LibStatut(2,0).'</option>';  	// brouillon 
print '<option '.($status=="3"?' selected ':'').' value="3">'.$generic_status->LibStatut(3,0).'</option>';  	// brouillon 
print '<option '.($status=="4"?' selected ':'').' value="4">'.$generic_status->LibStatut(4,0).'</option>';  	// brouillon 
...
print "window.open('".dol_buildpath("/process/propal/", 1);
...
print "	$.ajax({\n";
print "url: '".dol_buildpath("/process/propal/ajax/", 1);
