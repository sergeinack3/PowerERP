...
if (! empty($datedeb) && ! empty($datefin)) {
	...
	if ((int) DOL_VERSION >= 10)
		$sql = 'SELECT fa.rowid, fa.ref, fa.ref_supplier,';
	else
		$sql = 'SELECT fa.rowid, fa.facnumber as ref, fa.ref_supplier,';

	$sql.= ' fa.date_lim_reglement as dateo,';
	$sql.= ' fa.fk_user_author,';
	$sql.= ' fa.fk_projet,';
	$sql.= ' fa.fk_soc,';
	$sql.= ' p.color, p.step,';
	$sql.= ' fa.total ,';  // NOTE : préciser le type de total?
	$sql.= ' fa.paye, fa.fk_statut ';
	$sql.= ' FROM '.MAIN_DB_PREFIX.'facture as fa';
	$sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'process as p';
	$sql.= " ON fa.rowid = p.fk_element and p.element='facture'";
	$sql.= ' WHERE fa.entity in (0,'.$conf->entity.')';	// To limit to entity

	$sql.= ' AND (date_format(fa.date_lim_reglement, "%Y-%m-%d") >= "'.$datedeb.'"';
	$sql.= ' AND date_format(fa.date_lim_reglement, "%Y-%m-%d") <= "'.$datefin.'")';

	if ($periodsoc > 0) 	$sql.= ' AND fa.fk_soc = '.$periodsoc;
	if ($projectid > 0) 	$sql.= ' AND fa.fk_projet ='.$db->escape($projectid); 
	if ($colorid > 0) 		$sql.= ' AND p.color ='.$db->escape($colorid); 
	if ($stepid > 0) 		$sql.= ' AND p.step ='.$db->escape($stepid); 
	if ($statut >= 0) 		$sql.= ' AND fa.fk_statut ='.$db->escape($statut); 
...
//	$return['query']=$sql;
	...
	if ($resql) {
		...
		while ($j < $num) {
			...
			$title.=$langs->trans("TotalHT")." : ".price($objp->total)."\n";

			$event =array();
			$event['id'] = $objp->rowid;
			$event['title'] = $title;
			$event['start'] = $objp->dateo;
			$event['total'] = $objp->total;
			$event['color'] = "#".$bgcolor;
			$event['url'] = dol_buildpath("compta/facture", 1)."/card.php?id=".$objp->rowid;
