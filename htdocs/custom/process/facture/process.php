...
/**
 *  \file	   htdocs/process/factyre/process.php
 *  \ingroup	process
...
$rowidProcess=$process->fetch(0, $object->id, 'facture');
$process->id = $id;
$process->table_element="process_facture";
// set for the extrafields
$extrafields = new ExtraFields($db);
$extralabels=$extrafields->fetch_name_optionals_label('process_facture');
...
if ($id > 0 || ! empty($ref))
{
	...
	dol_fiche_head($head, 'process', $langs->trans("InvoiceCustomer"), 0, 'bill');

	print '<table class="border" width="100%">';

	$linkback = '<a href="'.DOL_URL_ROOT.'/facture/liste.php'.(! empty($socid)?'?socid='.$socid:'').'">'.$langs->trans("BackToList").'</a>';
	...
	print '<form name="selcolor" action="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'" method="post">';
 	...
 	$ColorFacture=explode(",", $conf->global->ColorBills);
	...
	for ($i=0;$i< $conf->global->NumberBills ;$i++)
	{
		print '<td align=center width=50px>';
		if ($i==$process->step)
			print img_picto($langs->trans("Step".$i), "facture/step_".$i."_sel@process" );
		else
		{
			...
			if ($process->accessright('facture', $i)==1)
			{
				...
				print img_picto($langs->trans("Step".$i), "facture/step_".$i."@process" );
				print '</a>';
			}
			//else
			//	print img_picto($langs->trans("Step".$i), "facture/step_".$i."_sel@process" );
