...
/**
 *	\file       ultimatepdf/core/modules/supplier_invoice/pdf/pdf_ultimate_supplierinvoice.modules.php
 *	\ingroup    fournisseur
...
/**
 *	Class to generate the supplier invoices with the ultimate_supplierinvoice model
 */
class pdf_ultimate_supplierinvoice extends ModelePDFSuppliersInvoices
{
	...
	*/
	...
	{
		global $user, $langs, $conf, $mysoc, $db, $hookmanager; //is $db usefull?
		...
		if ($conf->fournisseur->dir_output.'/facture')
		{
			...
			if (file_exists($dir))
			{
				...
				$tab_top_newpage = (empty($conf->global->ULTIMATE_SUPPLIERINVOICES_PDF_DONOTREPEAT_HEAD) ? $this->marge_haute + $logo_height + $top_shift : 10);
				
				$tab_height = 130 - $top_shift;
				$tab_height_newpage = 150;
				if (empty($conf->global->ULTIMATE_SUPPLIERINVOICES_PDF_DONOTREPEAT_HEAD)) $tab_height_newpage -= $top_shift;
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					// Add line
					if (! empty($conf->global->ULTIMATE_SUPPLIERINVOICES_PDF_DASH_BETWEEN_LINES) && $i < ($nblignes - 1))
					{
		...
		if ($conf->global->ULTIMATE_GENERATE_DOCUMENTS_WITHOUT_VAT == "no")
		{
			foreach( $this->tva as $tvakey => $tvaval )
			{
				if ($tvakey > 0)    // On affiche pas taux 0
				{
					...
					{
						$tvaval=$object->multicurrency_total_tva;
					}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		{
			// Logo					
			if (empty($conf->global->ULTIMATE_PDF_SUPPLIERINVOICE_DISABLE_MYCOMPANY_LOGO))
			{
        		$this->emetteur = $mysoc;
		...
		if(!empty($object->fk_project))
		{
			...
			$pdf->SetTextColorArray($textcolor);
			$pdf->SetFont('','B', $default_font_size);
			$pdf->MultiCell(100, 4, $outputlangs->transnoentities("RefProject") ." : ".$outputlangs->transnoentities($proj->ref), 0, 'R');
		}
	...
	function _pagefoot(&$pdf, $object, $outputlangs)
	{
		...
		return pdf_ultimatepagefoot($pdf,$outputlangs,'SUPPLIER_INVOICE_FREE_TEXT', $mysoc, $this->marge_basse, $this->marge_gauche, $this->page_hauteur, $object, $showdetails, $hidefreetext, $footertextcolor);
	}
    ...
    public function defineColumnField($object, $outputlangs, $hidedetails=0, $hidedesc=0, $hideref=0)
    {
		...
		if (!empty($conf->global->ULTIMATE_SUPPLIERINVOICES_WITH_LINE_NUMBER) || ($conf->global->ULTIMATE_DOCUMENTS_WITH_REF == "yes" && $this->atleastoneref==true))
	    {
