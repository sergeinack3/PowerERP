...
if ($action == 'setmod')
	powererp_set_const($db, "FACTORY_ADDON", $value, 'chaine', 0, '', $conf->entity);

if ($action == 'setdefaultother') {
	// save the setting
	$res = powererp_set_const(
...
if ($action == 'del') {
	...
	if ($db->query($sql))
		if ($conf->global->FACTORY_ADDON_PDF == "$value") 
			powererp_del_const($db, 'FACTORY_ADDON_PDF', $conf->entity);
...
if ($action == 'setdoc') {
	...
	if (powererp_set_const($db, "FACTORY_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity))
...
if ($action == 'addvirtualstock') {
	powererp_set_const($db, "FACTORY_AddVirtualstock", GETPOST('value'), 'chaine', 0, '', $conf->entity);
}
$addvirtualstock=$conf->global->FACTORY_AddVirtualstock;


if ($action == 'ChangePriceSetting') {
	powererp_set_const($db, "ChangePriceSetting", GETPOST('value'), 'chaine', 0, '', $conf->entity);
	// selon l'activation / d�sactivation on met  en place des tabs sur les cat�gorie de produits
	if (GETPOST('value') == 1) {
		// on ajoute l'onglet de param�trage des prix multiple
...
if ($action == 'componentprice') {
	powererp_set_const($db, "FACTORY_COMPONENT_BUYINGPRICE", $componentprice, 'chaine', 0, '', $conf->entity);
	powererp_set_const(
...
if ($action == 'getinfofromextrafield')
	powererp_set_const($db, "factory_extrafieldsNameInfo", GETPOST('extrafieldsName'), 'chaine', 0, '', $conf->entity);
...
foreach ($extrafields->attribute_type as $key => $value) {
	print "<option value=".$key." ".($conf->global->factory_extrafieldsNameInfo == $key ?'selected':'')." >";
	print $extrafields->attribute_label[$key]."</option>";
}
print "</select>";
...
if ( $addvirtualstock ==1) {
	print '<a href="'.$_SERVER["PHP_SELF"].'?action=addvirtualstock&value=0">';
	print img_picto($langs->trans("Enabled"), 'switch_on').'</a>';
} else {
	print '<a href="'.$_SERVER["PHP_SELF"].'?action=addvirtualstock&value=1">';
	print img_picto($langs->trans("Disabled"), 'switch_off').'</a>';
}
print '</td></tr>';
