...
/**
 * \file    test/phpunit/MouvementFunctionalTest.php
 * \ingroup mouvement
 ...
 *
 * @package Testmouvement
 */
class MouvementFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modMouvement')->click();
		} else {
			// Disable the module
			$this->byHref('modMouvement')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/mouvement/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('mouvement/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/mouvement/admin/about.php');
		$this->authenticate();
		return $this->assertContains('mouvement/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('mouvementwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modMouvement_MouvementTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modMouvement_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
