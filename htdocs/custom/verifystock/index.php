...
if($action == 'verifyStock')
{
	...
	$langs->trans('stockDataOk');
	$langs->trans('stockDataFail');
	$langs->trans('codeArt');
	$langs->trans('stockDataVerif');
	$langs->trans('launchCheck');
	$langs->trans('stockDataOk');
	$langs->trans('getPastStock');
	$langs->trans('stockInBase');
	$langs->trans('stockCalc');
	$langs->trans('stockDataOk');
	$langs->trans('dlStockFile');
...
if($action == 'getPastStock' && $stockDate != '')
{
		...
		if($res)
		{
			...
			if($transmit == 'file')
			{
			    ...
			    foreach ($prod_ref as $fk_product => $ref) {
			    	$stock_calc = (int) $stock_to_date[$fk_product];
		...
		<div>
			<table class="noborder">
				...
				foreach($stock_global AS $fk_product => $stock)
				{
					...
					$stock_calc = (int) $stock_global_calc[$fk_product];
					$hilite 	= ($stock_calc !== $stock)?' style="background: #FF0000; color: #FFF;"':'';
					$diff 		= (int) ($stock_calc - $stock);
