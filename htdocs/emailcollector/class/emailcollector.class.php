				...
				if (!empty($headers['References'])) {
					...
					foreach ($arrayofreferences as $reference) {
						...
						if (!empty($trackidfoundintorecipienttype)) {
							...
							$resultsearchtrackid = preg_match('/powererp-([a-z]+)([0-9]+)@'.preg_quote($host, '/').'/', $reference, $reg);
							if (empty($resultsearchtrackid) && getDolGlobalString('EMAIL_ALTERNATIVE_HOST_SIGNATURE')) {
								$resultsearchtrackid = preg_match('/powererp-([a-z]+)([0-9]+)@'.preg_quote(getDolGlobalString('EMAIL_ALTERNATIVE_HOST_SIGNATURE'), '/').'/', $reference, $reg);
							}
						}

						if (!empty($resultsearchtrackid)) {
							// We found a tracker (in recipient email or into a Reference matching the PowerERP server)
							$trackid = $reg[1].$reg[2];
