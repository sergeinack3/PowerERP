 ...
 */
class Tickets extends PowerERPApi
{
	/**
	...
	{
		if (!PowerERPApiAccess::$user->rights->ticket->read) {
		...
		if (!PowerERPApi::_checkAccessToResource('ticket', $this->ticket->id)) {
	...
	public function index($socid = 0, $sortfield = "t.rowid", $sortorder = "ASC", $limit = 100, $page = 0, $sqlfilters = '')
	{
		...
		if (!$socid && PowerERPApiAccess::$user->socid) {
	...
	public function post($request_data = null)
	{
		...
		if (!PowerERPApiAccess::$user->rights->ticket->write) {
		...
		if ($this->ticket->create(PowerERPApiAccess::$user) < 0) {
	...
	public function postNewMessage($request_data = null)
	{
		...
		if (!$this->ticket->createTicketMessage(PowerERPApiAccess::$user)) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->ticket->update($id, PowerERPApiAccess::$user)) {
			return $this->get($id);
		}
	...
	{
		if (!PowerERPApiAccess::$user->rights->ticket->delete) {
			throw new RestException(401);
		}
		$result = $this->ticket->fetch($id);
		if (!$result) {
