 ...
 */
class Contracts extends PowererpApi
{
	...
	{
		if (!PowererpApiAccess::$user->rights->contrat->lire) {
		...
		if (!PowererpApi::_checkAccessToResource('contrat', $this->contract->id)) {
	...
	{
		if (!PowererpApiAccess::$user->rights->contrat->creer) {
		...
		if ($this->contract->create(PowererpApiAccess::$user) < 0) {
	...
	public function activateLine($id, $lineid, $datestart, $dateend = null, $comment = null)
	{
		...
		$updateRes = $this->contract->active_line(PowererpApiAccess::$user, $lineid, $datestart, $dateend, $comment);
	...
	public function unactivateLine($id, $lineid, $datestart, $comment = null)
	{
		...
		$updateRes = $this->contract->close_line(PowererpApiAccess::$user, $lineid, $datestart, $comment);
	...
	public function deleteLine($id, $lineid)
	{
		...
		$updateRes = $this->contract->deleteline($lineid, PowererpApiAccess::$user);
		if ($updateRes > 0) {
			return $this->get($id);
		} else {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->contract->update(PowererpApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!PowererpApiAccess::$user->rights->contrat->supprimer) {
			throw new RestException(401);
		}
		$result = $this->contract->fetch($id);
		if (!$result) {
		...
		if (!$this->contract->delete(PowererpApiAccess::$user)) {
	...
	public function validate($id, $notrigger = 0)
	{
		...
		$result = $this->contract->validate(PowererpApiAccess::$user, '', $notrigger);
		if ($result == 0) {
	...
	public function close($id, $notrigger = 0)
	{
		...
		$result = $this->contract->closeAll(PowererpApiAccess::$user, $notrigger);
		if ($result == 0) {
