...
if ($action == 'create') {
	...
	$doleditor = new DolEditor('note_public', $note_public, '', '100', 'powererp_notes', 'In', 1, true, empty($conf->global->FCKEDITOR_ENABLE_NOTE_PUBLIC) ? 0 : 1, ROWS_3, '90%');
	print $doleditor->Create(1);
	print '</td></tr>';

	if (empty($user->socid)) {
		print '<tr><td>'.$langs->trans("NotePrivate").'</td><td class="tdtop">';
		$doleditor = new DolEditor('note_private', $note_private, '', '100', 'powererp_notes', 'In', 1, true, empty($conf->global->FCKEDITOR_ENABLE_NOTE_PRIVATE) ? 0 : 1, ROWS_3, '90%');
		print $doleditor->Create(1);
		print '</td></tr>';
	}
		...
		while ($cursorline <= $nbofservices) {
			...
			if ($result) {
				...
				if ($action != 'editline' || GETPOST('rowid') != $objp->rowid) {
					...
					$doleditor = new DolEditor('product_desc', $objp->description, '', 92, 'powererp_details', '', false, true, $enable, $nbrows, '90%');
					$doleditor->Create();
