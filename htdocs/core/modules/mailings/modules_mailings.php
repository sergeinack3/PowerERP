	...
	public function addTargetsToDatabase($mailing_id, $cibles)
	{
		...
		foreach ($cibles as $targetarray) {
			if (!empty($targetarray['email'])) { // avoid empty email address
				...
				$sql .= "'".$this->db->escape(dol_hash($powererp_main_instance_unique_id.";".$targetarray['email'].";".$targetarray['lastname'].";".$mailing_id.";".$conf->global->MAILING_EMAIL_UNSUBSCRIBE_KEY, 'md5'))."',";
