 ...
 *       \brief      Page called after a ping was done in js to the official powererp ping service.
...
if ($action == 'firstpingok') {
	...
	powererp_set_const($db, 'MAIN_FIRST_PING_OK_ID', $hash_unique_id, 'chaine', 0, '', $conf->entity);

	print 'First ping OK saved for entity '.$conf->entity;
} elseif ($action == 'firstpingko') {
	// If ko
	// Note: pings are by installation, done on entity 1.
	powererp_set_const($db, 'MAIN_LAST_PING_KO_DATE', dol_print_date($now, 'dayhourlog', 'gmt'), 'chaine', 0, '', $conf->entity); // erase last value
