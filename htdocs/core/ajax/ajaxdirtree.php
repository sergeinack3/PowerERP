...
if ($modulepart == 'ecm') {
	...
	$fullpathselecteddir = $powererp_main_data_root.'/medias/'.($selecteddir != '/' ? $selecteddir : '');
	$fullpathpreopened = $powererp_main_data_root.'/medias/'.($preopened != '/' ? $preopened : '');
}
					...
					if ($file != '.' && $file != '..' && ((!empty($val['fullrelativename']) && $val['id'] >= 0) || dol_is_dir($fullpathselecteddir.(preg_match('/\/$/', $fullpathselecteddir) ? '' : '/').$file))) {
						...
						if (preg_match('/^'.preg_quote($val['fullrelativename'].'/', '/').'/', $preopened)) {
							...
							if ($modulepart == 'ecm') {
								...
								$newfullpathselecteddir = $powererp_main_data_root.'/medias/'.($val['fullrelativename'] != '/' ? $val['fullrelativename'] : '');
							}
