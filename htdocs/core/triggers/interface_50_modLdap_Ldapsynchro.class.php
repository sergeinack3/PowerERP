 ...
 */
class InterfaceLdapsynchro extends PowerERPTriggers
{
	/**
	 * Constructor
	 *
	...
	public function runTrigger($action, $object, User $user, Translate $langs, Conf $conf)
	{
		...
		if ($action == 'USER_CREATE') {
			...
			if (!empty($conf->global->LDAP_SYNCHRO_ACTIVE) && getDolGlobalInt('LDAP_SYNCHRO_ACTIVE') === Ldap::SYNCHRO_POWERERP_TO_LDAP) {
				$ldap = new Ldap();
			...
			// Members
			dol_syslog("Trigger '".$this->name."' for action '$action' launched by ".__FILE__.". id=".$object->id);
			if (!empty($conf->global->LDAP_MEMBER_ACTIVE) && getDolGlobalInt('LDAP_MEMBER_ACTIVE') == Ldap::SYNCHRO_POWERERP_TO_LDAP) {
				$ldap = new Ldap();
				$result = $ldap->connect_bind();
				...
				if ($result > 0) {
					...
					// For member type
					if (!empty($conf->global->LDAP_MEMBER_TYPE_ACTIVE) && getDolGlobalInt('LDAP_MEMBER_TYPE_ACTIVE') === Ldap::SYNCHRO_POWERERP_TO_LDAP) {
						if ($object->typeid > 0) {
