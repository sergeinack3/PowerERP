	...
	public function setValues($db)
	{
		...
		if (!is_null($db) && is_object($db)) {
			...
			if ($resql) {
				...
				while ($i < $numr) {
					...
					if ($key) {
						...
						if (isset($_SERVER['POWERERP_'.$key])) {
							$value = $_SERVER['POWERERP_'.$key];
						} elseif (isset($_ENV['POWERERP_'.$key])) {
							$value = $_ENV['POWERERP_'.$key];
						}
			...
			if (!empty($this->global->LDAP_SYNCHRO_ACTIVE)) {
				if ($this->global->LDAP_SYNCHRO_ACTIVE == 'powererp2ldap') {
					$this->global->LDAP_SYNCHRO_ACTIVE = 1;
				} elseif ($this->global->LDAP_SYNCHRO_ACTIVE == 'ldap2powererp') {
					$this->global->LDAP_SYNCHRO_ACTIVE = 2;
				}
			}
			// For backward compatibility
			if (!empty($this->global->LDAP_MEMBER_ACTIVE) && $this->global->LDAP_MEMBER_ACTIVE == 'ldap2powererp') {
				$this->global->LDAP_MEMBER_ACTIVE = 2;
			}
			// For backward compatibility
			if (!empty($this->global->LDAP_MEMBER_TYPE_ACTIVE) && $this->global->LDAP_MEMBER_TYPE_ACTIVE == 'ldap2powererp') {
