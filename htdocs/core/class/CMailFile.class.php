...
class CMailFile
{
	...
	{
		global $conf, $powererp_main_data_root, $user;
		...
		$this->related_boundary = 'mul_'.dol_hash(uniqid("powererp2"), 3); // Force md5 hash (does not contains special chars)

		// On defini alternative_boundary
		$this->alternative_boundary = 'mul_'.dol_hash(uniqid("powererp3"), 3); // Force md5 hash (does not contains special chars)
		...
		if ($this->msgishtml) {
			...
			if (!empty($conf->global->MAIN_MAIL_ADD_INLINE_IMAGES_IF_IN_MEDIAS)) {	// Off by default
				...
				$findimg = $this->findHtmlImages($powererp_main_data_root.'/medias');
			}
	...
	{
		// phpcs:enable
		global $conf, $powererp_main_data_root;

		if (@is_writeable($powererp_main_data_root)) {	// Avoid fatal error on fopen with open_basedir
			$outputfile = $powererp_main_data_root."/powererp_mail.log";
	...
	public function save_dump_mail_in_err()
	{
		...
		if (@is_writeable($powererp_main_data_root)) {	// Avoid fatal error on fopen with open_basedir
			$srcfile = $powererp_main_data_root."/powererp_mail.log";
			if (getDolGlobalString('MAIN_MAIL_DEBUG_ERR_WITH_DATE')) {
				$destfile = $powererp_main_data_root."/powererp_mail.".dol_print_date(dol_now(), 'dayhourlog', 'gmt').".err";
			} else {
				$destfile = $powererp_main_data_root."/powererp_mail.err";
			}
	 ...
	 *
	 * @param	string	$images_dir		Location of physical images files. For example $powererp_main_data_root.'/medias'
	 ...
	 *
	 * @param	string	$images_dir		Location of where to store physicaly images files. For example $powererp_main_data_root.'/medias'
	 * @return	int 		        	>0 if OK, <0 if KO
	 */
