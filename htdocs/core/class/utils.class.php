...
class Utils
{
	...
	{
		global $conf, $langs, $powererp_main_data_root;
		...
		foreach ($choicesarray as $choice) {
			...
			if ($choice == 'tempfiles' || $choice == 'tempfilesold') {
				...
				if ($powererp_main_data_root) {
					$filesarray = dol_dir_list($powererp_main_data_root, "directories", 1, '^temp$', '', 'name', SORT_ASC, 2, 0, '', 1); // Do not follow symlinks
			...
			if ($choice == 'allfiles') {
				...
				if ($powererp_main_data_root) {
					$filesarray = dol_dir_list($powererp_main_data_root, "all", 0, '', 'install\.lock$', 'name', SORT_ASC, 0, 0, '', 1);	// No need to use recursive, we will delete directory
				}
			}
			...
			if ($choice == 'allfilesold') {
				// Delete all files (except install.lock, do not follow symbolic links)
				if ($powererp_main_data_root) {
					$filesarray = dol_dir_list($powererp_main_data_root, "files", 1, '', 'install\.lock$', 'name', SORT_ASC, 0, 0, '', 1, $nbsecondsold);	// No need to use recursive, we will delete directory
				}
			}
			...
			if ($choice == 'logfile' || $choice == 'logfiles') {
				// Define files log
				if ($powererp_main_data_root) {
					$filesarray = dol_dir_list($powererp_main_data_root, "files", 0, '.*\.log[\.0-9]*(\.gz)?$', 'install\.lock$', 'name', SORT_ASC, 0, 0, '', 1);
				}
	...
	{
		global $db, $conf, $langs, $powererp_main_data_root;
		global $powererp_main_db_name, $powererp_main_db_host, $powererp_main_db_user, $powererp_main_db_port, $powererp_main_db_pass;
		global $powererp_main_db_character_set;
		...
		if ($file == 'auto') {
			...
			$file = $prefix.'_'.$powererp_main_db_name.'_'.dol_sanitizeFileName(DOL_VERSION).'_'.dol_print_date(dol_now('gmt'), "dayhourlogsmall", 'tzuser').'.'.$ext;
		}
		...
		if ($type == 'mysql' || $type == 'mysqli') {
			...
			//$param=escapeshellarg($powererp_main_db_name)." -h ".escapeshellarg($powererp_main_db_host)." -u ".escapeshellarg($powererp_main_db_user)." -p".escapeshellarg($powererp_main_db_pass);
			$param = $powererp_main_db_name." -h ".$powererp_main_db_host;
			$param .= " -u ".$powererp_main_db_user;
			if (!empty($powererp_main_db_port)) {
				$param .= " -P ".$powererp_main_db_port." --protocol=tcp";
			}
			...
			if ($powererp_main_db_character_set == 'utf8mb4') {
			...
			if (!empty($powererp_main_db_pass)) {
				...
				$paramclear .= ' -p"'.str_replace(array('"', '`', '$'), array('\"', '\`', '\$'), $powererp_main_db_pass).'"';
			}
		...
		if ($type == 'postgresql' || $type == 'pgsql') {
			...
			//$param="-F c";
			$param = "-F p";
			$param .= " --no-tablespaces --inserts -h ".$powererp_main_db_host;
			$param .= " -U ".$powererp_main_db_user;
			if (!empty($powererp_main_db_port)) {
			...
			/*if (!empty($powererp_main_db_pass))
			 {
			 $paramcrypted.=" -W".preg_replace('/./i','*',$powererp_main_db_pass);
			 $paramclear.=" -W".$powererp_main_db_pass;
			 }*/
			$paramcrypted .= " -w ".$powererp_main_db_name;
			$paramclear .= " -w ".$powererp_main_db_name;
	...
	public function compressSyslogs()
	{
		...
		$tabfiles = dol_dir_list(DOL_DATA_ROOT, 'files', 0, '^(powererp_.+|odt2pdf)\.log$'); // Also handle other log files like powererp_install.log
	 ...
	 *	Note about foreign keys constraints: for PowerERP, since there are a lot of constraints and when imported the tables will be inserted in the dumped order, not in constraints order, then we ABSOLUTELY need to use SET FOREIGN_KEY_CHECKS=0; when importing the sql dump.
	...
	public function backupTables($outputfile, $tables = '*')
	{
		...
		$sqlhead = '';
		$sqlhead .= "-- ".$db::LABEL." dump via php with PowerERP ".DOL_VERSION."
--
	...
	public function sendBackup($sendto = '', $from = '', $subject = '', $message = '', $filename = '', $filter = '', $sizelimit = 100000000)
	{
		...
		if ($filepath) {
			if ($filesize > $sizelimit) {
				...
				$documenturl =  $powererp_main_url_root.'/document.php?modulepart=systemtools&atachement=1&file=backup/'.urlencode($filename[0]);
