...
abstract class CommonObject
{
	...
	{
		global $user, $powererp_main_url_root;
		...
		if (in_array($key, array('rowid', 'ref')) && method_exists($this, 'getNomUrl')) {
			...
			if ($resql) {
				if ($filter_categorie === false) {
					...
					foreach ($ways as $way) {
						$toprint[] = '<li class="select2-search-choice-powererp noborderoncategories"' . ($c->color ? ' style="background: #' . $c->color . ';"' : ' style="background: #aaa"') . '>' . img_object('', 'category') . ' ' . $way . '</li>';
					}
