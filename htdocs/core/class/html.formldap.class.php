...
class FormLdap
{
	 ...
	 *	@param	int			$scriptonly		Add warning if synchro only work with a script (0 = disable, 1 = Powererp2ldap, 2 = ldap2powererp, 3 = all)
	 * 	@param	int			$showempty		Add an empty field
	 *  @return	string						HTML select string
	 */
	public function selectLdapDnSynchroActive($selected = 0, $htmlname = 'activesynchro', $exclude = array(), $scriptonly = 0, $showempty = 0)
	{
		...
		$powererp2ldaplabel = $langs->trans("PowererpToLDAP") . (($scriptonly == 1 || $scriptonly == 3) ? " (".$langs->trans("SupportedForLDAPExportScriptOnly").")" : "");
		$ldap2powererplabel = $langs->trans("LDAPToPowererp") . (($scriptonly == 2 || $scriptonly == 3) ? " (".$langs->trans("SupportedForLDAPImportScriptOnly").")" : "");

		$arraylist = array(
			Ldap::SYNCHRO_NONE				=> $langs->trans("No"),
			Ldap::SYNCHRO_DOLIBARR_TO_LDAP	=> $powererp2ldaplabel,
			Ldap::SYNCHRO_LDAP_TO_DOLIBARR	=> $ldap2powererplabel
		);
