	...
	public function getTriggersList($forcedirtriggers = null)
	{
		...
		foreach ($orders as $key => $value) {
			...
			try {
				...
				if (is_subclass_of($objMod, 'PowerERPTriggers')) {
					...
					$triggers[$j]['fullpath'] = $fullpath[$key];
					$triggers[$j]['relpath'] = $relpath[$key];
					$triggers[$j]['status'] = img_picto('Error: Trigger '.$modName.' does not extends PowerERPTriggers', 'warning');

					//print 'Error: Trigger '.$modName.' does not extends PowerERPTriggers<br>';
					$text = 'Error: Trigger '.$modName.' does not extends PowerERPTriggers';
				}
			} catch (Exception $e) {
				print $e->getMessage();
			}
