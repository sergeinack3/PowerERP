	...
	public function send($notifcode, $object, $filename_list = array(), $mimetype_list = array(), $mimefilename_list = array())
	{
			...
			if ($num > 0) {
				...
				while ($i < $num && !$error) {	// For each notification couple defined (third party/actioncode)
					...
					if (dol_strlen($obj->email)) {
						...
						$subject = '['.$mysoc->name.'] '.$outputlangs->transnoentitiesnoconv("PowererpNotification").($projtitle ? ' '.$projtitle : '');
		...
		if (!$error) {
			foreach ($conf->global as $key => $val) {
				...
				$subject = '['.$mysoc->name.'] '.$langs->transnoentitiesnoconv("PowererpNotification").($projtitle ? ' '.$projtitle : '');
