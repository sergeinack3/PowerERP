...
class FormTicket
{
	...
	public function showForm($withdolfichehead = 0, $mode = 'edit', $public = 0, Contact $with_contact = null)
	{
		...
		$toolbarname = 'powererp_notes';
		if ($this->ispublic) {
			$toolbarname = 'powererp_details';
	...
	public function showMessageForm($width = '40%')
	{
		...
		if ($user->rights->ticket->write && !$user->socid) {
			...
			$doleditor = new DolEditor('mail_intro', $mail_intro, '100%', 90, 'powererp_details', '', false, $uselocalbrowser, getDolGlobalInt('FCKEDITOR_ENABLE_SOCIETE'), ROWS_2, 70);
		...
		if ($user->rights->ticket->write && !$user->socid) {
			...
			print '</td><td>';
			include_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
			$doleditor = new DolEditor('mail_signature', $mail_signature, '100%', 150, 'powererp_details', '', false, $uselocalbrowser, getDolGlobalInt('FCKEDITOR_ENABLE_SOCIETE'), ROWS_2, 70);
			$doleditor->Create();
			print '</td></tr>';
		}
