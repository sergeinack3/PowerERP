...
if ($action !== 'editcomment') {
	...
	$doleditor = new DolEditor('comment_description', $desc, '', 80, 'powererp_notes', 'In', 0, true, true, ROWS_3, '100%');
...
if (!empty($object->comments)) {
	...
	foreach ($object->comments as $comment) {
		...
		if ($action === 'editcomment' && $comment->id == $idcomment) {
			$doleditor = new DolEditor('comment_description', $comment->description, '', 80, 'powererp_notes', 'In', 0, true, true, ROWS_3, '100%');
			print $doleditor->Create(1);
		} else {
