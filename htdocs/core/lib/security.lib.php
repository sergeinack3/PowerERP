 ...
 *  \ingroup    core
 *  \brief		Set of function used for powererp security (common function included into filefunc.inc.php)
 ...
 *  Note: If a backup is restored onto another instance with a different $powererp_main_instance_unique_id, then decoded value will differ.
 *
 *	@param   string		$chain		string to encode
 *	@param   string		$key		If '', we use $powererp_main_instance_unique_id
...
function dolEncrypt($chain, $key = '', $ciphering = 'AES-256-CTR', $forceseed = '')
{
	...
	global $powererp_disable_dolcrypt_for_debug;
	...
	if (empty($key)) {
		$key = $powererp_main_instance_unique_id;
	}
	if (empty($ciphering)) {
	...
	if (function_exists('openssl_encrypt') && empty($powererp_disable_dolcrypt_for_debug)) {
		$ivlen = 16;
...
function dolGetLdapPasswordHash($password, $type = 'md5')
{
	...
	if ($type === 'md5') {
		...
		return '{MD5}' . base64_encode(hex2bin($password)); // Create OpenLDAP MD5 password from PowerERP MD5 password
	} elseif ($type === 'smd5') {
