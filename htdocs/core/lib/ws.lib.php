 ...
 *
 *  @param	array	$authentication     Array with authentication informations ('login'=>,'password'=>,'entity'=>,'powererpkey'=>)
...
function check_authentication($authentication, &$error, &$errorcode, &$errorlabel)
{
	...
	global $powererp_main_authentication, $powererp_auto_user;

	$fuser = new User($db);

	if (!$error && ($authentication['powererpkey'] != $conf->global->WEBSERVICES_KEY)) {
		$error++;
		$errorcode = 'BAD_VALUE_FOR_SECURITY_KEY';
		$errorlabel = 'Value provided into powererpkey entry field does not match security key defined in Webservice module setup';
	}
