...
function dol_check_secure_access_document($modulepart, $original_file, $entity, $fuser = '', $refname = '', $mode = 'read')
{
	...
	global $powererp_main_data_root, $powererp_main_document_root_alt;
	global $object;
	...
	if ($modulepart == 'medias' && !empty($powererp_main_data_root)) {
		...
		$accessallowed = ($user->admin && basename($original_file) == $original_file && preg_match('/^powererp.*\.(log|json)$/', basename($original_file)));
		...
		$original_file = $powererp_main_data_root.'/doctemplates/websites/'.$original_file;
	} elseif ($modulepart == 'packages' && !empty($powererp_main_data_root)) {
