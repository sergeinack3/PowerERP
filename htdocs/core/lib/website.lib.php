...
function dolWebsiteReplacementOfLinks($website, $content, $removephppart = 0, $contenttype = 'html', $containerid = '')
{
	...
	dol_syslog('dolWebsiteReplacementOfLinks start (contenttype='.$contenttype." containerid=".$containerid." USEPOWERERPEDITOR=".(defined('USEPOWERERPEDITOR') ? '1' : '')." USEPOWERERPSERVER=".(defined('USEPOWERERPSERVER') ? '1' : '').')', LOG_DEBUG);
...
function dolWebsiteOutput($content, $contenttype = 'html', $containerid = '')
{
	...
	global $powererp_main_url_root, $powererp_main_data_root;
	global $website;
	global $includehtmlcontentopened;

	$nbrep = 0;

	dol_syslog("dolWebsiteOutput start - contenttype=".$contenttype." containerid=".$containerid." USEPOWERERPEDITOR=".(defined('USEPOWERERPEDITOR') ? '1' : '')." USEPOWERERPSERVER=".(defined('USEPOWERERPSERVER') ? '1' : '').' includehtmlcontentopened='.$includehtmlcontentopened);
...
function dolWebsiteSaveContent($content)
{
	...
	//dol_syslog("dolWebsiteSaveContent start (mode=".(defined('USEPOWERERPSERVER')?'USEPOWERERPSERVER':'').')');
...
function getPagesFromSearchCriterias($type, $algo, $searchstring, $max = 25, $sortfield = 'date_creation', $sortorder = 'DESC', $langcode = '', $otherfilters = 'null', $status = 1)
{
	...
	if (!$error && (empty($max) || ($found < $max)) && (preg_match('/sitefiles/', $algo))) {
		...
		$pathofwebsite = $powererp_main_data_root.($conf->entity > 1 ? '/'.$conf->entity : '').'/website/'.$website->ref;
