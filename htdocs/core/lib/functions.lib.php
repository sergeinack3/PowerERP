...
/**
 * Return powererp user constant string value
 *
 * @param string $key 		key to return value, return '' if not set
 * @param string $default 	value to return
 * @return string
...
/**
 * Return powererp user constant int value
 *
 * @param string 	$key 			key to return value, return 0 if not set
 * @param int 		$default 		value to return
 * @return int
...
function GETPOST($paramname, $check = 'alphanohtml', $method = 0, $filter = null, $options = null, $noreplace = 0)
{
	...
	if ($paramname == 'backtopage' || $paramname == 'backtolist' || $paramname == 'backtourl') {
		...
		do {
			...
			$out = preg_replace(array('/^[^\?]*%/'), '', $out);				// We remove any % chars before the ?. Example in url: '/product/stock/card.php?action=create&backtopage=%2Fpowererp_dev%2Fhtdocs%2Fpro%25duct%2Fcard.php%3Fid%3Dabc'
	...
	function dol_getprefix($mode = '')
	{
		...
		if ($mode == 'email') {
			...
			if (!empty($conf->file->instance_unique_id)) {
				return sha1('powererp'.$conf->file->instance_unique_id);
			}
		...
		global $powererp_main_instance_unique_id, $powererp_main_cookie_cryptkey;	// This is loaded by filefunc.inc.php
		$tmp_instance_unique_id = empty($powererp_main_instance_unique_id) ? (empty($powererp_main_cookie_cryptkey) ? '' : $powererp_main_cookie_cryptkey) : $powererp_main_instance_unique_id; // Unique id of instance

		// The recommended value (may be not defined for old versions)
		if (!empty($tmp_instance_unique_id)) {
			return sha1('powererp'.$tmp_instance_unique_id);
		}
 ...
 *  @param	int		$type						0=Used for a Filesystem path, 1=Used for an URL path (output relative), 2=Used for an URL path (output full path using same host that current url), 3=Used for an URL path (output full path using host defined into $powererp_main_url_root of conf file)
...
function dol_syslog($message, $level = LOG_INFO, $ident = 0, $suffixinfilename = '', $restricttologhandler = '', $logcontext = null)
{
	...
	if (defined('USEEXTERNALSERVER') && !defined('USEPOWERERPSERVER') && !defined('USEPOWERERPEDITOR')) {
...
function dol_print_error($db = '', $error = '', $errors = null)
{
	...
	if ($_SERVER['DOCUMENT_ROOT']) {    // Mode web
		$out .= $langs->trans("PowerERPHasDetectedError").".<br>\n";
	...
	if (empty($powererp_main_prod) && $_SERVER['DOCUMENT_ROOT'] && function_exists('xdebug_print_function_stack') && function_exists('xdebug_call_file')) {
	...
	if (empty($powererp_main_prod)) {
		...
		print $langs->trans("PowerERPHasDetectedError").'. ';
		print $langs->trans("YouCanSetOptionPowerERPMainProdToZero");
...
/**
 * Function to forge a SQL criteria from a PowerERP filter syntax string.
