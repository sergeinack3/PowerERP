...
if (!function_exists('dol_loginfunction')) {
	...
	{
		global $powererp_main_demo, $powererp_main_force_https;
		...
		*/
		...
		if (!empty($conf->global->MAIN_SESSION_TIMEOUT)) {
			if (session_status() != PHP_SESSION_ACTIVE) {
				...
				setcookie($sessiontimeout, $conf->global->MAIN_SESSION_TIMEOUT, 0, "/", null, (empty($powererp_main_force_https) ? false : true), true);
			}
		}
		...
		$demologin = '';
		$demopassword = '';
		if (!empty($powererp_main_demo)) {
			$tab = explode(',', $powererp_main_demo);
			$demologin = $tab[0];
			$demopassword = $tab[1];
		}
...
function encodedecode_dbpassconf($level = 0)
{
	...
	if ($fp = fopen(DOL_DOCUMENT_ROOT.'/conf/conf.php', 'r')) {
		while (!feof($fp)) {
			...
			$reg = array();
			if (preg_match('/^[^#]*powererp_main_db_encrypted_pass[\s]*=[\s]*(.*)/i', $buffer, $reg)) {	// Old way to save crypted value
			...
			} elseif (preg_match('/^[^#]*powererp_main_db_pass[\s]*=[\s]*(.*)/i', $buffer, $reg)) {
				$val = trim($reg[1]); // This also remove CR/LF
				$val = preg_replace('/^["\']/', '', $val);
				$val = preg_replace('/["\'][\s;]*$/', '', $val);
			...
			if ($lineofpass) {
				...
				if ($level == 0) {
					$config .= '$powererp_main_db_pass=\''.$passwd.'\';'."\n";
				}
				if ($level == 1) {
					$config .= '$powererp_main_db_pass=\'crypted:'.$passwd_crypted.'\';'."\n";
				}
