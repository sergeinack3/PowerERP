			...
			if (preg_match('/^--\sV(MYSQL|PGSQL)([^\s]*)/i', $buf, $reg)) {
				...
				if ($qualified) {
					if (!empty($reg[2])) {
						...
						{
							$dbcollation = strtoupper(preg_replace('/_/', '', $conf->db->powererp_main_db_collation));
							//var_dump($reg[2]);
							//var_dump($dbcollation);
							if (empty($conf->db->powererp_main_db_collation) || ($reg[2] != $dbcollation)) {
								$qualified = 0;
							}
							//var_dump($qualified);
						}
 ...
 *	@return     int         			<0 if KO, >0 if OK
 *
 *	@see		powererp_get_const(), powererp_set_const(), dol_set_user_param()
 */
function powererp_del_const($db, $name, $entity = 1)
{
	global $conf;
	...
	dol_syslog("admin.lib::powererp_del_const", LOG_DEBUG);
	$resql = $db->query($sql);
	if ($resql) {
		$conf->global->$name = '';
		return 1;
 ...
 *
 *	@see		powererp_del_const(), powererp_set_const(), dol_set_user_param()
 */
function powererp_get_const($db, $name, $entity = 1)
{
	$value = '';
	...
	dol_syslog("admin.lib::powererp_get_const", LOG_DEBUG);
	$resql = $db->query($sql);
	if ($resql) {
		$obj = $db->fetch_object($resql);
		if ($obj) {
 ...
 *
 *	@see		powererp_del_const(), powererp_get_const(), dol_set_user_param()
 */
function powererp_set_const($db, $name, $value, $type = 'chaine', $visible = 0, $note = '', $entity = 1)
{
	global $conf;
	...
	// Check parameters
	if (empty($name)) {
		dol_print_error($db, "Error: Call to function powererp_set_const with wrong parameters", LOG_ERR);
		exit;
	}

	//dol_syslog("powererp_set_const name=$name, value=$value type=$type, visible=$visible, note=$note entity=$entity");
	...
	dol_syslog("admin.lib::powererp_set_const", LOG_DEBUG);
	$resql = $db->query($sql);
...
function activateModule($value, $withdeps = 1)
{
	...
	if (is_array($vermin) && versioncompare($verdol, $vermin) < 0) {
		$ret['errors'][] = $langs->trans("ErrorModuleRequirePowererpVersion", versiontostring($vermin));
		return $ret;
	}
		...
		if ($result) {
			...
			if ($const == 'ADHERENT_CARD_TYPE' || $const == 'ADHERENT_ETIQUETTE_TYPE') {
				...
				if ($obj->type == 'textarea' || in_array($const, array('ADHERENT_CARD_TEXT', 'ADHERENT_CARD_TEXT_RIGHT', 'ADHERENT_ETIQUETTE_TEXT'))) {
					...
					$doleditor = new DolEditor('constvalue'.(empty($strictw3c) ? '' : ($strictw3c == 3 ? '_'.$const : '[]')), $obj->value, '', 160, 'powererp_notes', '', false, false, $conf->fckeditor->enabled, ROWS_5, '90%');
					$doleditor->Create();
