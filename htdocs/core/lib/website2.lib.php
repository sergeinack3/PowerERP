...
function dolSaveMasterFile($filemaster)
{
	...
	$mastercontent .= "if (! defined('USEPOWERERPSERVER') && ! defined('USEPOWERERPEDITOR')) {\n";
...
function dolSavePageAlias($filealias, $object, $objectpage)
{
	...
	$aliascontent .= 'global $powererp_main_data_root;'."\n";
	$aliascontent .= 'if (empty($powererp_main_data_root)) require \'./page'.$objectpage->id.'.tpl.php\'; ';
	$aliascontent .= 'else require $powererp_main_data_root.\'/website/\'.$website->ref.\'/page'.$objectpage->id.'.tpl.php\';'."\n";
	$aliascontent .= '?>'."\n";
...
function dolSavePageContent($filetpl, Website $object, WebsitePage $objectpage, $backupold = 0)
{
	...
	$tplcontent .= "if (! defined('USEPOWERERPSERVER') && ! defined('USEPOWERERPEDITOR')) {\n";
	...
	$tplcontent .= '<meta name="generator" content="'.DOL_APPLICATION_TITLE.' '.DOL_VERSION.' (https://www.powererp.org)" />'."\n";
	$tplcontent .= '<meta name="powererp:pageid" content="'.dol_string_nohtmltag($objectpage->id).'" />'."\n";
	...
	$tplcontent .= '<!-- File generated by PowerERP website module editor -->'."\n";
...
function dolSaveIndexPage($pathofwebsite, $fileindex, $filetpl, $filewrapper, $object = null)
{
	...
	if ($fileindex) {
		...
		$indexcontent .= "if (! defined('USEPOWERERPSERVER') && ! defined('USEPOWERERPEDITOR')) { require_once './master.inc.php'; } // Load master if not already loaded\n";
			...
			if (empty($objectpage->lang) || !in_array($objectpage->lang, explode(',', $object->otherlang))) {
				if (empty($conf->global->WEBSITE_DISABLE_MAIN_LANGUAGE_INTO_LANGSUBDIR) && is_object($object) && !empty($object->otherlang)) {
					...
					foreach (explode(',', $object->otherlang) as $sublang) {
						...
						$indexcontent .= "// BEGIN PHP File generated to provide an index.php as Home Page or alias redirector - DO NOT MODIFY - It is just a generated wrapper.\n";
						$indexcontent .= '$websitekey=basename(__DIR__); if (empty($websitepagefile)) $websitepagefile=__FILE__;'."\n";
						$indexcontent .= "if (! defined('USEPOWERERPSERVER') && ! defined('USEPOWERERPEDITOR')) { require_once '".$relpath."/master.inc.php'; } // Load master if not already loaded\n";
						$indexcontent .= 'if (!empty($_GET[\'pageref\']) || !empty($_GET[\'pagealiasalt\']) || !empty($_GET[\'pageid\'])) {'."\n";
						$indexcontent .= "	require_once DOL_DOCUMENT_ROOT.'/core/lib/website.lib.php';\n";
						$indexcontent .= "	require_once DOL_DOCUMENT_ROOT.'/core/website.inc.php';\n";
						$indexcontent .= '	redirectToContainer($_GET[\'pageref\'], $_GET[\'pagealiasalt\'], $_GET[\'pageid\']);'."\n";
