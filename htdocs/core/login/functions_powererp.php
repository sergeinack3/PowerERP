 ...
 */
function check_user_password_powererp($usertotest, $passwordtotest, $entitytotest = 1)
{
	...
	if (!empty($usertotest)) {
		...
		dol_syslog("functions_powererp::check_user_password_powererp usertotest=".$usertotest." passwordtotest=".preg_replace('/./', '*', $passwordtotest)." entitytotest=".$entitytotest);
		...
		if ($resql) {
			...
			if ($obj) {
				...
				if ($cryptType == 'auto') {
					if ($passcrypted && dol_verifyHash($passtyped, $passcrypted, '0')) {
						$passok = true;
						dol_syslog("functions_powererp::check_user_password_powererp Authentification ok - hash ".$cryptType." of pass is ok");
					}
				}
				...
				if (!$passok) {
					if ((!$passcrypted || $passtyped)
						...
						$passok = true;
						dol_syslog("functions_powererp::check_user_password_powererp Authentification ok - found pass in database");
					}
				}
				...
				if ($passok) {
					...
					dol_syslog("functions_powererp::check_user_password_powererp Authentication KO bad password for '".$usertotest."', cryptType=".$cryptType, LOG_NOTICE);
				...
				if ($passok && !empty($conf->multicompany->enabled)) {	// We must check entity
					...
					if (!isset($mc)) {
						...
						if ($ret < 0) {
							dol_syslog("functions_powererp::check_user_password_powererp Authentication KO entity '".$entitytotest."' not allowed for user '".$obj->rowid."'", LOG_NOTICE);
							$login = ''; // force authentication failure
						}
					}
				}
			} else {
				dol_syslog("functions_powererp::check_user_password_powererp Authentication KO user not found for '".$usertotest."'", LOG_NOTICE);
				sleep(1);
			...
			dol_syslog("functions_powererp::check_user_password_powererp Authentication KO db error for '".$usertotest."' error=".$db->lasterror(), LOG_ERR);
			sleep(1);
