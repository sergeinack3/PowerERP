...
function check_user_password_ldap($usertotest, $passwordtotest, $entitytotest)
{
	...
	global $_POST;
	global $powererp_main_auth_ldap_host, $powererp_main_auth_ldap_port;
	global $powererp_main_auth_ldap_version, $powererp_main_auth_ldap_servertype;
	global $powererp_main_auth_ldap_login_attribute, $powererp_main_auth_ldap_dn;
	global $powererp_main_auth_ldap_admin_login, $powererp_main_auth_ldap_admin_pass;
	global $powererp_main_auth_ldap_filter;
	global $powererp_main_auth_ldap_debug;
	...
	if ($usertotest) {
		...
		$ldaphost = $powererp_main_auth_ldap_host;
		$ldapport = $powererp_main_auth_ldap_port;
		$ldapversion = $powererp_main_auth_ldap_version;
		$ldapservertype = (empty($powererp_main_auth_ldap_servertype) ? 'openldap' : $powererp_main_auth_ldap_servertype);

		$ldapuserattr = $powererp_main_auth_ldap_login_attribute;
		$ldapdn = $powererp_main_auth_ldap_dn;
		$ldapadminlogin = $powererp_main_auth_ldap_admin_login;
		$ldapadminpass = $powererp_main_auth_ldap_admin_pass;
		$ldapdebug = ((empty($powererp_main_auth_ldap_debug) || $powererp_main_auth_ldap_debug == "false") ? false : true);
		...
		$userSearchFilter = "";
		if (empty($powererp_main_auth_ldap_filter)) {
			$userSearchFilter = "(".$ldapuserattr."=".$usertotest.")";
		} else {
			$userSearchFilter = str_replace('%1%', $usertotest, $powererp_main_auth_ldap_filter);
		}
		...
		if ($ldapadminlogin || $powererp_main_auth_ldap_filter) {
		...
		if ($result > 0) {
			if ($result == 2) {	// Connection is ok for user/pass into LDAP
				...
				// ldap2powererp synchronisation
				if ($login && !empty($conf->ldap->enabled) && getDolGlobalInt('LDAP_SYNCHRO_ACTIVE') == Ldap::SYNCHRO_LDAP_TO_POWERERP) {	// ldap2powererp synchronization
					dol_syslog("functions_ldap::check_user_password_ldap Sync ldap2powererp");
					...
					if ($resultFetchUser > 0) {
						...
						//$resultUpdate = $usertmp->update_ldap2powererp($ldap);
					}
