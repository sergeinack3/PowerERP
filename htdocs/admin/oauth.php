...
if ($action == 'add') {		// $provider is OAUTH_XXX
	if ($provider && $provider != '-1') {
		...
		if (getDolGlobalString($constname)) {
			...
			powererp_set_const($db, $constname, $langs->trans('ToComplete'), 'chaine', 0, '', $conf->entity);
...
if ($action == 'update') {
	foreach ($conf->global as $key => $val) {
		if (!empty($val) && preg_match('/^OAUTH_.+_ID$/', $key)) {
			...
			if (!powererp_set_const($db, $constvalue.'_ID', GETPOST($constvalue.'_ID'), 'chaine', 0, '', $conf->entity)) {
				$error++;
			}
			// If we reset this provider, we also remove the secret
			if (!powererp_set_const($db, $constvalue.'_SECRET', GETPOST($constvalue.'_ID') ? GETPOST($constvalue.'_SECRET') : '', 'chaine', 0, '', $conf->entity)) {
				$error++;
			}
			if (GETPOSTISSET($constvalue.'_URLAUTHORIZE')) {
				if (!powererp_set_const($db, $constvalue.'_URLAUTHORIZE', GETPOST($constvalue.'_URLAUTHORIZE'), 'chaine', 0, '', $conf->entity)) {
					$error++;
				}
			}
			if (GETPOSTISSET($constvalue.'_TENANT')) {
				if (!powererp_set_const($db, $constvalue.'_TENANT', GETPOST($constvalue.'_TENANT'), 'chaine', 0, '', $conf->entity)) {
					$error++;
				}
			}
			if (GETPOSTISSET($constvalue.'_SCOPE')) {
				...
				if (!powererp_set_const($db, $constvalue.'_SCOPE', $scopestring, 'chaine', 0, '', $conf->entity)) {
					$error++;
				}
			} else {
				if (!powererp_set_const($db, $constvalue.'_SCOPE', '', 'chaine', 0, '', $conf->entity)) {
					$error++;
				}
			}
		}
...
if ($action == 'delete_entry') {
	...
	if (!powererp_del_const($db, $globalkey.'_NAME', $conf->entity) || !powererp_del_const($db, $globalkey.'_ID', $conf->entity) || !powererp_del_const($db, $globalkey.'_SECRET', $conf->entity) ||  !powererp_del_const($db, $globalkey.'_URLAUTHORIZE', $conf->entity) || !powererp_del_const($db, $globalkey.'_SCOPE', $conf->entity)) {
