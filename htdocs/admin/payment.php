...
if ($action == 'updateMask') {
	...
	if ($maskconstpayment && preg_match('/_MASK$/', $maskconstpayment)) {
		$res = powererp_set_const($db, $maskconstpayment, $maskpayment, 'chaine', 0, '', $conf->entity);
	}
...
if ($action == 'setmod') {
	powererp_set_const($db, "PAYMENT_ADDON", $value, 'chaine', 0, '', $conf->entity);
}

if ($action == 'setparams') {
	$freetext = GETPOST('FACTURE_PAYMENTS_ON_DIFFERENT_THIRDPARTIES_BILLS', 'restricthtml'); // No alpha here, we want exact string
	$res = powererp_set_const($db, "FACTURE_PAYMENTS_ON_DIFFERENT_THIRDPARTIES_BILLS", $freetext, 'chaine', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}

	$res = powererp_set_const($db, "PAYMENTS_REPORT_GROUP_BY_MOD", GETPOST('PAYMENTS_REPORT_GROUP_BY_MOD', 'int'), 'chaine', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
