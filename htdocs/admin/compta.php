...
if ($action == 'update') {
	...
	if (in_array($accounting_mode, $accounting_modes)) {
		if (!powererp_set_const($db, 'ACCOUNTING_MODE', $accounting_mode, 'chaine', 0, '', $conf->entity)) {
			$error++;
		}
	} else {
		$error++;
	...
	if (!empty($report_include_varpay)) {
		if ($report_include_varpay == 'yes') {
			if (!powererp_set_const($db, 'ACCOUNTING_REPORTS_INCLUDE_VARPAY', 1, 'chaine', 0, '', $conf->entity)) {
				$error++;
			}
		}
	}
	if ($report_include_varpay == 'no') {
		if (!powererp_del_const($db, 'ACCOUNTING_REPORTS_INCLUDE_VARPAY', $conf->entity)) {
			$error++;
		}
	}
	...
	if (!empty($report_include_loan)) {
		if ($report_include_loan == 'yes') {
			if (!powererp_set_const($db, 'ACCOUNTING_REPORTS_INCLUDE_LOAN', 1, 'chaine', 0, '', $conf->entity)) {
				$error++;
			}
		}
	}
	if ($report_include_loan == 'no') {
		if (!powererp_del_const($db, 'ACCOUNTING_REPORTS_INCLUDE_LOAN', $conf->entity)) {
			$error++;
		}
	}
