...
if ($action == 'setgeneraterule') {
	if (!powererp_set_const($db, 'USER_PASSWORD_GENERATED', GETPOST("value", "alphanohtml"), 'chaine', 0, '', $conf->entity)) {
		dol_print_error($db);
	}
}
...
if ($action == 'activate_encrypt') {
	...
	powererp_del_const($db, "DATABASE_PWD_ENCRYPTED", $conf->entity);
	// We set entity=0 (all) because DATABASE_PWD_ENCRYPTED is a setup into conf file, so always shared for everybody
	$entityforall = 0;
	powererp_set_const($db, "DATABASE_PWD_ENCRYPTED", "1", 'chaine', 0, '', $entityforall);
...
if ($action == 'activate_encryptdbpassconf') {
	...
	if ($result > 0) {
		...
		//powererp_set_const($db, "MAIN_DATABASE_PWD_CONFIG_ENCRYPTED", "1");
		header("Location: security.php");
		exit;
	} else {
		setEventMessages($langs->trans('InstrucToEncodePass', dol_encode($powererp_main_db_pass)), null, 'warnings');
	}
	...
	if ($result > 0) {
		...
		// database value not required
		//powererp_del_const($db, "MAIN_DATABASE_PWD_CONFIG_ENCRYPTED",$conf->entity);
		header("Location: security.php");
		exit;
	} else {
		setEventMessages($langs->trans('InstrucToClearPass', $powererp_main_db_pass), null, 'warnings');
	}
}

if ($action == 'activate_MAIN_SECURITY_DISABLEFORGETPASSLINK') {
	powererp_set_const($db, "MAIN_SECURITY_DISABLEFORGETPASSLINK", '1', 'chaine', 0, '', $conf->entity);
} elseif ($action == 'disable_MAIN_SECURITY_DISABLEFORGETPASSLINK') {
	powererp_del_const($db, "MAIN_SECURITY_DISABLEFORGETPASSLINK", $conf->entity);
}
...
if ($action == 'updatepattern') {
	...
	if (!$patternInError) {
		powererp_set_const($db, "USER_PASSWORD_PATTERN", $pattern, 'chaine', 0, '', $conf->entity);
...
if (empty($powererp_main_db_pass) && empty($powererp_main_db_encrypted_pass)) {
	...
	if (empty($powererp_main_db_encrypted_pass)) {
