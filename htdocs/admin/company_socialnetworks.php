	...
	foreach ($listofnetworks as $key => $value) {
		if (!empty($value['active'])) {
			...
			if (GETPOSTISSET($key.'url') && GETPOST($key.'url', 'alpha') != '') {
				powererp_set_const($db, $networkconstname, GETPOST($key.'url', 'alpha'), 'chaine', 0, '', $conf->entity);
				powererp_set_const($db, $networkconstid, GETPOST($key, 'alpha'), 'chaine', 0, '', $conf->entity);
				...
				if (!empty($listofnetworks[$key]['url'])) {
					...
					powererp_set_const($db, $networkconstname, $url, 'chaine', 0, '', $conf->entity);
				}
				powererp_set_const($db, $networkconstid, GETPOST($key, 'alpha'), 'chaine', 0, '', $conf->entity);
			} else {
				powererp_del_const($db, $networkconstname, $conf->entity);
				powererp_del_const($db, $networkconstid, $conf->entity);
			}
		}
	}
