...
print '<tr class="oddeven"><td width="300">'.$langs->trans("Password").'</td><td>'.preg_replace('/./i', '*', $powererp_main_db_pass).'</td></tr>'."\n";
...
if (!count($listofvars) && !count($listofstatus)) {
	...
	foreach ($arraylist as $listname) {
		...
		if (preg_match('/mysql/i', $db->type)) {
			$arraytest = array(
				'character_set_database'=>array('var'=>'powererp_main_db_character_set', 'valifempty'=>'utf8'),
				'collation_database'=>array('var'=>'powererp_main_db_collation', 'valifempty'=>'utf8_unicode_ci')
			);
		}
