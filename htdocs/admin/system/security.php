...
print '<span class="opacitymedium">'.$langs->trans("YouMayFindSecurityAdviceHere", 'hhttps://wiki.powererp.org/index.php/Security_information').'</span>';
print ' &nbsp; &nbsp; ';
...
$perms = fileperms($powererp_main_document_root.'/'.$conffile);
if ($perms) {
...
print '<strong>'.$langs->trans("PowerERPSetup").'</strong>: ';
...
print '<strong>$powererp_main_prod</strong>: '.($powererp_main_prod ? $powererp_main_prod : '0');
if (empty($powererp_main_prod)) {
	print ' &nbsp; '.img_picto('', 'warning').' '.$langs->trans("IfYouAreOnAProductionSetThis", 1);
}
print '<br>';

print '<strong>$powererp_nocsrfcheck</strong>: '.(empty($powererp_nocsrfcheck) ? '0' : $powererp_nocsrfcheck);
if (!empty($powererp_nocsrfcheck)) {
...
print '<strong>$powererp_main_restrict_ip</strong>: ';
if (empty($powererp_main_restrict_ip)) {
	print $langs->trans("None");
	//print ' <span class="opacitymedium">('.$langs->trans("RecommendedValueIs", $langs->transnoentitiesnoconv("IPsOfUsers")).')</span>';
} else {
	print $powererp_main_restrict_ip;
}
print '<br>';

print '<strong>$powererp_main_restrict_os_commands</strong>: ';
if (empty($powererp_main_restrict_os_commands)) {
	print $langs->trans("None");
} else {
	print $powererp_main_restrict_os_commands;
}
...
if (empty($conf->global->SECURITY_DISABLE_TEST_ON_OBFUSCATED_CONF)) {
	print '<strong>$powererp_main_db_pass</strong>: ';
	if (!empty($powererp_main_db_pass) && empty($powererp_main_db_encrypted_pass)) {
...
global $powererp_main_db_pass, $powererp_main_db_encrypted_pass;
...
print '<br>';
$urlexamplebase = 'https://github.com/PowerERP/powererp/blob/develop/dev/setup/fail2ban/filter.d/';
print '- Login process (see <a target="_blank" rel="noopener" href="'.$urlexamplebase.'web-powererp-rulesbruteforce.conf">fail2ban example on GitHub</a>)<br>';
print '- '.DOL_URL_ROOT.'/passwordforgotten.php (see <a target="_blank" rel="noopener" href="'.$urlexamplebase.'web-powererp-rulespassforgotten.conf">fail2ban example on GitHub</a>)<br>';
print '- '.DOL_URL_ROOT.'/public/* (see <a target="_blank" rel="noopener" href="'.$urlexamplebase.'web-powererp-limitpublic.conf">fail2ban example on GitHub</a>)<br>';
print '<br>';
$urlexamplebase = 'https://github.com/PowerERP/powererp/blob/develop/dev/setup/apache/';
