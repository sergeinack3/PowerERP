...
print load_fiche_titre($langs->trans("FileCheckPowerERP"), '', 'title_setup');
...
if (empty($error) && !empty($xml)) {
	...
	if (is_object($xml->powererp_htdocs_dir[0])) {
		//var_dump($xml->powererp_htdocs_dir[0]['includecustom']);exit;
		...
		print 'Error: Failed to found <b>powererp_htdocs_dir</b> into content of XML file:<br>'.dol_escape_htmltag(dol_trunc($xmlfile, 500));
		print '</div><br>';
		$error++;
	}
	...
	// Scan scripts
	/*
	if (is_object($xml->powererp_script_dir[0]))
	{
		$file_list = array();
		$ret = getFilesUpdated($file_list, $xml->powererp_htdocs_dir[0], '', ???, $checksumconcat);		// Fill array $file_list
	}*/
