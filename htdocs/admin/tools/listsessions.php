...
if ($action == 'confirm_lock' && $confirm == 'yes' && $user->admin) {
	if (powererp_set_const($db, 'MAIN_ONLY_LOGIN_ALLOWED', $user->login, 'text', 1, 'Logon is restricted to a particular user', 0) < 0) {
		dol_print_error($db);
	}
}
...
if ($action == 'confirm_unlock' && $user->admin) {
	if (powererp_del_const($db, 'MAIN_ONLY_LOGIN_ALLOWED', -1) < 0) {
		dol_print_error($db);
	}
}
