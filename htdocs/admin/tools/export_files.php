...
if ($compression == 'zip') {
	...
	if ($export_type == 'externalmodule' && !empty($what)) {
		...
		global $powererp_allow_download_external_modules;
		if (empty($powererp_allow_download_external_modules)) {
			print 'Download of external modules is not allowed by $powererp_allow_download_external_modules in conf.php file';
			$db->close();
			exit();
		}
	}
