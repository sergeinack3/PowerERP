...
if ($file && !$what) {
	//print DOL_URL_ROOT.'/powererp_export.php';
	header("Location: ".DOL_URL_ROOT.'/admin/tools/powererp_export.php?msg='.urlencode($langs->trans("ErrorFieldRequired", $langs->transnoentities("ExportMethod"))).(GETPOST('page_y', 'int') ? '&page_y='.GETPOST('page_y', 'int') : ''));
	exit;
}
...
if ($what == 'mysql') {
	...
	if (!empty($powererp_main_restrict_os_commands)) {
		$arrayofallowedcommand = explode(',', $powererp_main_restrict_os_commands);
		dol_syslog("Command are restricted to ".$powererp_main_restrict_os_commands.". We check that one of this command is inside ".$cmddump);
	...
	if (!$errormsg && $cmddump) {
		powererp_set_const($db, 'SYSTEMTOOLS_MYSQLDUMP', $cmddump, 'chaine', 0, '', $conf->entity);
	}
...
if ($what == 'postgresql') {
	...
	if (!$errormsg && $cmddump) {
		powererp_set_const($db, 'SYSTEMTOOLS_POSTGRESQLDUMP', $cmddump, 'chaine', 0, '', $conf->entity);
	}
...
if ($errormsg) {
	...
	/*else
	{
		setEventMessages($langs->trans("YouMustRunCommandFromCommandLineAfterLoginToUser",$powererp_main_db_user,$powererp_main_db_user), null, 'warnings');
	}*/
}
...
header("Location: powererp_export.php".(GETPOST('page_y', 'int') ? '?page_y='.GETPOST('page_y', 'int') : ''));
exit();
