...
if ($action == 'set') {
		...
		if (in_array($syslogHandler, $syslogModules)) {
			...
			foreach ($module->configure() as $option) {
				if (GETPOSTISSET($option['constant'])) {
					powererp_del_const($db, $option['constant'], -1);
					powererp_set_const($db, $option['constant'], trim(GETPOST($option['constant'])), 'chaine', 0, '', 0);
				}
			}
		}
	}
	...
	powererp_del_const($db, 'SYSLOG_HANDLERS', -1); // To be sure ther is not a setup into another entity
	powererp_set_const($db, 'SYSLOG_HANDLERS', json_encode($activeModules), 'chaine', 0, '', 0);
...
if ($action == 'setlevel') {
	$level = GETPOST("level");
	$res = powererp_set_const($db, "SYSLOG_LEVEL", $level, 'chaine', 0, '', 0);
	...
	if (!$error) {
		...
		$res = powererp_set_const($db, "SYSLOG_FILE_SAVES", $file_saves, 'chaine', 0, '', 0);
...
$syslogfacility = $defaultsyslogfacility = powererp_get_const($db, "SYSLOG_FACILITY", 0);
$syslogfile = $defaultsyslogfile = powererp_get_const($db, "SYSLOG_FILE", 0);
