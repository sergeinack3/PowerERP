 ...
 * Copyright (C) 2015      Bahfir Abbes         <contact@powererppar.org>
	...
	if (!$error && is_array($_POST)) {
		...
		foreach ($_POST as $key => $val) {
			...
			if (!empty($tmparray[0]) && !empty($tmparray[1])) {
				...
				//var_dump($constvalue);
				//var_dump($consttype);
				$res = powererp_set_const($db, $triggername.'_TEMPLATE', $constvalue, $consttype, 0, '', $conf->entity);
				if ($res < 0) {
					$error++;
					break;
				}
			} else {
				$res = powererp_del_const($db, $triggername.'_TEMPLATE', $conf->entity);
			}
		}
	}
...
if ($action == 'setvalue' && $user->admin) {
	...
	$result = powererp_set_const($db, "NOTIFICATION_EMAIL_FROM", GETPOST("email_from", "alphawithlgt"), 'chaine', 0, '', $conf->entity);
	if ($result < 0) {
		$error++;
	}

	$result = powererp_set_const($db, "NOTIFICATION_EMAIL_DISABLE_CONFIRM_MESSAGE", GETPOST("notif_disable", "alphawithlgt"), 'chaine', 0, '', $conf->entity);
	if ($result < 0) {
		$error++;
	}
	...
	if (!$error && is_array($_POST)) {
		...
		foreach ($_POST as $key => $val) {
			...
			if (preg_match('/^NOTIF_(.*)_old_(.*)_key/', $key, $reg)) {
				powererp_del_const($db, 'NOTIFICATION_FIXEDEMAIL_'.$reg[1].'_THRESHOLD_HIGHER_'.$reg[2], $conf->entity);
			...
			if ($newkey && $newval) {
				$result = powererp_set_const($db, $newkey, $newval, 'chaine', 0, '', $conf->entity);
			}
		}
	}
