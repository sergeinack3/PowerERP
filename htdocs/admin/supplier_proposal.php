...
if ($action == 'updateMask') {
	...
	if ($maskconstsupplier_proposal && preg_match('/_MASK$/', $maskconstsupplier_proposal)) {
		$res = powererp_set_const($db, $maskconstsupplier_proposal, $masksupplier_proposal, 'chaine', 0, '', $conf->entity);
	}
...
if ($action == 'set_SUPPLIER_PROPOSAL_DRAFT_WATERMARK') {
	...
	$res = powererp_set_const($db, "SUPPLIER_PROPOSAL_DRAFT_WATERMARK", trim($draft), 'chaine', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
...
if ($action == 'set_SUPPLIER_PROPOSAL_FREE_TEXT') {
	...
	$res = powererp_set_const($db, "SUPPLIER_PROPOSAL_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
...
if ($action == 'set_BANK_ASK_PAYMENT_BANK_DURING_SUPPLIER_PROPOSAL') {
	$res = powererp_set_const($db, "BANK_ASK_PAYMENT_BANK_DURING_SUPPLIER_PROPOSAL", $value, 'chaine', 0, '', $conf->entity);
...
if ($action == 'set') {
	...
	if ($ret > 0) {
		if ($conf->global->SUPPLIER_PROPOSAL_ADDON_PDF == "$value") {
			powererp_del_const($db, 'SUPPLIER_PROPOSAL_ADDON_PDF', $conf->entity);
		}
	}
} elseif ($action == 'setdoc') {
	if (powererp_set_const($db, "SUPPLIER_PROPOSAL_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
	...
	powererp_set_const($db, "SUPPLIER_PROPOSAL_ADDON", $value, 'chaine', 0, '', $conf->entity);
