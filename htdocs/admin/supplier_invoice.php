...
if ($action == 'specimen') {  // For invoices
	...
	if ($ret > 0) {
		if ($conf->global->INVOICE_SUPPLIER_ADDON_PDF == "$value") {
			powererp_del_const($db, 'INVOICE_SUPPLIER_ADDON_PDF', $conf->entity);
		}
	}
} elseif ($action == 'setdoc') {
	// Set default model
	if (powererp_set_const($db, "INVOICE_SUPPLIER_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
...
if ($action == 'setmod') {
	...
	powererp_set_const($db, "INVOICE_SUPPLIER_ADDON_NUMBER", $value, 'chaine', 0, '', $conf->entity);
}
...
if ($action == 'set_SUPPLIER_INVOICE_FREE_TEXT') {
	...
	$res = powererp_set_const($db, "SUPPLIER_INVOICE_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
