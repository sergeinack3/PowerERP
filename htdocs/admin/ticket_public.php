...
if ($action == 'setTICKET_ENABLE_PUBLIC_INTERFACE') {
	if (GETPOST('value')) {
		$res = powererp_set_const($db, 'TICKET_ENABLE_PUBLIC_INTERFACE', 1, 'chaine', 0, '', $conf->entity);
	} else {
		$res = powererp_set_const($db, 'TICKET_ENABLE_PUBLIC_INTERFACE', 0, 'chaine', 0, '', $conf->entity);
	}
	if (!($res > 0)) {
		$error++;
		$errors[] = $db->lasterror();
	...
	if (GETPOSTISSET('TICKET_ENABLE_PUBLIC_INTERFACE')) {	// only for no js case
		...
		$res = powererp_set_const($db, 'TICKET_ENABLE_PUBLIC_INTERFACE', $param_enable_public_interface, 'chaine', 0, '', $conf->entity);
		if (!($res > 0)) {
			$error++;
			$errors[] = $db->lasterror();
		}
	...
	if (GETPOSTISSET('TICKET_DISABLE_CUSTOMER_MAILS')) {	// only for no js case
		...
		$res = powererp_set_const($db, 'TICKET_DISABLE_CUSTOMER_MAILS', $param_disable_email, 'chaine', 0, '', $conf->entity);
		if (!($res > 0)) {
			$error++;
			$errors[] = $db->lasterror();
		}
	...
	if (GETPOSTISSET('TICKET_SHOW_COMPANY_LOGO')) {	// only for no js case
		...
		$res = powererp_set_const($db, 'TICKET_SHOW_COMPANY_LOGO', $param_show_module_logo, 'chaine', 0, '', $conf->entity);
		if (!($res > 0)) {
			$error++;
			$errors[] = $db->lasterror();
		}
	...
	if (!empty($topic_interface)) {
		$res = powererp_set_const($db, 'TICKET_PUBLIC_INTERFACE_TOPIC', $topic_interface, 'chaine', 0, '', $conf->entity);
	} else {
		$res = powererp_set_const($db, 'TICKET_PUBLIC_INTERFACE_TOPIC', '', 'chaine', 0, '', $conf->entity);
	}
	if (!($res > 0)) {
		$error++;
		$errors[] = $db->lasterror();
	...
	if (GETPOSTISSET('TICKET_PUBLIC_TEXT_HOME')) {
		$res = powererp_set_const($db, 'TICKET_PUBLIC_TEXT_HOME', $text_home, 'chaine', 0, '', $conf->entity);
	} else {
		$res = powererp_set_const($db, 'TICKET_PUBLIC_TEXT_HOME', $langs->trans('TicketPublicInterfaceTextHome'), 'chaine', 0, '', $conf->entity);
	}
	if (!($res > 0)) {
		$error++;
		$errors[] = $db->lasterror();
	...
	if (!empty($text_help)) {
		$res = powererp_set_const($db, 'TICKET_PUBLIC_TEXT_HELP_MESSAGE', $text_help, 'chaine', 0, '', $conf->entity);
	} else {
		$res = powererp_set_const($db, 'TICKET_PUBLIC_TEXT_HELP_MESSAGE', $langs->trans('TicketPublicPleaseBeAccuratelyDescribe'), 'chaine', 0, '', $conf->entity);
	}
	if (!($res > 0)) {
		$error++;
		$errors[] = $db->lasterror();
	...
	if (!empty($mail_new_ticket)) {
		$res = powererp_set_const($db, 'TICKET_MESSAGE_MAIL_NEW', $mail_new_ticket, 'chaine', 0, '', $conf->entity);
	} else {
		$res = powererp_set_const($db, 'TICKET_MESSAGE_MAIL_NEW', $langs->trans('TicketMessageMailNewText'), 'chaine', 0, '', $conf->entity);
	}
	if (!($res > 0)) {
		$error++;
		$errors[] = $db->lasterror();
	...
	if (!empty($url_interface)) {
		$res = powererp_set_const($db, 'TICKET_URL_PUBLIC_INTERFACE', $url_interface, 'chaine', 0, '', $conf->entity);
	} else {
		$res = powererp_set_const($db, 'TICKET_URL_PUBLIC_INTERFACE', '', 'chaine', 0, '', $conf->entity);
	}
	if (!($res > 0)) {
		$error++;
		$errors[] = $db->lasterror();
	...
	$res = powererp_set_const($db, 'TICKET_PUBLIC_NOTIFICATION_NEW_MESSAGE_DEFAULT_EMAIL', $param_public_notification_new_message_default_email, 'chaine', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
		$errors[] = $db->lasterror();
	}
	...
	if ($code == 'TICKET_NOTIFICATION_ALSO_MAIN_ADDRESS' && $conf->global->MAIN_FEATURES_LEVEL >= 2) {
		...
		if (!$error) {
			if ($code == 'TICKET_EMAIL_MUST_EXISTS') {
				$res = powererp_del_const($db, 'TICKET_CREATE_THIRD_PARTY_WITH_CONTACT_IF_NOT_EXIST', $conf->entity);
				if (!($res > 0)) {
					$error++;
					$errors[] = $db->lasterror();
				}
			} elseif ($code == 'TICKET_CREATE_THIRD_PARTY_WITH_CONTACT_IF_NOT_EXIST') {
				$res = powererp_del_const($db, 'TICKET_EMAIL_MUST_EXISTS', $conf->entity);
				if (!($res > 0)) {
					$error++;
					$errors[] = $db->lasterror();
				}
...
if (!empty($conf->global->TICKET_ENABLE_PUBLIC_INTERFACE)) {
	...
	$doleditor = new DolEditor('TICKET_PUBLIC_TEXT_HOME', $public_text_home, '100%', 180, 'powererp_notes', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_SOCIETE'), ROWS_2, 70);
	$doleditor->Create();
	print '</td>';
	print '<td class="center">';
	...
	print '</td><td>';
	require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
	$doleditor = new DolEditor('TICKET_PUBLIC_TEXT_HELP_MESSAGE', $public_text_help_message, '100%', 180, 'powererp_notes', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_SOCIETE'), ROWS_2, 70);
	$doleditor->Create();
	print '</td>';
	print '<td class="center">';
	...
	print '</td><td>';
	require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
	$doleditor = new DolEditor('TICKET_MESSAGE_MAIL_NEW', $mail_mesg_new, '100%', 120, 'powererp_mailings', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_MAIL'), ROWS_2, 70);
	$doleditor->Create();
	print '</td>';
	print '</tr>';
