...
if ($action == 'updateMask') {
	...
	if ($maskconstsupplierpayment) {
		$res = powererp_set_const($db, $maskconstsupplierpayment, $masksupplierpayment, 'chaine', 0, '', $conf->entity);
	}
	...
	powererp_set_const($db, "SUPPLIER_PAYMENT_ADDON", $value, 'chaine', 0, '', $conf->entity);
} elseif ($action == 'set') {
	// Activate a model
	...
	if ($ret > 0) {
		if ($conf->global->FACTURE_ADDON_PDF == "$value") {
			powererp_del_const($db, 'SUPPLIER_PAYMENT_ADDON_PDF', $conf->entity);
		}
	}
} elseif ($action == 'setdoc') {
	// Set default model
	if (powererp_set_const($db, "SUPPLIER_PAYMENT_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
	...
	if ($filefound) {
	   ...
	   $res = powererp_set_const($db, "PAYMENTS_FOURN_REPORT_GROUP_BY_MOD", GETPOST('PAYMENTS_FOURN_REPORT_GROUP_BY_MOD', 'int'), 'chaine', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
