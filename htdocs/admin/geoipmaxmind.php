...
if ($action == 'set') {
	...
	if (!$error) {
		$res1 = powererp_set_const($db, "GEOIP_VERSION", GETPOST('geoipversion', 'aZ09'), 'chaine', 0, '', $conf->entity);
		if (!$res1 > 0) {
			$error++;
		}

		$res2 = powererp_set_const($db, "GEOIPMAXMIND_COUNTRY_DATAFILE", $gimcdf, 'chaine', 0, '', $conf->entity);
		if (!$res2 > 0) {
			$error++;
		}
