...
if ($action == 'updateMask') {
	...
	if ($maskconstchequereceipts) {
		$res = powererp_set_const($db, $maskconstchequereceipts, $maskchequereceipts, 'chaine', 0, '', $conf->entity);
	}
...
if ($action == 'setmod') {
	powererp_set_const($db, "CHEQUERECEIPTS_ADDON", $value, 'chaine', 0, '', $conf->entity);
}
...
if ($action == 'set_BANK_CHEQUERECEIPT_FREE_TEXT') {
	...
	$res = powererp_set_const($db, "BANK_CHEQUERECEIPT_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
