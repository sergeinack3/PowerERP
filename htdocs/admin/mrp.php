...
if ($action == 'updateMask') {
	...
	if ($maskconstmrp) {
		$res = powererp_set_const($db, $maskconstmrp, $maskmrp, 'chaine', 0, '', $conf->entity);
	}
	...
	if ($ret > 0) {
		if ($conf->global->MRP_MO_ADDON_PDF == "$value") {
			powererp_del_const($db, 'MRP_MO_ADDON_PDF', $conf->entity);
		}
	}
} elseif ($action == 'setdoc') {
	// Set default model
	if (powererp_set_const($db, "MRP_MO_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
	...
	powererp_set_const($db, "MRP_MO_ADDON", $value, 'chaine', 0, '', $conf->entity);
} elseif ($action == 'set_MRP_MO_DRAFT_WATERMARK') {
	$draft = GETPOST("MRP_MO_DRAFT_WATERMARK");
	$res = powererp_set_const($db, "MRP_MO_DRAFT_WATERMARK", trim($draft), 'chaine', 0, '', $conf->entity);
	...
	$res = powererp_set_const($db, "MRP_MO_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
