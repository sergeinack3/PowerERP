...
if (GETPOSTISSET('mode')) {
	...
	if ($mode =='common' || $mode =='commonkanban')
		powererp_set_const($db, "MAIN_MODULE_SETUP_ON_LIST_BY_DEFAULT", $mode, 'chaine', 0, '', $conf->entity);
} else {
...
$urlpowererpmodules = 'https://www.dolistore.com/';
...
if ($action == 'install') {
	...
	if (!$original_file) {
		...
		if (!$error && !preg_match('/\.zip$/i', $original_file)) {
			$langs->load("errors");
			setEventMessages($langs->trans("ErrorFileMustBeAPowerERPPackage", $original_file), null, 'errors');
			$error++;
		}
		if (!$error && !preg_match('/^(module[a-zA-Z0-9]*|theme)_.*\-([0-9][0-9\.]*)\.zip$/i', $original_file)) {
			$langs->load("errors");
			setEventMessages($langs->trans("ErrorFilenameDosNotMatchPowerERPPackageRules", $original_file, 'module_*-x.y*.zip'), null, 'errors');
			$error++;
		}
...
if ($mode == 'deploy') {
	...
	$fullurl = '<a href="'.$urlpowererpmodules.'" target="_blank" rel="noopener noreferrer">'.$urlpowererpmodules.'</a>';
	$message = '';
	if ($allowonlineinstall) {
		if (!in_array('/custom', explode(',', $powererp_main_url_root_alt))) {
		...
		if (getDolGlobalString('MAIN_MESSAGE_INSTALL_MODULES_DISABLED_CONTACT_US')) {
			...
			$message = info_admin($langs->trans("InstallModuleFromWebHasBeenDisabledByFile", $powererpdataroot.'/installmodules.lock'));
		}
		$allowfromweb = 0;
	}
