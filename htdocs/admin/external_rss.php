...
if ($action == 'add' || GETPOST("modify")) {
	...
	if (GETPOST($external_rss_urlrss, 'alpha')) {
		...
		$result1 = powererp_set_const($db, "EXTERNAL_RSS_TITLE_".GETPOST("norss", 'int'), GETPOST($external_rss_title, 'alpha'), 'chaine', 0, '', $conf->entity);
		if ($result1) {
			$consttosave = "EXTERNAL_RSS_URLRSS_".GETPOST("norss", 'int');
			$urltosave = GETPOST($external_rss_urlrss, 'alpha');
			$result2 = powererp_set_const($db, $consttosave, $urltosave, 'chaine', 0, '', $conf->entity);
...
if (GETPOST("delete")) {
	if (GETPOST("norss", 'int')) {
		...
		$result1 = powererp_del_const($db, "EXTERNAL_RSS_TITLE_".GETPOST("norss", 'int'), $conf->entity);
		if ($result1) {
			$result2 = powererp_del_const($db, "EXTERNAL_RSS_URLRSS_".GETPOST("norss", 'int'), $conf->entity);
		}
...
print '<td>http://news.google.com/news?ned=us&topic=h&output=rss<br>http://www.powererp.org/rss</td>';
print '</tr>';
print '</table>';
