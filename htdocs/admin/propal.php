...
if ($action == 'updateMask') {
	...
	if ($maskconstpropal && preg_match('/_MASK$/', $maskconstpropal)) {
		$res = powererp_set_const($db, $maskconstpropal, $maskpropal, 'chaine', 0, '', $conf->entity);
	}
	...
	$res = powererp_set_const($db, "PROPALE_DRAFT_WATERMARK", trim($draft), 'chaine', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
	...
	$res = powererp_set_const($db, "PROPOSAL_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
	...
	$res = powererp_set_const($db, "PROPALE_VALIDITY_DURATION", $value, 'chaine', 0, '', $conf->entity);
	...
	$res = powererp_set_const($db, "BANK_ASK_PAYMENT_BANK_DURING_PROPOSAL", $value, 'chaine', 0, '', $conf->entity);
	...
	if ($ret > 0) {
		if ($conf->global->PROPALE_ADDON_PDF == "$value") {
			powererp_del_const($db, 'PROPALE_ADDON_PDF', $conf->entity);
		}
	}
} elseif ($action == 'setdoc') {
	if (powererp_set_const($db, "PROPALE_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
	...
	powererp_set_const($db, "PROPALE_ADDON", $value, 'chaine', 0, '', $conf->entity);
