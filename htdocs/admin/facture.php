...
if ($action == 'updateMask') {
	...
	if ($maskconstinvoice && preg_match('/_MASK_/', $maskconstinvoice)) {
		$res = powererp_set_const($db, $maskconstinvoice, $maskinvoice, 'chaine', 0, '', $conf->entity);
	}
	if ($maskconstreplacement && preg_match('/_MASK_/', $maskconstreplacement)) {
		$res = powererp_set_const($db, $maskconstreplacement, $maskreplacement, 'chaine', 0, '', $conf->entity);
	}
	if ($maskconstcredit && preg_match('/_MASK_/', $maskconstcredit)) {
		$res = powererp_set_const($db, $maskconstcredit, $maskcredit, 'chaine', 0, '', $conf->entity);
	}
	if ($maskconstdeposit && preg_match('/_MASK_/', $maskconstdeposit)) {
		$res = powererp_set_const($db, $maskconstdeposit, $maskdeposit, 'chaine', 0, '', $conf->entity);
	}
	...
	if ($ret > 0) {
		if ($conf->global->FACTURE_ADDON_PDF == "$value") {
			powererp_del_const($db, 'FACTURE_ADDON_PDF', $conf->entity);
		}
	}
} elseif ($action == 'setdoc') {
	// Set default model
	if (powererp_set_const($db, "FACTURE_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
	...
	powererp_set_const($db, "FACTURE_ADDON", $value, 'chaine', 0, '', $conf->entity);
} elseif ($action == 'setribchq') {
	$rib = GETPOST('rib', 'alpha');
	$chq = GETPOST('chq', 'alpha');

	$res = powererp_set_const($db, "FACTURE_RIB_NUMBER", $rib, 'chaine', 0, '', $conf->entity);
	$res = powererp_set_const($db, "FACTURE_CHQ_NUMBER", $chq, 'chaine', 0, '', $conf->entity);
	...
	$res = powererp_set_const($db, "FACTURE_DRAFT_WATERMARK", trim($draft), 'chaine', 0, '', $conf->entity);
	...
	$res = powererp_set_const($db, "INVOICE_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
	...
	$res = powererp_set_const($db, "FAC_FORCE_DATE_VALIDATION", $forcedate, 'chaine', 0, '', $conf->entity);
	...
	if (!empty($invoicetypemodels) && is_array($invoicetypemodels)) {
		...
		foreach ($invoicetypemodels as $type => $value) {
			$res = powererp_set_const($db, 'FACTURE_ADDON_PDF_'.intval($type), $value, 'chaine', 0, '', $conf->entity);
			if (!($res > 0)) {
				$error++;
			}
		}
	...
	$res = powererp_set_const($db, 'INVOICE_CHECK_POSTERIOR_DATE', $check_posterior_date, 'chaine', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
