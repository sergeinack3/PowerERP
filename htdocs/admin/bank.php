...
if ($action == 'setbankorder') {
	if (powererp_set_const($db, "BANK_SHOW_ORDER_OPTION", GETPOST('value', 'alpha'), 'chaine', 0, '', $conf->entity) > 0) {
	} else {
		dol_print_error($db);
	}
}
...
if ($action == 'setreportlastnumreleve') {
	if (powererp_set_const($db, "BANK_REPORT_LAST_NUM_RELEVE", 1, 'chaine', 0, '', $conf->entity) > 0) {
	} else {
		dol_print_error($db);
	}
} elseif ($action == 'unsetreportlastnumreleve') {
	if (powererp_set_const($db, "BANK_REPORT_LAST_NUM_RELEVE", 0, 'chaine', 0, '', $conf->entity) > 0) {
	} else {
		dol_print_error($db);
	}
}
...
if ($action == 'setbankcolorizemovement') {
	if (powererp_set_const($db, "BANK_COLORIZE_MOVEMENT", 1, 'chaine', 0, '', $conf->entity) > 0) {
	} else {
		dol_print_error($db);
	}
} elseif ($action == 'unsetbankcolorizemovement') {
	if (powererp_set_const($db, "BANK_COLORIZE_MOVEMENT", 0, 'chaine', 0, '', $conf->entity) > 0) {
	} else {
		dol_print_error($db);
	}
}
...
if ($actionsave) {
	...
	while ($i <= 2) {
		...
		$res = powererp_set_const($db, 'BANK_COLORIZE_MOVEMENT_COLOR'.$i, $color, 'chaine', 0, '', $conf->entity);
		if (!($res > 0)) {
			$error++;
		}
		$i++;
...
if ($action == 'set') {
	...
	if ($ret > 0) {
		if ($conf->global->BANKADDON_PDF == "$value") {
			powererp_del_const($db, 'BANKADDON_PDF', $conf->entity);
		}
	}
} elseif ($action == 'setdoc') {
	// Set default model
	if (powererp_set_const($db, "BANKADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
