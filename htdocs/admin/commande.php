...
if ($action == 'updateMask') {
	...
	if ($ret > 0) {
		if (getDolGlobalString('COMMANDE_ADDON_PDF') == $value) {
			powererp_del_const($db, 'COMMANDE_ADDON_PDF', $conf->entity);
		}
	}
} elseif ($action == 'setdoc') {
	// Set default model
	if (powererp_set_const($db, "COMMANDE_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
	...
	powererp_set_const($db, "COMMANDE_ADDON", $value, 'chaine', 0, '', $conf->entity);
} elseif ($action == 'set_COMMANDE_DRAFT_WATERMARK') {
	$draft = GETPOST("COMMANDE_DRAFT_WATERMARK");
	$res = powererp_set_const($db, "COMMANDE_DRAFT_WATERMARK", trim($draft), 'chaine', 0, '', $conf->entity);
	...
	$res = powererp_set_const($db, "ORDER_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
...
/*elseif ($action == 'set_BANK_ASK_PAYMENT_BANK_DURING_ORDER') {
	...
	$res = powererp_set_const($db, "BANK_ASK_PAYMENT_BANK_DURING_ORDER", $value, 'chaine', 0, '', $conf->entity);
	...
	$res = powererp_set_const($db, "WAREHOUSE_ASK_WAREHOUSE_DURING_ORDER", $value, 'chaine', 0, '', $conf->entity);
