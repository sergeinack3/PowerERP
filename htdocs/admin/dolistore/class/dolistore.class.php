...
class Dolistore
{
	...
	public function getRemoteProducts($options = array('start' => 0, 'end' => 10, 'per_page' => 50, 'categorie' => 0, 'search' => ''))
	{
		...
		try {
			...
			$opt['display']        = '[id,name,id_default_image,id_category_default,reference,price,condition,show_price,date_add,date_upd,description_short,description,module_version,powererp_min,powererp_max]';
			$opt['sort']           = 'id_desc';
			$opt['filter[active]'] = '[1]';
	...
	public function get_products()
	{
		...
		foreach ($this->products as $product) {
			...
			$version = '';
			if ($this->version_compare($product->powererp_min, DOL_VERSION) <= 0) {
				if ($this->version_compare($product->powererp_max, DOL_VERSION) >= 0) {
					//compatible
					$version = '<span class="compatible">'.$langs->trans(
						'CompatibleUpTo',
						$product->powererp_max,
						$product->powererp_min,
						$product->powererp_max
					).'</span>';
					$compatible = '';
				} else {
