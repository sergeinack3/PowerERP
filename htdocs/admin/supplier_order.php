...
if ($action == 'updateMask') {
	...
	if ($maskconstorder) {
		$res = powererp_set_const($db, $maskconstorder, $maskvalue, 'chaine', 0, '', $conf->entity);
	}
	...
	if ($ret > 0) {
		if ($conf->global->COMMANDE_SUPPLIER_ADDON_PDF == "$value") {
			powererp_del_const($db, 'COMMANDE_SUPPLIER_ADDON_PDF', $conf->entity);
		}
	}
} elseif ($action == 'setdoc') {
	// Set default model
	if (powererp_set_const($db, "COMMANDE_SUPPLIER_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
	...
	powererp_set_const($db, "COMMANDE_SUPPLIER_ADDON_NUMBER", $value, 'chaine', 0, '', $conf->entity);
	...
	$res1 = powererp_set_const($db, "SUPPLIER_ORDER_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
	$res2 = powererp_set_const($db, "SUPPLIER_ORDER_3_STEPS_TO_BE_APPROVED", $doubleapproval, 'chaine', 0, '', $conf->entity);
	...
	$res = powererp_set_const($db, "BANK_ASK_PAYMENT_BANK_DURING_SUPPLIER_ORDER", $value, 'chaine', 0, '', $conf->entity);
