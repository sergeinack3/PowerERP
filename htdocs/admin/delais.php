...
if ($action == 'update') {
		...
		if (!empty($conf->$module->enabled)) {
			foreach ($delays as $delay) {
				if (GETPOST($delay['code']) != '') {
					powererp_set_const($db, $delay['code'], GETPOST($delay['code']), 'chaine', 0, '', $conf->entity);
				}
			}
		}
	}

	powererp_set_const($db, "MAIN_DISABLE_METEO", GETPOST("MAIN_DISABLE_METEO"), 'chaine', 0, '', $conf->entity);
	powererp_set_const($db, "MAIN_USE_METEO_WITH_PERCENTAGE", GETPOST("MAIN_USE_METEO_WITH_PERCENTAGE"), 'chaine', 0, '', $conf->entity);
	...
	for ($i = 0; $i < 4; $i++) {
		if (GETPOSTISSET('MAIN_METEO'.$plus.'_LEVEL'.$i)) {
			powererp_set_const($db, 'MAIN_METEO'.$plus.'_LEVEL'.$i, GETPOST('MAIN_METEO'.$plus.'_LEVEL'.$i, 'int'), 'chaine', 0, '', $conf->entity);
		}
	}
