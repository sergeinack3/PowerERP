...
if (isModEnabled('expedition') && empty($conf->global->MAIN_SUBMODULE_EXPEDITION)) {
	...
	powererp_set_const($db, "MAIN_SUBMODULE_EXPEDITION", "1", 'chaine', 0, '', $conf->entity);
}
/*
 ...
 {
 powererp_del_const($db, "MAIN_SUBMODULE_EXPEDITION",$conf->entity);
 header("Location: confexped.php");
 exit;
 }
 */
...
if ($action == 'activate_delivery') {
	powererp_set_const($db, "MAIN_SUBMODULE_EXPEDITION", "1", 'chaine', 0, '', $conf->entity); // We must also enable this
	powererp_set_const($db, "MAIN_SUBMODULE_DELIVERY", "1", 'chaine', 0, '', $conf->entity);
	header("Location: delivery.php");
	exit;
} elseif ($action == 'disable_delivery') {
	powererp_del_const($db, "MAIN_SUBMODULE_DELIVERY", $conf->entity);
	header("Location: delivery.php");
	exit;
}
...
if ($action == 'updateMask') {
	...
	if ($maskconstdelivery && preg_match('/_MASK$/', $maskconstdelivery)) {
		$res = powererp_set_const($db, $maskconstdelivery, $maskdelivery, 'chaine', 0, '', $conf->entity);
	}
...
if ($action == 'set_DELIVERY_FREE_TEXT') {
	...
	$res = powererp_set_const($db, "DELIVERY_FREE_TEXT", $free, 'chaine', 0, '', $conf->entity);
...
if ($action == 'del') {
	...
	if ($ret > 0) {
		if ($conf->global->DELIVERY_ADDON_PDF == "$value") {
			powererp_del_const($db, 'DELIVERY_ADDON_PDF', $conf->entity);
		}
	}
}

if ($action == 'setdoc') {
	if (powererp_set_const($db, "DELIVERY_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
...
if ($action == 'setmod') {
	...
	powererp_set_const($db, "DELIVERY_ADDON_NUMBER", $value, 'chaine', 0, '', $conf->entity);
}
