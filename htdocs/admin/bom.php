...
if ($action == 'updateMask') {
	...
	if ($maskconstbom) {
		$res = powererp_set_const($db, $maskconstbom, $maskbom, 'chaine', 0, '', $conf->entity);
	}
	...
	if ($ret > 0) {
		if ($conf->global->BOM_ADDON_PDF == "$value") {
			powererp_del_const($db, 'BOM_ADDON_PDF', $conf->entity);
		}
	}
} elseif ($action == 'setdoc') {
	// Set default model
	if (powererp_set_const($db, "BOM_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
	...
	powererp_set_const($db, "BOM_ADDON", $value, 'chaine', 0, '', $conf->entity);
} elseif ($action == 'set_BOM_DRAFT_WATERMARK') {
	$draft = GETPOST("BOM_DRAFT_WATERMARK");
	$res = powererp_set_const($db, "BOM_DRAFT_WATERMARK", trim($draft), 'chaine', 0, '', $conf->entity);
	...
	$res = powererp_set_const($db, "BOM_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
