...
if ($action == 'create') {
	...
	foreach ($fieldsforcontent as $tmpfieldlist) {
		...
		if ($tmpfieldlist == 'topic') {
			...
			$doleditor = new DolEditor($tmpfieldlist, (!empty($obj->$tmpfieldlist) ? $obj->$tmpfieldlist : ''), '', 180, 'powererp_mailings', 'In', false, $acceptlocallinktomedia, $okforextended, ROWS_4, '90%');
				...
				foreach ($fieldsforcontent as $tmpfieldlist) {
					...
					if ($showfield) {
						...
						if ($tmpfieldlist == 'content') {
							...
							$doleditor = new DolEditor($tmpfieldlist.'-'.$rowid, (!empty($obj->{$tmpfieldlist}) ? $obj->{$tmpfieldlist} : ''), '', 500, 'powererp_mailings', 'In', 0, $acceptlocallinktomedia, $okforextended, ROWS_6, '90%');
							print $doleditor->Create(1);
						}
						if ($tmpfieldlist == 'content_lines') {
							print $form->textwithpicto($langs->trans("ContentForLines"), $tabhelp[$id][$tmpfieldlist], 1, 'help', '', 0, 2, $tmpfieldlist).'<br>';
							...
							$doleditor = new DolEditor($tmpfieldlist.'-'.$rowid, (!empty($obj->{$tmpfieldlist}) ? $obj->{$tmpfieldlist} : ''), '', 140, 'powererp_mailings', 'In', 0, $acceptlocallinktomedia, $okforextended, ROWS_6, '90%');
							print $doleditor->Create(1);
						}
						print '</td>';
						print '<td></td>';
