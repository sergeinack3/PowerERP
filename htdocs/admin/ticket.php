...
if ($action == 'updateMask') {
	...
	if ($maskconstticket) {
		$res = powererp_set_const($db, $maskconstticket, $maskticket, 'chaine', 0, '', $conf->entity);
	}
	...
	if ($ret > 0) {
		if ($conf->global->TICKET_ADDON_PDF == "$value") {
			powererp_del_const($db, 'TICKET_ADDON_PDF', $conf->entity);
		}
	}
	...
	if ($code == 'TICKET_NOTIFICATION_ALSO_MAIN_ADDRESS' && $conf->global->MAIN_FEATURES_LEVEL >= 2) {
		...
		$res = powererp_set_const($db, 'TICKET_NOTIFICATION_ALSO_MAIN_ADDRESS', $param_notification_also_main_addressemail, 'chaine', 0, '', $conf->entity);
		if (!($res > 0)) {
			$error++;
		}
	} else {
		$res = powererp_set_const($db, $code, $value, 'chaine', 0, '', $conf->entity);
		if (!($res > 0)) {
			$error++;
		}
	}
	...
	$code = $reg[1];
	$res = powererp_del_const($db, $code, $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
} elseif ($action == 'setdoc') {
	// Set default model
	if (powererp_set_const($db, "TICKET_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
	...
	powererp_set_const($db, "TICKET_ADDON", $value, 'chaine', 0, '', $conf->entity);
	...
	if (empty($conf->use_javascript_ajax)) {
		...
		$res = powererp_set_const($db, 'TICKET_AUTO_READ_WHEN_CREATED_FROM_BACKEND', $param_auto_read, 'chaine', 0, '', $conf->entity);
		if (!($res > 0)) {
			$error++;
		}

		$param_auto_assign = GETPOST('TICKET_AUTO_ASSIGN_USER_CREATE', 'alpha');
		$res = powererp_set_const($db, 'TICKET_AUTO_ASSIGN_USER_CREATE', $param_auto_assign, 'chaine', 0, '', $conf->entity);
		if (!($res > 0)) {
			$error++;
		}

		$param_auto_notify_close = GETPOST('TICKET_NOTIFY_AT_CLOSING', 'alpha');
		$res = powererp_set_const($db, 'TICKET_NOTIFY_AT_CLOSING', $param_auto_notify_close, 'chaine', 0, '', $conf->entity);
		if (!($res > 0)) {
			$error++;
		}
	}
	...
	$res = powererp_set_const($db, 'TICKET_LIMIT_VIEW_ASSIGNED_ONLY', $param_limit_view, 'chaine', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
	...
	if (GETPOSTISSET('product_category_id')) {
		...
		$res = powererp_set_const($db, 'TICKET_PRODUCT_CATEGORY', $param_ticket_product_category, 'chaine', 0, '', $conf->entity);
		if (!($res > 0)) {
			$error++;
		}
	}
	...
	$res = powererp_set_const($db, 'TICKET_DELAY_BEFORE_FIRST_RESPONSE', $param_delay_first_response, 'chaine', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}

	$param_delay_between_responses = GETPOST('delay_between_responses', 'int');
	$res = powererp_set_const($db, 'TICKET_DELAY_SINCE_LAST_RESPONSE', $param_delay_between_responses, 'chaine', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
	...
	$notification_email_description = "Sender of ticket replies sent from PowerERP";
	if (!empty($notification_email)) {
		$res = powererp_set_const($db, 'TICKET_NOTIFICATION_EMAIL_FROM', $notification_email, 'chaine', 0, $notification_email_description, $conf->entity);
	} else { // If an empty e-mail address is providen, use the global "FROM" since an empty field will cause other issues
		$res = powererp_set_const($db, 'TICKET_NOTIFICATION_EMAIL_FROM', $conf->global->MAIN_MAIL_EMAIL_FROM, 'chaine', 0, $notification_email_description, $conf->entity);
	}
	if (!($res > 0)) {
		$error++;
	}
	...
	$notification_email_to_description = "Notified e-mail for ticket replies sent from PowerERP";
	if (!empty($notification_email_to)) {
		$res = powererp_set_const($db, 'TICKET_NOTIFICATION_EMAIL_TO', $notification_email_to, 'chaine', 0, $notification_email_to_description, $conf->entity);
	} else {
		$res = powererp_set_const($db, 'TICKET_NOTIFICATION_EMAIL_TO', '', 'chaine', 0, $notification_email_to_description, $conf->entity);
	}
	if (!($res > 0)) {
		$error++;
	}
	...
	$mail_intro_description = "Introduction text of ticket replies sent from PowerERP";
	if (!empty($mail_intro)) {
		$res = powererp_set_const($db, 'TICKET_MESSAGE_MAIL_INTRO', $mail_intro, 'chaine', 0, $mail_intro_description, $conf->entity);
	} else {
		$res = powererp_set_const($db, 'TICKET_MESSAGE_MAIL_INTRO', '', 'chaine', 0, $mail_intro_description, $conf->entity);
	}
	if (!($res > 0)) {
		$error++;
	}
	...
	$signature_description = "Signature of ticket replies sent from PowerERP";
	if (!empty($mail_signature)) {
		$res = powererp_set_const($db, 'TICKET_MESSAGE_MAIL_SIGNATURE', $mail_signature, 'chaine', 0, $signature_description, $conf->entity);
	} else {
		$res = powererp_set_const($db, 'TICKET_MESSAGE_MAIL_SIGNATURE', '', 'chaine', 0, $signature_description, $conf->entity);
	}
	if (!($res > 0)) {
		$error++;
	}
...
$doleditor = new DolEditor('TICKET_MESSAGE_MAIL_INTRO', $mail_intro, '100%', 120, 'powererp_mailings', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_MAIL'), ROWS_2, 70);
$doleditor->Create();
print '</td>';
print '<td class="center">';
...
print '</td><td>';
require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
$doleditor = new DolEditor('TICKET_MESSAGE_MAIL_SIGNATURE', $mail_signature, '100%', 120, 'powererp_mailings', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_MAIL'), ROWS_2, 70);
$doleditor->Create();
print '</td>';
print '<td class="center">';
