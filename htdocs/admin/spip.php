...
if ($action == 'update' || $action == 'add') {
	...
	if ($action == 'update' || $action == 'add') {
		foreach ($constnamearray as $key => $val) {
			...
			$res = powererp_set_const($db, $constname, $constvalue, $type[$consttype], 0, $constnote, $conf->entity);
...
if ($action == 'set') {
	$result = powererp_set_const($db, $_GET["name"], $_GET["value"], '', 0, '', $conf->entity);
	if ($result < 0) {
		dol_print_error($db);
	}
}
...
if ($action == 'unset') {
	$result = powererp_del_const($db, $_GET["name"], $conf->entity);
	if ($result < 0) {
		dol_print_error($db);
	}
}
