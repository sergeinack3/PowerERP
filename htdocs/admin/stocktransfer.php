...
if ($action == 'updateMask') {
	...
	if ($maskconststocktransfer) {
		$res = powererp_set_const($db, $maskconststocktransfer, $maskstocktransfer, 'chaine', 0, '', $conf->entity);
		if ($res <= 0) $error++;
	}
	...
	if ($ret > 0) {
		...
		if ($conf->global->$constforval == "$value") powererp_del_const($db, $constforval, $conf->entity);
	}
