...
if ($action == 'add') {
	...
	if (!$error) {
		if (powererp_set_const($db, $constname, $constvalue, 'chaine', 1, $constnote, $entity) >= 0) {
...
if (!empty($consts) && $action == 'update') {
	...
	foreach ($consts as $const) {
		if (!empty($const["check"])) {
			if (powererp_set_const($db, $const["name"], $const["value"], $const["type"], 1, $const["note"], $const["entity"]) >= 0) {
				$nbmodified++;
			} else {
				dol_print_error($db);
			}
...
if (!empty($consts) && $action == 'delete') {
	...
	foreach ($consts as $const) {
		if (!empty($const["check"])) {	// Is checkbox checked
			if (powererp_del_const($db, $const["rowid"], -1) >= 0) {
				$nbdeleted++;
			} else {
				dol_print_error($db);
			}
...
if ($action == 'delete') {
	if (powererp_del_const($db, $rowid, $entity) >= 0) {
		setEventMessages($langs->trans("RecordDeleted"), null, 'mesgs');
	} else {
		dol_print_error($db);
	}
