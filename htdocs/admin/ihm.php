...
if (preg_match('/^(set|del)_([A-Z_]+)$/', $action, $regs)) {
	if ($regs[1] == 'set') {
		powererp_set_const($db, $regs[2], 1, 'chaine', 0, '', $conf->entity);
	} else {
		powererp_del_const($db, $regs[2], $conf->entity);
	}
}

if ($action == 'removebackgroundlogin' && !empty($conf->global->MAIN_LOGIN_BACKGROUND)) {
	powererp_set_const($db, "MAIN_IHM_PARAMS_REV", getDolGlobalInt('MAIN_IHM_PARAMS_REV') + 1, 'chaine', 0, '', $conf->entity);
	require_once DOL_DOCUMENT_ROOT.'/core/lib/files.lib.php';

	$logofile = $conf->mycompany->dir_output.'/logos/'.$conf->global->MAIN_LOGIN_BACKGROUND;
	dol_delete_file($logofile);
	powererp_del_const($db, "MAIN_LOGIN_BACKGROUND", $conf->entity);
	$mysoc->logo = '';

	/*$logosmallfile=$conf->mycompany->dir_output.'/logos/thumbs/'.$mysoc->logo_small;
	dol_delete_file($logosmallfile);
	powererp_del_const($db, "MAIN_INFO_SOCIETE_LOGO_SMALL",$conf->entity);
	$mysoc->logo_small='';

	$logominifile=$conf->mycompany->dir_output.'/logos/thumbs/'.$mysoc->logo_mini;
	dol_delete_file($logominifile);
	powererp_del_const($db, "MAIN_INFO_SOCIETE_LOGO_MINI",$conf->entity);
	$mysoc->logo_mini='';*/
}
...
if ($action == 'update') {
	...
	if ($mode == 'template') {
		//powererp_del_const($db, "MAIN_THEME", 0);	// To be sure we don't have this constant set for all entities

		powererp_set_const($db, "MAIN_THEME", GETPOST("main_theme", 'aZ09'), 'chaine', 0, '', $conf->entity);
		powererp_set_const($db, "MAIN_IHM_PARAMS_REV", getDolGlobalInt('MAIN_IHM_PARAMS_REV') + 1, 'chaine', 0, '', $conf->entity);

		if (GETPOSTISSET('THEME_DARKMODEENABLED')) {
			$val = GETPOST('THEME_DARKMODEENABLED');
			if (!$val) {
				powererp_del_const($db, "THEME_DARKMODEENABLED", $conf->entity);
			}
			if ($val) {
				powererp_set_const($db, "THEME_DARKMODEENABLED", $val, 'chaine', 0, '', $conf->entity);
			}
		}
		...
		if (GETPOSTISSET('THEME_TOPMENU_DISABLE_IMAGE')) {
			...
			if (!$val) {
				powererp_del_const($db, 'THEME_TOPMENU_DISABLE_IMAGE', $conf->entity);
			} else {
				powererp_set_const($db, 'THEME_TOPMENU_DISABLE_IMAGE', GETPOST('THEME_TOPMENU_DISABLE_IMAGE'), 'chaine', 0, '', $conf->entity);
			}
		}

		$val = (implode(',', (colorStringToArray(GETPOST('THEME_ELDY_BACKBODY'), array()))));
		if ($val == '') {
			powererp_del_const($db, 'THEME_ELDY_BACKBODY', $conf->entity);
		} else {
			powererp_set_const($db, 'THEME_ELDY_BACKBODY', $val, 'chaine', 0, '', $conf->entity);
		}

		$val = (implode(',', (colorStringToArray(GETPOST('THEME_ELDY_TOPMENU_BACK1'), array()))));
		if ($val == '') {
			powererp_del_const($db, 'THEME_ELDY_TOPMENU_BACK1', $conf->entity);
		} else {
			powererp_set_const($db, 'THEME_ELDY_TOPMENU_BACK1', $val, 'chaine', 0, '', $conf->entity);
		}

		$val = (implode(',', (colorStringToArray(GETPOST('THEME_ELDY_VERMENU_BACK1'), array()))));
		if ($val == '') {
			powererp_del_const($db, 'THEME_ELDY_VERMENU_BACK1', $conf->entity);
		} else {
			powererp_set_const($db, 'THEME_ELDY_VERMENU_BACK1', $val, 'chaine', 0, '', $conf->entity);
		}

		$val = (implode(',', (colorStringToArray(GETPOST('THEME_ELDY_TEXTTITLENOTAB'), array()))));
		if ($val == '') {
			powererp_del_const($db, 'THEME_ELDY_TEXTTITLENOTAB', $conf->entity);
		} else {
			powererp_set_const($db, 'THEME_ELDY_TEXTTITLENOTAB', $val, 'chaine', 0, '', $conf->entity);
		}

		$val = (implode(',', (colorStringToArray(GETPOST('THEME_ELDY_BACKTITLE1'), array()))));
		if ($val == '') {
			powererp_del_const($db, 'THEME_ELDY_BACKTITLE1', $conf->entity);
		} else {
			powererp_set_const($db, 'THEME_ELDY_BACKTITLE1', $val, 'chaine', 0, '', $conf->entity);
		}

		$val = (implode(',', (colorStringToArray(GETPOST('THEME_ELDY_TEXTTITLE'), array()))));
		if ($val == '') {
			powererp_del_const($db, 'THEME_ELDY_TEXTTITLE', $conf->entity);
		} else {
			powererp_set_const($db, 'THEME_ELDY_TEXTTITLE', $val, 'chaine', 0, '', $conf->entity);
		}

		$val = (implode(',', (colorStringToArray(GETPOST('THEME_ELDY_TEXTTITLELINK'), array()))));
		if ($val == '') {
			powererp_del_const($db, 'THEME_ELDY_TEXTTITLELINK', $conf->entity);
		} else {
			powererp_set_const($db, 'THEME_ELDY_TEXTTITLELINK', $val, 'chaine', 0, '', $conf->entity);
		}

		$val = (implode(',', (colorStringToArray(GETPOST('THEME_ELDY_LINEIMPAIR1'), array()))));
		if ($val == '') {
			powererp_del_const($db, 'THEME_ELDY_LINEIMPAIR1', $conf->entity);
		} else {
			powererp_set_const($db, 'THEME_ELDY_LINEIMPAIR1', $val, 'chaine', 0, '', $conf->entity);
		}
		$val = (implode(',', (colorStringToArray(GETPOST('THEME_ELDY_LINEIMPAIR1'), array()))));
		if ($val == '') {
			powererp_del_const($db, 'THEME_ELDY_LINEIMPAIR2', $conf->entity);
		} else {
			powererp_set_const($db, 'THEME_ELDY_LINEIMPAIR2', $val, 'chaine', 0, '', $conf->entity);
		}

		$val = (implode(',', (colorStringToArray(GETPOST('THEME_ELDY_LINEPAIR1'), array()))));
		if ($val == '') {
			powererp_del_const($db, 'THEME_ELDY_LINEPAIR1', $conf->entity);
		} else {
			powererp_set_const($db, 'THEME_ELDY_LINEPAIR1', $val, 'chaine', 0, '', $conf->entity);
		}
		$val = (implode(',', (colorStringToArray(GETPOST('THEME_ELDY_LINEPAIR1'), array()))));
		if ($val == '') {
			powererp_del_const($db, 'THEME_ELDY_LINEPAIR2', $conf->entity);
		} else {
			powererp_set_const($db, 'THEME_ELDY_LINEPAIR2', $val, 'chaine', 0, '', $conf->entity);
		}

		$val = (implode(',', (colorStringToArray(GETPOST('THEME_ELDY_TEXTLINK'), array()))));
		if ($val == '') {
			powererp_del_const($db, 'THEME_ELDY_TEXTLINK', $conf->entity);
		} else {
			powererp_set_const($db, 'THEME_ELDY_TEXTLINK', $val, 'chaine', 0, '', $conf->entity);
		}

		$val = (implode(',', (colorStringToArray(GETPOST('THEME_ELDY_USE_HOVER'), array()))));
		if ($val == '') {
			powererp_del_const($db, 'THEME_ELDY_USE_HOVER', $conf->entity);
		} else {
			powererp_set_const($db, "THEME_ELDY_USE_HOVER", $val, 'chaine', 0, '', $conf->entity);
		}

		$val = (implode(',', (colorStringToArray(GETPOST('THEME_ELDY_USE_CHECKED'), array()))));
		if ($val == '') {
			powererp_del_const($db, 'THEME_ELDY_USE_CHECKED', $conf->entity);
		} else {
			powererp_set_const($db, "THEME_ELDY_USE_CHECKED", $val, 'chaine', 0, '', $conf->entity);
		}

		$val=(implode(',', (colorStringToArray(GETPOST('THEME_ELDY_BTNACTION'), array()))));
		if ($val == '') {
			powererp_del_const($db, 'THEME_ELDY_BTNACTION', $conf->entity);
		} else {
			powererp_set_const($db, 'THEME_ELDY_BTNACTION', $val, 'chaine', 0, '', $conf->entity);
		}

		$val=(implode(',', (colorStringToArray(GETPOST('THEME_ELDY_TEXTBTNACTION'), array()))));
		if ($val == '') {
			powererp_del_const($db, 'THEME_ELDY_TEXTBTNACTION', $conf->entity);
		} else {
			powererp_set_const($db, 'THEME_ELDY_TEXTBTNACTION', $val, 'chaine', 0, '', $conf->entity);
		}
	}

	if ($mode == 'dashboard') {
		powererp_set_const($db, "MAIN_MOTD", dol_htmlcleanlastbr(GETPOST("main_motd", 'restricthtml')), 'chaine', 0, '', $conf->entity);
	}

	if ($mode == 'other') {
		powererp_set_const($db, "MAIN_LANG_DEFAULT", GETPOST("MAIN_LANG_DEFAULT", 'aZ09'), 'chaine', 0, '', $conf->entity);
		powererp_set_const($db, "MAIN_IHM_PARAMS_REV", getDolGlobalInt('MAIN_IHM_PARAMS_REV') + 1, 'chaine', 0, '', $conf->entity);

		powererp_set_const($db, "MAIN_SIZE_LISTE_LIMIT", GETPOST("main_size_liste_limit", 'int'), 'chaine', 0, '', $conf->entity);
		powererp_set_const($db, "MAIN_SIZE_SHORTLIST_LIMIT", GETPOST("main_size_shortliste_limit", 'int'), 'chaine', 0, '', $conf->entity);

		//powererp_set_const($db, "MAIN_DISABLE_JAVASCRIPT", GETPOST("MAIN_DISABLE_JAVASCRIPT", 'aZ09'), 'chaine', 0, '', $conf->entity);
		//powererp_set_const($db, "MAIN_BUTTON_HIDE_UNAUTHORIZED", GETPOST("MAIN_BUTTON_HIDE_UNAUTHORIZED", 'aZ09'), 'chaine', 0, '', $conf->entity);
		//powererp_set_const($db, "MAIN_MENU_HIDE_UNAUTHORIZED", GETPOST("MAIN_MENU_HIDE_UNAUTHORIZED", 'aZ09'), 'chaine', 0, '', $conf->entity);
		powererp_set_const($db, "MAIN_START_WEEK", GETPOST("MAIN_START_WEEK", 'int'), 'chaine', 0, '', $conf->entity);

		powererp_set_const($db, "MAIN_DEFAULT_WORKING_DAYS", GETPOST("MAIN_DEFAULT_WORKING_DAYS", 'alphanohtml'), 'chaine', 0, '', $conf->entity);
		powererp_set_const($db, "MAIN_DEFAULT_WORKING_HOURS", GETPOST("MAIN_DEFAULT_WORKING_HOURS", 'alphanohtml'), 'chaine', 0, '', $conf->entity);

		powererp_set_const($db, "MAIN_BUGTRACK_ENABLELINK", GETPOST("MAIN_BUGTRACK_ENABLELINK", 'alpha'), 'chaine', 0, '', $conf->entity);

		powererp_set_const($db, "MAIN_FIRSTNAME_NAME_POSITION", GETPOST("MAIN_FIRSTNAME_NAME_POSITION", 'aZ09'), 'chaine', 0, '', $conf->entity);
	}

	if ($mode == 'login') {
		powererp_set_const($db, "MAIN_HOME", dol_htmlcleanlastbr(GETPOST("main_home", 'restricthtml')), 'chaine', 0, '', $conf->entity);
		//powererp_set_const($db, "MAIN_HELP_DISABLELINK", GETPOST("MAIN_HELP_DISABLELINK", 'aZ09'), 'chaine', 0, '', 0); // Param for all entities
		...
		if ($_FILES[$varforimage]["tmp_name"]) {
			...
			if (preg_match('/([^\\/:]+)$/i', $_FILES[$varforimage]["name"], $reg)) {
				...
				if ($isimage >= 0) {
					...
					if ($result > 0) {
						powererp_set_const($db, "MAIN_LOGIN_BACKGROUND", $original_file, 'chaine', 0, '', $conf->entity);
	...
	if (GETPOST('dol_resetcache')) {
		powererp_set_const($db, "MAIN_IHM_PARAMS_REV", ((int) $conf->global->MAIN_IHM_PARAMS_REV) + 1, 'chaine', 0, '', $conf->entity);
	}
...
if ($mode == 'dashboard') {
	...
	$doleditor = new DolEditor('main_motd', (isset($conf->global->MAIN_MOTD) ? $conf->global->MAIN_MOTD : ''), '', 142, 'powererp_notes', 'In', false, true, true, ROWS_4, '90%');
	$doleditor->Create();
...
if ($mode == 'login') {
	...
	print '</td><td>';
	$doleditor = new DolEditor('main_home', (isset($conf->global->MAIN_HOME) ? $conf->global->MAIN_HOME : ''), '', 142, 'powererp_notes', 'In', false, true, true, ROWS_4, '90%');
	$doleditor->Create();
	print '</td></tr>' . "\n";
