...
if ($action == 'updateMask') {
	...
	if ($maskconst && preg_match('/_MASK$/', $maskconst)) {
		$res = powererp_set_const($db, $maskconst, $maskorder, 'chaine', 0, '', $conf->entity);
		if (!($res > 0)) {
			$error++;
		}
	}
	...
	foreach ($arrayofparameters as $constname => $val) {
		if ($val['enabled']==1) {
			...
			if ($val['type'] == 'textarea') {
				...
				$doleditor = new DolEditor($constname, $conf->global->{$constname}, '', 160, 'powererp_notes', '', false, false, isModEnabled('fckeditor'), ROWS_5, '90%');
				$doleditor->Create();
