 ...
 */
class Proposals extends PowerERPApi
{
	/**
	...
	private function _fetch($id, $ref = '', $ref_ext = '', $contact_list = 1)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('propal', $this->propal->id)) {
	...
	{
		if (!PowerERPApiAccess::$user->rights->propal->creer) {
		...
		if ($this->propal->create(PowerERPApiAccess::$user) < 0) {
	...
	public function getLines($id, $sqlfilters = '')
	{
		...
		if (!empty($sqlfilters)) {
			...
			$regexstring = '\(([^:\'\(\)]+:[^:\'\(\)]+:[^:\(\)]+)\)';
			$filters = " AND (".preg_replace_callback('/'.$regexstring.'/', 'PowerERPApi::_forge_criteria_callback', $sqlfilters).")";
		}
	...
	public function put($id, $request_data = null)
	{
		...
		if (!empty($this->propal->fin_validite)) {
			if ($this->propal->set_echeance(PowerERPApiAccess::$user, $this->propal->fin_validite) < 0) {
				throw new RestException(500, $this->propal->error);
			}
		}

		if ($this->propal->update(PowerERPApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
			throw new RestException(500, $this->propal->error);
		}
	...
	{
		if (!PowerERPApiAccess::$user->rights->propal->supprimer) {
			throw new RestException(401);
		}
		$result = $this->propal->fetch($id);
		if (!$result) {
		...
		if (!$this->propal->delete(PowerERPApiAccess::$user)) {
	...
	public function settodraft($id)
	{
		...
		$result = $this->propal->setDraft(PowerERPApiAccess::$user);
		if ($result == 0) {
	...
	public function validate($id, $notrigger = 0)
	{
		...
		$result = $this->propal->valid(PowerERPApiAccess::$user, $notrigger);
		if ($result == 0) {
	...
	public function close($id, $status, $note_private = '', $notrigger = 0)
	{
		...
		$result = $this->propal->closeProposal(PowerERPApiAccess::$user, $status, $note_private, $notrigger);
		if ($result == 0) {
	...
	public function setinvoiced($id)
	{
		...
		$result = $this->propal->classifyBilled(PowerERPApiAccess::$user);
		if ($result < 0) {
			throw new RestException(500, 'Error : '.$this->propal->error);
		}
