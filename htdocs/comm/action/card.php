...
if ($action == 'create') {
	...
	$doleditor = new DolEditor('note', (GETPOSTISSET('note') ? GETPOST('note', 'restricthtml') : $object->note_private), '', 120, 'powererp_notes', 'In', true, true, $conf->fckeditor->enabled, ROWS_4, '90%');
	$doleditor->Create();
	print '</td></tr>';
...
if ($id > 0) {
	...
	if ($action == 'edit') {
		...
		// Editeur wysiwyg
		require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
		$doleditor = new DolEditor('note', $object->note_private, '', 200, 'powererp_notes', 'In', true, true, $conf->fckeditor->enabled, ROWS_5, '90%');
		$doleditor->Create();
		print '</td></tr>';
