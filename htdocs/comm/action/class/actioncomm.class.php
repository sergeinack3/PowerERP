...
class ActionComm extends CommonObject
{
	...
	public function build_exportfile($format, $type, $cachedelay, $filename, $filters, $exportholiday = 0)
	{
		...
		// phpcs:enable
		global $conf, $langs, $powererp_main_url_root, $mysoc;
		...
		if ($buildfile) {
			...
			if ($resql) {
				...
				while ($obj = $this->db->fetch_object($resql)) {
					...
					$event = array();
					$event['uid'] = 'powererpagenda-'.$this->db->database_name.'-'.$obj->id."@".$_SERVER["SERVER_NAME"];
					$event['type'] = $type;
			...
			if ($exportholiday == 1) {
				...
				if ($resql) {
					...
					while ($i < $num) {
						...
						$event['uid']          = 'powererpholiday-'.$this->db->database_name.'-'.$obj->rowid."@".$_SERVER["SERVER_NAME"];
						$event['author']       = dolGetFirstLastname($obj->firstname, $obj->lastname);
						$event['type']         = 'event';
