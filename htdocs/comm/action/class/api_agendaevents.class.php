 ...
 */
class AgendaEvents extends PowerERPApi
{
	...
	{
		if (!PowerERPApiAccess::$user->rights->agenda->myactions->read) {
		...
		if (!PowerERPApiAccess::$user->rights->agenda->allactions->read && $this->actioncomm->userownerid != PowerERPApiAccess::$user->id) {
			throw new RestException(401, "Insufficient rights to read event for owner id ".$request_data['userownerid'].' Your id is '.PowerERPApiAccess::$user->id);
		}

		if (!PowerERPApi::_checkAccessToResource('agenda', $this->actioncomm->id, 'actioncomm', '', 'fk_soc', 'id')) {
	...
	public function index($sortfield = "t.id", $sortorder = 'ASC', $limit = 100, $page = 0, $user_ids = 0, $sqlfilters = '')
	{
		...
		$socid = 0;
		if (!empty(PowerERPApiAccess::$user->socid)) {
			$socid = PowerERPApiAccess::$user->socid;
		}

		// If the internal user must only see his customers, force searching by him
		$search_sale = 0;
		if (!PowerERPApiAccess::$user->rights->societe->client->voir && !$socid) {
		...
		if (isModEnabled("societe")) {
			if ((!PowerERPApiAccess::$user->rights->societe->client->voir && !$socid) || $search_sale > 0) {
	...
	{
		if (!PowerERPApiAccess::$user->rights->agenda->myactions->create) {
			throw new RestException(401, "Insufficient rights to create your Agenda Event");
		}
		if (!PowerERPApiAccess::$user->rights->agenda->allactions->create && PowerERPApiAccess::$user->id != $request_data['userownerid']) {
			throw new RestException(401, "Insufficient rights to create an Agenda Event for owner id ".$request_data['userownerid'].' Your id is '.PowerERPApiAccess::$user->id);
		}
		...
		if ($this->actioncomm->create(PowerERPApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('actioncomm', $this->actioncomm->id, 'actioncomm', '', 'fk_soc', 'id')) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}
		foreach ($request_data as $field => $value) {
			if ($field == 'id') {
		...
		if ($this->actioncomm->update(PowerERPApiAccess::$user, 1) > 0) {
			return $this->get($id);
		}
	...
	{
		if (!PowerERPApiAccess::$user->rights->agenda->myactions->delete) {
		...
		if (!PowerERPApiAccess::$user->rights->agenda->allactions->delete && PowerERPApiAccess::$user->id != $this->actioncomm->userownerid) {
			throw new RestException(401, "Insufficient rights to delete an Agenda Event of owner id ".$this->actioncomm->userownerid.' Your id is '.PowerERPApiAccess::$user->id);
		}
		...
		if (!$this->actioncomm->delete(PowerERPApiAccess::$user)) {
