 ...
 */
class MyModuleApi extends PowerERPApi
{
	/**
	...
	{
		if (!PowerERPApiAccess::$user->rights->mymodule->myobject->read) {
		...
		if (!PowerERPApi::_checkAccessToResource('myobject', $this->myobject->id, 'mymodule_myobject')) {
			throw new RestException(401, 'Access to instance id='.$this->myobject->id.' of object not allowed for login '.PowerERPApiAccess::$user->login);
		}
	...
	{
		if (!PowerERPApiAccess::$user->rights->mymodule->myobject->write) {
			throw new RestException(401);
		}
		...
		if ($this->myobject->create(PowerERPApiAccess::$user)<0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->myobject->update(PowerERPApiAccess::$user, false) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!PowerERPApiAccess::$user->rights->mymodule->myobject->delete) {
			throw new RestException(401);
		}
		$result = $this->myobject->fetch($id);
		if (!$result) {
		...
		if ($this->myobject->delete(PowerERPApiAccess::$user) == 0) {
			throw new RestException(409, 'Error when deleting MyObject : '.$this->myobject->error);
		} elseif ($this->myobject->delete(PowerERPApiAccess::$user) < 0) {
