...
print '<span class="opacitymedium hideonsmartphone">'.$langs->trans("ModuleBuilderDesc", 'https://wiki.powererp.org/index.php/Module_development#Create_your_module').'</span>';
...
if (empty($allowonlineinstall)) {
	if (getDolGlobalString('MAIN_MESSAGE_INSTALL_MODULES_DISABLED_CONTACT_US')) {
		...
		$message = info_admin($langs->trans("InstallModuleFromWebHasBeenDisabledByFile", $powererpdataroot.'/installmodules.lock'), 0, 0, 1, 'warning');
	}
				...
				if ($action != 'editfile' || empty($file)) {
					try {
						...
						if (!empty($tmpobject)) {
							...
							$htmltext = $langs->trans("PropertyDesc").'<br><br><a class="" href="https://wiki.powererp.org/index.php/Language_and_development_rules#Table_and_fields_structures" target="_blank" rel="noopener noreferrer external">'.$langs->trans("SeeExamples").'</a>';
			...
			if ($action != 'editfile' || empty($file)) {
				...
				if (count($tabs)) {
					foreach ($tabs as $tab) {
						...
						// See: https://wiki.powererp.org/index.php?title=Tabs_system#To_remove_an_existing_tab
						if ($tabName[0] === '-') {
							$tabTitle = '';
