...
if (in_array($action, array(
	...
	$res = powererp_set_const($db, $constname, $constvalue, 'yesno', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
...
if ($action == 'update') {
	...
	if (!$error) {
		foreach ($list as $constname) {
			...
			if (!powererp_set_const($db, $constname, $constvalue, 'chaine', 0, '', $conf->entity)) {
				$error++;
			}
		}
		if ($error) {
...
if ($action == 'setlistsorttodo') {
	...
	$res = powererp_set_const($db, "ACCOUNTING_LIST_SORT_VENTILATION_TODO", $setlistsorttodo, 'yesno', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
...
if ($action == 'setlistsortdone') {
	...
	$res = powererp_set_const($db, "ACCOUNTING_LIST_SORT_VENTILATION_DONE", $setlistsortdone, 'yesno', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
...
if ($action == 'setmanagezero') {
	...
	$res = powererp_set_const($db, "ACCOUNTING_MANAGE_ZERO", $setmanagezero, 'yesno', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
...
if ($action == 'setdisabledirectinput') {
	...
	$res = powererp_set_const($db, "BANK_DISABLE_DIRECT_INPUT", $setdisabledirectinput, 'yesno', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
...
if ($action == 'setenabledraftexport') {
	...
	$res = powererp_set_const($db, "ACCOUNTING_ENABLE_EXPORT_DRAFT_JOURNAL", $setenabledraftexport, 'yesno', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
...
if ($action == 'setenablesubsidiarylist') {
	...
	$res = powererp_set_const($db, "ACCOUNTANCY_COMBO_FOR_AUX", $setenablesubsidiarylist, 'yesno', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
...
if ($action == 'setdisablebindingonsales') {
	...
	$res = powererp_set_const($db, "ACCOUNTING_DISABLE_BINDING_ON_SALES", $setdisablebindingonsales, 'yesno', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
...
if ($action == 'setdisablebindingonpurchases') {
	...
	$res = powererp_set_const($db, "ACCOUNTING_DISABLE_BINDING_ON_PURCHASES", $setdisablebindingonpurchases, 'yesno', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
...
if ($action == 'setdisablebindingonexpensereports') {
	...
	$res = powererp_set_const($db, "ACCOUNTING_DISABLE_BINDING_ON_EXPENSEREPORTS", $setdisablebindingonexpensereports, 'yesno', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
...
if ($action == 'setenablelettering') {
	...
	$res = powererp_set_const($db, "ACCOUNTING_ENABLE_LETTERING", $setenablelettering, 'yesno', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
...
if ($action == 'setenableautolettering') {
	...
	$res = powererp_set_const($db, "ACCOUNTING_ENABLE_AUTOLETTERING", $setenableautolettering, 'yesno', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
