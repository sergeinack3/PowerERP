...
$conf->db->host = empty($powererp_main_db_host) ? '' : $powererp_main_db_host;
$conf->db->port = empty($powererp_main_db_port) ? '' : $powererp_main_db_port;
$conf->db->name = empty($powererp_main_db_name) ? '' : $powererp_main_db_name;
$conf->db->user = empty($powererp_main_db_user) ? '' : $powererp_main_db_user;
$conf->db->pass = empty($powererp_main_db_pass) ? '' : $powererp_main_db_pass;
$conf->db->type = $powererp_main_db_type;
$conf->db->prefix = $powererp_main_db_prefix;
...
$conf->file->main_limit_users = $powererp_main_limit_users;
$conf->file->mailing_limit_sendbyweb = empty($powererp_mailing_limit_sendbyweb) ? 0 : $powererp_mailing_limit_sendbyweb;
$conf->file->mailing_limit_sendbycli = empty($powererp_mailing_limit_sendbycli) ? 0 : $powererp_mailing_limit_sendbycli;
$conf->file->mailing_limit_sendbyday = empty($powererp_mailing_limit_sendbyday) ? 0 : $powererp_mailing_limit_sendbyday;
$conf->file->main_authentication = empty($powererp_main_authentication) ? 'powererp' : $powererp_main_authentication; // Identification mode
$conf->file->main_force_https = empty($powererp_main_force_https) ? '' : $powererp_main_force_https; // Force https
...
if (!empty($powererp_main_document_root_alt)) {
	...
	$values = preg_split('/[;,]/', $powererp_main_url_root_alt);
	$i = 0;
	foreach ($values as $value) {
		if (preg_match('/^http(s)?:/', $value)) {
			// Show error message
			$correct_value = str_replace($powererp_main_url_root, '', $value);
			print '<b>Error:</b><br>'."\n";
			print 'Wrong <b>$powererp_main_url_root_alt</b> value in <b>conf.php</b> file.<br>'."\n";
			print 'We now use a relative path to $powererp_main_url_root to build alternate URLs.<br>'."\n";
...
if (!defined('NOREQUIREDB')) {
	...
	if ($db->error) {
		...
		if (!defined('USEPOWERERPEDITOR') && !defined('USEPOWERERPSERVER') && !empty($_SERVER['SCRIPT_FILENAME']) && (strpos($_SERVER['SCRIPT_FILENAME'], DOL_DATA_ROOT.'/website') === 0)) {
...
//unset($powererp_main_db_pass); 	// We comment this because this constant is used in some other pages
