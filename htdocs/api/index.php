...
/**
 * 	\defgroup   api     Module PowerERPApi
 *  \brief      API loader
...
if (preg_match('/api\/index\.php\/explorer/', $url) && !empty($conf->global->API_EXPLORER_DISABLED)) {
	$langs->load("admin");
	dol_syslog("Call PowerERP API interfaces with module API REST disabled");
...
$api = new PowerERPApi($db, '', $refreshcache);
//var_dump($api->r->apiVersionMap);

// If MAIN_API_DEBUG is set to 1, we save logs into file "powererp_api.log"
...
$api->r->addAuthenticationClass('PowerERPApiAccess', '');
