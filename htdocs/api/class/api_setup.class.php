 ...
 */
class Setup extends PowerERPApi
{
	...
	public function getPaymentTypes($sortfield = "code", $sortorder = 'ASC', $limit = 100, $page = 0, $active = 1, $sqlfilters = '')
	{
		...
		if (!PowerERPApiAccess::$user->rights->propal->lire && !PowerERPApiAccess::$user->rights->commande->lire && !PowerERPApiAccess::$user->rights->facture->lire) {
			throw new RestException(401);
		}
	...
	public function getListOfExtrafields($sortfield = "t.pos", $sortorder = 'ASC', $type = '', $sqlfilters = '')
	{
		...
		if (!PowerERPApiAccess::$user->admin) {
	...
	public function getCompany()
	{
		...
		if (!PowerERPApiAccess::$user->admin
			&& (empty($conf->global->API_LOGINS_ALLOWED_FOR_GET_COMPANY) || PowerERPApiAccess::$user->login != $conf->global->API_LOGINS_ALLOWED_FOR_GET_COMPANY)) {
	...
	public function getConf($constantname)
	{
		...
		if (!PowerERPApiAccess::$user->admin
			&& (empty($conf->global->API_LOGINS_ALLOWED_FOR_CONST_READ) || PowerERPApiAccess::$user->login != $conf->global->API_LOGINS_ALLOWED_FOR_CONST_READ)) {
	...
	public function getCheckIntegrity($target)
	{
		...
		if (!PowerERPApiAccess::$user->admin
			&& (empty($conf->global->API_LOGINS_ALLOWED_FOR_INTEGRITY_CHECK) || PowerERPApiAccess::$user->login != $conf->global->API_LOGINS_ALLOWED_FOR_INTEGRITY_CHECK)) {
		...
		if ($xml) {
			...
			// Forced constants
			if (is_object($xml->powererp_constants[0])) {
				...
				$i = 0;
				foreach ($xml->powererp_constants[0]->constant as $constant) {    // $constant is a simpleXMLElement
			...
			// Scan htdocs
			if (is_object($xml->powererp_htdocs_dir[0])) {
				$includecustom = (empty($xml->powererp_htdocs_dir[0]['includecustom']) ? 0 : $xml->powererp_htdocs_dir[0]['includecustom']);
				...
				$ret = getFilesUpdated($file_list, $xml->powererp_htdocs_dir[0], '', DOL_DOCUMENT_ROOT, $checksumconcat); // Fill array $file_list
				...
				throw new RestException(500, 'Error: Failed to found powererp_htdocs_dir into XML file '.$xmlfile);
			}
			...
			$checksumtoget = trim((string) $xml->powererp_htdocs_dir_checksum);
	...
	public function getModules()
	{
		...
		if (!PowerERPApiAccess::$user->admin
			&& (empty($conf->global->API_LOGINS_ALLOWED_FOR_GET_MODULES) || PowerERPApiAccess::$user->login != $conf->global->API_LOGINS_ALLOWED_FOR_GET_MODULES)) {
