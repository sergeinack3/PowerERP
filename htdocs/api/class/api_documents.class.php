 ...
 */
class Documents extends PowerERPApi
{
	...
	public function index($modulepart, $original_file = '')
	{
		...
		$check_access = dol_check_secure_access_document($modulepart, $relativefile, $entity, PowerERPApiAccess::$user, '', 'read');
	...
	public function builddoc($modulepart, $original_file = '', $doctemplate = '', $langcode = '')
	{
		...
		$check_access = dol_check_secure_access_document($modulepart, $relativefile, $entity, PowerERPApiAccess::$user, '', 'write');
		$accessallowed              = $check_access['accessallowed'];
		$sqlprotectagainstexternals = $check_access['sqlprotectagainstexternals'];
		$original_file              = $check_access['original_file'];
	...
	public function getDocumentsListByElement($modulepart, $id = 0, $ref = '', $sortfield = '', $sortorder = '')
	{
		...
		if ($modulepart == 'societe' || $modulepart == 'thirdparty') {
			...
			if (!PowerERPApiAccess::$user->rights->user->user->lire && PowerERPApiAccess::$user->id != $id) {
				throw new RestException(401);
			}
			...
			if (!PowerERPApiAccess::$user->rights->adherent->lire) {
				throw new RestException(401);
			}
			...
			if (!PowerERPApiAccess::$user->rights->propal->lire) {
				throw new RestException(401);
			}
			...
			if (!PowerERPApiAccess::$user->rights->supplier_proposal->read) {
				throw new RestException(401);
			}
			...
			if (empty(PowerERPApiAccess::$user->rights->fournisseur->commande->lire) && empty(PowerERPApiAccess::$user->rights->supplier_order->lire)) {
				throw new RestException(401);
			}
			...
			if (!PowerERPApiAccess::$user->rights->expedition->lire) {
				throw new RestException(401);
			}
			...
			if (empty(PowerERPApiAccess::$user->rights->fournisseur->facture->lire) && empty(PowerERPApiAccess::$user->rights->supplier_invoice->lire)) {
				throw new RestException(401);
			}
			...
			if (!PowerERPApiAccess::$user->rights->agenda->myactions->read && !PowerERPApiAccess::$user->rights->agenda->allactions->read) {
				throw new RestException(401);
			}
			...
			if (!PowerERPApiAccess::$user->rights->expensereport->read && !PowerERPApiAccess::$user->rights->expensereport->read) {
				throw new RestException(401);
			}
			...
			if (!PowerERPApiAccess::$user->rights->knowledgemanagement->knowledgerecord->read && !PowerERPApiAccess::$user->rights->knowledgemanagement->knowledgerecord->read) {
				throw new RestException(401);
			}
			...
			// if (!PowerERPApiAccess::$user->rights->ecm->read) {
			// 	throw new RestException(401);
			// }
	...
	public function post($filename, $modulepart, $ref = '', $subdir = '', $filecontent = '', $fileencoding = '', $overwriteifexists = 0, $createdirifnotexists = 1)
	{
		...
		if (!PowerERPApiAccess::$user->rights->ecm->upload) {
			throw new RestException(401);
		}
		...
		// Define $uploadir
		$object = null;
		$entity = PowerERPApiAccess::$user->entity;
		if (empty($entity)) {
			$entity = 1;
		}
		...
		if ($ref) {
			...
			$tmp = dol_check_secure_access_document($modulepart, $relativefile, $entity, PowerERPApiAccess::$user, $ref, 'write');
			...
			$relativefile = $subdir;
			$tmp = dol_check_secure_access_document($modulepart, $relativefile, $entity, PowerERPApiAccess::$user, '', 'write');
			$upload_dir = $tmp['original_file']; // No dirname here, tmp['original_file'] is already the dir because dol_check_secure_access_document was called with param original_file that is only the dir
