...
class PowerERPApiAccess implements iAuthenticate
{
	...
	public function __isAllowed()
	{
		...
		if ($api_key) {
			...
			$sql .= " WHERE u.api_key = '".$this->db->escape($api_key)."' OR u.api_key = '".$this->db->escape(dolEncrypt($api_key, '', '', 'powererp'))."'";
		...
		Resources::$accessControlFunction = 'PowerERPApiAccess::verifyAccess';
	...
	{
		$requires = isset($m['class']['PowerERPApiAccess']['properties']['requires'])
				? $m['class']['PowerERPApiAccess']['properties']['requires']
				: false;
