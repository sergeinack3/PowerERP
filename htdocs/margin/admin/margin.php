...
if ($action == 'remises') {
	if (powererp_set_const($db, 'MARGIN_METHODE_FOR_DISCOUNT', $_POST['MARGIN_METHODE_FOR_DISCOUNT'], 'chaine', 0, '', $conf->entity) > 0) {
...
if ($action == 'typemarges') {
	if (powererp_set_const($db, 'MARGIN_TYPE', $_POST['MARGIN_TYPE'], 'chaine', 0, '', $conf->entity) > 0) {
		  setEventMessages($langs->trans("RecordModifiedSuccessfully"), null, 'mesgs');
	} else {
		dol_print_error($db);
	}
...
if ($action == 'contact') {
	if (powererp_set_const($db, 'AGENT_CONTACT_TYPE', $_POST['AGENT_CONTACT_TYPE'], 'chaine', 0, '', $conf->entity) > 0) {
		  setEventMessages($langs->trans("RecordModifiedSuccessfully"), null, 'mesgs');
	} else {
		dol_print_error($db);
	}
