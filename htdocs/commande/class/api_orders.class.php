 ...
 */
class Orders extends PowererpApi
{
	...
	private function _fetch($id, $ref = '', $ref_ext = '', $contact_list = 1)
	{
		...
		if (!PowererpApi::_checkAccessToResource('commande', $this->commande->id)) {
	...
	{
		if (!PowererpApiAccess::$user->rights->commande->creer) {
		...
		if ($this->commande->create(PowererpApiAccess::$user) < 0) {
	...
	public function deleteLine($id, $lineid)
	{
		...
		$updateRes = $this->commande->deleteline(PowererpApiAccess::$user, $lineid);
		if ($updateRes > 0) {
			return $this->get($id);
		} else {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->commande->update(PowererpApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!PowererpApiAccess::$user->rights->commande->supprimer) {
			throw new RestException(401);
		}
		$result = $this->commande->fetch($id);
		if (!$result) {
		...
		if (!$this->commande->delete(PowererpApiAccess::$user)) {
	...
	public function validate($id, $idwarehouse = 0, $notrigger = 0)
	{
		...
		$result = $this->commande->valid(PowererpApiAccess::$user, $idwarehouse, $notrigger);
		if ($result == 0) {
	...
	public function reopen($id)
	{
		...
		$result = $this->commande->set_reopen(PowererpApiAccess::$user);
		if ($result < 0) {
			throw new RestException(405, $this->commande->error);
		} elseif ($result == 0) {
	...
	public function setinvoiced($id)
	{
		...
		$result = $this->commande->classifyBilled(PowererpApiAccess::$user);
		if ($result < 0) {
			throw new RestException(400, $this->commande->error);
		}
	...
	public function close($id, $notrigger = 0)
	{
		...
		$result = $this->commande->cloture(PowererpApiAccess::$user, $notrigger);
		if ($result == 0) {
	...
	public function settodraft($id, $idwarehouse = -1)
	{
		...
		$result = $this->commande->setDraft(PowererpApiAccess::$user, $idwarehouse);
		if ($result == 0) {
	...
	public function createOrderFromProposal($proposalid)
	{
		...
		$result = $this->commande->createFromProposal($propal, PowererpApiAccess::$user);
		if ($result < 0) {
			throw new RestException(405, $this->commande->error);
		}
		$this->commande->fetchObjectLinked();
	...
	{
		require_once DOL_DOCUMENT_ROOT.'/expedition/class/expedition.class.php';
		if (!PowererpApiAccess::$user->rights->expedition->creer) {
			throw new RestException(401);
		}
		if ($warehouse_id <= 0) {
		...
		$result = $shipment->create(PowererpApiAccess::$user);
		if ($result <= 0) {
