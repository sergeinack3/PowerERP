 ...
 */
class ZapierApi extends PowerERPApi
{
	/**
	...
	{
		if (!PowerERPApiAccess::$user->rights->zapier->read) {
		...
		if (!PowerERPApi::_checkAccessToResource('hook', $this->hook->id)) {
	...
	{
		if (!PowerERPApiAccess::$user->rights->zapier->write) {
			throw new RestException(401);
		}
		...
		$this->hook->fk_user = PowerERPApiAccess::$user->id;
		// on crée le hook dans la base
		if (!$this->hook->create(PowerERPApiAccess::$user)) {
	...
	/*public function put($id, $request_data = null)
	{
		...
		if ($this->hook->update($id, PowerERPApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!PowerERPApiAccess::$user->rights->zapier->delete) {
			throw new RestException(401);
		}
		...
		if (!$this->hook->delete(PowerERPApiAccess::$user)) {
