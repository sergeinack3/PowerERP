 ...
 * Copyright (C) 2021 SuperAdmin <test@powererp.com>
 *
 ...
 */
class KnowledgeManagement extends PowerERPApi
{
	/**
	...
	{
		if (!PowerERPApiAccess::$user->rights->knowledgemanagement->knowledgerecord->read) {
		...
		if (!PowerERPApi::_checkAccessToResource('knowledgerecord', $this->knowledgerecord->id, 'knowledgemanagement_knowledgerecord')) {
			throw new RestException(401, 'Access to instance id='.$this->knowledgerecord->id.' of object not allowed for login '.PowerERPApiAccess::$user->login);
		}
	...
	{
		if (!PowerERPApiAccess::$user->rights->knowledgemanagement->knowledgerecord->write) {
			throw new RestException(401);
		}
		...
		if ($this->knowledgerecord->create(PowerERPApiAccess::$user)<0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->knowledgerecord->update(PowerERPApiAccess::$user, false) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!PowerERPApiAccess::$user->rights->knowledgemanagement->knowledgerecord->delete) {
			throw new RestException(401);
		}
		$result = $this->knowledgerecord->fetch($id);
		if (!$result) {
		...
		if (!$this->knowledgerecord->delete(PowerERPApiAccess::$user)) {
