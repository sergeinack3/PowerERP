...
if ($action == 'create') {
	...
	$doleditor = new DolEditor('answer', $object->answer, '', 200, 'powererp_notes', 'In', true, 0, true, ROWS_9, '100%');
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	print $langs->trans($object->fields['answer']['label']).'<br>';
	require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
	$doleditor = new DolEditor('answer', $object->answer, '', 200, 'powererp_notes', 'In', true, 0, true, ROWS_9, '100%', 1);
	$out = $doleditor->Create(1);
	print $out;
