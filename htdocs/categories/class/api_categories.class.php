 ...
 */
class Categories extends PowerERPApi
{
	/**
	...
	public function get($id, $include_childs = false)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('categorie', $this->category->id)) {
	...
	{
		if (!PowerERPApiAccess::$user->rights->categorie->creer) {
			throw new RestException(401);
		}
		...
		if ($this->category->create(PowerERPApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->category->update(PowerERPApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!PowerERPApiAccess::$user->rights->categorie->supprimer) {
			throw new RestException(401);
		}
		$result = $this->category->fetch($id);
		if (!$result) {
		...
		if (!$this->category->delete(PowerERPApiAccess::$user)) {
	...
	public function getListForObject($id, $type, $sortfield = "s.rowid", $sortorder = 'ASC', $limit = 0, $page = 0)
	{
		...
		if ($type == Categorie::TYPE_PRODUCT && !(PowerERPApiAccess::$user->rights->produit->lire || PowerERPApiAccess::$user->rights->service->lire)) {
			throw new RestException(401);
		} elseif ($type == Categorie::TYPE_CONTACT && !PowerERPApiAccess::$user->rights->contact->lire) {
			throw new RestException(401);
		} elseif ($type == Categorie::TYPE_CUSTOMER && !PowerERPApiAccess::$user->hasRight('societe', 'lire')) {
			throw new RestException(401);
		} elseif ($type == Categorie::TYPE_SUPPLIER && !PowerERPApiAccess::$user->rights->fournisseur->lire) {
			throw new RestException(401);
		} elseif ($type == Categorie::TYPE_MEMBER && !PowerERPApiAccess::$user->rights->adherent->lire) {
			throw new RestException(401);
		} elseif ($type == Categorie::TYPE_PROJECT && !PowerERPApiAccess::$user->rights->projet->lire) {
			throw new RestException(401);
		} elseif ($type == Categorie::TYPE_KNOWLEDGEMANAGEMENT && !PowerERPApiAccess::$user->rights->knowledgemanagement->knowledgerecord->read) {
			throw new RestException(401);
		}
	...
	public function linkObjectById($id, $type, $object_id)
	{
		...
		if ($type === Categorie::TYPE_PRODUCT) {
			if (!(PowerERPApiAccess::$user->rights->produit->creer || PowerERPApiAccess::$user->rights->service->creer)) {
				throw new RestException(401);
			}
			...
			if (!PowerERPApiAccess::$user->rights->societe->contact->creer) {
				throw new RestException(401);
			}
			$object = new Contact($this->db);
		} elseif ($type === Categorie::TYPE_MEMBER) {
			if (!PowerERPApiAccess::$user->hasRight('adherent', 'creer')) {
				throw new RestException(401);
			}
