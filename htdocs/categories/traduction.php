...
if ($action == 'edit') {
	...
	if (!empty($object->multilangs)) {
		foreach ($object->multilangs as $key => $value) {
			...
			$doleditor = new DolEditor("desc-$key", $desc, '', 160, 'powererp_notes', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_PRODUCTDESC'), ROWS_3, '90%');
			$doleditor->Create();
			print '</td></tr>';
...
if ($action == 'add' && ($user->rights->produit->creer || $user->rights->service->creer)) {
	...
	$doleditor = new DolEditor('desc', GETPOST('desc', 'restricthtml'), '', 160, 'powererp_notes', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_PRODUCTDESC'), ROWS_3, '90%');
	$doleditor->Create();
	print '</td></tr>';
