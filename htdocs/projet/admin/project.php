...
if ($action == 'updateMask') {
	...
	if ($maskconstproject) {
		$res = powererp_set_const($db, $maskconstproject, $maskproject, 'chaine', 0, '', $conf->entity);
	}
...
if ($action == 'updateMaskTask') {
	...
	if ($maskconstmasktask) {
		$res = powererp_set_const($db, $maskconstmasktask, $masktaskt, 'chaine', 0, '', $conf->entity);
	}
	...
	if ($ret > 0) {
		if ($conf->global->PROJECT_ADDON_PDF == "$value") {
			powererp_del_const($db, 'PROJECT_ADDON_PDF', $conf->entity);
		}
	}
	...
	if ($ret > 0) {
		if ($conf->global->PROJECT_TASK_ADDON_PDF == "$value") {
			powererp_del_const($db, 'PROJECT_TASK_ADDON_PDF', $conf->entity);
		}
	}
} elseif ($action == 'setdoc') {
	// Set default model
	powererp_set_const($db, "PROJECT_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity);
	...
	if (powererp_set_const($db, "PROJECT_TASK_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
	...
	powererp_set_const($db, "PROJECT_ADDON", $value, 'chaine', 0, '', $conf->entity);
} elseif ($action == 'setmodtask') {
	// TODO Verifier si module numerotation choisi peut etre active
	// par appel methode canBeActivated

	powererp_set_const($db, "PROJECT_TASK_ADDON", $value, 'chaine', 0, '', $conf->entity);
	...
	if (GETPOST('PROJECT_USE_SEARCH_TO_SELECT')) {
		...
		if (powererp_set_const($db, "PROJECT_USE_SEARCH_TO_SELECT", $companysearch, 'chaine', 0, '', $conf->entity)) {
	...
	if (GETPOST('PROJECT_ALLOW_TO_LINK_FROM_OTHER_COMPANY')) {
		...
		powererp_set_const($db, 'PROJECT_ALLOW_TO_LINK_FROM_OTHER_COMPANY', $projectToSelect, 'chaine', 0, '', $conf->entity); //Allow to disable this configuration if empty value
	}
	if (GETPOST('PROJECT_TIMESHEET_PREVENT_AFTER_MONTHS')) {
		$timesheetFreezeDuration = GETPOST('timesheetFreezeDuration', 'alpha');
		powererp_set_const($db, 'PROJECT_TIMESHEET_PREVENT_AFTER_MONTHS', intval($timesheetFreezeDuration), 'chaine', 0, '', $conf->entity); //Allow to disable this configuration if empty value
	}
} elseif (preg_match('/^(set|del)_?([A-Z_]+)$/', $action, $reg)) {
	// Set boolean (on/off) constants
	if (!powererp_set_const($db, $reg[2], ($reg[1] === 'set' ? '1' : '0'), 'chaine', 0, '', $conf->entity) > 0) {
		dol_print_error($db);
	}
}
