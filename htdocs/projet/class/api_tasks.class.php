 ...
 */
class Tasks extends PowerERPApi
{
	...
	{
		if (!PowerERPApiAccess::$user->rights->projet->lire) {
		...
		if (!PowerERPApi::_checkAccessToResource('task', $this->task->id)) {
	...
	{
		if (!PowerERPApiAccess::$user->rights->projet->creer) {
		...
		if ($this->task->create(PowerERPApiAccess::$user) < 0) {
	...
	public function getLines($id, $includetimespent=0)
	{
		...
		if( ! PowerERPApi::_checkAccessToResource('project',$this->project->id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}
		$this->project->getLinesArray(PowerERPApiAccess::$user);
		$result = array();
	...
	*/
	...
	public function getRoles($id, $userid = 0)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('tasks', $this->task->id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}

		$usert = PowerERPApiAccess::$user;
		if ($userid > 0) {
	...
	*/
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->task->update(PowerERPApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!PowerERPApiAccess::$user->rights->projet->supprimer) {
			throw new RestException(401);
		}
		$result = $this->task->fetch($id);
		if (!$result) {
		...
		if (!$this->task->delete(PowerERPApiAccess::$user)) {
	...
	public function addTimeSpent($id, $date, $duration, $user_id = 0, $note = '')
	{
		...
		if (!PowerERPApi::_checkAccessToResource('project', $this->task->fk_project)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}

		$uid = $user_id;
		if (empty($uid)) {
			$uid = PowerERPApiAccess::$user->id;
		}
		...
		$result = $this->task->addTimeSpent(PowerERPApiAccess::$user, 0);
		if ($result == 0) {
	...
	public function putTimeSpent($id, $timespent_id, $date, $duration, $user_id = 0, $note = '')
	{
		...
		$this->task->timespent_withhour = 1;
		$this->task->timespent_duration = $duration;
		$this->task->timespent_fk_user  = $user_id ?? PowerERPApiAccess::$user->id;
		$this->task->timespent_note     = $note;

		$result = $this->task->updateTimeSpent(PowerERPApiAccess::$user, 0);
		if ($result == 0) {
	...
	public function deleteTimeSpent($id, $timespent_id)
	{
		...
		if ($this->task->delTimeSpent(PowerERPApiAccess::$user, 0) < 0) {
