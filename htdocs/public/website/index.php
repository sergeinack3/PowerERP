...
if ($pageid == 'css') {   // No more used ?
	...
	$original_file = $powererp_main_data_root.($conf->entity > 1 ? '/'.$conf->entity : '').'/website/'.$websitekey.'/styles.css.php';
} else {
	$original_file = $powererp_main_data_root.($conf->entity > 1 ? '/'.$conf->entity : '').'/website/'.$websitekey.'/page'.$pageid.'.tpl.php';
}
...
// Output page content
define('USEPOWERERPSERVER', 1);
print '<!-- Page content '.$original_file.' rendered with POWERERP SERVER : Html with CSS link and html header + Body that was saved into tpl dir -->'."\n";
