...
if (empty($reshook) && $action == 'add') {
					...
					if (!empty(floatval($project->price_booth))) {
						...
						if (!$error) {
							...
							/*if (!$error) {
								...
								$reftouse = $facture->id;
								$redirection = $powererp_main_url_root.'/public/payment/newpayment.php?source='.$sourcetouse.'&ref='.$reftouse.'&booth='.$conforbooth->id;
	...
	if (!$error) {
		...
		$redirection = $powererp_main_url_root.'/public/eventorganization/subscriptionok.php?id='.$id.'&securekey='.$securekeyurl;
		Header("Location: ".$redirection);
		exit;
	} else {
		$db->rollback();
