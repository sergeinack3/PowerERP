	...
	foreach ($qualifiedjobs as $line) {
		...
		if (($line->datenextrun < $now) && (empty($line->datestart) || $line->datestart <= $now) && (empty($line->dateend) || $line->dateend >= $now)) {
			...
			if ($result < 0) {
				...
				echo "You can also enable module Log if not yet enabled, run again and take a look into powererp.log file\n";
			...
			if ($result < 0) {
				...
				echo "Enable module Log if not yet enabled, run again and take a look into powererp.log file\n";
