...
if (empty($reshook) && $action == 'add' && (!empty($conference->id) && $conference->status!=2  || !empty($project->id) && $project->status == Project::STATUS_VALIDATED)) {
	...
	if (!$error) {
		...
		if (!empty($confattendee->date_subscription) && !empty($confattendee->amount)) {
			...
			$redirection = $powererp_main_url_root.'/public/eventorganization/subscriptionok.php?id='.((int) $id).'&securekey='.urlencode($securekeyurl);
	...
	if (!$error) {
		...
		if (!empty(floatval($project->price_registration))) {
			...
			if (!$error) {
				...
				$reftouse = $facture->id;
				$redirection = $powererp_main_url_root.'/public/payment/newpayment.php?source='.urlencode($sourcetouse).'&ref='.urlencode($reftouse);
