...
if (!empty($withproject)) {
	...
	//print '</span>';
	print '</td><td>';
	$linksuggest = $powererp_main_url_root.'/public/project/index.php?id='.((int) $projectstatic->id);
	...
	//print '</span>';
	print '</td><td>';
	$link_subscription = $powererp_main_url_root.'/public/eventorganization/attendee_new.php?id='.((int) $projectstatic->id).'&type=global';
	$encodedsecurekey = dol_hash(getDolGlobalString('EVENTORGANIZATION_SECUREKEY').'conferenceorbooth'.((int) $projectstatic->id), 'md5');
