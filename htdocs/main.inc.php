...
if (!empty($_POST["DOL_AUTOSET_COOKIE"])) {
	...
	setcookie($cookiename, empty($cookievalue) ? '' : $cookievalue, empty($cookievalue) ? 0 : (time() + (86400 * 354)), '/', null, (empty($powererp_main_force_https) ? false : true), true); // keep cookie 1 year and add tag httponly
...
if (!defined('NOSESSION')) {
	session_set_cookie_params(0, '/', null, (empty($powererp_main_force_https) ? false : true), true); // Add tag secure and httponly on session cookie (same as setting session.cookie_httponly into php.ini). Must be called before the session_start.
