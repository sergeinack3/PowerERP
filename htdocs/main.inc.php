...
if (!empty($_POST["DOL_AUTOSET_COOKIE"])) {
	...
	if (PHP_VERSION_ID < 70300) {
		setcookie($cookiename, empty($cookievalue) ? '' : $cookievalue, empty($cookievalue) ? 0 : (time() + (86400 * 354)), '/', null, ((empty($powererp_main_force_https) && isHTTPS() === false) ? false : true), true); // keep cookie 1 year and add tag httponly
	} else {
		...
		$cookieparams = array(
			...
			'secure' => ((empty($powererp_main_force_https) && isHTTPS() === false) ? false : true),
			'httponly' => true,
...
if (!defined('NOSESSION')) {
	if (PHP_VERSION_ID < 70300) {
		session_set_cookie_params(0, '/', null, ((empty($powererp_main_force_https) && isHTTPS() === false) ? false : true), true); // Add tag secure and httponly on session cookie (same as setting session.cookie_httponly into php.ini). Must be called before the session_start.
	} else {
		// Only available for php >= 7.3
...
if (!empty($conf->file->main_force_https) && (empty($_SERVER["HTTPS"]) || $_SERVER["HTTPS"] != 'on')) {
	...
	if ($newurl) {
		...
		dol_syslog("main.inc: powererp_main_force_https is on, we make a redirect to ".$newurl);
		header("Location: ".$newurl);
		exit;
	} else {
		dol_syslog("main.inc: powererp_main_force_https is on but we failed to forge new https url so no redirect is done", LOG_WARNING);
	}
}

if (!defined('NOLOGIN') && !defined('NOIPCHECK') && !empty($powererp_main_restrict_ip)) {
	$listofip = explode(',', $powererp_main_restrict_ip);
	$found = false;
...
|| (empty($conf->global->MAIN_VERSION_LAST_UPGRADE) && !empty($conf->global->MAIN_VERSION_LAST_INSTALL) && ($conf->global->MAIN_VERSION_LAST_INSTALL != DOL_VERSION))) {
	...
	$powererpversionlastupgrade = preg_split('/[.-]/', $versiontocompare);
	$powererpversionprogram = preg_split('/[.-]/', DOL_VERSION);
	$rescomp = versioncompare($powererpversionprogram, $powererpversionlastupgrade);
...
//dol_syslog("aaaa - ".defined('NOCSRFCHECK')." - ".$powererp_nocsrfcheck." - ".$conf->global->MAIN_SECURITY_CSRF_WITH_TOKEN." - ".$_SERVER['REQUEST_METHOD']." - ".GETPOST('token', 'alpha'));
...
if ((!defined('NOCSRFCHECK') && empty($powererp_nocsrfcheck) && getDolGlobalInt('MAIN_SECURITY_CSRF_WITH_TOKEN')) || defined('CSRFCHECK_WITH_TOKEN')) {
		...
		if (!GETPOST('token', 'alpha') || GETPOST('token', 'alpha') == 'notrequired') {
			if (GETPOST('uploadform', 'int')) {
				...
				if (defined('CSRFCHECK_WITH_TOKEN')) {
					...
					print "If you access your server behind a proxy using url rewriting and the parameter is provided by caller, you might check that all HTTP header are propagated (or add the line \$powererp_nocsrfcheck=1 into your conf.php file or MAIN_SECURITY_CSRF_WITH_TOKEN to 0";
...
if (!defined('NOLOGIN')) {
	...
	if (!isset($_SESSION["dol_login"])) {
		...
		if (!empty($powererp_main_demo) && $_SERVER['PHP_SELF'] == DOL_URL_ROOT.'/index.php') {  // We ask index page
		...
		$usertotest = (!empty($_COOKIE['login_powererp']) ? preg_replace('/[^a-zA-Z0-9_\-]/', '', $_COOKIE['login_powererp']) : GETPOST("username", "alpha", $allowedmethodtopostusername));
		...
		if (GETPOST("username", "alpha", $allowedmethodtopostusername) || !empty($_COOKIE['login_powererp']) || GETPOST('openid_mode', 'alpha', 1)) {
			$goontestloop = true;
		}
		...
		// Note authmode is an array for example: array('0'=>'powererp', '1'=>'google');
		...
		if ($resultFetchUser <= 0) {
			...
			session_destroy();
			session_set_cookie_params(0, '/', null, (empty($powererp_main_force_https) ? false : true), true); // Add tag secure and httponly on session cookie
			session_name($sessionname);
			session_start();
			...
			if ($resultFetchUser == 0) {
				...
				$_SESSION["dol_loginmesg"] = $langs->transnoentitiesnoconv("ErrorCantLoadUserFromPowerERPDatabase", $login);

				$user->trigger_mesg = 'ErrorCantLoadUserFromPowerERPDatabase - login='.$login;
			}
			if ($resultFetchUser < 0) {
...
function top_htmlhead($head, $title = '', $disablejs = 0, $disablehead = 0, $arrayofjs = '', $arrayofcss = '', $disableforlogin = 0, $disablenofollow = 0, $disablenoindex = 0)
{
	...
	if (empty($disablehead)) {
		...
		// Favicon
		$favicon = DOL_URL_ROOT.'/theme/powererp_256x256_color.png';
...
function top_menu_user($hideloginname = 0, $urllogout = '')
{
	...
	$dropdownBody .= '<br><b>'.$langs->trans("AuthenticationMode").':</b> '.$_SESSION["dol_authmode"].(empty($powererp_main_demo) ? '' : ' (demo)');
...
if (!function_exists("llxFooter")) {
	...
	function llxFooter($comment = '', $zone = 'private', $disabledoutputofmessages = 0)
	{
		...
		if (($_SERVER["PHP_SELF"] == DOL_URL_ROOT.'/index.php') || $forceping) {
			...
			$hash_unique_id = md5('powererp'.$conf->file->instance_unique_id);	// Do not use dol_hash(), must not change if salt changes.
			...
			|| $forceping) {
				...
				if (strpos('alpha', DOL_VERSION) > 0 && !$forceping) {
					...
					if (!empty($conf->global->MAIN_LAST_PING_KO_DATE) && substr($conf->global->MAIN_LAST_PING_KO_DATE, 0, 6) == dol_print_date(dol_now(), '%Y%m') && !$forceping) {
						...
						if (!empty($powererp_distrib)) {
							$distrib = $powererp_distrib;
						}
						?>
							<script>
					...
					powererp_set_const($db, 'MAIN_FIRST_PING_OK_DATE', dol_print_date($now, 'dayhourlog', 'gmt'), 'chaine', 0, '', $conf->entity);
					powererp_set_const($db, 'MAIN_FIRST_PING_OK_ID', 'disabled', 'chaine', 0, '', $conf->entity);
				}
			}
		}
