 ...
 */
class Receptions extends PowerERPApi
{
	...
	{
		if (!PowerERPApiAccess::$user->rights->reception->lire) {
		...
		if (!PowerERPApi::_checkAccessToResource('reception', $this->reception->id)) {
	...
	{
		if (!PowerERPApiAccess::$user->rights->reception->creer) {
		...
		if ($this->reception->create(PowerERPApiAccess::$user) < 0) {
	...
	*/
	...
	public function deleteLine($id, $lineid)
	{
		...
		$updateRes = $this->reception->deleteline(PowerERPApiAccess::$user, $lineid);
		if ($updateRes > 0) {
			return $this->get($id);
		} else {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->reception->update(PowerERPApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!PowerERPApiAccess::$user->rights->reception->supprimer) {
			throw new RestException(401);
		}
		$result = $this->reception->fetch($id);
		if (!$result) {
		...
		if (!$this->reception->delete(PowerERPApiAccess::$user)) {
	...
	public function validate($id, $notrigger = 0)
	{
		...
		$result = $this->reception->valid(PowerERPApiAccess::$user, $notrigger);
		if ($result == 0) {
	...
	public function setinvoiced($id)
	{
		...
		$result = $this->reception->classifyBilled(PowerERPApiAccess::$user);
		if( $result < 0) {
	...
	*/
	...
	public function createShipmentFromOrder($orderid)
	{
		...
		$result = $this->reception->createFromOrder($order, PowerERPApiAccess::$user);
		if( $result < 0) {
				throw new RestException(405, $this->reception->error);
		}
		$this->reception->fetchObjectLinked();
	...
	*/
	public function close($id, $notrigger = 0)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('reception', $this->commande->id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}
