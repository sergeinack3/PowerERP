...
require_once $powererp_main_document_root.'/core/lib/security.lib.php'; // for dol_hash
...
if (@file_exists($forcedfile)) {
	...
	if ($force_install_noedit == 2) {
		if (!empty($force_install_powererplogin)) {
			$login = $force_install_powererplogin;
		}
	}
}

powererp_install_syslog("- step5: entering step5.php page");
...
if ($action == "set" || empty($action) || preg_match('/upgrade/i', $action)) {
	...
	if ($action == "set") {
		...
		powererp_install_syslog('step5: load module user '.DOL_DOCUMENT_ROOT."/core/modules/".$file, LOG_INFO);
		...
		if ($db->connected) {
			...
			if ($resql) {
				...
				if ($numrows == 0) {
					...
					powererp_set_const($db, "DATABASE_PWD_ENCRYPTED", "1", 'chaine', 0, '', $conf->entity);
					powererp_set_const($db, "MAIN_SECURITY_SALT", dol_print_date(dol_now(), 'dayhourlog'), 'chaine', 0, '', 0); // All entities
					if (function_exists('password_hash')) {
						powererp_set_const($db, "MAIN_SECURITY_HASH_ALGO", 'password_hash', 'chaine', 0, '', 0); // All entities
					} else {
						powererp_set_const($db, "MAIN_SECURITY_HASH_ALGO", 'sha1md5', 'chaine', 0, '', 0); // All entities
					}
				}

				powererp_install_syslog('step5: DATABASE_PWD_ENCRYPTED = '.$conf->global->DATABASE_PWD_ENCRYPTED.' MAIN_SECURITY_HASH_ALGO = '.$conf->global->MAIN_SECURITY_HASH_ALGO, LOG_INFO);
			}
			...
			if ($result > 0) {
				...
				if ($result == -6) {	//login or email already exists
					powererp_install_syslog('step5: AdminLoginAlreadyExists', LOG_WARNING);
					print '<br><div class="warning">'.$newuser->error."</div><br>";
					$success = 1;
				} else {
					powererp_install_syslog('step5: FailedToCreateAdminLogin '.$newuser->error, LOG_ERR);
			...
			if ($success) {
				...
				powererp_install_syslog('step5: set MAIN_VERSION_LAST_INSTALL const to '.$targetversion, LOG_DEBUG);
				...
				if ($useforcedwizard) {
					powererp_install_syslog('step5: set MAIN_REMOVE_INSTALL_WARNING const to 1', LOG_DEBUG);
				...
				if (!empty($force_install_module)) {
					if (!defined('DOL_DOCUMENT_ROOT') && !empty($powererp_main_document_root)) {
						define('DOL_DOCUMENT_ROOT', $powererp_main_document_root);
					}
					...
					foreach ($tmparray as $modtoactivate) {
						...
						powererp_install_syslog('step5: activate module file='.$file);
				...
				powererp_install_syslog('step5: remove MAIN_NOT_INSTALLED const');
		...
		if ($db->connected) {
			...
			if ($tagdatabase) {
				powererp_install_syslog('step5: set MAIN_VERSION_LAST_UPGRADE const to value '.$targetversion);
				...
				powererp_install_syslog('step5: we run an upgrade to version '.$targetversion.' but database was already upgraded to '.$conf->global->MAIN_VERSION_LAST_UPGRADE.'. We keep MAIN_VERSION_LAST_UPGRADE as it is.');
			}
		} else {
			print $langs->trans("ErrorFailedToConnect")."<br>";
		}
...
if ($action == "set") {
	if ($success) {
		if (empty($conf->global->MAIN_VERSION_LAST_UPGRADE) || ($conf->global->MAIN_VERSION_LAST_UPGRADE == DOL_VERSION)) {
			...
			print '<div class="center"><a href="'.$powererp_main_url_root.'/install/index.php">';
	...
	if (empty($conf->global->MAIN_VERSION_LAST_UPGRADE) || ($conf->global->MAIN_VERSION_LAST_UPGRADE == DOL_VERSION)) {
		...
		$morehtml .= '<span class="fas fa-link-alt"></span> '.$langs->trans("GoToPowerERP").'...';
		$morehtml .= '</a></div><br>';
	} else {
		// If here MAIN_VERSION_LAST_UPGRADE is not empty
		print $langs->trans("VersionLastUpgrade").': <b><span class="ok">'.$conf->global->MAIN_VERSION_LAST_UPGRADE.'</span></b><br>';
