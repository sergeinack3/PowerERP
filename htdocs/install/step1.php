...
if (@file_exists($forcedfile)) {
	...
	if ($force_install_noedit) {
		$main_dir = detect_powererp_main_document_root();
		if (!empty($argv[1])) {
		...
		if (!empty($force_install_main_data_root)) {
			...
			$main_data_dir = detect_powererp_main_data_root($main_dir);
		}
		$main_url = detect_powererp_main_url_root();
	...
	if (!empty($force_install_distrib)) {
		$powererp_main_distrib = $force_install_distrib;
	}
}
...
powererp_install_syslog("--- step1: entering step1.php page");
...
if (!$error) {
	...
	if ($result) {
		...
		if (!empty($db_create_database) && !$userroot) {
			print '<div class="error">'.$langs->trans("YouAskDatabaseCreationSoPowerERPNeedToConnect", $db_name).'</div>';
			print '<br>';
			print $langs->trans("BecauseConnectionFailedParametersMayBeWrong").'<br><br>';
			print $langs->trans("ErrorGoBackAndCorrectParameters");
			$error++;
		...
		if (!empty($db_create_user) && !$userroot) {
			print '<div class="error">'.$langs->trans("YouAskLoginCreationSoPowerERPNeedToConnect", $db_user).'</div>';
			print '<br>';
			print $langs->trans("BecauseConnectionFailedParametersMayBeWrong").'<br><br>';
			print $langs->trans("ErrorGoBackAndCorrectParameters");
			$error++;
...
if (!$error && $db->connected) {
	...
	print '<input type="hidden" name="powererp_main_db_character_set" value="'.$defaultCharacterSet.'">';
	print '<input type="hidden" name="powererp_main_db_collation" value="'.$defaultDBSortingCollation.'">';
	$db_character_set = $defaultCharacterSet;
	$db_collation = $defaultDBSortingCollation;
	powererp_install_syslog("step1: db_character_set=".$db_character_set." db_collation=".$db_collation);
}
...
if (!$error && $db->connected && $action == "set") {
	...
	if (is_array($_POST)) {
		foreach ($_POST as $key => $value) {
			if (!preg_match('/^db_pass/i', $key)) {
				powererp_install_syslog("step1: choice for ".$key." = ".$value);
			}
		}
	}
	...
	if (!$error) {
		if (!is_dir($main_dir)) {
			powererp_install_syslog("step1: directory '".$main_dir."' is unavailable or can't be accessed");
	...
	if (!$error) {
		powererp_install_syslog("step1: directory '".$main_dir."' exists");
	}
	...
	if (!$error) {
		...
		if (!is_dir($main_data_dir)) {
			...
			if (!file_exists($pathhtaccess)) {
				powererp_install_syslog("step1: .htaccess file did not exist, we created it in '".$main_data_dir."'");
				...
				if ($handlehtaccess) {
					...
					powererp_install_syslog("step1: .htaccess file created");
				}
			}
			...
			for ($i = 0; $i < $num; $i++) {
				if (is_dir($dir[$i])) {
					powererp_install_syslog("step1: directory '".$dir[$i]."' exists");
				} else {
					if (dol_mkdir($dir[$i]) < 0) {
						...
						powererp_install_syslog("step1: directory '".$dir[$i]."' created");
					}
				}
			}
	...
	if (!$error) {
		...
		conf($powererp_main_document_root);
		...
		if (isset($db_create_user) && ($db_create_user == "1" || $db_create_user == "on")) {
			powererp_install_syslog("step1: create database user: ".$powererp_main_db_user);
			...
			if (!$error) {
				if ($db->connected) {
					...
					if (empty($powererp_main_db_pass)) {
						powererp_install_syslog("step1: failed to create user, password is empty", LOG_ERR);
						print '<tr><td>';
						print $langs->trans("UserCreation").' : ';
						print $powererp_main_db_user;
						print '</td>';
						print '<td>'.$langs->trans("Error").": A password for database user is mandatory.</td></tr>";
					} else {
						// Create user
						$result = $db->DDLCreateUser($powererp_main_db_host, $powererp_main_db_user, $powererp_main_db_pass, $powererp_main_db_name);

						// Create user bis
						if ($databasefortest == 'mysql') {
							if (!in_array($powererp_main_db_host, array('127.0.0.1', '::1', 'localhost', 'localhost.local'))) {
								$resultbis = $db->DDLCreateUser('%', $powererp_main_db_user, $powererp_main_db_pass, $powererp_main_db_name);
							}
						}
						...
						if ($result > 0 && $resultbis > 0) {
							...
							|| $db->errno() == 'DB_ERROR_USER_ALREADY_EXISTS') {
								powererp_install_syslog("step1: user already exists");
								print '<tr><td>';
								print $langs->trans("UserCreation").' : ';
								print $powererp_main_db_user;
								print '</td>';
								...
								powererp_install_syslog("step1: failed to create user", LOG_ERR);
								print '<tr><td>';
								print $langs->trans("UserCreation").' : ';
								print $powererp_main_db_user;
								print '</td>';
					...
					print $langs->trans("YouAskDatabaseCreationSoPowerERPNeedToConnect", $powererp_main_db_user, $powererp_main_db_host, $userroot);
					print '<br>';
					print $langs->trans("BecauseConnectionFailedParametersMayBeWrong").'<br><br>';
					print $langs->trans("ErrorGoBackAndCorrectParameters").'<br><br>';
					print '</td></tr>';
		...
		if (!$error && (isset($db_create_database) && ($db_create_database == "1" || $db_create_database == "on"))) {
			powererp_install_syslog("step1: create database: ".$powererp_main_db_name." ".$powererp_main_db_character_set." ".$powererp_main_db_collation." ".$powererp_main_db_user);
			...
			if ($newdb->connected) {
				$result = $newdb->DDLCreateDb($powererp_main_db_name, $powererp_main_db_character_set, $powererp_main_db_collation, $powererp_main_db_user);

				if ($result) {
					print '<tr><td>';
					print $langs->trans("DatabaseCreation")." (".$langs->trans("User")." ".$userroot.") : ";
					print $powererp_main_db_name;
					print '</td>';
					print '<td><img src="../theme/eldy/img/tick.png" alt="Ok"></td></tr>';

					$newdb->select_db($powererp_main_db_name);
					$check1 = $newdb->getDefaultCharacterSetDatabase();
					$check2 = $newdb->getDefaultCollationDatabase();
					powererp_install_syslog('step1: new database is using charset='.$check1.' collation='.$check2);

					// If values differs, we save conf file again
					//if ($check1 != $powererp_main_db_character_set) powererp_install_syslog('step1: value for character_set is not the one asked for database creation', LOG_WARNING);
					//if ($check2 != $powererp_main_db_collation)     powererp_install_syslog('step1: value for collation is not the one asked for database creation', LOG_WARNING);
				} else {
					// warning message
					print '<tr><td colspan="2"><br>';
					print $langs->trans("ErrorFailedToCreateDatabase", $powererp_main_db_name).'<br>';
					print $newdb->lasterror().'<br>';
					print $langs->trans("IfDatabaseExistsGoBackAndCheckCreate");
					print '<br>';
					print '</td></tr>';

					powererp_install_syslog('step1: failed to create database '.$powererp_main_db_name.' '.$newdb->lasterrno().' '.$newdb->lasterror(), LOG_ERR);
					$error++;
				}
				$newdb->close();
			} else {
		...
		// We test access with powererp database user (not admin)
		if (!$error) {
			powererp_install_syslog("step1: connection type=".$conf->db->type." on host=".$conf->db->host." port=".$conf->db->port." user=".$conf->db->user." name=".$conf->db->name);
			...
			if ($db->connected) {
				powererp_install_syslog("step1: connection to server by user ".$conf->db->user." ok");
				print "<tr><td>";
				print $langs->trans("ServerConnection")." (".$langs->trans("User")." ".$conf->db->user.") : ";
				print $powererp_main_db_host;
				print "</td><td>";
				...
				if ($db->database_selected) {
					powererp_install_syslog("step1: connection to database ".$conf->db->name." by user ".$conf->db->user." ok");
					print "<tr><td>";
					...
					powererp_install_syslog("step1: connection to database ".$conf->db->name." by user ".$conf->db->user." failed", LOG_ERR);
					print "<tr><td>";
					print $langs->trans("DatabaseConnection")." (".$langs->trans("User")." ".$conf->db->user.") : ";
					print $powererp_main_db_name;
					print '</td><td>';
					...
					// warning message
					print '<tr><td colspan="2"><br>';
					print $langs->trans('CheckThatDatabasenameIsCorrect', $powererp_main_db_name).'<br>';
				...
				powererp_install_syslog("step1: connection to server by user ".$conf->db->user." failed", LOG_ERR);
				print "<tr><td>";
				print $langs->trans("ServerConnection")." (".$langs->trans("User")." ".$conf->db->user.") : ";
				print $powererp_main_db_host;
				print '</td><td>';
...
powererp_install_syslog("--- step1: end");
...
function write_conf_file($conffile)
{
	...
	global $powererp_main_url_root, $powererp_main_document_root, $powererp_main_data_root, $powererp_main_db_host;
	global $powererp_main_db_port, $powererp_main_db_name, $powererp_main_db_user, $powererp_main_db_pass;
	global $powererp_main_db_type, $powererp_main_db_character_set, $powererp_main_db_collation, $powererp_main_authentication;
	global $powererp_main_distrib;
	...
	global $force_powererp_lib_ADODB_PATH, $force_powererp_lib_NUSOAP_PATH;
	global $force_powererp_lib_TCPDF_PATH, $force_powererp_lib_FPDI_PATH;
	global $force_powererp_lib_GEOIP_PATH;
	global $force_powererp_lib_ODTPHP_PATH, $force_powererp_lib_ODTPHP_PATHTOPCLZIP;
	global $force_powererp_js_CKEDITOR, $force_powererp_js_JQUERY, $force_powererp_js_JQUERY_UI;
	global $force_powererp_font_DOL_DEFAULT_TTF, $force_powererp_font_DOL_DEFAULT_TTF_BOLD;
	...
	if ($fp) {
		...
		fputs($fp, '<?php'."\n");
		fputs($fp, '//'."\n");
		fputs($fp, '// File generated by PowerERP installer '.DOL_VERSION.' on '.dol_print_date(dol_now(), '')."\n");
		fputs($fp, '//'."\n");
		...
		fputs($fp, '$powererp_main_url_root=\''.str_replace("'", "\'", trim($main_url)).'\';');
		fputs($fp, "\n");

		fputs($fp, '$powererp_main_document_root=\''.str_replace("'", "\'", trim($main_dir)).'\';');
		fputs($fp, "\n");

		fputs($fp, $main_use_alt_dir.'$powererp_main_url_root_alt=\''.str_replace("'", "\'", trim("/".$main_alt_dir_name)).'\';');
		fputs($fp, "\n");

		fputs($fp, $main_use_alt_dir.'$powererp_main_document_root_alt=\''.str_replace("'", "\'", trim($main_dir."/".$main_alt_dir_name)).'\';');
		fputs($fp, "\n");

		fputs($fp, '$powererp_main_data_root=\''.str_replace("'", "\'", trim($main_data_dir)).'\';');
		fputs($fp, "\n");

		fputs($fp, '$powererp_main_db_host=\''.str_replace("'", "\'", trim($db_host)).'\';');
		fputs($fp, "\n");

		fputs($fp, '$powererp_main_db_port=\''.str_replace("'", "\'", trim($db_port)).'\';');
		fputs($fp, "\n");

		fputs($fp, '$powererp_main_db_name=\''.str_replace("'", "\'", trim($db_name)).'\';');
		fputs($fp, "\n");

		fputs($fp, '$powererp_main_db_prefix=\''.str_replace("'", "\'", trim($main_db_prefix)).'\';');
		fputs($fp, "\n");

		fputs($fp, '$powererp_main_db_user=\''.str_replace("'", "\'", trim($db_user)).'\';');
		fputs($fp, "\n");
		fputs($fp, '$powererp_main_db_pass=\''.str_replace("'", "\'", trim($db_pass)).'\';');
		fputs($fp, "\n");

		fputs($fp, '$powererp_main_db_type=\''.str_replace("'", "\'", trim($db_type)).'\';');
		fputs($fp, "\n");

		fputs($fp, '$powererp_main_db_character_set=\''.str_replace("'", "\'", trim($db_character_set)).'\';');
		fputs($fp, "\n");

		fputs($fp, '$powererp_main_db_collation=\''.str_replace("'", "\'", trim($db_collation)).'\';');
		fputs($fp, "\n");
		...
		fputs($fp, '$powererp_main_authentication=\'powererp\';');
		fputs($fp, "\n\n");

		fputs($fp, '//$powererp_main_demo=\'autologin,autopass\';');
		fputs($fp, "\n");

		fputs($fp, '// Security settings');
		fputs($fp, "\n");

		fputs($fp, '$powererp_main_prod=\'0\';');
		fputs($fp, "\n");

		fputs($fp, '$powererp_main_force_https=\''.$main_force_https.'\';');
		fputs($fp, "\n");

		fputs($fp, '$powererp_main_restrict_os_commands=\'mysqldump, mysql, pg_dump, pgrestore\';');
		fputs($fp, "\n");

		fputs($fp, '$powererp_nocsrfcheck=\'0\';');
		fputs($fp, "\n");

		fputs($fp, '$powererp_main_instance_unique_id=\''.$key.'\';');
		fputs($fp, "\n");

		fputs($fp, '$powererp_mailing_limit_sendbyweb=\'0\';');
		fputs($fp, "\n");
		fputs($fp, '$powererp_mailing_limit_sendbycli=\'0\';');
		fputs($fp, "\n");

		// Write params to overwrites default lib path
		fputs($fp, "\n");
		if (empty($force_powererp_lib_FPDF_PATH)) {
			fputs($fp, '//'); $force_powererp_lib_FPDF_PATH = '';
		}
		fputs($fp, '$powererp_lib_FPDF_PATH=\''.$force_powererp_lib_FPDF_PATH.'\';');
		fputs($fp, "\n");
		if (empty($force_powererp_lib_TCPDF_PATH)) {
			fputs($fp, '//'); $force_powererp_lib_TCPDF_PATH = '';
		}
		fputs($fp, '$powererp_lib_TCPDF_PATH=\''.$force_powererp_lib_TCPDF_PATH.'\';');
		fputs($fp, "\n");
		if (empty($force_powererp_lib_FPDI_PATH)) {
			fputs($fp, '//'); $force_powererp_lib_FPDI_PATH = '';
		}
		fputs($fp, '$powererp_lib_FPDI_PATH=\''.$force_powererp_lib_FPDI_PATH.'\';');
		fputs($fp, "\n");
		if (empty($force_powererp_lib_TCPDI_PATH)) {
			fputs($fp, '//'); $force_powererp_lib_TCPDI_PATH = '';
		}
		fputs($fp, '$powererp_lib_TCPDI_PATH=\''.$force_powererp_lib_TCPDI_PATH.'\';');
		fputs($fp, "\n");
		if (empty($force_powererp_lib_ADODB_PATH)) {
			fputs($fp, '//'); $force_powererp_lib_ADODB_PATH = '';
		}
		fputs($fp, '$powererp_lib_ADODB_PATH=\''.$force_powererp_lib_ADODB_PATH.'\';');
		fputs($fp, "\n");
		if (empty($force_powererp_lib_GEOIP_PATH)) {
			fputs($fp, '//'); $force_powererp_lib_GEOIP_PATH = '';
		}
		fputs($fp, '$powererp_lib_GEOIP_PATH=\''.$force_powererp_lib_GEOIP_PATH.'\';');
		fputs($fp, "\n");
		if (empty($force_powererp_lib_NUSOAP_PATH)) {
			fputs($fp, '//'); $force_powererp_lib_NUSOAP_PATH = '';
		}
		fputs($fp, '$powererp_lib_NUSOAP_PATH=\''.$force_powererp_lib_NUSOAP_PATH.'\';');
		fputs($fp, "\n");
		if (empty($force_powererp_lib_ODTPHP_PATH)) {
			fputs($fp, '//'); $force_powererp_lib_ODTPHP_PATH = '';
		}
		fputs($fp, '$powererp_lib_ODTPHP_PATH=\''.$force_powererp_lib_ODTPHP_PATH.'\';');
		fputs($fp, "\n");
		if (empty($force_powererp_lib_ODTPHP_PATHTOPCLZIP)) {
			fputs($fp, '//'); $force_powererp_lib_ODTPHP_PATHTOPCLZIP = '';
		}
		fputs($fp, '$powererp_lib_ODTPHP_PATHTOPCLZIP=\''.$force_powererp_lib_ODTPHP_PATHTOPCLZIP.'\';');
		fputs($fp, "\n");
		if (empty($force_powererp_js_CKEDITOR)) {
			fputs($fp, '//'); $force_powererp_js_CKEDITOR = '';
		}
		fputs($fp, '$powererp_js_CKEDITOR=\''.$force_powererp_js_CKEDITOR.'\';');
		fputs($fp, "\n");
		if (empty($force_powererp_js_JQUERY)) {
			fputs($fp, '//'); $force_powererp_js_JQUERY = '';
		}
		fputs($fp, '$powererp_js_JQUERY=\''.$force_powererp_js_JQUERY.'\';');
		fputs($fp, "\n");
		if (empty($force_powererp_js_JQUERY_UI)) {
			fputs($fp, '//'); $force_powererp_js_JQUERY_UI = '';
		}
		fputs($fp, '$powererp_js_JQUERY_UI=\''.$force_powererp_js_JQUERY_UI.'\';');
		fputs($fp, "\n");

		// Write params to overwrites default font path
		fputs($fp, "\n");
		if (empty($force_powererp_font_DOL_DEFAULT_TTF)) {
			fputs($fp, '//'); $force_powererp_font_DOL_DEFAULT_TTF = '';
		}
		fputs($fp, '$powererp_font_DOL_DEFAULT_TTF=\''.$force_powererp_font_DOL_DEFAULT_TTF.'\';');
		fputs($fp, "\n");
		if (empty($force_powererp_font_DOL_DEFAULT_TTF_BOLD)) {
			fputs($fp, '//'); $force_powererp_font_DOL_DEFAULT_TTF_BOLD = '';
		}
		fputs($fp, '$powererp_font_DOL_DEFAULT_TTF_BOLD=\''.$force_powererp_font_DOL_DEFAULT_TTF_BOLD.'\';');
		fputs($fp, "\n");

		// Other
		fputs($fp, '$powererp_main_distrib=\''.str_replace("'", "\'", trim($powererp_main_distrib)).'\';');
		fputs($fp, "\n");
