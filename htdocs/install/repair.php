...
include_once $powererp_main_document_root.'/core/lib/images.lib.php';
require_once $powererp_main_document_root.'/core/class/extrafields.class.php';
...
powererp_install_syslog("--- repair: entering upgrade.php page");
if (!is_object($conf)) {
	powererp_install_syslog("repair: conf file not initialized", LOG_ERR);
}
...
// For encryption
$conf->db->powererp_main_db_encryption = isset($powererp_main_db_encryption) ? $powererp_main_db_encryption : '';
$conf->db->powererp_main_db_cryptkey = isset($powererp_main_db_cryptkey) ? $powererp_main_db_cryptkey : '';

$db = getDoliDBInstance($conf->db->type, $conf->db->host, $conf->db->user, $conf->db->pass, $conf->db->name, $conf->db->port);

if ($db->connected) {
	print '<tr><td class="nowrap">';
	print $langs->trans("ServerConnection")." : $powererp_main_db_host</td><td class=\"right\">".$langs->trans("OK")."</td></tr>";
	powererp_install_syslog("repair: ".$langs->transnoentities("ServerConnection").": ".$powererp_main_db_host.$langs->transnoentities("OK"));
	$ok = 1;
} else {
	print "<tr><td>".$langs->trans("ErrorFailedToConnectToDatabase", $powererp_main_db_name)."</td><td class=\"right\">".$langs->transnoentities("Error")."</td></tr>";
	powererp_install_syslog("repair: ".$langs->transnoentities("ErrorFailedToConnectToDatabase", $powererp_main_db_name));
	$ok = 0;
}
...
if ($ok) {
	if ($db->database_selected) {
		print '<tr><td class="nowrap">';
		print $langs->trans("DatabaseConnection")." : ".$powererp_main_db_name."</td><td class=\"right\">".$langs->trans("OK")."</td></tr>";
		powererp_install_syslog("repair: database connection successful: ".$powererp_main_db_name);
		$ok = 1;
	} else {
		print "<tr><td>".$langs->trans("ErrorFailedToConnectToDatabase", $powererp_main_db_name)."</td><td class=\"right\">".$langs->trans("Error")."</td></tr>";
		powererp_install_syslog("repair: ".$langs->transnoentities("ErrorFailedToConnectToDatabase", $powererp_main_db_name));
		$ok = 0;
	}
}
...
if ($ok) {
	...
	powererp_install_syslog("repair: ".$langs->transnoentities("ServerVersion").": ".$version);
	...
	if ($resql) {
		...
		while ($i < $num) {
			...
			*/
			...
			if (!empty($name)) {
				$filetotest = $powererp_main_data_root.'/societe/logos/'.$name.$ext;
				$filetotestsmall = $powererp_main_data_root.'/societe/logos/thumbs/'.$name.'_small'.$ext;
				...
				if ($exists) {
					$filetarget = $powererp_main_data_root.'/societe/'.$obj->rowid.'/logos/'.$name.$ext;
					$filetargetsmall = $powererp_main_data_root.'/societe/'.$obj->rowid.'/logos/thumbs/'.$name.'_small'.$ext;
					$existt = dol_is_file($filetarget);
					if (!$existt) {
						if (GETPOST('restore_thirdparties_logos', 'alpha') == 'confirmed') {
							dol_mkdir($powererp_main_data_root.'/societe/'.$obj->rowid.'/logos');
						}
					...
					if (!$existtt) {
						if (GETPOST('restore_thirdparties_logos', 'alpha') == 'confirmed') {
							dol_mkdir($powererp_main_data_root.'/societe/'.$obj->rowid.'/logos/thumbs');
						}
...
if ($ok && GETPOST('restore_user_pictures', 'alpha')) {
	...
	if ($resql) {
		...
		while ($i < $num) {
			...
			if (!empty($name)) {
				$filetotest = $powererp_main_data_root.'/users/'.substr(sprintf('%08d', $obj->rowid), -1, 1).'/'.substr(sprintf('%08d', $obj->rowid), -2, 1).'/'.$name.$ext;
				$filetotestsmall = $powererp_main_data_root.'/users/'.substr(sprintf('%08d', $obj->rowid), -1, 1).'/'.substr(sprintf('%08d', $obj->rowid), -2, 1).'/thumbs/'.$name.'_small'.$ext;
				$filetotestmini = $powererp_main_data_root.'/users/'.substr(sprintf('%08d', $obj->rowid), -1, 1).'/'.substr(sprintf('%08d', $obj->rowid), -2, 1).'/thumbs/'.$name.'_mini'.$ext;
				$exists = dol_is_file($filetotest);
				...
				if ($exists) {
					$filetarget = $powererp_main_data_root.'/users/'.$obj->rowid.'/'.$name.$ext;
					$filetargetsmall = $powererp_main_data_root.'/users/'.$obj->rowid.'/thumbs/'.$name.'_small'.$ext;
					$filetargetmini = $powererp_main_data_root.'/users/'.$obj->rowid.'/thumbs/'.$name.'_mini'.$ext;

					$existt = dol_is_file($filetarget);
					if (!$existt) {
						if (GETPOST('restore_user_pictures', 'alpha') == 'confirmed') {
							dol_mkdir($powererp_main_data_root.'/users/'.$obj->rowid);
						}
					...
					if (!$existtt) {
						if (GETPOST('restore_user_pictures', 'alpha') == 'confirmed') {
							dol_mkdir($powererp_main_data_root.'/users/'.$obj->rowid.'/thumbs');
						}
...
if ($ok && GETPOST('rebuild_product_thumbs', 'alpha')) {
	...
	if ($resql) {
		...
		while ($i < $num) {
			...
			if (!empty($obj->ref)) {
				$files = dol_dir_list($powererp_main_data_root.'/produit/'.$obj->ref, 'files', 0);
...
if ($ok && GETPOST('force_utf8_on_tables', 'alpha')) {
	...
	if ($db->type == "mysql" || $db->type == "mysqli") {
		...
		foreach ($listoftables as $table) {
			...
			if ($conf->db->powererp_main_db_encryption != 0 && preg_match('/\_const$/', $table)) {
				continue;
			}
...
if ($ok && GETPOST('repair_link_dispatch_lines_supplier_order_lines')) {
	/*
	 * This script is meant to be run when upgrading from a powererp version < 3.8
	 * to a newer version.
	 *
	 ...
	 * old supplier orders whose lines were dispatched using the old powererp version
...
if ($oneoptionset) {
	...
	print $langs->trans("GoToPowererp");
	print '</a></div>';
} else {
...
powererp_install_syslog("--- repair: end");
pFooter(1, $setuplang);
