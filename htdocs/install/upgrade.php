...
powererp_install_syslog("--- upgrade: Entering upgrade.php page");
if (!is_object($conf)) {
...
if (!GETPOST('action', 'aZ09') || preg_match('/upgrade/i', GETPOST('action', 'aZ09'))) {
	...
	if ($db->connected) {
		...
		print $langs->trans("ServerConnection")." : ".$powererp_main_db_host.'</td><td class="right"><span class="neutral">'.$langs->trans("OK").'</span></td></tr>'."\n";
		powererp_install_syslog("upgrade: ".$langs->transnoentities("ServerConnection").": $powererp_main_db_host ".$langs->transnoentities("OK"));
		$ok = 1;
	} else {
		print "<tr><td>".$langs->trans("ErrorFailedToConnectToDatabase", $powererp_main_db_name).'</td><td class="right"><span class="error">'.$langs->transnoentities("Error")."</span></td></tr>\n";
		powererp_install_syslog("upgrade: ".$langs->transnoentities("ErrorFailedToConnectToDatabase", $powererp_main_db_name));
		$ok = 0;
	}
	...
	if ($ok) {
		if ($db->database_selected) {
			print '<tr><td class="nowrap">';
			print $langs->trans("DatabaseConnection")." : ".$powererp_main_db_name.'</td><td class="right"><span class="neutral">'.$langs->trans("OK")."</span></td></tr>\n";
			powererp_install_syslog("upgrade: Database connection successful: ".$powererp_main_db_name);
			$ok = 1;
		} else {
			print "<tr><td>".$langs->trans("ErrorFailedToConnectToDatabase", $powererp_main_db_name).'</td><td class="right"><span class="ok">'.$langs->trans("Error")."</span></td></tr>\n";
			powererp_install_syslog("upgrade: ".$langs->transnoentities("ErrorFailedToConnectToDatabase", $powererp_main_db_name));
			$ok = 0;
		}
	}
	...
	if ($ok) {
		...
		powererp_install_syslog("upgrade: ".$langs->transnoentities("ServerVersion").": ".$version);
		if ($db->type == 'mysqli' && function_exists('mysqli_get_charset')) {
			...
			powererp_install_syslog("upgrade: ".$langs->transnoentities("ClientCharset").": ".$tmparray->charset);
			print '<tr><td>'.$langs->trans("ClientSortingCharset").'</td>';
			print '<td class="right">'.$tmparray->collation.'</td></tr>';
			powererp_install_syslog("upgrade: ".$langs->transnoentities("ClientCollation").": ".$tmparray->collation);
		}
		...
		if (count($versionmindb) && count($versionarray)
			...
			powererp_install_syslog("upgrade: ".$langs->transnoentities("ErrorDatabaseVersionTooLow", join('.', $versionarray), join('.', $versionmindb)));
			$ok = 0;
		}
		...
		if (empty($ignoredbversion)) {
			...
			foreach ($dbversion_disallowed as $dbversion_totest) {
				...
				if ($dbversion_totest['type'] == $db->type
					...
					powererp_install_syslog("upgrade: ".$langs->transnoentities("ErrorDatabaseVersionForbiddenForMigration", join('.', $versionarray), $listofforbiddenversion));
					$ok = 0;
					break;
				}
			}
	...
	if ($ok && preg_match('/mysql/', $db->type)) {
		...
		&& versioncompare($versioncommande, $versionarray) <= 0) {	// Si mysql >= 4.0
			powererp_install_syslog("Clean database from bad named constraints");
	...
	if ($ok) {
		...
		powererp_install_syslog("Scan sql files for migration files in ".$dir);
		...
		if (count($filelist) == 0) {
			...
			foreach ($filelist as $file) {
				...
				// for modules htdocs/module/sql or htdocs/custom/module/sql (files called "powererp_x.y.z-a.b.c.sql" or "powererp_always.sql")
				$modulesfile = array();
					...
					if (is_resource($handlemodule)) {
						while (($filemodule = readdir($handlemodule)) !== false) {
							if (!preg_match('/\./', $filemodule) && is_dir($dirroot.'/'.$filemodule.'/sql')) {	// We exclude filemodule that contains . (are not directories) and are not directories.
								...
								if (is_file($dirroot.'/'.$filemodule.'/sql/powererp_'.$file)) {
									$modulesfile[$dirroot.'/'.$filemodule.'/sql/powererp_'.$file] = '/'.$filemodule.'/sql/powererp_'.$file;
								}
								if (is_file($dirroot.'/'.$filemodule.'/sql/powererp_allversions.sql')) {
									$modulesfile[$dirroot.'/'.$filemodule.'/sql/powererp_allversions.sql'] = '/'.$filemodule.'/sql/powererp_allversions.sql';
								}
							}
						}
...
powererp_install_syslog("--- upgrade: end ".((!$ok && empty($_GET["ignoreerrors"])) || $dirmodule));
