...
if (is_readable($conffile) && filesize($conffile) > 8) {
	powererp_install_syslog("check: conf file '".$conffile."' already defined");
	$confexists = 1;
	...
	// If not, we create it
	powererp_install_syslog("check: we try to create conf file '".$conffile."'");
	$confexists = 0;

	// First we try by copying example
	if (@copy($conffile.".example", $conffile)) {
		// Success
		powererp_install_syslog("check: successfully copied file ".$conffile.".example into ".$conffile);
	} else {
		// If failed, we try to create an empty file
		powererp_install_syslog("check: failed to copy file ".$conffile.".example into ".$conffile.". We try to create it.", LOG_WARNING);
		...
		if ($fp) {
			...
			powererp_install_syslog("check: failed to create a new file ".$conffile." into current dir ".getcwd().". Please check permissions.", LOG_ERR);
		}
	}
...
if (!file_exists($conffile)) {
	...
	if ($checksok) {
		...
		if (file_exists($conffile)) {
			include_once $conffile;
			if (!empty($powererp_main_db_type) && !empty($powererp_main_document_root)) {
				if (!file_exists($powererp_main_document_root."/core/lib/admin.lib.php")) {
					print '<span class="error">A '.$conffiletoshow.' file exists with a powererp_main_document_root to '.$powererp_main_document_root.' that seems wrong. Try to fix or remove the '.$conffiletoshow.' file.</span><br>'."\n";
					dol_syslog("A '".$conffiletoshow."' file exists with a powererp_main_document_root to ".$powererp_main_document_root." that seems wrong. Try to fix or remove the '".$conffiletoshow."' file.", LOG_WARNING);
				} else {
					...
					if (preg_match('/crypted:/i', $powererp_main_db_pass) || !empty($powererp_main_db_encrypted_pass)) {
						...
						if (preg_match('/crypted:/i', $powererp_main_db_pass)) {
							$powererp_main_db_encrypted_pass = preg_replace('/crypted:/i', '', $powererp_main_db_pass); // We need to set this as it is used to know the password was initially crypted
		...
		if ($ok) {
			...
			$powererpversiontoinstallarray = versionpowererparray();
		}
		...
		if (empty($powererp_main_db_host)) {	// This means install process was not run
		...
		foreach ($migrationscript as $migarray) {
			...
			$powererpversionfromarray = preg_split('/[\.-]/', $versionfrom);
			$powererpversiontoarray = preg_split('/[\.-]/', $versionto);
			...
			$newversionfrombis = '';
			if (versioncompare($powererpversiontoarray, $versionarray) < -2) {	// From x.y.z -> x.y.z+1
				$newversionfrombis = ' '.$langs->trans("or").' '.$versionto;
			}

			if ($ok) {
				if (count($powererplastupgradeversionarray) >= 2) {	// If database access is available and last upgrade version is known
					// Now we check if this is the first qualified choice
					if ($allowupgrade && empty($foundrecommandedchoice) &&
						(versioncompare($powererpversiontoarray, $powererplastupgradeversionarray) > 0 || versioncompare($powererpversiontoarray, $versionarray) < -2)
					) {
						$foundrecommandedchoice = 1; // To show only once
...
powererp_install_syslog("- check: end");
