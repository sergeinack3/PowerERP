...
require_once $powererp_main_document_root.'/comm/propal/class/propal.class.php';
require_once $powererp_main_document_root.'/contrat/class/contrat.class.php';
require_once $powererp_main_document_root.'/commande/class/commande.class.php';
require_once $powererp_main_document_root.'/fourn/class/fournisseur.commande.class.php';
require_once $powererp_main_document_root.'/core/lib/price.lib.php';
require_once $powererp_main_document_root.'/core/class/menubase.class.php';
require_once $powererp_main_document_root.'/core/lib/files.lib.php';
...
powererp_install_syslog("--- upgrade2: entering upgrade2.php page ".$versionfrom." ".$versionto." ".$enablemodules);
if (!is_object($conf)) {
	powererp_install_syslog("upgrade2: conf file not initialized", LOG_ERR);
}
...
if (!GETPOST('action', 'aZ09') || preg_match('/upgrade/i', GETPOST('action', 'aZ09'))) {
	...
	if ((!empty($powererp_main_db_pass) && preg_match('/crypted:/i', $powererp_main_db_pass)) || !empty($powererp_main_db_encrypted_pass)) {
		require_once $powererp_main_document_root.'/core/lib/security.lib.php';
		if (!empty($powererp_main_db_pass) && preg_match('/crypted:/i', $powererp_main_db_pass)) {
			$powererp_main_db_pass = preg_replace('/crypted:/i', '', $powererp_main_db_pass);
			$powererp_main_db_pass = dol_decode($powererp_main_db_pass);
			$powererp_main_db_encrypted_pass = $powererp_main_db_pass; // We need to set this as it is used to know the password was initially crypted
		} else {
			$powererp_main_db_pass = dol_decode($powererp_main_db_encrypted_pass);
		}
	}

	// $conf is already instancied inside inc.php
	$conf->db->type = $powererp_main_db_type;
	$conf->db->host = $powererp_main_db_host;
	$conf->db->port = $powererp_main_db_port;
	$conf->db->name = $powererp_main_db_name;
	$conf->db->user = $powererp_main_db_user;
	$conf->db->pass = $powererp_main_db_pass;
	...
	if (!$db->connected) {
		...
		powererp_install_syslog('upgrade2: failed to connect to database :'.$conf->db->name.' on '.$conf->db->host.' for user '.$conf->db->user, LOG_ERR);
		$error++;
	}

	if (!$error) {
		if ($db->database_selected) {
			powererp_install_syslog('upgrade2: database connection successful :'.$powererp_main_db_name);
		} else {
			$error++;
		}
	}
	...
	foreach ($listofentities as $entity) {
		...
		if (!$error) {
			...
			$powererplastupgradeversionarray = preg_split('/[\.-]/', isset($conf->global->MAIN_VERSION_LAST_UPGRADE) ? $conf->global->MAIN_VERSION_LAST_UPGRADE : (isset($conf->global->MAIN_VERSION_LAST_INSTALL) ? $conf->global->MAIN_VERSION_LAST_INSTALL : ''));
...
powererp_install_syslog("--- upgrade2: end");
...
function migrate_paiements($db, $langs, $conf)
{
	...
	if ($obj) {
		...
		powererp_install_syslog("upgrade2::migrate_paiements");
		if ($resql) {
			$i = 0;
			$row = array();
...
function migrate_paiements_orphelins_1($db, $langs, $conf)
{
	...
	if ($obj) {
		...
		powererp_install_syslog("upgrade2::migrate_paiements_orphelins_1");
		$row = array();
		if ($resql) {
			$i = $j = 0;
			$num = $db->num_rows($resql);
...
function migrate_paiements_orphelins_2($db, $langs, $conf)
{
	...
	if ($obj) {
		...
		powererp_install_syslog("upgrade2::migrate_paiements_orphelins_2");
		$row = array();
		if ($resql) {
			$i = $j = 0;
			$num = $db->num_rows($resql);
...
function migrate_contracts_det($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_contracts_det");
	if ($resql) {
		$i = 0;
		$row = array();
		$num = $db->num_rows($resql);
...
function migrate_links_transfert($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_links_transfert");
	if ($resql) {
		$i = 0;
		$row = array();
		$num = $db->num_rows($resql);
		...
		if ($num) {
			...
			while ($i < $num) {
				...
				//print $sql.'<br>';
				powererp_install_syslog("migrate_links_transfert");
...
function migrate_contracts_date1($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_contracts_date1");
	$resql = $db->query($sql);
	if (!$resql) {
		dol_print_error($db);
	}
...
function migrate_contracts_date2($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_contracts_date2");
	if ($resql) {
		$i = 0;
		$row = array();
		$num = $db->num_rows($resql);
...
function migrate_contracts_date3($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_contracts_date3");
	$resql = $db->query($sql);
	if (!$resql) {
		dol_print_error($db);
	}
...
function migrate_contracts_open($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_contracts_open");
	$resql = $db->query($sql);
	if (!$resql) {
		dol_print_error($db);
	}
...
function migrate_paiementfourn_facturefourn($db, $langs, $conf)
{
	...
	if ($obj) {
		...
		powererp_install_syslog("upgrade2::migrate_paiementfourn_facturefourn");
...
function migrate_price_facture($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_price_facture");
	$resql = $db->query($sql);
	if ($resql) {
		$num = $db->num_rows($resql);
		$i = 0;
		if ($num) {
			while ($i < $num) {
				...
				powererp_install_syslog("upgrade2: line ".$rowid.": facid=".$obj->facid." pu=".$pu." qty=".$qty." vatrate=".$vatrate." remise_percent=".$remise_percent." remise_global=".$remise_percent_global." -> ".$total_ht.", ".$total_tva.", ".$total_ttc);
				print ". ";
...
function migrate_price_propal($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_price_propal");
	$resql = $db->query($sql);
	if ($resql) {
		$num = $db->num_rows($resql);
		$i = 0;
		if ($num) {
			while ($i < $num) {
				...
				powererp_install_syslog("upgrade2: Line ".$rowid.": propalid=".$obj->rowid." pu=".$pu." qty=".$qty." vatrate=".$vatrate." remise_percent=".$remise_percent." remise_global=".$remise_percent_global." -> ".$total_ht.", ".$total_tva.", ".$total_ttc);
				print ". ";
...
function migrate_price_contrat($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_price_contrat");
	$resql = $db->query($sql);
	if ($resql) {
		$num = $db->num_rows($resql);
		$i = 0;
		if ($num) {
			while ($i < $num) {
				...
				powererp_install_syslog("upgrade2: Line ".$rowid.": contratdetid=".$obj->rowid." pu=".$pu." qty=".$qty." vatrate=".$vatrate." remise_percent=".$remise_percent."  -> ".$total_ht.", ".$total_tva." , ".$total_ttc);
				print ". ";
...
function migrate_price_commande($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_price_commande");
	$resql = $db->query($sql);
	if ($resql) {
		$num = $db->num_rows($resql);
		$i = 0;
		if ($num) {
			while ($i < $num) {
				...
				powererp_install_syslog("upgrade2: Line ".$rowid." : commandeid=".$obj->rowid." pu=".$pu." qty=".$qty." vatrate=".$vatrate." remise_percent=".$remise_percent." remise_global=".$remise_percent_global."  -> ".$total_ht.", ".$total_tva.", ".$total_ttc);
				print ". ";
...
function migrate_price_commande_fournisseur($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_price_commande_fournisseur");
	$resql = $db->query($sql);
	if ($resql) {
		$num = $db->num_rows($resql);
		$i = 0;
...
function migrate_modeles($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_modeles");
...
{
	powererp_install_syslog("upgrade2::migrate_commande_expedition");
...
{
	powererp_install_syslog("upgrade2::migrate_commande_livraison");
...
{
	powererp_install_syslog("upgrade2::migrate_detail_livraison");
...
{
	powererp_install_syslog("upgrade2::migrate_stocks");
...
{
	powererp_install_syslog("upgrade2::migrate_menus");
...
{
	powererp_install_syslog("upgrade2::migrate_commande_deliveryaddress");
...
{
	powererp_install_syslog("upgrade2::migrate_restore_missing_links");

	if (($db->type == 'mysql' || $db->type == 'mysqli')) {
		if (versioncompare($db->getVersionArray(), array(4, 0)) < 0) {
			powererp_install_syslog("upgrade2::migrate_restore_missing_links Version of database too old to make this migrate action");
			return 0;
		}
	}
	print '<tr><td colspan="4">';
	...
	powererp_install_syslog("upgrade2::migrate_restore_missing_links DIRECTION 1");
	$resql = $db->query($sql);
	if ($resql) {
		$i = 0;
		$num = $db->num_rows($resql);
	...
	powererp_install_syslog("upgrade2::migrate_restore_missing_links DIRECTION 2");
	$resql = $db->query($sql);
	if ($resql) {
		$i = 0;
		$num = $db->num_rows($resql);
...
{
	powererp_install_syslog("upgrade2::migrate_project_user_resp");
...
{
	powererp_install_syslog("upgrade2::migrate_project_task_actors");
...
function migrate_relationship_tables($db, $langs, $conf, $table, $fk_source, $sourcetype, $fk_target, $targettype)
{
	...
	if ($db->DDLInfoTable(MAIN_DB_PREFIX.$table)) {
		powererp_install_syslog("upgrade2::migrate_relationship_tables table = ".MAIN_DB_PREFIX.$table);
...
{
	powererp_install_syslog("upgrade2::migrate_project_task_time");
...
function migrate_customerorder_shipping($db, $langs, $conf)
{
	...
	if (!$obj1 && !$obj2) {
		powererp_install_syslog("upgrade2::migrate_customerorder_shipping");
...
function migrate_shipping_delivery($db, $langs, $conf)
{
	...
	$obj = $db->fetch_object($result);
	if ($obj) {
		powererp_install_syslog("upgrade2::migrate_shipping_delivery");
...
function migrate_shipping_delivery2($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_shipping_delivery2");
...
function migrate_actioncomm_element($db, $langs, $conf)
{
	...
	foreach ($elements as $type => $field) {
		...
		$obj = $db->fetch_object($result);
		if ($obj) {
			powererp_install_syslog("upgrade2::migrate_actioncomm_element field=".$field);
...
function migrate_mode_reglement($db, $langs, $conf)
{
	...
	foreach ($elements['old_id'] as $key => $old_id) {
		...
		powererp_install_syslog("upgrade2::migrate_mode_reglement code=".$elements['code'][$key]);
...
function migrate_clean_association($db, $langs, $conf)
{
	...
	if ($result) {	// result defined for version 3.2 or -
		...
		if ($obj) {	// It table categorie_association exists
			...
			powererp_install_syslog("upgrade: search duplicate");
			$resql = $db->query($sql);
			if ($resql) {
				$num = $db->num_rows($resql);
				...
				powererp_install_syslog("upgrade: result is num=".$num." count(couples)=".count($couples));
				...
				if (count($couples) > 0 && $num > count($couples)) {
					...
					powererp_install_syslog("upgrade: delete association");
					$resqld = $db->query($sql);
					if ($resqld) {
						...
						foreach ($couples as $key => $val) {
							...
							powererp_install_syslog("upgrade: insert association");
							$resqli = $db->query($sql);
							if (!$resqli) {
								$error++;
							}
...
function migrate_categorie_association($db, $langs, $conf)
{
	...
	if ($db->DDLInfoTable(MAIN_DB_PREFIX."categorie_association")) {
		powererp_install_syslog("upgrade2::migrate_categorie_association");
...
function migrate_event_assignement($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_event_assignement");
...
function migrate_reset_blocked_log($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_reset_blocked_log");
...
function migrate_remise_entity($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_remise_entity");
...
function migrate_remise_except_entity($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_remise_except_entity");
...
function migrate_user_rights_entity($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_user_rights_entity");
...
function migrate_usergroup_rights_entity($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_usergroup_rights_entity");
...
{
	powererp_install_syslog("upgrade2::migrate_rename_directories");

	if (is_dir(DOL_DATA_ROOT.$oldname) && !file_exists(DOL_DATA_ROOT.$newname)) {
		powererp_install_syslog("upgrade2::migrate_rename_directories move ".DOL_DATA_ROOT.$oldname.' into '.DOL_DATA_ROOT.$newname);
...
function migrate_delete_old_files($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_delete_old_files");
	...
	$filetodeletearray = array(
		...
		'/core/modules/mailings/powererp_services_expired.modules.php',
...
function migrate_delete_old_dir($db, $langs, $conf)
{
	...
	powererp_install_syslog("upgrade2::migrate_delete_old_dir");
...
function migrate_reload_modules($db, $langs, $conf, $listofmodule = array(), $force = 0)
{
	...
	powererp_install_syslog("upgrade2::migrate_reload_modules force=".$force.", listofmodule=".join(',', array_keys($listofmodule)));
	...
	foreach ($listofmodule as $moduletoreload => $reloadmode) {	// reloadmodule can be 'noboxes', 'newboxdefonly', 'forceactivate'
		...
		if ($moduletoreload == 'MAIN_MODULE_AGENDA') {
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Agenda module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Rest API module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Barcode module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate BlockedLog module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Cron module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate ExternalSite module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Societe module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Produit module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Service module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Commande module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Facture module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Fournisseur module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Leave Request module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Deplacement module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Expense Report module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Don module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate ECM module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Knowledge Management");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules EventOrganization");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Paybox module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Supplier Proposal module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Opensurvey module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules ProductBatch module");
			...
			powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate Takepos module");
			...
			if (!empty($reg[1])) {
				...
				powererp_install_syslog("upgrade2::migrate_reload_modules Reactivate module ".$moduletoreloadshort." with mode ".$reloadmode);
				...
				if ($res) {
					...
					powererp_install_syslog('Failed to include '.DOL_DOCUMENT_ROOT.'/core/modules/mod'.$moduletoreloadshort.'.class.php');
					...
					if ($res) {
						...
						powererp_install_syslog('Failed to include '.strtolower($moduletoreloadshort).'/core/modules/mod'.$moduletoreloadshort.'.class.php', LOG_ERR);
						print "Error, can't find module with name ".$moduletoreload."\n";
						return -1;
					}
				}
			} else {
				powererp_install_syslog("Error, can't find module with name ".$moduletoreload, LOG_ERR);
				print "Error, can't find module with name ".$moduletoreload."\n";
				return -1;
			}
		}
...
{
	global $conf;
	powererp_install_syslog("upgrade2::migrate_reload_menu");
