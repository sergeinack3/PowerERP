...
pHeader($langs->trans("ConfigurationFile"), "step1", "set", "", (empty($force_powererp_js_JQUERY) ? '' : $force_powererp_js_JQUERY.'/'), 'main-inside-bis');

// Test if we can run a first install process
if (!is_writable($conffile)) {
	print $langs->trans("ConfFileIsNotWritable", $conffiletoshow);
	powererp_install_syslog("fileconf: config file is not writable", LOG_WARNING);
	powererp_install_syslog("- fileconf: end");
...
<table class="nobordernopadding<?php if ($force_install_noedit) {
	...
	<!-- Documents root $powererp_main_document_root -->
	<tr>
...
if (empty($powererp_main_document_root)) {
	$powererp_main_document_root = GETPOSTISSET('main_dir') ? GETPOST('main_dir') : detect_powererp_main_document_root();
}
?>
		<td class="label">
			<input type="text"
				   ...
				   id="main_dir"
				   name="main_dir"
				   value="<?php print $powererp_main_document_root ?>"
<?php
...
?>
	...
	<!-- Documents URL $powererp_main_data_root -->
	<tr>
		<td class="label"><label for="main_data_dir"><b><?php print $langs->trans("DocumentsDirectory"); ?></b></label></td>
		<?php
		if (!empty($force_install_main_data_root)) {
			$powererp_main_data_root = @$force_install_main_data_root;
		}
		if (empty($powererp_main_data_root)) {
			$powererp_main_data_root = GETPOSTISSET('main_data_dir') ? GETPOST('main_data_dir') : detect_powererp_main_data_root($powererp_main_document_root);
		}
		?>
		<td class="label">
			<input type="text"
				   ...
				   id="main_data_dir"
				   name="main_data_dir"
				   value="<?php print $powererp_main_data_root ?>"
<?php if (!empty($force_install_noedit)) {
	...
	<!-- Root URL $powererp_main_url_root -->
	<?php
	if (empty($powererp_main_url_root)) {
		$powererp_main_url_root = GETPOSTISSET('main_url') ? GETPOST('main_url') : detect_powererp_main_url_root();
	}
	?>
	<tr>
		...
		<td class="label">
			<input type="text"
				   ...
				   id="main_url"
				   name="main_url"
				   value="<?php print $powererp_main_url_root; ?> "
<?php if (!empty($force_install_noedit)) {
	print ' disabled';
}
?>
	...
	<tr>
		...
		<h3><img class="valignmiddle inline-block paddingright" src="../theme/common/octicons/build/svg/database.svg" width="20" alt="webserver"> <?php echo $langs->trans("PowerERPDatabase"); ?></h3>
		</td>
	</tr>
	...
	<?php
	if (!isset($powererp_main_db_host)) {
	...
	<tr>
		...
		$defaultype = !empty($powererp_main_db_type) ? $powererp_main_db_type : (empty($force_install_type) ? 'mysqli' : $force_install_type);
	...
	<tr class="hidesqlite">
		...
		<td class="label">
			<input type="text"
				   id="db_host"
				   name="db_host"
				   value="<?php print (!empty($force_install_dbserver) ? $force_install_dbserver : (!empty($powererp_main_db_host) ? $powererp_main_db_host : 'localhost')); ?>"
	...
	<tr class="hidesqlite">
		...
		<td class="label">
			<input type="text"
				   name="db_port"
				   id="db_port"
				   value="<?php print (!empty($force_install_port)) ? $force_install_port : $powererp_main_db_port; ?>"
	...
	<tr class="hidesqlite">
		...
		<td class="label">
			<input type="text"
				   id="db_prefix"
				   name="db_prefix"
				   value="<?php echo(!empty($force_install_prefix) ? $force_install_prefix : (!empty($powererp_main_db_prefix) ? $powererp_main_db_prefix : 'llx_')); ?>"
	...
	<tr class="hidesqlite">
		...
		<td class="label">
			<input type="text"
				   id="db_user"
				   name="db_user"
				   value="<?php print (!empty($force_install_databaselogin)) ? $force_install_databaselogin : $powererp_main_db_user; ?>"
		...
		<td class="label">
			<input type="password" class="text-security";
				   ...
				   value="<?php
					...
					if (!empty($powererp_main_prod) && empty($_SESSION['dol_save_pass'])) {    // So value can't be found if install page still accessible
						$autofill = '';
					}
