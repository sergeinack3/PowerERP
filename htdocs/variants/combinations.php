		...
		if ($action == 'add') {
			...
			if (!empty($features)) {
				...
				foreach ($features as $feature) {
					...
					$toprint[] = '<li class="select2-search-choice-powererp noborderoncategories" style="background: #ddd;">' . $prodattr->label.' : '.$prodattr_val->value .
				...
				$listofvariantselected .= '<div class="select2-container-multi-powererp" style="width: 90%;"><ul class="select2-choices-powererp">' . implode(' ', $toprint) . '</ul></div>';
			}
