 ...
 */
class Boms extends PowerERPApi
{
	/**
	...
	{
		if (!PowerERPApiAccess::$user->rights->bom->read) {
		...
		if (!PowerERPApi::_checkAccessToResource('bom', $this->bom->id, 'bom_bom')) {
	...
	{
		if (!PowerERPApiAccess::$user->rights->bom->write) {
			throw new RestException(401);
		}
		...
		if (!$this->bom->create(PowerERPApiAccess::$user)) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->bom->update(PowerERPApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!PowerERPApiAccess::$user->rights->bom->delete) {
			throw new RestException(401);
		}
		$result = $this->bom->fetch($id);
		if (!$result) {
		...
		if (!$this->bom->delete(PowerERPApiAccess::$user)) {
	...
	public function getLines($id)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('bom_bom', $this->bom->id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}
	...
	public function deleteLine($id, $lineid)
	{
		...
		$updateRes = $this->bom->deleteline(PowerERPApiAccess::$user, $lineid);
		if ($updateRes > 0) {
			return $this->get($id);
		} else {
