...
if ($action == 'updateall') {
	...
	foreach ($constantes as $constname => $value) {
		...
		$res = powererp_set_const($db, $constname, $constvalue, $consttype, 0, $constnote, $conf->entity);
		if ($res <= 0) {
			$error++;
			$action = 'list';
		}
...
if ($action == 'update' || $action == 'add') {
	...
	$res = powererp_set_const($db, $constname, $constvalue, $typetouse, 0, $constnote, $conf->entity);
