...
if ($rowid > 0) {
	...
	if (!empty($conf->societe->enabled)) {
		...
		print $langs->trans("LinkedToPowerERPThirdParty");
		print '</td>';
	...
	if (($action == 'addsubscription' || $action == 'create_thirdparty') && $user->rights->adherent->cotisation->creer) {
		...
		if ($action == 'create_thirdparty') {
			...
			print $form->formconfirm($_SERVER["PHP_SELF"]."?rowid=".$object->id, $langs->trans("CreatePowerERPThirdParty"), $langs->trans("ConfirmCreateThirdParty"), "confirm_create_thirdparty", $formquestion, 1);
		}
			...
			if ((!empty($conf->banque->enabled) || isModEnabled('facture')) && empty($conf->global->ADHERENT_SUBSCRIPTION_HIDECOMPLEMENTARYACTIONS)) {
				...
				if (!empty($conf->societe->enabled) && isModEnabled('facture')) {
					...
					if ($object->fk_soc) {
						...
						print $langs->trans("CreatePowerERPThirdParty");
						print '</a>)';
					}
