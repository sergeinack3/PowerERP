 ...
 */
class Members extends PowerERPApi
{
	/**
	...
	public function get($id)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('adherent', $member->id) && $id > 0) {
	...
	public function getByThirdparty($thirdparty)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('adherent', $member->id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}
	...
	public function post($request_data = null)
	{
		...
		if ($member->create(PowerERPApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('member', $member->id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}
		...
		if ($member->update(PowerERPApiAccess::$user) >= 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!PowerERPApiAccess::$user->rights->adherent->supprimer) {
			throw new RestException(401);
		}
		$member = new Adherent($this->db);
		$result = $member->fetch($id);
		...
		$res = $member->delete($member->id, PowerERPApiAccess::$user);
		if ($res < 0) {
	...
	public function getSubscriptions($id)
	{
		...
		if (!PowerERPApiAccess::$user->rights->adherent->cotisation->lire) {
			throw new RestException(401);
		}
	...
	{
		if (!PowerERPApiAccess::$user->rights->adherent->cotisation->creer) {
			throw new RestException(401);
		}
