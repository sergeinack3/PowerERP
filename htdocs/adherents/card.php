...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
	...
	if ($action == 'edit') {
		...
		if ($object->user_id) {
			...
			print $langs->trans("NoPowerERPAccess");
		}
		print '</td></tr>';
	...
	if ($id > 0 && $action != 'edit') {
		...
		if ($action == 'create_user') {
			...
			if (isModEnabled('societe') && $object->socid > 0) {
				...
				$formquestion[] = array('label' => $langs->trans("UserWillBe"), 'type' => 'radio', 'name' => 'internalorexternal', 'default'=>'external', 'values' => array('external'=>$langs->trans("External").' - '.$langs->trans("LinkedToPowerERPThirdParty").' '.$object->thirdparty->getNomUrl(1, '', 0, 1), 'internal'=>$langs->trans("Internal")));
			}
			$text = '';
			if (isModEnabled('societe') && $object->socid <= 0) {
			...
			print $form->formconfirm($_SERVER["PHP_SELF"]."?rowid=".$object->id, $langs->trans("CreatePowerERPLogin"), $text, "confirm_create_user", $formquestion, 'yes');
		}
		...
		if ($action == 'create_thirdparty') {
			...
			print $form->formconfirm($_SERVER["PHP_SELF"]."?rowid=".$object->id, $langs->trans("CreatePowerERPThirdParty"), $langs->trans("ConfirmCreateThirdParty"), "confirm_create_thirdparty", $formquestion, 'yes');
		}
		...
		if (isModEnabled('societe')) {
			...
			print $form->editfieldkey('LinkedToPowerERPThirdParty', 'thirdparty', '', $object, $editenable);
		...
		if ($action == 'editlogin') {
			...
			if ($object->user_id) {
				...
				print '<span class="opacitymedium">'.$langs->trans("NoPowerERPAccess").'</span>';
			}
		}
		print '</td></tr>';
			...
			if ($action != 'editlogin' && $action != 'editthirdparty') {
				...
				if (isModEnabled('societe') && !$object->socid) {
					if ($user->hasRight('societe', 'creer')) {
						if (Adherent::STATUS_DRAFT != $object->statut) {
							print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?rowid='.((int) $object->id).'&amp;action=create_thirdparty" title="'.dol_escape_htmltag($langs->trans("CreatePowerERPThirdPartyDesc")).'">'.$langs->trans("CreatePowerERPThirdParty").'</a>'."\n";
						} else {
							print '<a class="butActionRefused classfortooltip" href="#" title="'.dol_escape_htmltag($langs->trans("ValidateBefore")).'">'.$langs->trans("CreatePowerERPThirdParty").'</a>'."\n";
						}
					} else {
						print '<span class="butActionRefused classfortooltip" title="'.dol_escape_htmltag($langs->trans("NotEnoughPermissions")).'">'.$langs->trans("CreatePowerERPThirdParty").'</span>'."\n";
					}
				}
				...
				if (!$user->socid && !$object->user_id) {
					if ($user->hasRight('user', 'user', 'creer')) {
						if (Adherent::STATUS_DRAFT != $object->statut) {
							print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?rowid='.((int) $object->id).'&amp;action=create_user" title="'.dol_escape_htmltag($langs->trans("CreatePowerERPLoginDesc")).'">'.$langs->trans("CreatePowerERPLogin").'</a>'."\n";
						} else {
							print '<a class="butActionRefused classfortooltip" href="#" title="'.dol_escape_htmltag($langs->trans("ValidateBefore")).'">'.$langs->trans("CreatePowerERPLogin").'</a>'."\n";
						}
					} else {
						print '<span class="butActionRefused classfortooltip" title="'.dol_escape_htmltag($langs->trans("NotEnoughPermissions")).'">'.$langs->trans("CreatePowerERPLogin").'</span>'."\n";
					}
				}
