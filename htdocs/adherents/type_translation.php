...
if ($action == 'edit') {
	...
	if (!empty($object->multilangs)) {
		foreach ($object->multilangs as $key => $value) {
			...
			$doleditor = new DolEditor("desc-$key", $object->multilangs[$key]["description"], '', 160, 'powererp_notes', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_SOCIETE'), ROWS_3, '90%');
			$doleditor->Create();
			print '</td></tr>';
			print '</td></tr>';
			print '</table>';
...
if ($action == 'create' && $user->hasRight('adherent', 'configurer')) {
	...
	print '<tr><td class="tdtop">'.$langs->trans('Description').'</td><td>';
	$doleditor = new DolEditor('desc', '', '', 160, 'powererp_notes', '', false, true, empty($conf->fckeditor->enabled) ? false : $conf->fckeditor->enabled, ROWS_3, '90%');
	$doleditor->Create();
	print '</td></tr>';
