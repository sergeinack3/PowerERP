	...
	public function assign_values(&$action, $id)
	{
		...
		if ($action == 'view' || $action == 'edit' || $action == 'delete') {
			...
			if ($this->object->user_id) {
				...
				$this->tpl['powererp_user'] = $powererp_user->getLoginUrl(1);
			} else {
				$this->tpl['powererp_user'] = $langs->trans("NoPowererpAccess");
			}
		}
		...
		if ($action == 'create_user') {
			...
			$this->tpl['action_create_user'] = $form->formconfirm($_SERVER["PHP_SELF"]."?id=".$this->object->id, $langs->trans("CreatePowererpLogin"), $langs->trans("ConfirmCreateAdherent"), "confirm_create_user", $formquestion, 'no');
		}
	}
