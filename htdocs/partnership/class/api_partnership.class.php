 ...
 */
class PartnershipApi extends PowerERPApi
{
	/**
	...
	{
		if (!PowerERPApiAccess::$user->rights->partnership->read) {
		...
		if (!PowerERPApi::_checkAccessToResource('partnership', $this->partnership->id, 'partnership')) {
			throw new RestException(401, 'Access to instance id='.$this->partnership->id.' of object not allowed for login '.PowerERPApiAccess::$user->login);
		}
	...
	{
		if (!PowerERPApiAccess::$user->rights->partnership->write) {
			throw new RestException(401);
		}
		...
		if ($this->partnership->create(PowerERPApiAccess::$user)<0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->partnership->update(PowerERPApiAccess::$user, false) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!PowerERPApiAccess::$user->rights->partnership->delete) {
			throw new RestException(401);
		}
		$result = $this->partnership->fetch($id);
		if (!$result) {
		...
		if (!$this->partnership->delete(PowerERPApiAccess::$user)) {
