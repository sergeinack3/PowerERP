...
class PartnershipUtils
{
	 ...
	 * @return  int                 0 if OK, <>0 if KO (this function is used also by cron so only 0 is OK)
	 */
	public function doWarningOfPartnershipIfPowererpBacklinkNotfound()
	{
		global $conf, $langs, $user;
		...
		dol_syslog(get_class($this)."::doWarningOfPartnershipIfPowererpBacklinkNotfound Warning of partnership");
		...
		if ($resql) {
			...
			while ($ifetchpartner < $numofexpiredmembers) {
				...
				if ($obj) {
					...
					if ($somethingdoneonpartnership >= $MAXPERCALL) {
						dol_syslog("We reach the limit of ".$MAXPERCALL." partnership processed, so we quit loop for this batch doWarningOfPartnershipIfPowererpBacklinkNotfound to avoid to reach email quota.", LOG_WARNING);
						break;
					}
					...
					if (empty($website)) {
						...
						$backlinkfound = $this->checkPowererpBacklink($website);
					}
	 ...
	 */
	private function checkPowererpBacklink($website = null)
	{
		global $conf, $langs, $user;
