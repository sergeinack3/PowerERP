 ...
 */
class SupplierOrders extends PowerERPApi
{
	/**
	 *
	...
	{
		if (!PowerERPApiAccess::$user->rights->fournisseur->commande->lire) {
		...
		if (!PowerERPApi::_checkAccessToResource('fournisseur', $this->order->id, 'commande_fournisseur', 'commande')) {
	...
	{
		if (empty(PowerERPApiAccess::$user->rights->fournisseur->commande->creer) && empty(PowerERPApiAccess::$user->rights->supplier_order->creer)) {
		...
		if ($this->order->create(PowerERPApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->order->update(PowerERPApiAccess::$user)) {
			return $this->get($id);
		}
	...
	{
		if (!PowerERPApiAccess::$user->rights->fournisseur->commande->supprimer) {
			throw new RestException(401);
		}
		$result = $this->order->fetch($id);
		if (!$result) {
		...
		if ($this->order->delete(PowerERPApiAccess::$user) < 0) {
	...
	public function validate($id, $idwarehouse = 0, $notrigger = 0)
	{
		...
		$result = $this->order->valid(PowerERPApiAccess::$user, $idwarehouse, $notrigger);
		if ($result == 0) {
	...
	public function approve($id, $idwarehouse = 0, $secondlevel = 0)
	{
		...
		$result = $this->order->approve(PowerERPApiAccess::$user, $idwarehouse, $secondlevel);
		if ($result == 0) {
	...
	public function makeOrder($id, $date, $method, $comment = '')
	{
		...
		$result = $this->order->commande(PowerERPApiAccess::$user, $date, $method, $comment);
		if ($result == 0) {
	...
	public function receiveOrder($id, $closeopenorder, $comment, $lines)
	{
		...
		foreach ($lines as $line) {
			...
			$result=$this->order->dispatchProduct(PowerERPApiAccess::$user,
				  $lineObj->fk_product,
				  $lineObj->qty,
				  $lineObj->warehouse,
				  $lineObj->price,
		...
		$result = $this->order->calcAndSetStatusDispatch(PowerERPApiAccess::$user, $closeopenorder, $comment);
