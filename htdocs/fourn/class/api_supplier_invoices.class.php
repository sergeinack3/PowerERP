 ...
 */
class SupplierInvoices extends PowererpApi
{
	/**
	 *
	...
	{
		if (!PowererpApiAccess::$user->rights->fournisseur->facture->lire) {
		...
		if (!PowererpApi::_checkAccessToResource('fournisseur', $this->invoice->id, 'facture_fourn', 'facture')) {
	...
	public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $thirdparty_ids = '', $status = '', $sqlfilters = '')
	{
		...
		$search_sale = 0;
		if (!PowererpApiAccess::$user->rights->societe->client->voir) {
		...
		if (!PowererpApiAccess::$user->rights->societe->client->voir || $search_sale > 0) {
	...
	{
		if (!PowererpApiAccess::$user->rights->fournisseur->facture->creer) {
		...
		if ($this->invoice->create(PowererpApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->invoice->update($id, PowererpApiAccess::$user)) {
			return $this->get($id);
		}
	...
	{
		if (!PowererpApiAccess::$user->rights->fournisseur->facture->supprimer) {
			throw new RestException(401);
		}
		$result = $this->invoice->fetch($id);
		if (!$result) {
		...
		if ($this->invoice->delete(PowererpApiAccess::$user) < 0) {
	...
	public function addPayment($id, $datepaye, $payment_mode_id, $closepaidinvoices, $accountid, $num_payment = '', $comment = '', $chqemetteur = '', $chqbank = '')
	{
		...
		$paiement_id = $paiement->create(PowererpApiAccess::$user, ($closepaidinvoices == 'yes' ? 1 : 0)); // This include closing invoices
		if ($paiement_id < 0) {
			$this->db->rollback();
		...
		if (!empty($conf->banque->enabled)) {
			$result = $paiement->addPaymentToBank(PowererpApiAccess::$user, 'payment_supplier', '(SupplierInvoicePayment)', $accountid, $chqemetteur, $chqbank);
			if ($result < 0) {
				$this->db->rollback();
	 ...
	 *
	 * Note: socid = powererp_order_id, pu_ht = net price, remise = discount
	 *
