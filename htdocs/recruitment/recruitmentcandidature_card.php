			...
			if ($object->status == $object::STATUS_CONTRACT_SIGNED) {
				if ($user->rights->user->user->creer) {
					...
					if (empty($useralreadyexists)) {
						print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=create_user">'.$langs->trans("CreatePowerERPLogin").'</a></div>';
					} else {
						print '<div class="inline-block divButAction"><a class="butActionRefused" href="#">'.$langs->trans("CreatePowerERPLogin").'</a></div>';
					}
				} else {
					print '<div class="inline-block divButAction"><span class="butActionRefused classfortooltip" href="#" title="'.dol_escape_htmltag($langs->trans("NotEnoughPermissions")).'">'.$langs->trans("CreatePowerERPLogin")."</span></div>";
				}
			}
