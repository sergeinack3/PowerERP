 ...
 */
class Recruitment extends PowerERPApi
{
	/**
	...
	{
		if (!PowerERPApiAccess::$user->rights->recruitment->recruitmentjobposition->read) {
		...
		if (!PowerERPApi::_checkAccessToResource('recruitment', $this->jobposition->id, 'recruitment_recruitmentjobposition')) {
			throw new RestException(401, 'Access to instance id='.$this->jobposition->id.' of object not allowed for login '.PowerERPApiAccess::$user->login);
		}
	...
	public function getCandidature($id)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('recruitment', $this->candidature->id, 'recruitment_recruitmentcandidature')) {
			throw new RestException(401, 'Access to instance id='.$this->candidature->id.' of object not allowed for login '.PowerERPApiAccess::$user->login);
		}
	...
	{
		if (!PowerERPApiAccess::$user->rights->recruitment->recruitmentjobposition->write) {
			throw new RestException(401);
		}
		...
		if ($this->jobposition->create(PowerERPApiAccess::$user)<0) {
	...
	*/
	public function postCandidature($request_data = null)
	{
		...
		if ($this->candidature->create(PowerERPApiAccess::$user)<0) {
	...
	public function putJobPosition($id, $request_data = null)
	{
		...
		if ($this->jobposition->update(PowerERPApiAccess::$user, false) > 0) {
			return $this->get($id);
		} else {
	...
	public function putCandidature($id, $request_data = null)
	{
		...
		if ($this->candidature->update(PowerERPApiAccess::$user, false) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!PowerERPApiAccess::$user->rights->recruitment->recruitmentjobposition->delete) {
			throw new RestException(401);
		}
		$result = $this->jobposition->fetch($id);
		if (!$result) {
		...
		if (!$this->jobposition->delete(PowerERPApiAccess::$user)) {
	...
	public function deleteCandidature($id)
	{
		...
		if (!$this->candidature->delete(PowerERPApiAccess::$user)) {
