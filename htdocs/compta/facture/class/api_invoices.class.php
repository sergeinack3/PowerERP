 ...
 */
class Invoices extends PowerERPApi
{
	/**
	 *
	...
	{
		if (!PowerERPApiAccess::$user->rights->facture->lire) {
		...
		if (!PowerERPApi::_checkAccessToResource('facture', $this->invoice->id)) {
	...
	public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $thirdparty_ids = '', $status = '', $sqlfilters = '')
	{
		...
		$socids = PowerERPApiAccess::$user->socid ? PowerERPApiAccess::$user->socid : $thirdparty_ids;

		// If the internal user must only see his customers, force searching by him
		$search_sale = 0;
		if (!PowerERPApiAccess::$user->rights->societe->client->voir && !$socids) {
			$search_sale = PowerERPApiAccess::$user->id;
		}

		$sql = "SELECT t.rowid";
		if ((!PowerERPApiAccess::$user->rights->societe->client->voir && !$socids) || $search_sale > 0) {
	...
	{
		if (!PowerERPApiAccess::$user->rights->facture->creer) {
		...
		if ($this->invoice->create(PowerERPApiAccess::$user, 0, (empty($request_data["date_lim_reglement"]) ? 0 : $request_data["date_lim_reglement"])) < 0) {
	...
	public function createInvoiceFromOrder($orderid)
	{
		...
		if (!PowerERPApiAccess::$user->rights->commande->lire) {
			throw new RestException(401);
		}
		if (!PowerERPApiAccess::$user->rights->facture->creer) {
			throw new RestException(401);
		...
		$result = $this->invoice->createFromOrder($order, PowerERPApiAccess::$user);
		if ($result < 0) {
	...
	public function postContact($id, $contactid, $type)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('invoice', $this->invoice->id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}
	...
	public function deleteLine($id, $lineid)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('facture', $id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->invoice->update(PowerERPApiAccess::$user)) {
			return $this->get($id);
		}
	...
	{
		if (!PowerERPApiAccess::$user->rights->facture->supprimer) {
			throw new RestException(401);
		}
		$result = $this->invoice->fetch($id);
		if (!$result) {
		...
		$result = $this->invoice->delete(PowerERPApiAccess::$user);
		if ($result < 0) {
	...
	public function settodraft($id, $idwarehouse = -1)
	{
		...
		$result = $this->invoice->setDraft(PowerERPApiAccess::$user, $idwarehouse);
		if ($result == 0) {
	...
	public function validate($id, $idwarehouse = 0, $notrigger = 0)
	{
		...
		$result = $this->invoice->validate(PowerERPApiAccess::$user, '', $idwarehouse, $notrigger);
		if ($result == 0) {
	...
	public function settopaid($id, $close_code = '', $close_note = '')
	{
		...
		$result = $this->invoice->setPaid(PowerERPApiAccess::$user, $close_code, $close_note);
		if ($result == 0) {
			throw new RestException(304, 'Error nothing done. May be object is already validated');
		}
		if ($result < 0) {
	...
	public function settounpaid($id)
	{
		...
		$result = $this->invoice->setUnpaid(PowerERPApiAccess::$user);
		if ($result == 0) {
			throw new RestException(304, 'Nothing done');
		}
		if ($result < 0) {
	...
	public function markAsCreditAvailable($id)
	{
		...
		if ($canconvert) {
			...
			if ($this->invoice->type == Facture::TYPE_STANDARD || $this->invoice->type == Facture::TYPE_REPLACEMENT || $this->invoice->type == Facture::TYPE_SITUATION) {
				...
				$result = $discount->create(PowerERPApiAccess::$user);
				if ($result < 0) {
					$error++;
				}
			}
			...
			if (empty($error)) {
				if ($this->invoice->type != Facture::TYPE_DEPOSIT) {
					// Classe facture
					$result = $this->invoice->setPaid(PowerERPApiAccess::$user);
					if ($result >= 0) {
						$this->db->commit();
					} else {
						$this->db->rollback();
	...
	public function addPayment($id, $datepaye, $paymentid, $closepaidinvoices, $accountid, $num_payment = '', $comment = '', $chqemetteur = '', $chqbank = '')
	{
		...
		if (!PowerERPApi::_checkAccessToResource('facture', $id)) {
			throw new RestException(403, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}
		...
		$payment_id = $paymentobj->create(PowerERPApiAccess::$user, ($closepaidinvoices == 'yes' ? 1 : 0)); // This include closing invoices
		if ($payment_id < 0) {
			$this->db->rollback();
		...
		if (isModEnabled("banque")) {
			...
			$result = $paymentobj->addPaymentToBank(PowerERPApiAccess::$user, 'payment', $label, $accountid, $chqemetteur, $chqbank);
			if ($result < 0) {
				$this->db->rollback();
	...
	public function addPaymentDistributed($arrayofamounts, $datepaye, $paymentid, $closepaidinvoices, $accountid, $num_payment = '', $comment = '', $chqemetteur = '', $chqbank = '', $ref_ext = '', $accepthigherpayment = false)
	{
		...
		foreach ($arrayofamounts as $id => $amount) {
			...
			if (!PowerERPApi::_checkAccessToResource('facture', $id)) {
				throw new RestException(403, 'Access not allowed on invoice ID '.$id.' for login '.PowerERPApiAccess::$user->login);
			}
		}
	...
	private function _fetchTemplateInvoice($id, $ref = '', $ref_ext = '', $contact_list = 1)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('facturerec', $this->template_invoice->id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}
