...
if ($action == 'create') {
	...
	$doleditor = new DolEditor('note_public', GETPOST('note_public', 'restricthtml'), '', 200, 'powererp_notes', 'In', false, true, empty($conf->global->FCKEDITOR_ENABLE_NOTE_PUBLIC) ? 0 : 1, ROWS_8, '90%');
	...
	if (empty($user->socid)) {
		...
		$doleditor = new DolEditor('note_private', GETPOST('note_private', 'restricthtml'), '', 200, 'powererp_notes', 'In', false, true, empty($conf->global->FCKEDITOR_ENABLE_NOTE_PRIVATE) ? 0 : 1, ROWS_8, '90%');
		print $doleditor->Create(1);
	...
	if ($result > 0) {
		...
		if ($action == 'edit' && $user->rights->deplacement->creer) {
			...
			$doleditor = new DolEditor('note_public', $object->note_public, '', 200, 'powererp_notes', 'In', false, true, empty($conf->global->FCKEDITOR_ENABLE_NOTE_PUBLIC) ? 0 : 1, ROWS_8, '90%');
			print $doleditor->Create(1);
			...
			if (empty($user->socid)) {
				...
				$doleditor = new DolEditor('note_private', $object->note_private, '', 200, 'powererp_notes', 'In', false, true, empty($conf->global->FCKEDITOR_ENABLE_NOTE_PRIVATE) ? 0 : 1, ROWS_8, '90%');
				print $doleditor->Create(1);
