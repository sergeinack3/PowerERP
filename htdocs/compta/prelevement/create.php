...
if (empty($reshook)) {
	...
	if ($action == 'modify') {
		for ($i = 1; $i < 9; $i++) {
			powererp_set_const($db, GETPOST("nom$i"), GETPOST("value$i"), 'chaine', 0, '', $conf->entity);
		}
	}
	if ($action == 'create') {
		...
		if ($id_bankaccount != $conf->global->$default_account) {
			$res = powererp_set_const($db, $default_account, $id_bankaccount, 'chaine', 0, '', $conf->entity); // Set as default
		}
