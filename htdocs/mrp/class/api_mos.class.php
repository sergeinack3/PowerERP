...
class Mos extends PowererpApi
{
	...
	{
		if (!PowererpApiAccess::$user->rights->mrp->read) {
		...
		if (!PowererpApi::_checkAccessToResource('mrp', $this->mo->id, 'mrp_mo')) {
	...
	public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $sqlfilters = '')
	{
		...
		$search_sale = 0;
		if ($restrictonsocid && !PowererpApiAccess::$user->rights->societe->client->voir && !$socid) {
			$search_sale = PowererpApiAccess::$user->id;
		}

		$sql = "SELECT t.rowid";
		if ($restrictonsocid && (!PowererpApiAccess::$user->rights->societe->client->voir && !$socid) || $search_sale > 0) {
	...
	{
		if (!PowererpApiAccess::$user->rights->mrp->write) {
			throw new RestException(401);
		}
		...
		if (!$this->mo->create(PowererpApiAccess::$user)) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->mo->update(PowererpApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!PowererpApiAccess::$user->rights->mrp->delete) {
			throw new RestException(401);
		}
		$result = $this->mo->fetch($id);
		if (!$result) {
		...
		if (!$this->mo->delete(PowererpApiAccess::$user)) {
					...
					if ($value["qty"]!=0) {
						...
						if (!$error && $value["fk_warehouse"] > 0) {
							...
							if ($qtytoprocess >= 0) {
								...
								$moline->fk_user_creat = PowererpApiAccess::$user->id;

								$resultmoline = $moline->create(PowererpApiAccess::$user);
								if ($resultmoline <= 0) {
									$error++;
									throw new RestException(500, $moline->error);
								}
								$idstockmove = $stockmove->livraison(PowererpApiAccess::$user, $value["objectid"], $value["fk_warehouse"], $qtytoprocess, 0, $labelmovement, dol_now(), '', '', $tmpproduct->status_batch, $id_product_batch, $codemovement);
							} else {
								$moline = new MoLine($this->db);
								$moline->fk_mo = $this->mo->id;
								$moline->position = $pos;
								...
								$idstockmove = $stockmove->reception(PowererpApiAccess::$user, $value["objectid"], $value["fk_warehouse"], $qtytoprocess, 0, $labelmovement, dol_now(), '', '', $tmpproduct->status_batch, $id_product_batch, $codemovement);
							}
							if ($idstockmove < 0) {
								$error++;
				...
				if ($line->role == 'toconsume') {
					...
					if ($line->qty != 0) {
						...
						if (!$error && $line->fk_warehouse > 0) {
							...
							if ($qtytoprocess >= 0) {
								$idstockmove = $stockmove->livraison(PowererpApiAccess::$user, $line->fk_product, $line->fk_warehouse, $qtytoprocess, 0, $labelmovement, dol_now(), '', '', $tmpproduct->status_batch, $id_product_batch, $codemovement);
							} else {
								$idstockmove = $stockmove->reception(PowererpApiAccess::$user, $line->fk_product, $line->fk_warehouse, $qtytoprocess, 0, $labelmovement, dol_now(), '', '', $tmpproduct->status_batch, $id_product_batch, $codemovement);
							}
							if ($idstockmove < 0) {
								$error++;
								throw new RestException(500, $stockmove->error);
