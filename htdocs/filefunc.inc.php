...
if (!$result && !empty($_SERVER["GATEWAY_INTERFACE"])) {    // If install not done and we are in a web session
	...
	/*
	print '<br><center>';
	print 'The conf/conf.php file was not found or is not readable by the web server. If this is your first access, <a href="'.$path.'install/index.php">click here to start the PowerERP installation process</a> to create it...';
	print '</center><br>';
	*/
...
// Clean parameters
$powererp_main_data_root = trim($powererp_main_data_root);
$powererp_main_url_root = trim(preg_replace('/\/+$/', '', $powererp_main_url_root));
$powererp_main_url_root_alt = (empty($powererp_main_url_root_alt) ? '' : trim($powererp_main_url_root_alt));
$powererp_main_document_root = trim($powererp_main_document_root);
$powererp_main_document_root_alt = (empty($powererp_main_document_root_alt) ? '' : trim($powererp_main_document_root_alt));

if (empty($powererp_main_db_port)) {
	$powererp_main_db_port = 3306; // For compatibility with old configs, if not defined, we take 'mysql' type
}
if (empty($powererp_main_db_type)) {
	$powererp_main_db_type = 'mysqli'; // For compatibility with old configs, if not defined, we take 'mysql' type
}
...
if (empty($powererp_main_db_prefix)) {
	$powererp_main_db_prefix = 'llx_';
}
if (empty($powererp_main_db_character_set)) {
	$powererp_main_db_character_set = ($powererp_main_db_type == 'mysqli' ? 'utf8' : ''); // Old installation
}
if (empty($powererp_main_db_collation)) {
	$powererp_main_db_collation = ($powererp_main_db_type == 'mysqli' ? 'utf8_unicode_ci' : ''); // Old installation
}
...
if (empty($powererp_main_limit_users)) {
	$powererp_main_limit_users = 0;
}
if (empty($powererp_mailing_limit_sendbyweb)) {
	$powererp_mailing_limit_sendbyweb = 0;
}
if (empty($powererp_mailing_limit_sendbycli)) {
	$powererp_mailing_limit_sendbycli = 0;
}
if (empty($powererp_mailing_limit_sendbyday)) {
	$powererp_mailing_limit_sendbyday = 0;
}
if (empty($powererp_strict_mode)) {
	$powererp_strict_mode = 0; // For debug in php strict mode
}
...
if (!defined('NOCSRFCHECK') && empty($powererp_nocsrfcheck)) {
	if (!empty($_SERVER['REQUEST_METHOD']) && !in_array($_SERVER['REQUEST_METHOD'], array('GET', 'HEAD')) && !empty($_SERVER['HTTP_HOST'])) {
		...
		if ($csrfattack) {
			...
			print "If you access your server behind a proxy using url rewriting, you might check that all HTTP headers are propagated (or add the line \$powererp_nocsrfcheck=1 into your conf.php file to remove this security check).\n";
			die;
		}
	}
...
if (empty($powererp_main_document_root_alt)) {
	$powererp_main_document_root_alt = $powererp_main_document_root.'/custom';
}

if (empty($powererp_main_data_root)) {
	// If directory not defined, we use the default hardcoded value
	$powererp_main_data_root = str_replace("/htdocs", "", $powererp_main_document_root);
...
define('DOL_DATA_ROOT', $powererp_main_data_root); // Filesystem data (documents)
...
foreach ($paths as $tmppath) {	// We check to find (B+start of C)=A
	...
	//print $_SERVER["SCRIPT_NAME"].'-'.$pathroot.'-'.$concatpath.'-'.$real_powererp_main_document_root.'-'.realpath($pathroot.$concatpath).'<br>';
	if ($real_powererp_main_document_root == @realpath($pathroot.$concatpath)) {    // @ avoid warning when safe_mode is on.
...
//print "found=".$found." powererp_main_url_root=".$powererp_main_url_root."\n";
if (!$found) {
	$tmp = $powererp_main_url_root; // If autodetect fails (Ie: when using apache alias that point outside default DOCUMENT_ROOT).
} else {
...
define('MAIN_DB_PREFIX', $powererp_main_db_prefix);
...
if (!defined('ADODB_PATH')) {
	define('ADODB_PATH', (!isset($powererp_lib_ADODB_PATH)) ?DOL_DOCUMENT_ROOT.'/includes/adodbtime/' : (empty($powererp_lib_ADODB_PATH) ? '' : $powererp_lib_ADODB_PATH.'/'));
}
if (!defined('TCPDF_PATH')) {
	define('TCPDF_PATH', (empty($powererp_lib_TCPDF_PATH)) ?DOL_DOCUMENT_ROOT.'/includes/tecnickcom/tcpdf/' : $powererp_lib_TCPDF_PATH.'/');
}
if (!defined('TCPDI_PATH')) {
	define('TCPDI_PATH', (empty($powererp_lib_TCPDI_PATH)) ?DOL_DOCUMENT_ROOT.'/includes/tcpdi/' : $powererp_lib_TCPDI_PATH.'/');
}
if (!defined('NUSOAP_PATH')) {
	define('NUSOAP_PATH', (!isset($powererp_lib_NUSOAP_PATH)) ?DOL_DOCUMENT_ROOT.'/includes/nusoap/lib/' : (empty($powererp_lib_NUSOAP_PATH) ? '' : $powererp_lib_NUSOAP_PATH.'/'));
}
if (!defined('PHPEXCELNEW_PATH')) {
	define('PHPEXCELNEW_PATH', (!isset($powererp_lib_PHPEXCELNEW_PATH)) ?DOL_DOCUMENT_ROOT.'/includes/phpoffice/phpspreadsheet/src/PhpSpreadsheet/' : (empty($powererp_lib_PHPEXCELNEW_PATH) ? '' : $powererp_lib_PHPEXCELNEW_PATH.'/'));
}
if (!defined('ODTPHP_PATH')) {
	define('ODTPHP_PATH', (!isset($powererp_lib_ODTPHP_PATH)) ?DOL_DOCUMENT_ROOT.'/includes/odtphp/' : (empty($powererp_lib_ODTPHP_PATH) ? '' : $powererp_lib_ODTPHP_PATH.'/'));
}
if (!defined('ODTPHP_PATHTOPCLZIP')) {
	define('ODTPHP_PATHTOPCLZIP', (!isset($powererp_lib_ODTPHP_PATHTOPCLZIP)) ?DOL_DOCUMENT_ROOT.'/includes/odtphp/zip/pclzip/' : (empty($powererp_lib_ODTPHP_PATHTOPCLZIP) ? '' : $powererp_lib_ODTPHP_PATHTOPCLZIP.'/'));
}
if (!defined('JS_CKEDITOR')) {
	define('JS_CKEDITOR', (!isset($powererp_js_CKEDITOR)) ? '' : (empty($powererp_js_CKEDITOR) ? '' : $powererp_js_CKEDITOR.'/'));
}
if (!defined('JS_JQUERY')) {
	define('JS_JQUERY', (!isset($powererp_js_JQUERY)) ? '' : (empty($powererp_js_JQUERY) ? '' : $powererp_js_JQUERY.'/'));
}
if (!defined('JS_JQUERY_UI')) {
	define('JS_JQUERY_UI', (!isset($powererp_js_JQUERY_UI)) ? '' : (empty($powererp_js_JQUERY_UI) ? '' : $powererp_js_JQUERY_UI.'/'));
}
// Other required path
if (!defined('DOL_DEFAULT_TTF')) {
	define('DOL_DEFAULT_TTF', (!isset($powererp_font_DOL_DEFAULT_TTF)) ?DOL_DOCUMENT_ROOT.'/includes/fonts/Aerial.ttf' : (empty($powererp_font_DOL_DEFAULT_TTF) ? '' : $powererp_font_DOL_DEFAULT_TTF));
}
if (!defined('DOL_DEFAULT_TTF_BOLD')) {
	define('DOL_DEFAULT_TTF_BOLD', (!isset($powererp_font_DOL_DEFAULT_TTF_BOLD)) ?DOL_DOCUMENT_ROOT.'/includes/fonts/AerialBd.ttf' : (empty($powererp_font_DOL_DEFAULT_TTF_BOLD) ? '' : $powererp_font_DOL_DEFAULT_TTF_BOLD));
}
...
// If password is encoded, we decode it. Note: When page is called for install, $powererp_main_db_pass may not be defined yet.
if ((!empty($powererp_main_db_pass) && preg_match('/crypted:/i', $powererp_main_db_pass)) || !empty($powererp_main_db_encrypted_pass)) {
	if (!empty($powererp_main_db_pass) && preg_match('/crypted:/i', $powererp_main_db_pass)) {
		...
		$powererp_main_db_encrypted_pass = $powererp_main_db_pass; // We need to set this so we can use it later to know the password was initially crypted
	} else {
