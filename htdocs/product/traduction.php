...
if ($action == 'edit') {
	...
	if (!empty($object->multilangs)) {
		...
		foreach ($object->multilangs as $key => $value) {
			...
			$doleditor = new DolEditor("desc-$key", $object->multilangs[$key]["description"], '', 160, 'powererp_notes', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_PRODUCTDESC'), ROWS_3, '90%');
			$doleditor->Create();
			print '</td></tr>';
			if (!empty($conf->global->PRODUCT_USE_OTHER_FIELD_IN_TRANSLATION)) {
				...
				$doleditor = new DolEditor("other-$key", $object->multilangs[$key]["other"], '', 160, 'powererp_notes', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_PRODUCTDESC'), ROWS_3, '90%');
				$doleditor->Create();
			}
			print '</td></tr>';
			print '</table>';
...
if ($action == 'add' && ($user->rights->produit->creer || $user->rights->service->creer)) {
	...
	print '<tr><td class="tdtop">'.$langs->trans('Description').'</td><td>';
	$doleditor = new DolEditor('desc', '', '', 160, 'powererp_notes', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_PRODUCTDESC'), ROWS_3, '90%');
	$doleditor->Create();
	print '</td></tr>';
	// Other field (not used)
	if (!empty($conf->global->PRODUCT_USE_OTHER_FIELD_IN_TRANSLATION)) {
		print '<tr><td class="tdtop">'.$langs->trans('Other').' ('.$langs->trans("NotUsed").'</td><td>';
		$doleditor = new DolEditor('other', '', '', 160, 'powererp_notes', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_PRODUCTDESC'), ROWS_3, '90%');
		$doleditor->Create();
		print '</td></tr>';
	}
	print '</table>';
