...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
	...
	if ($action == 'create' && $usercancreate) {
		...
		if ($action == 'edit' && $usercancreate) {
			...
			// We use powererp_details as type of DolEditor here, because we must not accept images as description is included into PDF and not accepted by TCPDF.
			$doleditor = new DolEditor('desc', $object->description, '', 160, 'powererp_details', '', false, true, $conf->global->FCKEDITOR_ENABLE_PRODUCTDESC, ROWS_4, '90%');
			$doleditor->Create();
			...
			if (!empty($conf->global->MAIN_DISABLE_NOTES_TAB)) {
				...
				$doleditor = new DolEditor('note_private', $object->note_private, '', 140, 'powererp_notes', '', false, true, $conf->global->FCKEDITOR_ENABLE_PRODUCTDESC, ROWS_4, '90%');
				$doleditor->Create();
