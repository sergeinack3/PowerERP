...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
	...
	if ($action == 'create' && $usercancreate) {
		...
		$doleditor = new DolEditor('desc', GETPOST('desc', 'restricthtml'), '', 160, 'powererp_details', '', false, true, getDolGlobalString('FCKEDITOR_ENABLE_PRODUCTDESC'), ROWS_4, '90%');
		$doleditor->Create();
		...
		//{
			...
			// We use powererp_details as type of DolEditor here, because we must not accept images as description is included into PDF and not accepted by TCPDF.
			$doleditor = new DolEditor('note_private', GETPOST('note_private', 'restricthtml'), '', 140, 'powererp_details', '', false, true, getDolGlobalString('FCKEDITOR_ENABLE_PRODUCTDESC'), ROWS_8, '90%');
			$doleditor->Create();
		...
		if ($action == 'edit' && $usercancreate) {
			...
			// We use powererp_details as type of DolEditor here, because we must not accept images as description is included into PDF and not accepted by TCPDF.
			$doleditor = new DolEditor('desc', GETPOSTISSET('desc') ? GETPOST('desc', 'restricthtml') : $object->description, '', 160, 'powererp_details', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_PRODUCTDESC'), ROWS_4, '90%');
			$doleditor->Create();
			...
			if (!empty($conf->global->MAIN_DISABLE_NOTES_TAB)) {
				...
				$doleditor = new DolEditor('note_private', $object->note_private, '', 140, 'powererp_notes', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_PRODUCTDESC'), ROWS_4, '90%');
				$doleditor->Create();
