...
if ($action == 'create') {
	...
	$doleditor = new DolEditor('desc', (!empty($object->description) ? $object->description : ''), '', 180, 'powererp_notes', 'In', false, true, empty($conf->fckeditor->enabled) ? '' : $conf->fckeditor->enabled, ROWS_5, '90%');
	$doleditor->Create();
	print '</td></tr>';
	...
	if ($id > 0 || $ref) {
		...
		if ($action == 'edit' || $action == 're-edit') {
			...
			// Editeur wysiwyg
			require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
			$doleditor = new DolEditor('desc', $object->description, '', 180, 'powererp_notes', 'In', false, true, $conf->fckeditor->enabled, ROWS_5, '90%');
			$doleditor->Create();
			print '</td></tr>';
