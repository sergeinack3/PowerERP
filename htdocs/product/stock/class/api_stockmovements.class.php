 ...
 */
class StockMovements extends PowerERPApi
{
	/**
	...
	public function get($id)
	{
		...
		if( ! PowerERPApi::_checkAccessToResource('warehouse',$this->stockmovement->id)) {
	...
	public function post($product_id, $warehouse_id, $qty, $type = 2, $lot = '', $movementcode = '', $movementlabel = '', $price = '', $datem = '', $dlc = '', $dluo = '', $origin_type = '', $origin_id = 0)
	{
		...
		if ($this->stockmovement->_create(PowerERPApiAccess::$user, $product_id, $warehouse_id, $qty, $type, $price, $movementlabel, $movementcode, $dateMvt, $eatBy, $sellBy, $lot) <= 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if( ! PowerERPApi::_checkAccessToResource('stock',$this->stockmovement->id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}
		...
		if($this->stockmovement->update($id, PowerERPApiAccess::$user))
			return $this->get ($id);
	...
	public function delete($id)
	{
		...
		if (! $this->stockmovement->delete(PowerERPApiAccess::$user)) {
