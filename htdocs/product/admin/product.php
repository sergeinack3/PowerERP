...
if (!empty($conf->global->PRODUIT_MULTIPRICES) && empty($conf->global->PRODUIT_MULTIPRICES_LIMIT)) {
	powererp_set_const($db, 'PRODUIT_MULTIPRICES_LIMIT', 5, 'chaine', 0, '', $conf->entity);
}
...
if ($action == 'setcodeproduct') {
	if (powererp_set_const($db, "PRODUCT_CODEPRODUCT_ADDON", $value, 'chaine', 0, '', $conf->entity) > 0) {
...
if ($action == 'other' && GETPOST('value_PRODUIT_LIMIT_SIZE') >= 0) {
	$res = powererp_set_const($db, "PRODUIT_LIMIT_SIZE", GETPOST('value_PRODUIT_LIMIT_SIZE'), 'chaine', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
}
if ($action == 'other' && GETPOST('value_PRODUIT_MULTIPRICES_LIMIT') > 0) {
	$res = powererp_set_const($db, "PRODUIT_MULTIPRICES_LIMIT", GETPOST('value_PRODUIT_MULTIPRICES_LIMIT'), 'chaine', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
}
	...
	foreach ($select_pricing_rules as $rule => $label) { // Loop on each possible mode
		if ($rule == $princingrules) { // We are on selected rule, we enable it
			if ($princingrules == 'PRODUCT_PRICE_UNIQ') { // For this case, we disable entries manually
				$res = powererp_set_const($db, 'PRODUIT_MULTIPRICES', 0, 'chaine', 0, '', $conf->entity);
				$res = powererp_set_const($db, 'PRODUIT_CUSTOMER_PRICES_BY_QTY', 0, 'chaine', 0, '', $conf->entity);
				$res = powererp_set_const($db, 'PRODUIT_CUSTOMER_PRICES', 0, 'chaine', 0, '', $conf->entity);
				powererp_set_const($db, 'PRODUCT_PRICE_UNIQ', 1, 'chaine', 0, '', $conf->entity);
			} else {
				$multirule = explode('&', $princingrules);
				foreach ($multirule as $rulesselected) {
					$res = powererp_set_const($db, $rulesselected, 1, 'chaine', 0, '', $conf->entity);
				}
			}
		} else // We clear this mode
		{
			if (strpos($rule, '&') === false) {
				$res = powererp_set_const($db, $rule, 0, 'chaine', 0, '', $conf->entity);
			}
		}
	}

	$value = GETPOST('price_base_type', 'alpha');
	$res = powererp_set_const($db, "PRODUCT_PRICE_BASE_TYPE", $value, 'chaine', 0, '', $conf->entity);

	/*$value = GETPOST('PRODUIT_SOUSPRODUITS', 'alpha');
	$res = powererp_set_const($db, "PRODUIT_SOUSPRODUITS", $value, 'chaine', 0, '', $conf->entity);*/

	$value = GETPOST('activate_viewProdDescInForm', 'alpha');
	$res = powererp_set_const($db, "PRODUIT_DESC_IN_FORM", $value, 'chaine', 0, '', $conf->entity);

	$value = GETPOST('activate_viewProdTextsInThirdpartyLanguage', 'alpha');
	$res = powererp_set_const($db, "PRODUIT_TEXTS_IN_THIRDPARTY_LANGUAGE", $value, 'chaine', 0, '', $conf->entity);

	$value = GETPOST('activate_mergePropalProductCard', 'alpha');
	$res = powererp_set_const($db, "PRODUIT_PDF_MERGE_PROPAL", $value, 'chaine', 0, '', $conf->entity);

	$value = GETPOST('activate_usesearchtoselectproduct', 'alpha');
	$res = powererp_set_const($db, "PRODUIT_USE_SEARCH_TO_SELECT", $value, 'chaine', 0, '', $conf->entity);

	$value = GETPOST('activate_FillProductDescAuto', 'alpha');
	$res = powererp_set_const($db, "PRODUIT_AUTOFILL_DESC", $value, 'chaine', 0, '', $conf->entity);

	$value = GETPOST('PRODUIT_FOURN_TEXTS', 'alpha');
	$res = powererp_set_const($db, "PRODUIT_FOURN_TEXTS", $value, 'chaine', 0, '', $conf->entity);

	$value = GETPOST('PRODUCT_USE_SUPPLIER_PACKAGING', 'alpha');
	$res = powererp_set_const($db, "PRODUCT_USE_SUPPLIER_PACKAGING", $value, 'chaine', 0, '', $conf->entity);
}
...
if ($action == 'del') {
	...
	if ($ret > 0) {
		if ($conf->global->PRODUCT_ADDON_PDF == "$value") {
			powererp_del_const($db, 'PRODUCT_ADDON_PDF', $conf->entity);
		}
	}
}
...
// Set default model
if ($action == 'setdoc') {
	if (powererp_set_const($db, "PRODUCT_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
...
if ($action == 'set') {
	...
	if (GETPOST('value', 'alpha')) {
		$res = powererp_set_const($db, $const, $value, 'chaine', 0, '', $conf->entity);
	} else {
		$res = powererp_del_const($db, $const, $conf->entity);
	}
	if (!($res > 0)) {
		$error++;
	}
...
if (preg_match('/set_(.+)/', $action, $reg)) {
	...
	if ($keyforvar) {
		$value = 1;
		$res = powererp_set_const($db, $keyforvar, $value, 'chaine', 0, '', $conf->entity);
	}
}
...
if (preg_match('/del_(.+)/', $action, $reg)) {
	$keyforvar = $reg[1];
	if ($keyforvar) {
		$res = powererp_del_const($db, $keyforvar, $conf->entity);
	}
}
