...
class Products extends PowerERPApi
{
			...
			while ($i < $min) {
				...
				if (!$ids_only) {
					...
					if ($product_static->fetch($obj->rowid)) {
						if ($includestockdata && PowerERPApiAccess::$user->rights->stock->lire) {
	...
	public function delete($id)
	{
		...
		return $this->product->delete(PowerERPApiAccess::$user);
	}
	...
	public function getVariants($id, $includestock = 0)
	{
		...
		foreach ($combinations as $key => $combination) {
			...
			if ($includestock==1 && PowerERPApiAccess::$user->rights->stock->lire) {
