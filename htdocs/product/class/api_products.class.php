 ...
 */
class Products extends PowerERPApi
{
	/**
	...
	public function index($sortfield = "t.ref", $sortorder = 'ASC', $limit = 100, $page = 0, $mode = 0, $category = 0, $sqlfilters = '', $ids_only = false, $variant_filter = 0, $pagination_data = false, $includestockdata = 0)
	{
		...
		if (!PowerERPApiAccess::$user->rights->produit->lire) {
			throw new RestException(403);
		}

		$obj_ret = array();

		$socid = PowerERPApiAccess::$user->socid ? PowerERPApiAccess::$user->socid : '';
			...
			while ($i < $min) {
				...
				if (!$ids_only) {
					...
					if ($product_static->fetch($obj->rowid)) {
						if (!empty($includestockdata) && PowerERPApiAccess::$user->rights->stock->lire) {
	...
	{
		if (!PowerERPApiAccess::$user->rights->produit->creer) {
		...
		if ($this->product->create(PowerERPApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('product', $this->product->id)) {
		...
		$result = $this->product->update($id, PowerERPApiAccess::$user, 1, 'update', $updatetype);
		...
		if ($result > 0 && !empty($conf->global->PRODUCT_PRICE_UNIQ)) {
			...
			if ($pricemodified) {
				...
				$result = $this->product->updatePrice($newprice, $this->product->price_base_type, PowerERPApiAccess::$user, $newvat, $newpricemin, 0, $newnpr, 0, 0, array(), $newvatsrccode);
			}
		}
	...
	{
		if (!PowerERPApiAccess::$user->rights->produit->supprimer) {
			throw new RestException(401);
		}
		$result = $this->product->fetch($id);
		if (!$result) {
		...
		$res = $this->product->delete(PowerERPApiAccess::$user);
		if ($res < 0) {
	...
	public function getSubproducts($id)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('product', $id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}
	...
	{
		if (!PowerERPApiAccess::$user->rights->categorie->lire) {
			throw new RestException(401);
		}
	...
	public function getCustomerPricesPerCustomer($id, $thirdparty_id = '')
	{
		...
		if ($socid > 0 && $socid != $thirdparty_id) {
			throw new RestException(401, 'Getting prices for all customers or for the customer ID '.$thirdparty_id.' is not allowed for login '.PowerERPApiAccess::$user->login);
		}
	...
	public function addPurchasePrice($id, $qty, $buyprice, $price_base_type, $fourn_id, $availability, $ref_fourn, $tva_tx, $charges = 0, $remise_percent = 0, $remise = 0, $newnpr = 0, $delivery_time_days = 0, $supplier_reputation = '', $localtaxes_array = array(), $newdefaultvatcode = '', $multicurrency_buyprice = 0, $multicurrency_price_base_type = 'HT', $multicurrency_tx = 1, $multicurrency_code = '', $desc_fourn = '', $barcode = '', $fk_barcode_type = null)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('product', $this->productsupplier->id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}

		$socid = PowerERPApiAccess::$user->socid ? PowerERPApiAccess::$user->socid : '';
		if ($socid > 0 && $socid != $fourn_id) {
			throw new RestException(401, 'Adding purchase price for the supplier ID '.$fourn_id.' is not allowed for login '.PowerERPApiAccess::$user->login);
		}

		$result = $this->productsupplier->add_fournisseur(PowerERPApiAccess::$user, $fourn_id, $ref_fourn, $qty);
		if ($result < 0) {
		...
		$result = $this->productsupplier->update_buyprice($qty, $buyprice, PowerERPApiAccess::$user, $price_base_type, $fourn, $availability, $ref_fourn, $tva_tx, $charges, $remise_percent, $remise, $newnpr, $delivery_time_days, $supplier_reputation, $localtaxes_array, $newdefaultvatcode, $multicurrency_buyprice, $multicurrency_price_base_type, $multicurrency_tx, $multicurrency_code, $desc_fourn, $barcode, $fk_barcode_type);
	...
	public function addAttributes($ref, $label, $ref_ext = '')
	{
		...
		$resid = $prodattr->create(PowerERPApiAccess::$user);
		if ($resid <= 0) {
	...
	public function putAttributes($id, $request_data = null)
	{
		...
		if ($prodattr->update(PowerERPApiAccess::$user) > 0) {
			$result = $prodattr->fetch((int) $id);
			if ($result == 0) {
				throw new RestException(404, 'Attribute not found');
			} elseif ($result < 0) {
	...
	public function deleteAttributes($id)
	{
		...
		$prodattr = new ProductAttribute($this->db);
		$prodattr->id = (int) $id;
		$result = $prodattr->delete(PowerERPApiAccess::$user);
	...
	public function deleteAttributeValueByRef($id, $ref)
	{
		...
		$attrval->id = $result->rowid;
		$result = $attrval->delete(PowerERPApiAccess::$user);
		if ($result > 0) {
			return 1;
		}
	...
	public function addAttributeValue($id, $ref, $value)
	{
		...
		if ($objectval->create(PowerERPApiAccess::$user) > 0) {
			return $objectval->id;
		}
	...
	public function putAttributeValue($id, $request_data)
	{
		...
		if ($objectval->update(PowerERPApiAccess::$user) > 0) {
			$result = $objectval->fetch((int) $id);
			if ($result == 0) {
				throw new RestException(404, 'Attribute not found');
			} elseif ($result < 0) {
	...
	public function deleteAttributeValueById($id)
	{
		...
		if ($objectval->delete(PowerERPApiAccess::$user) > 0) {
			return 1;
		}
		throw new RestException(500, "Error deleting attribute value");
	}
	...
	public function getVariants($id, $includestock = 0)
	{
		...
		foreach ($combinations as $key => $combination) {
			...
			if (!empty($includestock) && PowerERPApiAccess::$user->rights->stock->lire) {
	...
	public function addVariant($id, $weight_impact, $price_impact, $price_impact_is_percent, $features, $reference = '', $ref_ext = '')
	{
		...
		$result = $prodcomb->createProductCombination(PowerERPApiAccess::$user, $this->product, $features, array(), $price_impact_is_percent, $price_impact, $weight_impact, $reference, $ref_ext);
		if ($result > 0) {
			return $result;
		} else {
	...
	public function addVariantByProductRef($ref, $weight_impact, $price_impact, $price_impact_is_percent, $features)
	{
		...
		if (!$prodcomb->fetchByProductCombination2ValuePairs($this->product->id, $features)) {
			$result = $prodcomb->createProductCombination(PowerERPApiAccess::$user, $this->product, $features, array(), $price_impact_is_percent, $price_impact, $weight_impact);
			if ($result > 0) {
				return $result;
			} else {
				throw new RestException(500, "Error creating new product variant");
	...
	public function putVariant($id, $request_data = null)
	{
		...
		$result = $prodcomb->update(PowerERPApiAccess::$user);
		if ($result > 0) {
			return 1;
		}
	...
	public function deleteVariant($id)
	{
		...
		$prodcomb = new ProductCombination($this->db);
		$prodcomb->id = (int) $id;
		$result = $prodcomb->delete(PowerERPApiAccess::$user);
		if ($result <= 0) {
	...
	public function getStock($id, $selected_warehouse_id = null)
	{
		...
		if (!PowerERPApiAccess::$user->rights->produit->lire || !PowerERPApiAccess::$user->rights->stock->lire) {
			throw new RestException(401);
		}
	...
	protected function _cleanObjectDatas($object)
	{
		...
		if (empty(PowerERPApiAccess::$user->rights->stock->lire)) {
