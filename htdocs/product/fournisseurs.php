				...
				if ($rowid) {
					...
					if (empty($reshook)) {
						if (empty($form->result)) {
							print ' - <a href="'.DOL_URL_ROOT.'/societe/card.php?action=create&type=f&backtopage='.urlencode($_SERVER["PHP_SELF"].'?id='.$object->id.'&action='.$action).'">'.$langs->trans("CreatePowererpThirdPartySupplier").'</a>';
						}
					}
				}
				print '</td></tr>';
...
END;
				...
				if (!empty($conf->global->PRODUIT_FOURN_TEXTS)) {
					...
					$doleditor = new DolEditor('supplier_description', $object->desc_supplier, '', 160, 'powererp_details', '', false, true, $conf->global->FCKEDITOR_ENABLE_PRODUCTDESC, ROWS_4, '90%');
					$doleditor->Create();
