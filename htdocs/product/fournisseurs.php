				...
				if ($rowid) {
					...
					if (empty($reshook)) {
						if (empty($form->result)) {
							...
							print img_picto($langs->trans("CreatePowerERPThirdPartySupplier"), 'add', 'class="marginleftonly"');
							print '</a>';
						}
					}
				}
...
END;
				...
				if (!empty($conf->global->PRODUIT_FOURN_TEXTS)) {
					...
					$doleditor = new DolEditor('supplier_description', $object->desc_supplier, '', 160, 'powererp_details', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_PRODUCTDESC'), ROWS_4, '90%');
					$doleditor->Create();
