...
$pathofwebsite = $powererp_main_data_root.($conf->entity > 1 ? '/'.$conf->entity : '').'/website/'.$websitekey;
...
if ($action == 'seteditinline') {
	powererp_set_const($db, 'WEBSITE_EDITINLINE', 1);
	setEventMessages($langs->trans("FeatureNotYetAvailable"), null, 'warnings');
	//powererp_set_const($db, 'WEBSITE_SUBCONTAINERSINLINE', 0); // Force disable of 'Include dynamic content'
	header("Location: ".$_SERVER["PHP_SELF"].'?website='.GETPOST('website', 'alphanohtml').'&pageid='.GETPOST('pageid', 'int'));
	exit;
}
if ($action == 'unseteditinline') {
	powererp_del_const($db, 'WEBSITE_EDITINLINE');
	header("Location: ".$_SERVER["PHP_SELF"].'?website='.GETPOST('website', 'alphanohtml').'&pageid='.GETPOST('pageid', 'int'));
	exit;
}
if ($action == 'setshowsubcontainers') {
	powererp_set_const($db, 'WEBSITE_SUBCONTAINERSINLINE', 1);
	//powererp_set_const($db, 'WEBSITE_EDITINLINE', 0); // Force disable of edit inline
	header("Location: ".$_SERVER["PHP_SELF"].'?website='.GETPOST('website', 'alphanohtml').'&pageid='.GETPOST('pageid', 'int'));
	exit;
}
if ($action == 'unsetshowsubcontainers') {
	powererp_del_const($db, 'WEBSITE_SUBCONTAINERSINLINE');
	header("Location: ".$_SERVER["PHP_SELF"].'?website='.GETPOST('website', 'alphanohtml').'&pageid='.GETPOST('pageid', 'int'));
	exit;
}
...
if ($massaction == 'replace' && GETPOST('confirmmassaction', 'alpha') && $usercanedit) {
	...
	if (dol_is_file($powererpdataroot.'/installmodules.lock')) {
	...
	if (!$allowimportsite) {
		...
		if (getDolGlobalString('MAIN_MESSAGE_INSTALL_MODULES_DISABLED_CONTACT_US')) {
			...
			$message = $langs->trans("InstallModuleFromWebHasBeenDisabledByFile", $powererpdataroot.'/installmodules.lock');
		}
...
if ($action == 'addcontainer' && $usercanedit) {
			...
			if (!$error) {
				...
				foreach ($regs[0] as $key => $val) {
					...
					if ($tmpgeturl['curl_error_no']) {
						...
						// We try to convert the CSS we got by adding a prefix .bodywebsite with lessc to avoid conflict with CSS of PowerERP.
				...
				// Normalize links href to PowerERP internal naming
	...
	if (!$error) {
		...
		if (!dol_is_file($filerobot)) {
			$robotcontent = "# Robot file. Generated with PowerERP\nUser-agent: *\nAllow: /public/\nDisallow: /administrator/";
		...
		if (!dol_is_file($filereadme)) {
			$readmecontent = "Website generated by PowerERP ERP CRM";
	...
	if (GETPOST('refreshsite') || GETPOST('refreshsite_x') || GETPOST('refreshsite.x') || GETPOST('refreshpage') || GETPOST('refreshpage_x') || GETPOST('refreshpage.x')) {
		...
		if (!$error) {
			...
			if (!$errorphpcheck) {
				...
				$htmlheadercontent.= "if (! defined('USEPOWERERPSERVER') && ! defined('USEPOWERERPEDITOR')) { require_once './master.inc.php'; } // Load env if not already loaded"."\n";
			...
			if (!$errorphpcheck) {
				...
				$csscontent .= "if (! defined('USEPOWERERPSERVER') && ! defined('USEPOWERERPEDITOR')) { require_once __DIR__.'/master.inc.php'; } // Load env if not already loaded\n"; // For the css, we need to set path of master using the dirname of css file.
			...
			if (!$errorphpcheck) {
				...
				$jscontent .= "if (! defined('USEPOWERERPSERVER') && ! defined('USEPOWERERPEDITOR')) { require_once __DIR__.'/master.inc.php'; } // Load env if not already loaded\n"; // For the css, we need to set path of master using the dirname of css file.
			...
			if (!$errorphpcheck) {
				...
				$robotcontent.= "if (! defined('USEPOWERERPSERVER') && ! defined('USEPOWERERPEDITOR')) { require_once './master.inc.php'; } // Load env if not already loaded"."\n";
			...
			if (!$errorphpcheck) {
				...
				$manifestjsoncontent .= "if (! defined('USEPOWERERPSERVER') && ! defined('USEPOWERERPEDITOR')) { require_once __DIR__.'/master.inc.php'; } // Load env if not already loaded\n"; // For the css, we need to set path of master using the dirname of css file.
			...
			if (!$errorphpcheck) {
				...
				/*$readmecontent.= "<?php // BEGIN PHP\n";
				   ...
				   $readmecontent.= "if (! defined('USEPOWERERPSERVER') && ! defined('USEPOWERERPEDITOR')) { require_once __DIR__.'/master.inc.php'; } // Load env if not already loaded"."\n";	// For the css, we need to set path of master using the dirname of css file.
	...
	if ($action == 'confirm_createpagefromclone') {
		...
		if (!$error) {
			...
			if ($newwebsiteid > 0 && $newwebsiteid != $object->id) {
				...
				$pathofwebsitenew = $powererp_main_data_root.($conf->entity > 1 ? '/'.$conf->entity : '').'/website/'.$tmpwebsite->ref;
			} else {
				$tmpwebsite = $object;
			}
			...
			if (!empty($_FILES) || GETPOSTISSET('templateuserfile')) {
				...
				if (!$error && GETPOSTISSET('templateuserfile')) {
					...
					if ($result < 0) {
						...
						powererp_set_const($db, 'WEBSITE_SUBCONTAINERSINLINE', 1, 'chaine', 0, '', $conf->entity);
	...
	if ($resql) {
		...
		if ($num_rows > 0) {
			...
			while ($i < $num_rows) {
				...
				//$pathofpage = $powererp_main_url_root.'/'.$pageurl.'.php';
...
if (!GETPOST('hide_websitemenu')) {
	...
	if ($websitekey && $websitekey != '-1' && (!in_array($action, array('editcss', 'editmenu', 'importsite', 'file_manager', 'replacesiteconfirm'))) && (!in_array($mode, array('replacesite'))) && !$file_manager) {
		...
		if (($pageid > 0 && $atleastonepage) && ($action == 'preview' || $action == 'createfromclone' || $action == 'createpagefromclone' || $action == 'deletesite')) {
			...
			$htmltext = $langs->trans("PreviewSiteServedByPowerERP", $langs->transnoentitiesnoconv("Page"), $langs->transnoentitiesnoconv("Page"), $realpage, $dataroot);
		...
		if ($action == 'editsource' || $action == 'editcontent' || GETPOST('editsource', 'alpha') || GETPOST('editcontent', 'alpha')) {
			$url = 'https://wiki.powererp.org/index.php/Module_Website';
...
if ((empty($action) || $action == 'preview' || $action == 'createfromclone' || $action == 'createpagefromclone') && !in_array($mode, array('replacesite'))) {
	if ($pageid > 0 && $atleastonepage) {
		...
		// Ouput page under the PowerERP top menu
		...
		*/
		...
		// Clean style that may affect global style of PowerERP
