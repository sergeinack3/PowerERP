...
class Website extends CommonObject
{
	...
	public function createFromClone($user, $fromid, $newref, $newlang = '')
	{
		...
		$pathofwebsiteold = $powererp_main_data_root.($conf->entity > 1 ? '/'.$conf->entity : '').'/website/'.dol_sanitizeFileName($oldref);
		$pathofwebsitenew = $powererp_main_data_root.($conf->entity > 1 ? '/'.$conf->entity : '').'/website/'.dol_sanitizeFileName($newref);
	...
	public function exportWebSite()
	{
		...
		foreach ($listofpages as $pageid => $objectpageold) {
			...
			$line = '-- File generated by PowerERP '.DOL_VERSION.' -- '.dol_print_date(dol_now('gmt'), 'standard', 'gmt').' UTC --;';
			$line .= "\n";
