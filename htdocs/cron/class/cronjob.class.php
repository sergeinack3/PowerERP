...
class Cronjob extends CommonObject
{
	...
	public function run_jobs($userlogin)
	{
		...
		if ($this->jobtype == 'command') {
			global $powererp_cron_allow_cli;

			if (empty($powererp_cron_allow_cli)) {
				$langs->load("errors");
				$this->error      = $langs->trans("FailedToExecutCommandJob");
				$this->lastoutput = '';
				$this->lastresult = $langs->trans("ErrorParameterMustBeEnabledToAllwoThisFeature", 'powererp_cron_allow_cli');
			} else {
