...
if ($setterminal > 0) {
	...
	setcookie("takeposterminal", $setterminal, (time() + (86400 * 354)), '/', null, (empty($powererp_main_force_https) ? false : true), true); // Permanent takeposterminal var in a cookie
}
...
function PowerERPOpenDrawer() {
	console.log("PowerERPOpenDrawer call ajax url /takepos/ajax/ajax.php?action=opendrawer&token=<?php echo newToken();?>&term=<?php print urlencode($_SESSION["takeposterminal"]); ?>");
	$.ajax({
		type: "GET",
...
if (getDolGlobalString('TAKEPOS_BAR_RESTAURANT')) {
	...
	if (getDolGlobalString('TAKEPOS_BAR_RESTAURANT')) {
		if (getDolGlobalString('TAKEPOS_PRINT_METHOD') == "takeposconnector") {
			...
			$menus[$r++] = array('title'=>'<span class="fa fa-receipt paddingrightonly"></span><div class="trunc">'.$langs->trans("Receipt").'</div>', 'action'=>'PowerERPTakeposPrinting(placeid);');
		} else {
