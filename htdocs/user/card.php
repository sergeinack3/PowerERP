...
if ($action == 'create' || $action == 'adduserldap') {
	...
	if (!empty($conf->ldap->enabled) && (isset($conf->global->LDAP_SYNCHRO_ACTIVE) && getDolGlobalInt('LDAP_SYNCHRO_ACTIVE') === Ldap::SYNCHRO_LDAP_TO_POWERERP)) {
	...
	print '<td>';
	$valuetoshow = '';
	if (preg_match('/ldap/', $powererp_main_authentication)) {
	...
	if (preg_match('/powererp/', $powererp_main_authentication)) {
		if (!empty($ldap_pass)) {	// For very old system comaptibilty. Now clear password can't be viewed from LDAP read
			$valuetoshow .= ($valuetoshow ? ' + ' : '').'<input type="hidden" name="password" value="'.dol_escape_htmltag($ldap_pass).'">'; // PowerERP password is preffiled with LDAP known password
			$valuetoshow .= preg_replace('/./i', '*', $ldap_pass);
		} else {
			// We do not use a field password but a field text to show new password to use.
			$valuetoshow .= ($valuetoshow ? ' + '.$langs->trans("PowerERPPassword") : '').'<input class="minwidth300 maxwidth400 widthcentpercentminusx" maxsize="32" type="text" id="password" name="password" value="'.dol_escape_htmltag($password).'" autocomplete="new-password">';
			if (!empty($conf->use_javascript_ajax)) {
	...
	$doleditor = new DolEditor('signature', GETPOST('signature', 'restricthtml'), '', 138, 'powererp_notes', 'In', true, $acceptlocallinktomedia, empty($conf->global->FCKEDITOR_ENABLE_USERSIGN) ? 0 : 1, ROWS_4, '90%');
	...
	print '</td><td>';
	require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
	$doleditor = new DolEditor('note_public', GETPOSTISSET('note_public') ? GETPOST('note_public', 'restricthtml') : '', '', 100, 'powererp_notes', '', false, true, getDolGlobalString('FCKEDITOR_ENABLE_NOTE_PUBLIC'), ROWS_3, '90%');
	$doleditor->Create();
	print "</td></tr>\n";
	...
	print '</td><td>';
	require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
	$doleditor = new DolEditor('note_private', GETPOSTISSET('note_private') ? GETPOST('note_private', 'restricthtml') : '', '', 100, 'powererp_notes', '', false, true, getDolGlobalString('FCKEDITOR_ENABLE_NOTE_PRIVATE'), ROWS_3, '90%');
	$doleditor->Create();
	print "</td></tr>\n";
	...
	if ($id > 0) {
		...
		if ($action != 'edit') {
			...
			if (!empty($object->ldap_sid) && $object->statut == 0) {
				...
				print $langs->trans("LoginAccountDisableInPowerERP");
				print '</td>';
			} else {
				print '<td>';
				$addadmin = '';
			...
			if (isModEnabled('adherent')) {
				$langs->load("members");
				print '<tr><td>'.$langs->trans("LinkedToPowerERPMember").'</td>';
				print '<td>';
				if ($object->fk_member) {
					$adh = new Adherent($db);
		...
		if ($action == 'edit' && ($canedituser || $caneditpassword)) {
			...
			if ($caneditfield) {
				...
				$doleditor = new DolEditor('signature', $object->signature, '', 138, 'powererp_notes', 'In', false, $acceptlocallinktomedia, empty($conf->global->FCKEDITOR_ENABLE_USERSIGN) ? 0 : 1, ROWS_4, '90%');
				print $doleditor->Create(1);
			} else {
				print dol_htmlentitiesbr($object->signature);
			}
