...
if ($object->ldap_sid) {
	print '<td class="warning">'.$langs->trans("LoginAccountDisableInPowerERP").'</td>';
} else {
...
if (getDolGlobalInt('LDAP_SYNCHRO_ACTIVE') === Ldap::SYNCHRO_POWERERP_TO_LDAP) {
	print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=powererp2ldap">'.$langs->trans("ForceSynchronize").'</a>';
}
