...
class User extends CommonObject
{
	...
	public function update($user, $notrigger = 0, $nosyncmember = 0, $nosyncmemberpass = 0, $nosynccontact = 0)
	{
		...
		$sql .= ", api_key = ".($this->api_key ? "'".$this->db->escape(dolEncrypt($this->api_key, '', '', 'powererp'))."'" : "null");
	...
	public function send_password($user, $password = '', $changelater = 0)
	{
		...
		if (!$changelater) {
			...
			//print $password.'-'.$this->id.'-'.$powererp_main_instance_unique_id;
			$url = $urlwithroot.'/user/passwordforgotten.php?action=validatenewpassword';
			$url .= '&username='.urlencode($this->login)."&passworduidhash=".urlencode(dol_hash($password.'-'.$this->id.'-'.$powererp_main_instance_unique_id));
	...
	public function getNomUrl($withpictoimg = 0, $option = '', $infologin = 0, $notooltip = 0, $maxlen = 24, $hidethirdpartylogo = 0, $mode = '', $morecss = '', $save_lastsearch_value = -1)
	{
		...
		if ($infologin > 0) {
			...
			$label .= '<br><b>'.$langs->trans("AuthenticationMode").':</b> '.dol_string_nohtmltag($_SESSION["dol_authmode"].(empty($powererp_main_demo) ? '' : ' (demo)'));
	 ...
	 *  @return int  				<0 if KO, >0 if OK
	 */
	public function update_ldap2powererp(&$ldapuser)
	{
		// phpcs:enable
		...
		dol_syslog(get_class($this)."::update_ldap2powererp result=".$result, LOG_DEBUG);
