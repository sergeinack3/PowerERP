 ...
 */
class Users extends PowererpApi
{
	/**
	...
	public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $user_ids = 0, $category = 0, $sqlfilters = '')
	{
		...
		if (empty(PowererpApiAccess::$user->rights->user->user->lire) && empty(PowererpApiAccess::$user->admin)) {
		...
		//$socid = PowererpApiAccess::$user->socid ? PowererpApiAccess::$user->socid : $societe;
	...
	{
		if (empty(PowererpApiAccess::$user->rights->user->user->lire) && empty(PowererpApiAccess::$user->admin) && $id != 0 && PowererpApiAccess::$user->id != $id) {
		...
		if ($id > 0 && !PowererpApi::_checkAccessToResource('user', $this->useraccount->id, 'user')) {
	...
	public function getByLogin($login, $includepermissions = 0)
	{
		...
		if (empty(PowererpApiAccess::$user->rights->user->user->lire) && empty(PowererpApiAccess::$user->admin) && PowererpApiAccess::$user->login != $login) {
			throw new RestException(401, 'Not allowed');
		}
		...
		if (!PowererpApi::_checkAccessToResource('user', $this->useraccount->id, 'user')) {
			throw new RestException(401, 'Access not allowed for login '.PowererpApiAccess::$user->login);
		}
	...
	public function getByEmail($email, $includepermissions = 0)
	{
		...
		if (empty(PowererpApiAccess::$user->rights->user->user->lire) && empty(PowererpApiAccess::$user->admin) && PowererpApiAccess::$user->email != $email) {
			throw new RestException(401, 'Not allowed');
		}
	...
	{
		if (empty(PowererpApiAccess::$user->rights->user->self->creer) && empty(PowererpApiAccess::$user->rights->user->user->lire) && empty(PowererpApiAccess::$user->admin)) {
			throw new RestException(401, 'Not allowed');
		}

		$apiUser = PowererpApiAccess::$user;
	...
	public function post($request_data = null)
	{
		...
		if (empty(PowererpApiAccess::$user->rights->user->creer) && empty(PowererpApiAccess::$user->admin)) {
			throw new RestException(401, "User creation not allowed for login ".PowererpApiAccess::$user->login);
		}
		...
		foreach ($request_data as $field => $value) {
			...
			/*if ($field == 'pass') {
				if (empty(PowererpApiAccess::$user->rights->user->user->password)) {
		...
		if ($this->useraccount->create(PowererpApiAccess::$user) < 0) {
	...
	{
		// Check user authorization
		if (empty(PowererpApiAccess::$user->rights->user->user->creer) && empty(PowererpApiAccess::$user->admin)) {
		...
		foreach ($request_data as $field => $value) {
			...
			if ($field == 'pass') {
				if ($this->useraccount->id != PowererpApiAccess::$user->id && empty(PowererpApiAccess::$user->rights->user->user->password)) {
					throw new RestException(401, 'You are not allowed to modify password of other users');
				}
				if ($this->useraccount->id == PowererpApiAccess::$user->id && empty(PowererpApiAccess::$user->rights->user->self->password)) {
					throw new RestException(401, 'You are not allowed to modify your own password');
				}
			}
			if (PowererpApiAccess::$user->admin) {	// If user for API is admin
		...
		if ($this->useraccount->update(PowererpApiAccess::$user) >= 0) {
			return $this->get($id);
		} else {
	...
	public function setGroup($id, $group, $entity = 1)
	{
		...
		if (!empty($conf->multicompany->enabled) && !empty($conf->global->MULTICOMPANY_TRANSVERSE_MODE) && !empty(PowererpApiAccess::$user->admin) && empty(PowererpApiAccess::$user->entity)) {
			...
			$entity = (PowererpApiAccess::$user->entity > 0 ? PowererpApiAccess::$user->entity : $conf->entity);
		}
	...
	public function listGroups($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $group_ids = 0, $sqlfilters = '')
	{
		...
		if ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && empty(PowererpApiAccess::$user->rights->user->user->lire) && empty(PowererpApiAccess::$user->admin)) ||
			!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && empty(PowererpApiAccess::$user->rights->user->group_advance->read) && empty(PowererpApiAccess::$user->admin)) {
	...
	{
		if (empty(PowererpApiAccess::$user->rights->user->user->supprimer) && empty(PowererpApiAccess::$user->admin)) {
			throw new RestException(401, 'Not allowed');
		}
		$result = $this->useraccount->fetch($id);
		if (!$result) {
		...
		return $this->useraccount->delete(PowererpApiAccess::$user);
	}
	...
	protected function _cleanObjectDatas($object)
	{
		...
		$canreadsalary = ((!empty($conf->salaries->enabled) && !empty(PowererpApiAccess::$user->rights->salaries->read)) || (empty($conf->salaries->enabled)));
