 ...
 */
class ExpenseReports extends PowerERPApi
{
	...
	{
		if (!PowerERPApiAccess::$user->rights->expensereport->lire) {
		...
		if (!PowerERPApi::_checkAccessToResource('expensereport', $this->expensereport->id)) {
	...
	{
		if (!PowerERPApiAccess::$user->rights->expensereport->creer) {
		...
		if ($this->expensereport->create(PowerERPApiAccess::$user) < 0) {
	...
	*/
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->expensereport->update(PowerERPApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!PowerERPApiAccess::$user->rights->expensereport->supprimer) {
			throw new RestException(401);
		}
		...
		if (!$this->expensereport->delete(PowerERPApiAccess::$user)) {
	...
	public function validate($id, $idwarehouse=0)
	{
		...
		if( ! $this->expensereport->valid(PowerERPApiAccess::$user, $idwarehouse)) {
