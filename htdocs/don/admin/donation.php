...
if ($action == 'specimen') {
	...
	// Set default model
	if (powererp_set_const($db, "DON_ADDON_MODEL", $value, 'chaine', 0, '', $conf->entity)) {
	...
	if ($ret > 0) {
		if ($conf->global->DON_ADDON_MODEL == "$value") {
			powererp_del_const($db, 'DON_ADDON_MODEL', $conf->entity);
		}
	}
}
...
if ($action == 'set_DONATION_ACCOUNTINGACCOUNT') {
	...
	$res = powererp_set_const($db, "DONATION_ACCOUNTINGACCOUNT", $account, 'chaine', 0, '', $conf->entity);
...
if ($action == 'set_DONATION_MESSAGE') {
	...
	$res = powererp_set_const($db, "DONATION_MESSAGE", $freemessage, 'chaine', 0, '', $conf->entity);
