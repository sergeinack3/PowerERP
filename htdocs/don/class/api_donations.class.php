 ...
 */
class Donations extends PowerERPApi
{
	...
	{
		if (!PowerERPApiAccess::$user->rights->don->lire) {
		...
		if (!PowerERPApi::_checkAccessToResource('don', $this->don->id)) {
	...
	{
		if (!PowerERPApiAccess::$user->rights->don->creer) {
		...
		if ($this->don->create(PowerERPApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('donation', $this->don->id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}
		foreach ($request_data as $field => $value) {
			if ($field == 'id') {
		...
		if ($this->don->update(PowerERPApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!PowerERPApiAccess::$user->rights->don->supprimer) {
			throw new RestException(401);
		}
		...
		if (!$this->don->delete(PowerERPApiAccess::$user)) {
	...
	public function validate($id, $idwarehouse = 0, $notrigger = 0)
	{
		...
		$result = $this->don->valid_promesse($id, PowerERPApiAccess::$user->id, $notrigger);
		if ($result == 0) {
