...
if ($action == 'create') {
	...
	if ($socid > 0) {
		...
		print '<td>';
		$doleditor = new DolEditor('note_public', $note_public, '', 80, 'powererp_notes', 'In', 0, false, empty($conf->global->FCKEDITOR_ENABLE_NOTE_PUBLIC) ? 0 : 1, ROWS_3, '90%');
		...
		if (empty($user->socid)) {
			...
			print '<td>';
			$doleditor = new DolEditor('note_private', $note_private, '', 80, 'powererp_notes', 'In', 0, false, empty($conf->global->FCKEDITOR_ENABLE_NOTE_PRIVATE) ? 0 : 1, ROWS_3, '90%');
			print $doleditor->Create(1);
		...
		if ($resql) {
			...
			while ($i < $num) {
				...
				if ($object->statut == 0 && $action == 'editline' && $user->rights->ficheinter->creer && GETPOST('line_id', 'int') == $objp->rowid) {
					...
					// Editeur wysiwyg
					require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
					$doleditor = new DolEditor('np_desc', $objp->description, '', 164, 'powererp_details', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_DETAILS'), ROWS_2, '90%');
					$doleditor->Create();
			...
			if ($object->statut == 0 && $user->rights->ficheinter->creer && $action <> 'editline' && empty($conf->global->FICHINTER_DISABLE_DETAILS)) {
				...
				if (empty($conf->global->FICHINTER_EMPTY_LINE_DESC)) {
					require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
					$doleditor = new DolEditor('np_desc', GETPOST('np_desc', 'restricthtml'), '', 100, 'powererp_details', '', false, true, !empty($conf->global->FCKEDITOR_ENABLE_DETAILS), ROWS_2, '90%');
					$doleditor->Create();
				}
