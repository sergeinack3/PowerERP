 ...
 *
 */
class Thirdparties extends PowerERPApi
{
	/**
	 *
	...
	public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $mode = 0, $category = 0, $sqlfilters = '')
	{
		...
		if (!PowerERPApiAccess::$user->hasRight('societe', 'lire')) {
			throw new RestException(401);
		}

		// case of external user, we force socids
		$socids = PowerERPApiAccess::$user->socid ? PowerERPApiAccess::$user->socid : '';
	...
	{
		if (!PowerERPApiAccess::$user->rights->societe->creer) {
			throw new RestException(401);
		}
		...
		if ($this->company->create(PowerERPApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('societe', $this->company->id)) {
		...
		if ($this->company->update($id, PowerERPApiAccess::$user, 1, '', '', 'update', 1)) {
			return $this->get($id);
		}
	...
	public function merge($id, $idtodelete)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('societe', $this->companytoremove->id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}
	...
	{
		if (!PowerERPApiAccess::$user->hasRight('societe', 'supprimer')) {
			throw new RestException(401);
		}
		$result = $this->company->fetch($id);
		if (!$result) {
	...
	public function setThirdpartyPriceLevel($id, $priceLevel)
	{
		...
		if (empty(PowerERPApiAccess::$user->rights->societe->creer)) {
			throw new RestException(401, 'Access to thirdparty '.$id.' not allowed for login '.PowerERPApiAccess::$user->login);
		}
		...
		if (empty(PowerERPApi::_checkAccessToResource('societe', $this->company->id))) {
			throw new RestException(401, 'Access to thirdparty '.$id.' not allowed for login '.PowerERPApiAccess::$user->login);
		}

		$result = $this->company->setPriceLevel($priceLevel, PowerERPApiAccess::$user);
		if ($result <= 0) {
	...
	public function addCategory($id, $category_id)
	{
		...
		if (!PowerERPApi::_checkAccessToResource('category', $category->id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}
	...
	public function getOutStandingProposals($id, $mode = 'customer')
	{
		...
		if (!PowerERPApi::_checkAccessToResource('societe', $id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login);
		}
	...
	public function getSalesRepresentatives($id, $mode = 0)
	{
		...
		$result = $this->company->getSalesRepresentatives(PowerERPApiAccess::$user, $mode);
	...
	{
		if (!PowerERPApiAccess::$user->rights->societe->lire) {
			throw new RestException(401);
		}
		if (empty($id)) {
			throw new RestException(400, 'Thirdparty ID is mandatory');
	...
	public function createCompanyBankAccount($id, $request_data = null)
	{
		...
		if ($account->create(PowerERPApiAccess::$user) < 0) {
		...
		if ($account->update(PowerERPApiAccess::$user) < 0) {
	...
	public function deleteCompanyBankAccount($id, $bankaccount_id)
	{
		...
		return $account->delete(PowerERPApiAccess::$user);
	}
	...
	public function generateBankAccountDocument($id, $companybankid = null, $model = 'sepamandate')
	{
		...
		$this->company->setDocModel(PowerERPApiAccess::$user, $model);
	...
	public function deleteSocieteAccount($id, $site)
	{
		...
		if ($result && $this->db->num_rows($result) == 0) {
			...
			if ($account->delete(PowerERPApiAccess::$user) < 0) {
	...
	private function _fetch($rowid, $ref = '', $ref_ext = '', $barcode = '', $idprof1 = '', $idprof2 = '', $idprof3 = '', $idprof4 = '', $idprof5 = '', $idprof6 = '', $email = '', $ref_alias = '')
	{
		...
		if (!PowerERPApiAccess::$user->hasRight('societe', 'lire')) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login.'. No read permission on thirdparties.');
		}
		...
		if (!PowerERPApi::_checkAccessToResource('societe', $this->company->id)) {
			throw new RestException(401, 'Access not allowed for login '.PowerERPApiAccess::$user->login.' on this thirdparty');
		}
		if (isModEnabled('mailing')) {
