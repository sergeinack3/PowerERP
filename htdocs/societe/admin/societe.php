...
if ($action == 'setcodeclient') {
	if (powererp_set_const($db, "SOCIETE_CODECLIENT_ADDON", $value, 'chaine', 0, '', $conf->entity) > 0) {
...
if ($action == 'setcodecompta') {
	if (powererp_set_const($db, "SOCIETE_CODECOMPTA_ADDON", $value, 'chaine', 0, '', $conf->entity) > 0) {
		header("Location: ".$_SERVER["PHP_SELF"]);
		exit;
	} else {
		dol_print_error($db);
...
if ($action == 'setprofid') {
	...
	if (powererp_set_const($db, $idprof, $status, 'chaine', 0, '', $conf->entity) > 0) {
		//header("Location: ".$_SERVER["PHP_SELF"]);
		//exit;
	} else {
		dol_print_error($db);
...
if ($action == 'sethideinactivethirdparty') {
	...
	if (powererp_set_const($db, "COMPANY_HIDE_INACTIVE_IN_COMBOBOX", $status, 'chaine', 0, '', $conf->entity) > 0) {
		header("Location: ".$_SERVER["PHP_SELF"]);
		exit;
	} else {
		dol_print_error($db);
